# Oracle 11g - queries005
Source: https://docs.oracle.com/cd/E11882_01/server.112/e41084/queries005.htm
...
When character values are compared linguistically for the `ORDER` `BY` clause, they are first transformed to collation keys and then compared like `RAW` values. The collation keys are generated either explicitly as specified in `NLSSORT` or implicitly using the same method that `NLSSORT` uses. Both explicitly and implicitly generated collation keys are subject to the same restrictions that are described in ["NLSSORT"](functions113.md#i78399). As a result of these restrictions, two values may compare as linguistically equal if they do not differ in the prefix that was used to produce the collation key, even if they differ in the rest of the value.
