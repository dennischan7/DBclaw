# Oracle 11g - statements_10008
Source: https://docs.oracle.com/cd/E11882_01/server.112/e41084/statements_10008.htm
...
[ALTER SESSION](statements_2013.md#i2231814)
...
If `column` is part of the partitioning key of a partitioned table, then `UPDATE` will fail if you change a value in the column that would move the row to a different partition or subpartition, unless you enable row movement. Refer to the `row_movement_clause` of [CREATE TABLE](statements_7002.md#i2095331) or [ALTER TABLE](statements_3001.md#CJAHHIBI).
...
You can use the `flashback_query_clause` within the subquery to update `table` with past data. Refer to the [flashback\_query\_clause](statements_10002.md#i2112818) of `SELECT` for more information on this clause.
...
column, then during subsequent queries, Oracle Database will perform a full table scan rather than using any index that might exist on the
...
The `where_clause` lets you restrict the rows updated to those for which the specified `condition` is true. If you omit this clause, then the database updates all rows in the table or view. Refer to [Chapter 7, "Conditions"](conditions.md#g1077361) for the syntax of `condition`.
...
The error\_logging\_clause has the same behavior in an `UPDATE` statement as it does in an `INSERT` statement. Refer to the `INSERT` statement [error\_logging\_clause](statements_9014.md#BGBEIACB) for more information.
...
Updating an Object Table: Example The following statement creates two object tables, `people_demo1` and `people_demo2`, of the `people_typ` object created in [Table Collections: Examples](statements_10002.md#i2071643). The example shows how to update a row of `people_demo1` by selecting a row from `people_demo2`:
...
Correlated Update: Example For an example that uses a correlated subquery to update nested table rows, refer to ["Table Collections: Examples"](statements_10002.md#i2071643).
