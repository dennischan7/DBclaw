# Oracle 11g - clauses009
Source: https://docs.oracle.com/cd/E11882_01/server.112/e41084/clauses009.htm
...
is interpreted differently for locally managed tablespaces. For locally managed tablespaces, Oracle Database uses
...
to compute how many extents are allocated when the object is first created. After object creation, these parameters are ignored. For more information, see

[CREATE TABLESPACE](statements_7003.md#i2231734)

.

INITIAL

Specify the size of the first extent of the object. Oracle allocates space for this extent when you create the schema object. Refer to [size\_clause](clauses008.md#CHDEIJBC) for information on that clause.
...
Specify in bytes the size of the next extent to be allocated to the object. Refer to [size\_clause](clauses008.md#CHDEIJBC) for information on that clause.
...
Restriction on MAXEXTENTS `MAXEXTENTS` is ignored for objects residing in a locally managed tablespace, unless the value of `ALLOCATION_TYPE` is `USER` for the tablespace in the `DBA_TABLESPACES` data dictionary view.
...
has the same storage parameters as the tablespace in which the rollback segment is created. Thus, if you create a tablespace with

`MAXEXTENTS UNLIMITED`

, then the rollback segment will have this same default.
...
This clause is not valid or useful if you have specified the `SECUREFILE` parameter of [LOB\_parameters](statements_7002.md#BABFFFBE). If you specify both the `SECUREFILE` parameter and `FREELISTS`, then the database silently ignores the `FREELISTS` specification.
...
This clause is not valid or useful if you have specified the `SECUREFILE` parameter of [LOB\_parameters](statements_7002.md#BABFFFBE). If you specify both the `SECUREFILE` parameter and `FREELIST` `GROUPS`, then the database silently ignores the `FREELIST` `GROUPS` specification.
...
The `OPTIMAL` keyword is relevant only to rollback segments. It specifies an optimal size in bytes for a rollback segment. Refer to [size\_clause](clauses008.md#CHDEIJBC) for information on that clause.
...
Restrictions on the BUFFER\_POOL Parameter `BUFFER_POOL` is subject to the following restrictions:
