# Oracle 11g - statements_1004
Source: https://docs.oracle.com/cd/E11882_01/server.112/e41084/statements_1004.htm
...
redo\_log\_file\_spec Each `redo_log_file_spec` specifies a redo log file group containing one or more members (copies). If you do not specify a filename for the new log file, then Oracle Database creates Oracle Managed Files according to the rules described in the ["LOGFILE Clause"](statements_5004.md#i2160227) of `CREATE` `DATABASE`.
...
* To drop a log file in the current log, you must first issue an `ALTER` `SYSTEM` `SWITCH` `LOGFILE` statement. Refer to [ALTER SYSTEM](statements_2014.md#i2053602) for more information.
...
[standby databases](../../server.112/e41134/manage_ps.md#SBYDB00708)
...
You can no longer change the database character set or the national character set using the `ALTER` `DATABASE` statement. Refer to [Oracle Database Globalization Support Guide](../../server.112/e10729/ch11charsetmig.md#NLSPG011) for information on database character set migration.
...
This clause has the same semantics in `CREATE` `DATABASE` and `ALTER` `DATABASE` statements. When used in with `ALTER` `DATABASE`, this clause resets the time zone of the database. To determine the time zone of the database, query the built-in function [DBTIMEZONE](functions048.md#i1327612). After setting or changing the time zone with this clause, you must restart the database for the new time zone to take effect.
...
For a full description of this clause, refer to [set\_time\_zone\_clause](statements_5004.md#i2082011) in the documentation on `CREATE` `DATABASE`.

security\_clause

Use the `security_clause` (`GUARD`) to protect data in the database from being changed. You can override this setting for a current session using the `ALTER` `SESSION` `DISABLE` `GUARD` statement. Refer to [ALTER SESSION](statements_2013.md#i2231814) for more information.
