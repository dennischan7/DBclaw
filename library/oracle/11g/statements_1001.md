# Oracle 11g - statements_1001
Source: https://docs.oracle.com/cd/E11882_01/server.112/e41084/statements_1001.htm
...
The `SELECT` statement is supported in PL/SQL only when executed dynamically. However, you can use the similar PL/SQL statement [`SELECT` `INTO`](../../appdev.112/e25519/selectinto_statement.md#LNPLS01345) in PL/SQL code, and you do not have to execute it dynamically. The `CALL` and `EXPLAIN` `PLAN` statements are supported in PL/SQL only when executed dynamically. All other DML statements are fully supported in PL/SQL.
...
All transaction control statements, except certain forms of the `COMMIT` and `ROLLBACK` commands, are supported in PL/SQL. For information on the restrictions, see [COMMIT](statements_4010.md#i2060233) and [ROLLBACK](statements_9021.md#i2104635).
