# Oracle 11g - sql_elements001
Source: https://docs.oracle.com/cd/E11882_01/server.112/e41084/sql_elements001.htm
...
To ensure proper data conversion between databases with different character sets, you must ensure that `VARCHAR2` data consists of well-formed strings. See [Oracle Database Globalization Support Guide](../../server.112/e10729/ch2charset.md#NLSPG002) for more information on character set support.
...
[Table 3-2](#g196646) show how Oracle stores data using different precisions and scales.
...
You can specify floating-point numbers within the limits listed in [Table 3-3](#g195556). The format for specifying floating-point numbers is defined in ["Numeric Literals"](sql_elements003.md#i139891).
...
* The SQL function `SQRT` implements square root. See [SQRT](functions168.md#i1009289).
* The SQL function `REMAINDER` implements remainder. See [REMAINDER](functions152.md#i1300767).
...
If any implicit conversion is needed and fails, then the operation fails. Refer to [Table 3-10, "Implicit Type Conversion Matrix"](sql_elements002.md#g195937) for more information on implicit conversion.
...
Both datetimes and intervals are made up of fields. The values of these fields determine the value of the data type. [Table 3-4](#BABFDAEI) lists the datetime fields and their possible values for datetimes and intervals.
...
| `TIMEZONE_REGION` | Query the `TZNAME` column of the `V$TIMEZONE_NAMES` data dictionary view. Not applicable for `DATE` or `TIMESTAMP`. For a complete listing of all time zone region names, refer to [Oracle Database Globalization Support Guide](../../server.112/e10729/ch4datetime.md#NLSPG004). | Not applicable |
...
are specified together and interpreted as an entity in the format
...
, with values ranging from -12:59 to +14:00. Refer to

[Oracle Data Provider for .NET Developer's Guide](../../win.112/e23174/OracleTimeStampStructure.md#ODPNT0002)
...
[Table 3-5](#g196492) is a matrix of datetime arithmetic operations. Dashes represent operations that are not supported.
...
For a complete listing of the time zone region names in both files, refer to [Oracle Database Globalization Support Guide](../../server.112/e10729/applocaledata.md#NLSPG0141).
...
Oracle strongly recommends that you convert `LONG` `RAW` columns to binary LOB (`BLOB`) columns. LOB columns are subject to far fewer restrictions than `LONG` columns. See [TO\_LOB](functions205.md#i79464) for more information.
...
Restrictions on LOB Columns  LOB columns are subject to a number of rules and restrictions. See [Oracle Database SecureFiles and Large Objects Developer's Guide](../../appdev.112/e18294/adlob_working.md#ADLOB2010) for a complete listing.
...
You can change the filename and path of a `BFILE` without affecting the base table by using the `BFILENAME` function. Refer to [BFILENAME](functions019.md#i76871) for more information on this built-in SQL function.
