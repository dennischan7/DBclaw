# Oracle 11g - pseudocolumns001
Source: https://docs.oracle.com/cd/E11882_01/server.112/e41084/pseudocolumns001.htm
...
For each row returned by a hierarchical query, the `LEVEL` pseudocolumn returns 1 for a root row, 2 for a child of a root, and so on. A root row is the highest row within an inverted tree. A child row is any nonroot row. A parent row is any row that has children. A leaf row is any row without children. [Figure 2-1](#i1009270) shows the nodes of an inverted tree with their `LEVEL` values.