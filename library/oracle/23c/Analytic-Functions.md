...
As a start point, `CURRENT` `ROW` specifies that the window begins at the current row or value (depending on whether you have specified `ROW`, `RANGE`, or `GROUPS` respectively). In this case the end point cannot be `value_expr` `PRECEDING`.

As an end point, `CURRENT` `ROW` specifies that the window ends at the current row or value (depending on whether you have specified `ROW`, `RANGE`, or `GROUPS`respectively). In this case the start point cannot be `value_expr` `FOLLOWING`.

value\_expr PRECEDING or value\_expr FOLLOWING

For `RANGE`, `ROW`, or `GROUPS`:
...
Use the `QUALIFY` clause to filter the results of an analytic function that appears in a `SELECT` list or in the predicate specified within the `QUALIFY` clause.
...
* All `WHERE`, `GROUB BY`, `HAVING` clauses and analytic functions are completed before the `QUALIFY` clause is processed. The general order of operations is as follows: `FROM` -> `WHERE` -> `GROUP BY` -> `HAVING` -> `WINDOW` -> `QUALIFY` -> `DISTINCT` -> `ORDER BY` -> `FETCH FIRST`.
