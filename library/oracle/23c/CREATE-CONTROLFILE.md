# Oracle 23c - CREATE-CONTROLFILE
Source: https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/CREATE-CONTROLFILE.html
...
Use the `redo_log_file_spec` form of `file_specification` (see [file\_specification](file_specification.md#GUID-580FA726-F712-4410-90CF-783A2DA89688)) to list regular redo log files in an operating system file system or to list Oracle ASM disk group redo log files. When using a form of `ASM_filename`, you cannot specify the `autoextend_clause` of the `redo_log_file_spec`.
