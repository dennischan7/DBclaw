# Oracle 23c - INSERT
Source: https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/INSERT.html
...
You can insert rows into a remote table or view only if you are using the database distributed functionality.

To insert rows into a remote table you must have both `INSERT` and `SELECT` privileges on the table.

If you omit `dblink`, then the database assumes that the table or view is on the local database. You can insert rows into a local table with just the `INSERT` privilege.
