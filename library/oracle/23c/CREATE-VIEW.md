# Oracle 23c - CREATE-VIEW
Source: https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/CREATE-VIEW.html
...
You cannot replace a conventional view with an editioning view or an editioning view with a conventional view. See [Oracle AI Database
Development Guide](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/26/sqlrf&id=ADFNS0202) for more information on editioning views.
...
Use these clauses to specify whether the view becomes an editioned or noneditioned object if editioning is enabled for the schema object type `VIEW` in `schema`. The default is `EDITIONABLE`. For information about editioned and noneditioned objects, see [Oracle AI Database
Development Guide](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/26/sqlrf&id=ADFNS99923).
...
For more information on JSON relational duality views, fully updateable views on top of relational tables, see [CREATE JSON RELATIONAL DUALITY VIEW](create-json-relational-duality-view.md#GUID-64B579AD-BF97-4B27-BF22-94C1FB6FD6DF) .

For more on JSON collections, see [JSON Collections](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/26/sqlrf&id=ADJSN-GUID-F69A381E-7A6A-4D18-A148-B0C541C174BD) of the JSON Developer's Guide.
...
Specify the schema to contain the view. If you omit `schema`, then the database creates the view in your own schema.
...
Specify names for the expressions selected by the defining query of the view. The number of aliases must match the number of expressions selected by the view. Aliases must follow the rules for naming the database schema objects. Aliases must be unique within the view.
...
The database does not enforce view constraints. For a full discussion of view constraints, including restrictions, refer to "[View Constraints](constraint.md#GUID-1055EA97-BA6F-4764-A15F-1024FD5B6DFE__I1002565)".
...
If you omit this clause, then the default collation for the view is set to the effective schema default collation of the schema containing the view. Refer to the [DEFAULT\_COLLATION](ALTER-SESSION.md#GUID-27186B28-7EFC-4998-B1ED-2B905CC0211B__DEFAULT_COLLATION-39CF7241) clause of `ALTER` `SESSION` for more information on the effective schema default collation.
...
* The view must not contain any of the following constructs:
  ...
  + Joins, with some exceptions, as documented in [Oracle AI Database
    Administratorâs Guide](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/26/sqlrf&id=ADMIN020)
...
* If you want a join view to be updatable, then all of the following conditions must be true:
  ...
  + For a `DELETE` statement, if the join results in more than one key-preserved table, then the database deletes from the first table named in the `FROM` clause, whether or not the view was created `WITH` `CHECK` `OPTION`.
...
Restriction on the subquery\_restriction\_clause
...
Creating a View with Column and View Level Annotations: Example
