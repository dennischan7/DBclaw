# Oracle 23c - Comments
Source: https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/Comments.html

Hints are comments in a SQL statement that pass instructions to the database optimizer. The optimizer uses these hints to choose an execution plan for the statement, unless some condition exists that prevents the optimizer from doing so.
...
The database ignores hints and does not return an error under the following circumstances:
...
With 19c you can use `DBMS_XPLAN` to find out whether a hint is used or not used. For more information, see the [Database SQL Tuning Guide](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/26/sqlrf&id=TGSQL).
...
The database ignores global hints that refer to multiple query blocks. To avoid this issue, Oracle recommends that you specify the object alias in the hint instead of using `tablespec` and `indexspec`.
