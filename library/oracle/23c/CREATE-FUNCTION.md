# Oracle 23c - CREATE-FUNCTION
Source: https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/CREATE-FUNCTION.html

Purpose

Functions are defined in PL/SQL. Therefore, this section provides some general information but refers to [Oracle Database PL/SQL Language Reference](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/26/sqlrf&id=LNPLS01370) for details of syntax and semantics.
...
[`ALTER` `FUNCTION`](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/26/sqlrf&id=SQLRF00804) for information on recompiling functions using SQL
...
Use these clauses to specify whether the function is an editioned or noneditioned object if editioning is enabled for the schema object type `FUNCTION` in `schema`. The default is `EDITIONABLE`. For information about editioned and noneditioned objects, see [Oracle Database Development Guide](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/26/sqlrf&id=ADFNS99923).