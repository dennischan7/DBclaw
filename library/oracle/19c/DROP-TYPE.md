# Oracle 19c - DROP-TYPE
Source: https://docs.oracle.com/en/database/oracle/oracle-database/19/sqlrf/DROP-TYPE.html

Purpose

Object types are defined using PL/SQL. Refer to [Oracle Database PL/SQL Language Reference](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/19/sqlrf&id=LNPLS99989) for complete information on creating, altering, and dropping object types.
...
Specify `IF EXISTS` to drop an existing type.
...
The following statement removes object type `person_t`. See [Oracle Database PL/SQL Language Reference](/pls/topic/lookup?ctx=en/database/oracle/oracle-database/19/sqlrf&id=LNPLS01383) for the example that creates this object type. Any columns that are dependent on person\_t are marked `UNUSED` and become inaccessible.
