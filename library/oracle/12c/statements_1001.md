# Oracle 12c - statements_1001
Source: https://docs.oracle.com/database/121/SQLRF/statements_1001.htm
...
Many DDL statements may cause Oracle Database to recompile or reauthorize schema objects. For information on how Oracle Database recompiles and reauthorizes schema objects and the circumstances under which a DDL statement would cause this, see [Oracle Database Concepts](../CNCPT/sqllangu.md#CNCPT216).
...
["Session Control Statements"](#i2099318)

and

["System Control Statement"](#i2099335)
...
The `SELECT` statement is supported in PL/SQL only when executed dynamically. However, you can use the similar PL/SQL statement [`SELECT` `INTO`](../LNPLS/selectinto_statement.md#LNPLS01345) in PL/SQL code, and you do not have to execute it dynamically. The `CALL` and `EXPLAIN` `PLAN` statements are supported in PL/SQL only when executed dynamically. All other DML statements are fully supported in PL/SQL.
...
All transaction control statements, except certain forms of the `COMMIT` and `ROLLBACK` commands, are supported in PL/SQL. For information on the restrictions, see [COMMIT](statements_4011.md#i2060233) and [ROLLBACK](statements_9023.md#i2104635).
