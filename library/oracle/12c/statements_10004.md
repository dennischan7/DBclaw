# Oracle 12c - statements_10004
Source: https://docs.oracle.com/database/121/SQLRF/statements_10004.htm
...
* For most roles, you cannot enable or disable a role unless it was granted to you either directly or through other roles. However, a secure application role can be granted and enabled by its associated PL/SQL package. See the `CREATE` `ROLE` semantics for [USING package](statements_6014.md#BABCJFBJ) and [Oracle Database Security Guide](../DBSEG/app_devs.md#DBSEG005) for information about secure application roles.
* `SET` `ROLE` succeeds only if there are no definer's rights units on the call stack. If at least one DR unit is on the call stack, then issuing the `SET` `ROLE` command causes `ORA`-`06565`. See [Oracle Database PL/SQL Language Reference](../LNPLS/subprograms.md#LNPLS00809) for more information about definer's rights units.
* To run the `SET` `ROLE` command from PL/SQL, you must use dynamic SQL, preferably the `EXECUTE` `IMMEDIATE` statement. See [Oracle Database PL/SQL Language Reference](../LNPLS/dynamic.md#LNPLS01115) for more information about this statement.
...
* You cannot use this clause to enable a secure application role, which is a role that can be enabled only by applications using an authorized package. Refer to [Oracle Database Security Guide](../DBSEG/app_devs.md#DBSEG30399) for information on creating a secure application role and [Oracle Database 2 Day + Security Guide](../TDPSG/GUID-800F33CF-26E2-4246-86D5-7DD5B3B7B2ED.md#TDPSG30032) for a tutorial.
