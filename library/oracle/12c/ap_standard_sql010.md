# Oracle 12c - ap_standard_sql010
Source: https://docs.oracle.com/database/121/SQLRF/ap_standard_sql010.htm
...
| X025, `XMLCast` | Oracle supports this feature, with the following restrictions:   * The source expression must be of `XMLType` and the target data type may not be `XMLType`. (Since Oracle has only one XML type, there is no need to cast from XML to XML.) * The target data type may not be a domain (Oracle does not support domains). * Oracle does not support <XML passing mechanism>; the behavior is the same as `BY` `VALUE` in the standard.   Oracle extends this feature with the ability to cast to type `REF` `XMLTYPE`. |
...
| X070, `XMLSerialize`: Character string serialization and `CONTENT` option | Oracle supports this feature, with this restriction:  Oracle extends this feature as follows: the standard requires a target data type; Oracle defaults to `CLOB`. |
