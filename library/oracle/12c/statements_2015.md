# Oracle 12c - statements_2015
Source: https://docs.oracle.com/database/121/SQLRF/statements_2015.htm
...
[Oracle Database Administrator's Guide](../ADMIN/schema.md#ADMIN014)
...
CONTAINER Use this clause in a multitenant container database (CDB) to switch to the container specified by `container_name`.
...
ROW ARCHIVAL VISIBILITY Use this clause to configure row archival visibility for the session. This clause lets you implement In-Database Archiving, which allows you to designate table rows as active or archived. You can then perform queries on only the active rows within the table.
...
The `TIME_ZONE` parameter specifies the default local time zone offset or region name for the current SQL session. `TIME_ZONE` is a session parameter only, not an initialization parameter. To determine the time zone of the current session, query the built-in function `SESSIONTIMEZONE` (see [SESSIONTIMEZONE](functions176.md#i999827)).
