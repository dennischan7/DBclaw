# Oracle 12c - functions088
Source: https://docs.oracle.com/database/121/SQLRF/functions088.htm
...
`INSERTXMLBEFORE` inserts a user-supplied value into the target XML before the node indicated by the XPath expression. This function is similar to INSERTXMLAFTER, but it inserts before, not after, the target node. Compare this function with [INSERTCHILDXML](functions084.md#CIHIJEBB).
...
The following example is similar to that for [INSERTCHILDXML](functions084.md#CIHIJEBB), but it adds a third `/Owner` node before the `/Owner` node added in the other example. The output of the query has been formatted for readability.
