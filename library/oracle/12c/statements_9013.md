# Oracle 12c - statements_9013
Source: https://docs.oracle.com/database/121/SQLRF/statements_9013.htm
...
Oracle strongly recommends that you run your database in automatic undo mode by leaving the
...
, which is the default. In addition, set the

`UNDO_RETENTION`

initialization parameter to an interval large enough to include the oldest data you anticipate needing. For more information refer to the documentation on the

[`UNDO_MANAGEMENT`](../REFRN/GUID-5DD8D2CD-F0DD-40F9-8CE6-EB84CC006ED6.md#REFRN10224)

and

[`UNDO_RETENTION`](../REFRN/GUID-D238E854-A2C5-463E-A146-31F61300124E.md#REFRN10225)
...
Row movement must be enabled for all tables in the Flashback list unless you are flashing back the table `TO` `BEFORE` `DROP`. That operation is called a flashback drop operation, and it uses dropped data in the recycle bin rather than undo data. Refer to [row\_movement\_clause](statements_3001.md#i2192391) for information on enabling row movement.
...
statement, wait a minimum of 5 minutes prior to issuing the following statement. This wait would not be necessary if a previously existing table were being used in this example.
