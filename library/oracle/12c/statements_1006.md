# Oracle 12c - statements_1006
Source: https://docs.oracle.com/database/121/SQLRF/statements_1006.htm

Semantics

Notes on Using ALTER DATABASE in a CDB When you issue the `ALTER` `DATABASE` statement while connected to a CDB, the behavior of the statement depends on the current container and the clause(s) you specify.
...
If the current container is a PDB, then `ALTER` `DATABASE` statements modify that PDB. In this case, you can issue only `ALTER` `DATABASE` clauses that are also supported by the `ALTER` `PLUGGABLE` `DATABASE` statement. This functionality is provided to maintain backward compatibility for applications that have been migrated to a CDB environment. The exception is modifying PDB storage limits, for which you must use the `pdb_storage_clause` of `ALTER` `PLUGGABLE` `DATABASE`. Refer to the documentation on [ALTER PLUGGABLE DATABASE](statements_2008.md#CCHJFGJD) for complete information on these clauses.

database

Specify the name of the database to be altered. If you omit `database`, then Oracle Database alters the database identified by the value of the initialization parameter `DB_NAME`. You can alter only the database whose control files are specified by the initialization parameter `CONTROL_FILES`. The database identifier is not related to the Oracle Net database specification.
...
Notes on Using the recovery\_clauses in a CDB When the current container is the root, you can specify all of the `recovery_clauses` to back up and recover the entire CDB.
...
You can also specify the preceding subclauses using the `pdb_recovery_clauses` of `ALTER` `PLUGGABLE` `DATABASE`. Refer to the syntax diagram [pdb\_recovery\_clauses::=](statements_2008.md#CCHDJFHH) of `ALTER` `PLUGGABLE` `DATABASE`.
...
Parallelism is enabled by default during full or partial database recovery and logfile recovery. The database computes the degree of parallelism. You can disable parallelism of these operations by specifying

`NOPARALLEL`

, or specify a degree of parallelism with

`PARALLEL` `integer`

, as shown in the respective syntax diagrams.
...
Beginning with Oracle Database 12
...
is enabled by default during Redo Apply. Real-time apply recovers redo from the standby redo log files as soon as they are written, without requiring them to be archived first at the physical standby database. You can disable real-time apply with the
...
Parallelism is enabled by default during Redo Apply. The database computes the degree of parallelism. You can disable parallelism of these operations by specifying
...
Restrictions on Managed Standby Recovery The same restrictions listed under [general\_recovery](#i2208874) apply to this clause.
...
[ALTER TABLESPACE](statements_3002.md#i2093894)

for information on moving all data files in an individual tablespace into and out of online backup mode
...
* To take an individual data file out of online backup mode, use the `ALTER` `DATABASE` `DATAFILE` ... `END` `BACKUP` statement. See [database\_file\_clauses](#i2078824) .
...
END BACKUP Specify `END` `BACKUP` to take the data file out of online backup mode. The `END` `BACKUP` clause is described more fully at the top level of the syntax of `ALTER` `DATABASE`. See ["END BACKUP Clause"](#i2150184).
...
You can specify the original data file using the `file_name`, `ASM_filename`, or `file_number`. Refer to [ASM\_filename](clauses004.md#i1026664) for information on ASM file names. If you identify the file by number, then `file_number` is an integer representing the number found in the `FILE#` column of the `V$DATAFILE` dynamic performance view or in the `FILE_ID` column of the `DBA_DATA_FILES` data dictionary view.
...
Use the `autoextend_clause` to enable or disable the automatic extension of a new or existing data file or temp file. Refer to [file\_specification](clauses004.md#g1057086) for information about this clause.
...
This clause has the same function for logfiles that it has for data files and temp files. See ["RENAME FILE Clause"](#i2162839).
...
* You must specify `UNARCHIVED` if you want to reuse a redo log that was not archived.
  ...
  makes backups unusable if the redo log is needed for recovery.
...
ADD LOGFILE Clause Use the `ADD` `LOGFILE` clause to add one or more redo log file groups to the online redo log or standby redo log.
...
redo\_log\_file\_spec Each `redo_log_file_spec` specifies a redo log file group containing one or more members (copies). If you do not specify a filename for the new log file, then Oracle Database creates Oracle Managed Files according to the rules described in the ["LOGFILE Clause"](statements_5005.md#i2160227) of `CREATE` `DATABASE`.
...
* To drop a log file in the current log, you must first issue an `ALTER` `SYSTEM` `SWITCH` `LOGFILE` statement. Refer to [ALTER SYSTEM](statements_2017.md#i2053602) for more information.
...
For a full discussion of the `supplemental_id_clause`, refer to [supplemental\_id\_key\_clause](statements_7002.md#i2215249) in the documentation on `CREATE` `TABLE`.
...
keywords have been replaced for clarity but are still supported.
...
Specify the `REGISTER` `LOGFILE` clause from the standby database to manually register log files from the failed primary. Use the `redo_log_file_spec` form of `file_specification` (see [file\_specification](clauses004.md#g1057086)) to list regular redo log files in an operating system file system or to list Oracle ASM disk group redo log files.
...
[standby databases](../SBYDB/manage_ps.md#SBYDB00708)
...
You can no longer change the database character set or the national character set using the `ALTER` `DATABASE` statement. Refer to [Oracle Database Globalization Support Guide](../NLSPG/ch11charsetmig.md#NLSPG011) for information on database character set migration.
...
Use this clause to enable or disable the force full database caching mode. In contrast to the default mode, which is automatic, the force full database caching mode considers the entire database, including `NOCACHE` LOBs, as eligible for caching in the buffer cache. This mode is available starting with Oracle Database 12c Release 1 (12.1.0.2).
...
This clause has the same semantics in `CREATE` `DATABASE` and `ALTER` `DATABASE` statements. When used in with `ALTER` `DATABASE`, this clause resets the time zone of the database. To determine the time zone of the database, query the built-in function [DBTIMEZONE](functions056.md#i1327612). After setting or changing the time zone with this clause, you must restart the database for the new time zone to take effect.
...
For a full description of this clause, refer to [set\_time\_zone\_clause](statements_5005.md#i2082011) in the documentation on `CREATE` `DATABASE`.

security\_clause

Use the `security_clause` (`GUARD`) to protect data in the database from being changed. You can override this setting for a current session using the `ALTER` `SESSION` `DISABLE` `GUARD` statement. Refer to [ALTER SESSION](statements_2015.md#i2231814) for more information.
