...
See [Variable Substitution](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+VariableSubstitution "/confluence/display/Hive/LanguageManual+VariableSubstitution") for examples of using the `hiveconf` option.
...
* Example of running a script non-interactively from local disk
  ...
  ```
* Example of running a script non-interactively from a Hadoop supported filesystem (starting in [Hive 0.14](https://issues.apache.org/jira/browse/HIVE-7136 "https://issues.apache.org/jira/browse/HIVE-7136"))
...
See [Hive Logging in Getting Started](https://cwiki.apache.org/confluence/display/Hive/GettingStarted#GettingStarted-HiveLogging "/confluence/display/Hive/GettingStarted#GettingStarted-HiveLogging") for more information.

### Tool to Clear Dangling Scratch Directories

See [Scratch Directory Management](https://cwiki.apache.org/confluence/display/Hive/Setting+Up+HiveServer2#SettingUpHiveServer2-ScratchDirectoryManagement "/confluence/display/Hive/Setting+Up+HiveServer2#SettingUpHiveServer2-ScratchDirectoryManagement") in Setting Up HiveServer2 for information about scratch directories and a command-line [tool for removing dangling scratch directories](https://cwiki.apache.org/confluence/display/Hive/Setting+Up+HiveServer2#SettingUpHiveServer2-ClearDanglingScratchDirTool "/confluence/display/Hive/Setting+Up+HiveServer2#SettingUpHiveServer2-ClearDanglingScratchDirTool") that can be used in the Hive CLI as well as HiveServer2.
...
See [HIVE-7136](https://issues.apache.org/jira/browse/HIVE-7136 "https://issues.apache.org/jira/browse/HIVE-7136") for more details.
...
| reset | Resets the configuration to the default values (as of Hive 0.10: see [HIVE-3202](https://issues.apache.org/jira/browse/HIVE-3202 "https://issues.apache.org/jira/browse/HIVE-3202")). |
...
| add FILE[S] <filepath> <filepath>\*   add JAR[S] <filepath> <filepath>\*   add ARCHIVE[S] <filepath> <filepath>\* | Adds one or more files, jars, or archives to the list of resources in the distributed cache. See [Hive Resources](#LanguageManualCli-HiveResources "#LanguageManualCli-HiveResources") below for more information. |
| add FILE[S] <ivyurl> <ivyurl>\*   add JAR[S] <ivyurl> <ivyurl>\*   add ARCHIVE[S] <ivyurl> <ivyurl>\* | As of [Hive 1.2.0](https://issues.apache.org/jira/browse/HIVE-9664 "https://issues.apache.org/jira/browse/HIVE-9664"), adds one or more files, jars or archives to the list of resources in the distributed cache using an [Ivy](http://ant.apache.org/ivy/ "http://ant.apache.org/ivy/") URL of the form ivy://group:module:version?query\_string. See [Hive Resources](#LanguageManualCli-HiveResources "#LanguageManualCli-HiveResources") below for more information. |
| list FILE[S]   list JAR[S]   list ARCHIVE[S] | Lists the resources already added to the distributed cache. See [Hive Resources](#LanguageManualCli-HiveResources "#LanguageManualCli-HiveResources") below for more information. |
| list FILE[S] <filepath>\*   list JAR[S] <filepath>\*   list ARCHIVE[S] <filepath>\* | Checks whether the given resources are already added to the distributed cache or not. See [Hive Resources](#LanguageManualCli-HiveResources "#LanguageManualCli-HiveResources") below for more information. |
| delete FILE[S] <filepath>\*   delete JAR[S] <filepath>\*   delete ARCHIVE[S] <filepath>\* | Removes the resource(s) from the distributed cache. |
| delete FILE[S] <ivyurl> <ivyurl>\*   delete JAR[S] <ivyurl> <ivyurl>\*   delete ARCHIVE[S] <ivyurl> <ivyurl>\* | As of [Hive 1.2.0](https://issues.apache.org/jira/browse/HIVE-9664 "https://issues.apache.org/jira/browse/HIVE-9664"), removes the resource(s) which were added using the <ivyurl> from the distributed cache. See [Hive Resources](#LanguageManualCli-HiveResources "#LanguageManualCli-HiveResources") below for more information. |
...
* JAR resources are also added to the Java classpath. This is required in order to reference objects they contain such as UDFs. See [Hive Plugins](https://cwiki.apache.org/confluence/display/Hive/HivePlugins "/confluence/display/Hive/HivePlugins") for more information about custom UDFs.
...
As of [Hive 1.2.0](https://issues.apache.org/jira/browse/HIVE-9664 "https://issues.apache.org/jira/browse/HIVE-9664"), resources can be added and deleted using [Ivy](http://ant.apache.org/ivy/ "http://ant.apache.org/ivy/") URLs of the form ivy://group:module:version?query\_string.
...
See [HIVE-9664](https://issues.apache.org/jira/browse/HIVE-9664?focusedCommentId=14338279&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-14338279 "https://issues.apache.org/jira/browse/HIVE-9664?focusedCommentId=14338279&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-14338279") for more details.
...
Many (but not all) `hcat` commands can be issued as `hive` commands, and vice versa. See the HCatalog [Command Line Interface](https://cwiki.apache.org/confluence/display/Hive/HCatalog+CLI "/confluence/display/Hive/HCatalog+CLI") document in the [HCatalog manual](https://cwiki.apache.org/confluence/display/Hive/HCatalog "/confluence/display/Hive/HCatalog") for more information.