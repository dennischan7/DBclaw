...
Introduced in Hive version [0.11.0](https://issues.apache.org/jira/browse/HIVE-3874 "https://issues.apache.org/jira/browse/HIVE-3874").

The *Optimized Row Columnar* ([ORC](https://orc.apache.org "https://orc.apache.org")) file format provides a highly efficient way to store Hive data. It was designed to overcome limitations of the other Hive file formats. Using ORC files improves performance when Hive is reading, writing, and processing data.
...
A complete specification of the format is given in the [ORC specification](#LanguageManualORC-orc-spec "#LanguageManualORC-orc-spec").
...
The parameters are all placed in the TBLPROPERTIES (see [Create Table](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+DDL#LanguageManualDDL-CreateTable "/confluence/display/Hive/LanguageManual+DDL#LanguageManualDDL-CreateTable")). They are:
...
Specifying `-d` in the command will cause it to dump the ORC file data rather than the metadata (Hive [1.1.0](https://issues.apache.org/jira/browse/HIVE-7896 "https://issues.apache.org/jira/browse/HIVE-7896") and later).

Specifying `--rowindex` with a comma separated list of column ids will cause it to print [row indexes](#LanguageManualORC-StripeStructure "#LanguageManualORC-StripeStructure") for the specified columns, where 0 is the top level struct containing all of the columns and 1 is the first column id (Hive [1.1.0](https://issues.apache.org/jira/browse/HIVE-7896 "https://issues.apache.org/jira/browse/HIVE-7896") and later).
...
*<location-of-orc-file-or-directory>* is the URI of the ORC file or directory. From [Hive 1.3.0](https://issues.apache.org/jira/browse/HIVE-11669 "https://issues.apache.org/jira/browse/HIVE-11669") onward, this URI can be a directory containing ORC files.

## ORC Configuration Parameters

The ORC configuration parameters are described in [Hive Configuration Properties – ORC File Format](https://cwiki.apache.org/confluence/display/Hive/Configuration+Properties#ConfigurationProperties-ORCFileFormat "/confluence/display/Hive/Configuration+Properties#ConfigurationProperties-ORCFileFormat").

# ORC Format Specification

The ORC specification has moved to [ORC project](https://orc.apache.org/specification/ "https://orc.apache.org/specification/").