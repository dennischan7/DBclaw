...
- [ndb-applier-allow-skip-epoch](#page-175-0): Lets replication applier skip epochs.
- [ndb-batch-size](#page-175-1): Size (in bytes) to use for NDB transaction batches.
- [ndb-blob-read-batch-bytes](#page-177-0): Specifies size in bytes that large BLOB reads should be batched into. 0 = no limit.
- [ndb-blob-write-batch-bytes](#page-177-1): Specifies size in bytes that large BLOB writes should be batched into. 0 = no limit.
- [ndb-cluster-connection-pool](#page-176-0): Number of connections to cluster used by MySQL.
- [ndb-cluster-connection-pool-nodeids](#page-176-1): Comma-separated list of node IDs for connections to cluster used by MySQL; number of nodes in list must match value set for --ndb-cluster-connectionpool.
- [ndb-connectstring](#page-178-0): Address of NDB management server distributing configuration information for this cluster.
- [ndb-default-column-format](#page-178-1): Use this value (FIXED or DYNAMIC) by default for COLUMN\_FORMAT and ROW\_FORMAT options when creating or adding table columns.
- [ndb-deferred-constraints](#page-178-2): Specifies that constraint checks on unique indexes (where these are supported) should be deferred until commit time. Not normally needed or used; for testing purposes only.

- [ndb-distribution](#page-179-0): Default distribution for new tables in NDBCLUSTER (KEYHASH or LINHASH, default is KEYHASH).
- [ndb-log-apply-status](#page-179-1): Cause MySQL server acting as replica to log mysql.ndb\_apply\_status updates received from its immediate source in its own binary log, using its own server ID. Effective only if server is started with --ndbcluster option.
- [ndb-log-empty-epochs](#page-180-0): When enabled, causes epochs in which there were no changes to be written to ndb\_apply\_status and ndb\_binlog\_index tables, even when --log-slave-updates is enabled.
- [ndb-log-empty-update](#page-180-1): When enabled, causes updates that produced no changes to be written to ndb\_apply\_status and ndb\_binlog\_index tables, even when --log-slave-updates is enabled.
- [ndb-log-exclusive-reads](#page-181-0): Log primary key reads with exclusive locks; allow conflict resolution based on read conflicts.
- [ndb-log-fail-terminate](#page-181-1): Terminate mysqld process if complete logging of all found row events is not possible.
- [ndb-log-orig](#page-181-2): Log originating server id and epoch in mysql.ndb\_binlog\_index table.
- [ndb-log-transaction-dependency](#page-182-0): Make binary log thread calculate transaction dependencies for every transaction it writes to binary log.
- [ndb-log-transaction-id](#page-182-1): Write NDB transaction IDs in binary log. Requires --log-bin-v1 events=OFF.
- [ndb-log-update-minimal](#page-183-0): Log updates in minimal format.
- [ndb-log-updated-only](#page-183-1): Log updates only (ON) or complete rows (OFF).
- [ndb-log-update-as-write](#page-182-2): Toggles logging of updates on source between updates (OFF) and writes (ON).
- [ndb-mgmd-host](#page-184-0): Set host (and port, if desired) for connecting to management server.
- [ndb-nodeid](#page-184-1): NDB Cluster node ID for this MySQL server.
- [ndb-optimized-node-selection](#page-185-0): Enable optimizations for selection of nodes for transactions. Enabled by default; use --skip-ndb-optimized-node-selection to disable.
- [ndb-transid-mysql-connection-map](#page-186-0): Enable or disable ndb\_transid\_mysql\_connection\_map plugin; that is, enable or disable INFORMATION\_SCHEMA table having that name.
- [ndb-wait-connected](#page-186-1): Time (in seconds) for MySQL server to wait for connection to cluster management and data nodes before accepting MySQL client connections.
- [ndb-wait-setup](#page-186-2): Time (in seconds) for MySQL server to wait for NDB engine setup to complete.
- [ndb-allow-copying-alter-table](#page-175-2): Set to OFF to keep ALTER TABLE from using copying operations on NDB tables.
...
- [ndb\\_autoincrement\\_prefetch\\_sz](#page-187-1): NDB auto-increment prefetch size.
- [ndb\\_clear\\_apply\\_status](#page-187-2): Causes RESET SLAVE/RESET REPLICA to clear all rows from ndb\_apply\_status table; ON by default.
...
- [ndb\\_conflict\\_role](#page-188-0): Role for replica to play in conflict detection and resolution. Value is one of PRIMARY, SECONDARY, PASS, or NONE (default). Can be changed only when replication SQL thread is stopped. See documentation for further information.
...
- [ndb\\_data\\_node\\_neighbour](#page-188-1): Specifies cluster data node "closest" to this MySQL Server, for transaction hinting and fully replicated tables.
- [ndb\\_default\\_column\\_format](#page-189-0): Sets default row format and column format (FIXED or DYNAMIC) used for new NDB tables.
- [ndb\\_deferred\\_constraints](#page-189-1): Specifies that constraint checks should be deferred (where these are supported). Not normally needed or used; for testing purposes only.
- [ndb\\_dbg\\_check\\_shares](#page-189-2): Check for any lingering shares (debug builds only).
- [ndb-schema-dist-timeout](#page-179-2): How long to wait before detecting timeout during schema distribution.
- [ndb\\_distribution](#page-190-0): Default distribution for new tables in NDBCLUSTER (KEYHASH or LINHASH, default is KEYHASH).
...
- [ndb\\_eventbuffer\\_free\\_percent](#page-190-1): Percentage of free memory that should be available in event buffer before resumption of buffering, after reaching limit set by ndb\_eventbuffer\_max\_alloc.
- [ndb\\_eventbuffer\\_max\\_alloc](#page-190-2): Maximum memory that can be allocated for buffering events by NDB API. Defaults to 0 (no limit).
- Ndb\_execute\_count: Number of round trips to NDB kernel made by operations.
- [ndb\\_extra\\_logging](#page-191-0): Controls logging of NDB Cluster schema, connection, and data distribution events in MySQL error log.
- Ndb\_fetch\_table\_stats: Number of times table statistics were fetched from tables rather than cache.
- [ndb\\_force\\_send](#page-191-1): Forces sending of buffers to NDB immediately, without waiting for other threads.
- [ndb\\_fully\\_replicated](#page-191-2): Whether new NDB tables are fully replicated.
- [ndb\\_index\\_stat\\_enable](#page-192-0): Use NDB index statistics in query optimization.
- [ndb\\_index\\_stat\\_option](#page-192-1): Comma-separated list of tunable options for NDB index statistics; list should contain no spaces.
- [ndb\\_join\\_pushdown](#page-194-0): Enables pushing down of joins to data nodes.
...
- [ndb\\_log\\_apply\\_status](#page-196-1): Whether or not MySQL server acting as replica logs mysql.ndb\_apply\_status updates received from its immediate source in its own binary log, using its own server ID.
- [ndb\\_log\\_bin](#page-196-0): Write updates to NDB tables in binary log. Effective only if binary logging is enabled with --log-bin.
- [ndb\\_log\\_binlog\\_index](#page-196-2): Insert mapping between epochs and binary log positions into ndb\_binlog\_index table. Defaults to ON. Effective only if binary logging is enabled.
- [ndb\\_log\\_cache\\_size](#page-196-3): Set size of transaction cache used for recording NDB binary log.
- [ndb\\_log\\_empty\\_epochs](#page-197-0): When enabled, epochs in which there were no changes are written to ndb\_apply\_status and ndb\_binlog\_index tables, even when log\_replica\_updates or log\_slave\_updates is enabled.

- [ndb\\_log\\_empty\\_update](#page-197-1): When enabled, updates which produce no changes are written to ndb\_apply\_status and ndb\_binlog\_index tables, even when log\_replica\_updates or log\_slave\_updates is enabled.
- [ndb\\_log\\_exclusive\\_reads](#page-197-2): Log primary key reads with exclusive locks; allow conflict resolution based on read conflicts.
- [ndb\\_log\\_orig](#page-197-3): Whether id and epoch of originating server are recorded in mysql.ndb\_binlog\_index table. Set using --ndb-log-orig option when starting mysqld.
- [ndb\\_log\\_transaction\\_id](#page-198-0): Whether NDB transaction IDs are written into binary log (Read-only).
- [ndb\\_log\\_transaction\\_compression](#page-198-1): Whether to compress NDB binary log; can also be enabled on startup by enabling --binlog-transaction-compression option.
- [ndb\\_log\\_transaction\\_compression\\_level\\_zstd](#page-199-0): The ZSTD compression level to use when writing compressed transactions to the NDB binary log.
- [ndb\\_metadata\\_check](#page-199-1): Enable auto-detection of NDB metadata changes with respect to MySQL data dictionary; enabled by default.
- Ndb\_metadata\_blacklist\_size: Number of NDB metadata objects that NDB binlog thread has failed to synchronize; renamed in NDB 8.0.22 as Ndb\_metadata\_excluded\_count.
...
- [ndb-optimization-delay](#page-185-1): Number of milliseconds to wait between processing sets of rows by OPTIMIZE TABLE on NDB tables.
...
- [ndbcluster](#page-174-0): Enable NDB Cluster (if this version of MySQL supports it). Disabled by [--skip](#page-187-3)[ndbcluster](#page-187-3).
- [ndbinfo](#page-185-2): Enable ndbinfo plugin, if supported.
...
- [skip-ndbcluster](#page-187-3): Disable NDB Cluster storage engine.
...
# <span id="page-46-0"></span>**25.4.3 NDB Cluster Configuration Files**
