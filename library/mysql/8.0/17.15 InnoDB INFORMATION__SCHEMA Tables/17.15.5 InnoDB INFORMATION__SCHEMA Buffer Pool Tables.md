...
title: 17.15.5 InnoDB INFORMATION__SCHEMA Buffer Pool Tables
---
...
For more information about SHOW ENGINE INNODB STATUS output, see [Section 17.17.3, "InnoDB](#page-118-0) [Standard Monitor and Lock Monitor Output"](#page-118-0).
...
## <span id="page-98-0"></span>**17.15.6 InnoDB INFORMATION\_SCHEMA Metrics Table**
...
• [innodb\\_monitor\\_enable](#page-47-1): Enables counters.

```
SET GLOBAL innodb_monitor_enable = [counter-name|module_name|pattern|all];
```

• [innodb\\_monitor\\_disable](#page-47-0): Disables counters.

```
SET GLOBAL innodb_monitor_disable = [counter-name|module_name|pattern|all];
```

• [innodb\\_monitor\\_reset](#page-47-2): Resets counter values to zero.

```
SET GLOBAL innodb_monitor_reset = [counter-name|module_name|pattern|all];
```

• [innodb\\_monitor\\_reset\\_all](#page-48-0): Resets all counter values. A counter must be disabled before using [innodb\\_monitor\\_reset\\_all](#page-48-0).
...
|---------------------------------------------------------------------------------------------|--------------------------------|------------|
| ++++<br>  name<br>++++                                                                      | subsystem                      | status<br> |
...
Module names can be used with [innodb\\_monitor\\_enable](#page-47-1) and related variables.
...
8. After the counter is disabled, you can reset all counter values using the [innodb\\_monitor\\_reset\\_all](#page-48-0) option. All values are set to zero or NULL.
