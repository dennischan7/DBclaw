...
Use the group\_replication\_get\_write\_concurrency() function to inspect a group's event horizon value at runtime by issuing:
...
Use the group\_replication\_set\_write\_concurrency() function to set the maximum number of consensus instances that the system can execute in parallel by issuing:

```
SELECT group_replication_set_write_concurrency(instances);
```

where instances is the new maximum number of consensus instances. The GROUP\_REPLICATION\_ADMIN privilege is required to use this function.

# <span id="page-123-0"></span>**20.5.1.4 Setting a Group's Communication Protocol Version**

From MySQL 8.0.16, Group Replication has the concept of a communication protocol for the group. The Group Replication communication protocol version can be managed explicitly, and set to accommodate the oldest MySQL Server version that you want the group to support. This enables

groups to be formed from members at different MySQL Server versions while ensuring backward compatibility.

- Versions from MySQL 5.7.14 allow compression of messages (see [Section 20.7.4, "Message](#page-172-0) [Compression"](#page-172-0)).
- Versions from MySQL 8.0.16 also allow fragmentation of messages (see [Section 20.7.5, "Message](#page-174-0) [Fragmentation"\)](#page-174-0).
- Versions from MySQL 8.0.27 also allow the group communication engine to operate with a single consensus leader when the group is in single-primary mode and group\_replication\_paxos\_single\_leader is set to true (see [Section 20.7.3, "Single](#page-171-0) [Consensus Leader"](#page-171-0)).
...
You can inspect the communication protocol in use by a group by using the group\_replication\_get\_communication\_protocol() function, which returns the oldest MySQL Server version that the group supports. All existing members of the group return the same communication protocol version. For example:
