...
The following table shows the plugin and library file names. The file name suffix might differ on your system. The file must be located in the directory named by the plugin\_dir system variable. For installation information, see [Installing Kerberos Pluggable Authentication](#page-85-0).
...
- [Prerequisites for Kerberos Pluggable Authentication](#page-84-0)
- [How Kerberos Authentication of MySQL Users Works](#page-84-1)
- [Installing Kerberos Pluggable Authentication](#page-85-0)
- [Using Kerberos Pluggable Authentication](#page-87-0)
- [Kerberos Authentication Debugging](#page-94-1)

For general information about pluggable authentication in MySQL, see Section 8.2.17, "Pluggable Authentication".

## <span id="page-84-0"></span>**Prerequisites for Kerberos Pluggable Authentication**
...
## <span id="page-84-1"></span>**How Kerberos Authentication of MySQL Users Works**

This section provides an overview of how MySQL and Kerberos work together to authenticate MySQL users. For examples showing how to set up MySQL accounts to use the Kerberos authentication plugins, see [Using Kerberos Pluggable Authentication](#page-87-0).
...
- 1. The MySQL server sends to the client application its service principal name. This SPN must be registered in the Kerberos system, and is configured on the server side using the [authentication\\_kerberos\\_service\\_principal](#page-109-1) system variable.
...
- On the client host, a client keytab file may be used to obtain a TGT and ST without supplying a password. See [Client Configuration Parameters for Kerberos Authentication](#page-93-0).
- On the MySQL server host, a server-side service keytab file is used to verify service tickets received by the MySQL server from clients. The keytab file name is configured using the [authentication\\_kerberos\\_service\\_key\\_tab](#page-108-0) system variable.
...
## <span id="page-85-0"></span>**Installing Kerberos Pluggable Authentication**
...
- [authentication\\_kerberos\\_service\\_principal](#page-109-1): The MySQL service principal name (SPN). This name is sent to clients that attempt to authenticate using Kerberos. The SPN must be present in the database managed by the KDC server. The default is mysql/host\_name@realm\_name.
- [authentication\\_kerberos\\_service\\_key\\_tab](#page-108-0): The keytab file for authenticating tickets received from clients. This file must exist and contain a valid key for the SPN or authentication of clients will fail. The default is mysql.keytab in the data directory.

For details about all Kerberos authentication system variables, see [Section 8.4.1.13, "Pluggable](#page-107-0) [Authentication System Variables"](#page-107-0).
...
The [authentication\\_kerberos\\_service\\_principal](#page-109-1) system variable can be set and persisted at runtime without restarting the server, by using a SET PERSIST statement:
...
To associate MySQL accounts with the Kerberos plugin, see [Using Kerberos Pluggable Authentication](#page-87-0).

## <span id="page-87-0"></span>**Using Kerberos Pluggable Authentication**

This section describes how to enable MySQL accounts to connect to the MySQL server using Kerberos pluggable authentication. It is assumed that the server is running with the server-side plugin enabled, as described in [Installing Kerberos Pluggable Authentication](#page-85-0), and that the client-side plugin is available on the client host.

- [Verify Kerberos Availability](#page-87-1)
- [Create a MySQL Account That Uses Kerberos Authentication](#page-88-0)
- [Use the MySQL Account to Connect to the MySQL Server](#page-89-0)
- [Client Configuration Parameters for Kerberos Authentication](#page-93-0)

#### <span id="page-87-1"></span>**Verify Kerberos Availability**
...
## <span id="page-88-0"></span>**Create a MySQL Account That Uses Kerberos Authentication**
...
Assume that the Kerberos user principal name has been registered by the appropriate service administrator, and that, as previously described in [Installing Kerberos Pluggable Authentication](#page-85-0), the MySQL server has been started with appropriate configuration settings for the server-side Kerberos plugin. To create a MySQL account that corresponds to a Kerberos UPN of user@realm\_name, the MySQL DBA uses a statement like this:
...
### <span id="page-89-0"></span>**Use the MySQL Account to Connect to the MySQL Server**
...
When the client-side Kerberos authentication plugin itself obtains the TGT, the client user may not want the TGT to be reused. As described in [Client](#page-93-0) [Configuration Parameters for Kerberos Authentication](#page-93-0), the local /etc/ krb5.conf file can be used to cause the client-side plugin to destroy the TGT when done with it.
...
- The destroy\_tickets parameter, described in [Client Configuration Parameters for Kerberos](#page-93-0) [Authentication](#page-93-0), is not supported because the MIT Kerberos library on Windows does not support the required API member (get\_profile\_boolean) to read its value from configuration file.

## <span id="page-93-0"></span>**Client Configuration Parameters for Kerberos Authentication**
...
## <span id="page-94-1"></span>**Kerberos Authentication Debugging**
...
# <span id="page-94-0"></span>**8.4.1.9 No-Login Pluggable Authentication**
...
- [Installing No-Login Pluggable Authentication](#page-95-0)
- [Uninstalling No-Login Pluggable Authentication](#page-96-0)
- [Using No-Login Pluggable Authentication](#page-96-1)
...
## <span id="page-95-0"></span>**Installing No-Login Pluggable Authentication**
...
To associate MySQL accounts with the no-login plugin, see [Using No-Login Pluggable Authentication](#page-96-1).

## <span id="page-96-0"></span>**Uninstalling No-Login Pluggable Authentication**
...
# <span id="page-96-1"></span>**Using No-Login Pluggable Authentication**

This section describes how to use the no-login authentication plugin to prevent accounts from being used for connecting from MySQL client programs to the server. It is assumed that the server is running with the no-login plugin enabled, as described in [Installing No-Login Pluggable Authentication](#page-95-0).
...
The mysql\_no\_login plugin is also useful in proxying scenarios. (For a discussion of concepts involved in proxying, see Section 8.2.19, "Proxy Users".) An account that authenticates using mysql\_no\_login may be used as a proxied user for proxy accounts:
