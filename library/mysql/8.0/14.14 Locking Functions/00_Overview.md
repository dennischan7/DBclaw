...
<span id="page-55-1"></span>• [GET\\_LOCK\(](#page-55-1)str,timeout)
...
A lock obtained with [GET\\_LOCK\(\)](#page-55-1) is released explicitly by executing [RELEASE\\_LOCK\(\)](#page-57-3) or implicitly when your session terminates (either normally or abnormally). Locks obtained with [GET\\_LOCK\(\)](#page-55-1) are not released when transactions commit or roll back.

[GET\\_LOCK\(\)](#page-55-1) is implemented using the metadata locking (MDL) subsystem. Multiple simultaneous locks can be acquired and [GET\\_LOCK\(\)](#page-55-1) does not release any existing locks. For example, suppose that you execute these statements:
...
The second [GET\\_LOCK\(\)](#page-55-1) acquires a second lock and both [RELEASE\\_LOCK\(\)](#page-57-3) calls return 1 (success).
...
[GET\\_LOCK\(\)](#page-55-1) is unsafe for statement-based replication. A warning is logged if you use this function when binlog\_format is set to STATEMENT.
...
A different locking interface is available as either a plugin service or a set of loadable functions. This interface provides lock namespaces and distinct read and write locks, unlike the interface provided by [GET\\_LOCK\(\)](#page-55-1) and related functions. For details, see Section 7.6.9.1, "The Locking Service".

<span id="page-57-0"></span>• [IS\\_FREE\\_LOCK\(](#page-57-0)str)
...
<span id="page-57-1"></span>• [IS\\_USED\\_LOCK\(](#page-57-1)str)
...
<span id="page-57-2"></span>• [RELEASE\\_ALL\\_LOCKS\(\)](#page-57-2)

Releases all named locks held by the current session and returns the number of locks released (0 if there were none)

This function is unsafe for statement-based replication. A warning is logged if you use this function when binlog\_format is set to STATEMENT.

<span id="page-57-3"></span>• [RELEASE\\_LOCK\(](#page-57-3)str)

Releases the lock named by the string str that was obtained with [GET\\_LOCK\(\)](#page-55-1). Returns 1 if the lock was released, 0 if the lock was not established by this thread (in which case the lock is not released), and NULL if the named lock did not exist. The lock does not exist if it was never obtained by a call to [GET\\_LOCK\(\)](#page-55-1) or if it has previously been released.

The DO statement is convenient to use with [RELEASE\\_LOCK\(\)](#page-57-3). See Section 15.2.3, "DO Statement".
