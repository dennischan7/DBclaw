...
row has a Host value 'localhost' that is more specific than '%', it is used in preference to the new row when connecting from localhost! The correct procedure is to insert a second row with Host='localhost' and User='some\_user', or to delete the row with Host='localhost' and User=''. After deleting the row, remember to issue a FLUSH PRIVILEGES statement to reload the grant tables. See also Section 8.2.6, "Access Control, Stage 1: Connection Verification".
...
- If you have any other problems with the MySQL grant tables and ask on the [MySQL Community](https://mysqlcommunity.slack.com/) [Slack](https://mysqlcommunity.slack.com/), always provide a dump of the MySQL grant tables. You can dump the tables with the mysqldump mysql command. To file a bug report, see the instructions at Section 1.5, "How to Report Bugs or Problems". In some cases, you may need to restart mysqld with --skip-granttables to run mysqldump.