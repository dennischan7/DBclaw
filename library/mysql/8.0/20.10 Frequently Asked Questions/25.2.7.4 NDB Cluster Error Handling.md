...
title: 25.2.7.4 NDB Cluster Error Handling
---
...
See also [Section 25.2.7.2, "Limits and Differences of NDB Cluster from Standard MySQL Limits".](#page-181-0)

## <span id="page-185-0"></span>**25.2.7.5 Limits Associated with Database Objects in NDB Cluster**

Some database objects such as tables and indexes have different limitations when using the [NDBCLUSTER](#page-128-0) storage engine:

- **Number of database objects.** The maximum number of all [NDB](#page-128-0) database objects in a single NDB Cluster—including databases, tables, and indexes—is limited to 20320.
...
## <span id="page-185-1"></span>**25.2.7.6 Unsupported or Missing Features in NDB Cluster**

A number of features supported by other storage engines are not supported for [NDB](#page-128-0) tables. Trying to use any of these features in NDB Cluster does not cause errors in or of itself; however, errors may occur in applications that expects the features to be supported or enforced. Statements referencing such features, even if effectively ignored by NDB, must be syntactically and otherwise valid.
...
See [Section 25.2.7.3, "Limits Relating to Transaction Handling in NDB Cluster"](#page-182-0), for more information relating to limitations on transaction handling in [NDB](#page-128-0).