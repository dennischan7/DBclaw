...
title: 7.6.6.1 Version Tokens Elements
---

Version Tokens is based on a plugin library that implements these elements:

- A server-side plugin named version\_tokens holds the list of version tokens associated with the server and subscribes to notifications for statement execution events. The version\_tokens plugin uses the [audit plugin API](https://dev.mysql.com/doc/extending-mysql/8.0/en/plugin-types.md#audit-plugin-type) to monitor incoming statements from clients and matches each client's session-specific version token list against the server version token list. If there is a match, the plugin lets the statement through and the server continues to process it. Otherwise, the plugin returns an error to the client and the statement fails.
- A set of loadable functions provides an SQL-level API for manipulating and inspecting the list of server version tokens maintained by the plugin. The [VERSION\\_TOKEN\\_ADMIN](#page-126-2) privilege (or the deprecated [SUPER](#page-117-0) privilege) is required to call any of the Version Token functions.
- When the version\_tokens plugin loads, it defines the [VERSION\\_TOKEN\\_ADMIN](#page-126-2) dynamic privilege. This privilege can be granted to users of the functions.
...
## <span id="page-26-0"></span>**7.6.6.2 Installing or Uninstalling Version Tokens**
...
The plugin library file base name is version\_tokens. The file name suffix differs per platform (for example, .so for Unix and Unix-like systems, .dll for Windows).
...
## <span id="page-27-0"></span>**7.6.6.3 Using Version Tokens**

Before using Version Tokens, install it according to the instructions provided at [Section 7.6.6.2,](#page-26-0) ["Installing or Uninstalling Version Tokens"](#page-26-0).
...
When Version Tokens initializes on a given server, the server's version token list is empty. Token list maintenance is performed by calling functions. The [VERSION\\_TOKEN\\_ADMIN](#page-126-2) privilege (or the deprecated [SUPER](#page-117-0) privilege) is required to call any of the Version Token functions, so token list modification is expected to be done by a management or administrative application that has that privilege.
...
To define the version token list for a server, the management application calls the [version\\_tokens\\_set\(\)](#page-34-0) function. (There are also functions for modifying and displaying the token list, described later.) For example, the application might send these statements to a group of three servers:
...
Before communicating with the servers, a client application contacts the management application and retrieves information about server assignments. Then the client selects a server based on those assignments. Suppose that a client wants to perform both reads and writes on the emp database. Based on the preceding assignments, only server 2 qualifies. The client connects to server 2 and registers its server requirements there by setting its [version\\_tokens\\_session](#page-35-0) system variable:
...
As long as the assignment of server 2 does not change, the client continues to use it for reads and writes. But suppose that the management application wants to change server assignments so that writes for the emp database must be sent to server 1 instead of server 2. To do this, it uses [version\\_tokens\\_edit\(\)](#page-33-0) to modify the emp token value on the two servers (and updates its cache of server assignments):
...
[version\\_tokens\\_edit\(\)](#page-33-0) modifies the named tokens in the server token list and leaves other tokens unchanged.
...
- Take a shared lock for each token named in the client token list (that is, in the [version\\_tokens\\_session](#page-35-0) value)
...
- [version\\_tokens\\_set\(\)](#page-34-0) completely replaces the current list and assigns a new list. The argument is a semicolon-separated list of name=value pairs.
- [version\\_tokens\\_edit\(\)](#page-33-0) enables partial modifications to the current list. It can add new tokens or change the values of existing tokens. The argument is a semicolon-separated list of name=value pairs.
- [version\\_tokens\\_delete\(\)](#page-33-1) deletes tokens from the current list. The argument is a semicolonseparated list of token names.
- [version\\_tokens\\_show\(\)](#page-34-1) displays the current token list. It takes no argument.
...
As mentioned previously, version tokens are defined using a semicolon-separated list of name=value pairs. Consider this invocation of [version\\_tokens\\_set\(\)](#page-34-0):
...
- Whitespace around names and values is ignored. Whitespace within names and values is permitted. (For [version\\_tokens\\_delete\(\)](#page-33-1), which takes a list of names without values, whitespace around names is ignored.)
...
Given those rules, the preceding [version\\_tokens\\_set\(\)](#page-34-0) call results in a token list with two tokens: tok1 has the value 1'2 3"4, and tok2 has the value a = b. To verify this, call [version\\_tokens\\_show\(\)](#page-34-1):
...
If the token list contains two tokens, why did [version\\_tokens\\_set\(\)](#page-34-0) return the value 3 version tokens set? That occurred because the original token list contained two definitions for tok1, and the second definition replaced the first.
...
- Token values cannot contain ; characters. Length of values is constrained by the value of the max\_allowed\_packet system variable.
...
- [version\\_tokens\\_lock\\_exclusive\(\)](#page-34-2) acquires exclusive version token locks. It takes a list of one or more lock names and a timeout value.
- [version\\_tokens\\_lock\\_shared\(\)](#page-34-3) acquires shared version token locks. It takes a list of one or more lock names and a timeout value.
- [version\\_tokens\\_unlock\(\)](#page-35-1) releases version token locks (exclusive and shared). It takes no argument.
