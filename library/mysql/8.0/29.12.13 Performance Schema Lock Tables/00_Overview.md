...
- [data\\_locks](#page-33-0): Data locks held and requested
- [data\\_lock\\_waits](#page-36-0): Relationships between data lock owners and data lock requestors blocked by those owners
- [metadata\\_locks](#page-39-0): Metadata locks held and requested
- [table\\_handles](#page-41-0): Table locks held and requested

The following sections describe these tables in more detail.

## <span id="page-33-0"></span>**29.12.13.1 The data\_locks Table**

The [data\\_locks](#page-33-0) table shows data locks held and requested. For information about which lock requests are blocked by which held locks, see [Section 29.12.13.2, "The data\\_lock\\_waits Table".](#page-36-0)
...
Use the [data\\_locks](#page-33-0) table to help diagnose performance problems that occur during times of heavy concurrent load. For InnoDB, see the discussion of this topic at Section 17.15.2, "InnoDB INFORMATION\_SCHEMA Transaction and Locking Information".

The [data\\_locks](#page-33-0) table has these columns:
...
The thread ID of the session that created the lock. To obtain details about the thread, join this column with the THREAD\_ID column of the Performance Schema [threads](#page-99-0) table.
...
To obtain details about the parent event, join the THREAD\_ID and EVENT\_ID columns with the columns of like name in the appropriate parent event table. See [Section 29.19.2, "Obtaining Parent](#page-137-0) [Event Information".](#page-137-0)
...
use (see Section 17.6.2.1, "Clustered and Secondary Indexes"). LOCK\_DATA reports "supremum pseudo-record" for a lock taken on a supremum pseudo-record. If the page containing the locked record is not in the buffer pool because it was written to disk while the lock was held, InnoDB does not fetch the page from disk. Instead, LOCK\_DATA reports NULL.

The [data\\_locks](#page-33-0) table has these indexes:
...
TRUNCATE TABLE is not permitted for the [data\\_locks](#page-33-0) table.

![](_page_36_Picture_8.jpeg)

### **Note**

Prior to MySQL 8.0.1, information similar to that in the Performance Schema [data\\_locks](#page-33-0) table is available in the INFORMATION\_SCHEMA.INNODB\_LOCKS table, which provides information about each lock that an InnoDB transaction has requested but not yet acquired, and each lock held by a transaction that is blocking another transaction. INFORMATION\_SCHEMA.INNODB\_LOCKS is deprecated and is removed as of MySQL 8.0.1. [data\\_locks](#page-33-0) should be used instead.

Differences between INNODB\_LOCKS and [data\\_locks](#page-33-0):

- If a transaction holds a lock, INNODB\_LOCKS displays the lock only if another transaction is waiting for it. [data\\_locks](#page-33-0) displays the lock regardless of whether any transaction is waiting for it.
- The [data\\_locks](#page-33-0) table has no columns corresponding to LOCK\_SPACE, LOCK\_PAGE, or LOCK\_REC.
- The INNODB\_LOCKS table requires the global PROCESS privilege. The [data\\_locks](#page-33-0) table requires the usual Performance Schema privilege of SELECT on the table to be selected from.

The following table shows the mapping from INNODB\_LOCKS columns to [data\\_locks](#page-33-0) columns. Use this information to migrate applications from one table to the other.
...
| LOCK_MODE                                | LOCK_MODE                                                |
| LOCK_TYPE                                | LOCK_TYPE                                                |
| LOCK_TABLE (combined schema/table names) | OBJECT_SCHEMA (schema name), OBJECT_NAME<br>(table name) |
...
### <span id="page-36-0"></span>**29.12.13.2 The data\_lock\_waits Table**

The [data\\_lock\\_waits](#page-36-0) table implements a many-to-many relationship showing which data lock requests in the [data\\_locks](#page-33-0) table are blocked by which held data locks in the [data\\_locks](#page-33-0) table. Held locks in [data\\_locks](#page-33-0) appear in [data\\_lock\\_waits](#page-36-0) only if they block some lock request.
...
Use the [data\\_lock\\_waits](#page-36-0) table to help diagnose performance problems that occur during times of heavy concurrent load. For InnoDB, see the discussion of this topic at Section 17.15.2, "InnoDB INFORMATION\_SCHEMA Transaction and Locking Information".

Because the columns in the [data\\_lock\\_waits](#page-36-0) table are similar to those in the [data\\_locks](#page-33-0) table, the column descriptions here are abbreviated. For more detailed column descriptions, see [Section 29.12.13.1, "The data\\_locks Table"](#page-33-0).

The [data\\_lock\\_waits](#page-36-0) table has these columns:
...
The ID of the lock requested by the storage engine. To obtain details about the lock, join this column with the ENGINE\_LOCK\_ID column of the [data\\_locks](#page-33-0) table.
...
The ID of the blocking lock. To obtain details about the lock, join this column with the ENGINE\_LOCK\_ID column of the [data\\_locks](#page-33-0) table.
...
The [data\\_lock\\_waits](#page-36-0) table has these indexes:
...
TRUNCATE TABLE is not permitted for the [data\\_lock\\_waits](#page-36-0) table.

![](_page_38_Picture_19.jpeg)

### **Note**

Prior to MySQL 8.0.1, information similar to that in the Performance Schema [data\\_lock\\_waits](#page-36-0) table is available in the INFORMATION\_SCHEMA.INNODB\_LOCK\_WAITS table, which provides information about each blocked InnoDB transaction, indicating the lock it has requested and any locks that are blocking that request. INFORMATION\_SCHEMA.INNODB\_LOCK\_WAITS is deprecated and is removed as of MySQL 8.0.1. [data\\_lock\\_waits](#page-36-0) should be used instead.

The tables differ in the privileges required: The INNODB\_LOCK\_WAITS table requires the global PROCESS privilege. The [data\\_lock\\_waits](#page-36-0) table requires the usual Performance Schema privilege of SELECT on the table to be selected from.

The following table shows the mapping from INNODB\_LOCK\_WAITS columns to [data\\_lock\\_waits](#page-36-0) columns. Use this information to migrate applications from one table to the other.
...
### <span id="page-39-0"></span>**29.12.13.3 The metadata\_locks Table**
...
The Performance Schema exposes metadata lock information through the [metadata\\_locks](#page-39-0) table:
...
The [metadata\\_locks](#page-39-0) table is read only and cannot be updated. It is autosized by default; to configure the table size, set the [performance\\_schema\\_max\\_metadata\\_locks](#page-120-0) system variable at server startup.
...
To control metadata lock instrumentation state at runtime, update the setup\_instruments table:
...
The Performance Schema maintains [metadata\\_locks](#page-39-0) table content as follows, using the LOCK\_STATUS column to indicate the status of each lock:
...
- When a pending lock request is canceled by the deadlock detector to break a deadlock ([ER\\_LOCK\\_DEADLOCK](https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.md#error_er_lock_deadlock)), its row status is updated from PENDING to VICTIM.
- When a pending lock request times out ([ER\\_LOCK\\_WAIT\\_TIMEOUT](https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.md#error_er_lock_wait_timeout)), its row status is updated from PENDING to TIMEOUT.
...
The [metadata\\_locks](#page-39-0) table has these columns:
...
The [metadata\\_locks](#page-39-0) table has these indexes:
...
TRUNCATE TABLE is not permitted for the [metadata\\_locks](#page-39-0) table.

## <span id="page-41-0"></span>**29.12.13.4 The table\_handles Table**

The Performance Schema exposes table lock information through the [table\\_handles](#page-41-0) table to show the table locks currently in effect for each opened table handle. [table\\_handles](#page-41-0) reports what is recorded by the table lock instrumentation. This information shows which table handles the server has open, how they are locked, and by which sessions.

The [table\\_handles](#page-41-0) table is read only and cannot be updated. It is autosized by default; to configure the table size, set the [performance\\_schema\\_max\\_table\\_handles](#page-125-0) system variable at server startup.
...
To control table lock instrumentation state at runtime, update the setup\_instruments table:
...
The [table\\_handles](#page-41-0) table has these columns:
...
The [table\\_handles](#page-41-0) table has these indexes:

- Primary key on (OBJECT\_INSTANCE\_BEGIN)
- Index on (OBJECT\_TYPE, OBJECT\_SCHEMA, OBJECT\_NAME)
- Index on (OWNER\_THREAD\_ID, OWNER\_EVENT\_ID)

TRUNCATE TABLE is not permitted for the [table\\_handles](#page-41-0) table.

# <span id="page-42-0"></span>**29.12.14 Performance Schema System Variable Tables**
...
- [global\\_variables](#page-42-0): Global system variables. An application that wants only global values should use this table.
- [session\\_variables](#page-42-0): System variables for the current session. An application that wants all system variable values for its own session should use this table. It includes the session variables for its session, as well as the values of global variables that have no session counterpart.
- [variables\\_by\\_thread](#page-42-0): Session system variables for each active session. An application that wants to know the session variable values for specific sessions should use this table. It includes session variables only, identified by thread ID.

- [persisted\\_variables](#page-43-0): Provides a SQL interface to the mysqld-auto.cnf file that stores persisted global system variable settings. See [Section 29.12.14.1, "Performance Schema](#page-43-0) [persisted\\_variables Table"](#page-43-0).
- [variables\\_info](#page-44-0): Shows, for each system variable, the source from which it was most recently set, and its range of values. See [Section 29.12.14.2, "Performance Schema variables\\_info Table"](#page-44-0).
...
The session variable tables ([session\\_variables](#page-42-0), [variables\\_by\\_thread](#page-42-0)) contain information only for active sessions, not terminated sessions.

The [global\\_variables](#page-42-0) and [session\\_variables](#page-42-0) tables have these columns:
...
The system variable value. For [global\\_variables](#page-42-0), this column contains the global value. For [session\\_variables](#page-42-0), this column contains the variable value in effect for the current session.

The [global\\_variables](#page-42-0) and [session\\_variables](#page-42-0) tables have these indexes:

• Primary key on (VARIABLE\_NAME)

The [variables\\_by\\_thread](#page-42-0) table has these columns:
...
Then [persisted\\_variables](#page-43-0) has these contents:
...
The [persisted\\_variables](#page-43-0) table has these columns:
...
[persisted\\_variables](#page-43-0) has these indexes:

• Primary key on (VARIABLE\_NAME)

TRUNCATE TABLE is not permitted for the [persisted\\_variables](#page-43-0) table.

### <span id="page-44-0"></span>**29.12.14.2 Performance Schema variables\_info Table**

The [variables\\_info](#page-44-0) table shows, for each system variable, the source from which it was most recently set, and its range of values.

The [variables\\_info](#page-44-0) table has these columns:
...
The [variables\\_info](#page-44-0) table has no indexes.

TRUNCATE TABLE is not permitted for the [variables\\_info](#page-44-0) table.
...
If a system variable has an unexpected VARIABLE\_SOURCE value, consider your server startup method. For example, mysqld\_safe reads option files and passes certain options it finds there as part of the command line that it uses to start mysqld. Consequently, some system variables that you set in option files might display in [variables\\_info](#page-44-0) as COMMAND\_LINE, rather than as GLOBAL or SERVER as you might otherwise expect.

Some sample queries that use the [variables\\_info](#page-44-0) table, with representative output:
...
• Join [variables\\_info](#page-44-0) with the [global\\_variables](#page-42-0) table to display the current values of persisted variables, together with their range of values:
...
# <span id="page-47-0"></span>**29.12.15 Performance Schema Status Variable Tables**
...
- [global\\_status](#page-47-0): Global status variables. An application that wants only global values should use this table.
- [session\\_status](#page-47-0): Status variables for the current session. An application that wants all status variable values for its own session should use this table. It includes the session variables for its session, as well as the values of global variables that have no session counterpart.
- [status\\_by\\_thread](#page-47-0): Session status variables for each active session. An application that wants to know the session variable values for specific sessions should use this table. It includes session variables only, identified by thread ID.

There are also summary tables that provide status variable information aggregated by account, host name, and user name. See [Section 29.12.20.12, "Status Variable Summary Tables".](#page-86-0)

The session variable tables ([session\\_status](#page-47-0), [status\\_by\\_thread](#page-47-0)) contain information only for active sessions, not terminated sessions.

The Performance Schema collects statistics for global status variables only for threads for which the INSTRUMENTED value is YES in the [threads](#page-99-0) table. Statistics for session status variables are always collected, regardless of the INSTRUMENTED value.

The Performance Schema does not collect statistics for Com\_xxx status variables in the status variable tables. To obtain global and per-session statement execution counts, use the [events\\_statements\\_summary\\_global\\_by\\_event\\_name](#page-65-0) and [events\\_statements\\_summary\\_by\\_thread\\_by\\_event\\_name](#page-65-0) tables, respectively. For example:
...
The [global\\_status](#page-47-0) and [session\\_status](#page-47-0) tables have these columns:
...
The status variable value. For [global\\_status](#page-47-0), this column contains the global value. For [session\\_status](#page-47-0), this column contains the variable value for the current session.

The [global\\_status](#page-47-0) and [session\\_status](#page-47-0) tables have these indexes:

• Primary key on (VARIABLE\_NAME)

The [status\\_by\\_thread](#page-47-0) table contains the status of each active thread. It has these columns:
...
The [status\\_by\\_thread](#page-47-0) table has these indexes:

• Primary key on (THREAD\_ID, VARIABLE\_NAME)

The [status\\_by\\_thread](#page-47-0) table contains status variable information only about foreground threads. If the [performance\\_schema\\_max\\_thread\\_instances](#page-127-1) system variable is not autoscaled (signified by a value of −1) and the maximum permitted number of instrumented thread objects is not greater than the number of background threads, the table is empty.

The Performance Schema supports TRUNCATE TABLE for status variable tables as follows:

- [global\\_status](#page-47-0): Resets thread, account, host, and user status. Resets global status variables except those that the server never resets.
- [session\\_status](#page-47-0): Not supported.
- [status\\_by\\_thread](#page-47-0): Aggregates status for all threads to the global status and account status, then resets thread status. If account statistics are not collected, the session status is added to host and user status, if host and user status are collected.
