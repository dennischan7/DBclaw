...
(For more information on connection strings, see [Section 25.4.3.3, "NDB Cluster Connection Strings"](#page-53-0).)
...
For additional NDB variables that can be set in the my.cnf file, see [NDB Cluster System Variables.](#page-187-0)
...
section contains parameters that apply to a particular data node, whereas an [ndbd default] section contains parameters that apply to all data nodes. Suppose that all data nodes should use the same data memory size. To configure them all, create an [ndbd default] section that contains a [DataMemory](#page-71-0) line to specify the data memory size.
...
In some older releases of NDB Cluster, there was no default value for [NoOfReplicas](#page-68-0), which always had to be specified explicitly in the [ndbd default] section. Although this parameter now has a default value of 2, which is the recommended setting in most common usage scenarios, it is still recommended practice to set this parameter explicitly.
...
#### <span id="page-49-0"></span>**Sections of the config.ini File**
...
- [computer]: Defines cluster hosts. This is not required to configure a viable NDB Cluster, but be may used as a convenience when setting up a large cluster. See [Section 25.4.3.4, "Defining](#page-54-1) [Computers in an NDB Cluster"](#page-54-1), for more information.
- [ndbd]: Defines a cluster data node (ndbd process). See [Section 25.4.3.6, "Defining NDB Cluster](#page-64-0) [Data Nodes"](#page-64-0), for details.
- [mysqld]: Defines the cluster's MySQL server nodes (also called SQL or API nodes). For a discussion of SQL node configuration, see [Section 25.4.3.7, "Defining SQL and Other API Nodes in](#page-162-1) [an NDB Cluster".](#page-162-1)
- [mgm] or [ndb\_mgmd]: Defines a cluster management server (MGM) node. For information concerning the configuration of management nodes, see [Section 25.4.3.5, "Defining an NDB Cluster](#page-55-0) [Management Server".](#page-55-0)
...
# <span id="page-50-0"></span>**25.4.3.2 Recommended Starting Configuration for NDB Cluster**
...
**Required my.cnf options for SQL nodes.** MySQL servers acting as NDB Cluster SQL nodes must always be started with the [--ndbcluster](#page-174-0) and [--ndb-connectstring](#page-178-0) options, either on the command line or in my.cnf.

# <span id="page-53-0"></span>**25.4.3.3 NDB Cluster Connection Strings**
...
- For backward compatibility, two other options are available, using the same syntax:
  - 1. Set the NDB\_CONNECTSTRING environment variable to contain the connection string. This should be considered deprecated, and not used in new installations.
...
# <span id="page-54-1"></span>**25.4.3.4 Defining Computers in an NDB Cluster**
...
#### <span id="page-55-1"></span>• HostName
...
# <span id="page-55-0"></span>**25.4.3.5 Defining an NDB Cluster Management Server**
...
| Default               | []                                                                                                                                                                                                                 |
| Range                 | 1 - 255                                                                                                                                                                                                            |
| Restart Type          | Initial System<br>Restart:<br>Requires a<br>complete<br>shutdown of the<br>cluster, wiping<br>and restoring<br>the cluster file<br>system from a<br>backup, and<br>then restarting<br>the cluster.<br>(NDB 8.0.13) |
...
The use of the Id parameter for identifying management nodes is deprecated in favor of [NodeId](#page-56-1). Although Id continues to be supported for backward compatibility, it now generates a warning and is subject to removal in a future version of NDB Cluster.

# <span id="page-56-1"></span>• NodeId
...
NodeId is the preferred parameter name to use when identifying management nodes. Although the older [Id](#page-56-0) continues to be supported for backward compatibility, it is now deprecated and generates a warning when used; it is also subject to removal in a future NDB Cluster release.

#### <span id="page-57-0"></span>• ExecuteOnComputer
...
| Restart Type          | System<br>Restart:<br>Requires a<br>complete<br>shutdown and<br>restart of the<br>cluster. (NDB<br>8.0.13) |
...
This parameter is deprecated, and is subject to removal in a future release. Use the [HostName](#page-58-0) parameter instead.

#### <span id="page-57-1"></span>• PortNumber
...
The node ID for this node can be given out only to connections that explicitly request it. A management server that requests "any" node ID cannot use this one. This parameter can be used when running multiple management servers on the same host, and [HostName](#page-58-0) is not sufficient for distinguishing among processes.

#### <span id="page-58-0"></span>• HostName
...
#### <span id="page-58-1"></span>• [LocationDomainId](#page-58-1)
...
<span id="page-59-0"></span>• [LogDestination](#page-59-0)
...
<span id="page-60-0"></span>• ArbitrationRank
...
You can disable arbitration completely either by setting ArbitrationRank to 0 on all management and SQL nodes, or by setting the [Arbitration](#page-117-0) parameter in the [ndbd default] section of the config.ini global configuration file. Setting [Arbitration](#page-117-0) causes any settings for ArbitrationRank to be disregarded.

# <span id="page-61-0"></span>• ArbitrationDelay
...
#### <span id="page-61-1"></span>• DataDir
...
# <span id="page-62-2"></span>• [Wan](#page-62-2)
...
<span id="page-62-1"></span>• HeartbeatThreadPriority
...
<span id="page-62-0"></span>• ExtraSendBufferMemory
...
This parameter specifies the amount of transporter send buffer memory to allocate in addition to any that has been set using [TotalSendBufferMemory](#page-63-1), SendBufferMemory, or both. <sup>4233</sup> <span id="page-63-1"></span>• TotalSendBufferMemory
...
<span id="page-63-0"></span>• HeartbeatIntervalMgmdMgmd
...
# <span id="page-64-0"></span>**25.4.3.6 Defining NDB Cluster Data Nodes**
...
The parameter [NoOfReplicas](#page-68-0) should be defined in the [ndbd default] section, as it is common to all Cluster data nodes. It is not strictly necessary to set [NoOfReplicas](#page-68-0), but it is good practice to set it explicitly.
...
Information about configuration parameters specific to NDB Cluster Disk Data tables can be found later in this section (see [Disk Data Configuration Parameters\)](#page-149-1).

All of these parameters also apply to ndbmtd (the multithreaded version of ndbd). Three additional data node configuration parameters—[MaxNoOfExecutionThreads](#page-136-1), [ThreadConfig](#page-142-0), and [NoOfFragmentLogParts](#page-140-1)—apply to ndbmtd only; these have no effect when used with ndbd. For more information, see [Multi-Threading Configuration Parameters \(ndbmtd\)](#page-135-2). See also Section 25.5.3, "ndbmtd — The NDB Cluster Data Node Daemon (Multi-Threaded)".
...
| Range                 | 1 - 48                                                                                                                                                                                                             |
|-----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Version (or<br>later) | NDB 8.0.18                                                                                                                                                                                                         |
| Type or units         | unsigned                                                                                                                                                                                                           |
| Default               | []                                                                                                                                                                                                                 |
| Range                 | 1 - 144                                                                                                                                                                                                            |
| Restart Type          | Initial System<br>Restart:<br>Requires a<br>complete<br>shutdown of the<br>cluster, wiping<br>and restoring<br>the cluster file<br>system from a<br>backup, and<br>then restarting<br>the cluster.<br>(NDB 8.0.13) |
...
#### <span id="page-65-0"></span>• ExecuteOnComputer
...
This parameter is deprecated, and is subject to removal in a future release. Use the [HostName](#page-66-0) parameter instead.

The node ID for this node can be given out only to connections that explicitly request it. A management server that requests "any" node ID cannot use this one. This parameter can be used when running multiple management servers on the same host, and [HostName](#page-66-0) is not sufficient for distinguishing among processes.

#### <span id="page-66-0"></span>• HostName
...
#### <span id="page-66-1"></span>• [ServerPort](#page-66-1)
...
Connections from data nodes to management nodes is done using the ndb\_mgmd management port (the management server's [PortNumber](#page-57-1)) so outgoing connections to that port from any data nodes should always be permitted.

# <span id="page-67-2"></span>• TcpBind\_INADDR\_ANY
...
#### <span id="page-67-1"></span>• [NodeGroup](#page-67-1)
...
The [NodeGroup](#page-67-1) parameter is chiefly intended for use in adding a new node group to a running NDB Cluster without having to perform a rolling restart. For this purpose, you should set it to 65536 (the maximum value). You are not required to set a [NodeGroup](#page-67-1) value for all cluster data nodes, only for those nodes which are to be started and added to the cluster as a new node group at a later time. For more information, see Section 25.6.7.3, "Adding NDB Cluster Data Nodes Online: Detailed Example".

#### <span id="page-67-0"></span>• [LocationDomainId](#page-67-0)
...
#### <span id="page-68-0"></span>• [NoOfReplicas](#page-68-0)
...
| Default               | 2                                                                                                                                                                                                                  |
| Range                 | 1 - 2                                                                                                                                                                                                              |
| Version (or<br>later) | NDB 8.0.19                                                                                                                                                                                                         |
| Type or units         | integer                                                                                                                                                                                                            |
| Default               | 2                                                                                                                                                                                                                  |
| Range                 | 1 - 4                                                                                                                                                                                                              |
| Restart Type          | Initial System<br>Restart:<br>Requires a<br>complete<br>shutdown of the<br>cluster, wiping<br>and restoring<br>the cluster file<br>system from a<br>backup, and<br>then restarting<br>the cluster.<br>(NDB 8.0.13) |
...
The number of data nodes in the cluster must be evenly divisible by the value of this parameter. For example, if there are two data nodes, then [NoOfReplicas](#page-68-0) must be equal to either 1 or 2, since 2/3 and 2/4 both yield fractional values; if there are four data nodes, then NoOfReplicas must be equal to 1, 2, or 4.

## <span id="page-69-0"></span>• DataDir
...
| Default               |                                                                                                                                                         |
| Range                 |                                                                                                                                                         |
| Restart Type          | Initial Node<br>Restart:<br>Requires a<br>rolling restart<br>of the cluster;<br>each data<br>node must be<br>restarted with<br>initial.<br>(NDB 8.0.13) |

This parameter specifies the directory where trace files, log files, pid files and error logs are placed.

The default is the data node process working directory.

# • [FileSystemPath](#page-69-1)
...
rolling restart of the cluster; each data node must be restarted with --initial. (NDB 8.0.13)
...
#### <span id="page-70-0"></span>• [BackupDataDir](#page-70-0)
...
The string '/BACKUP' is always appended to this value. For example, if you set the value of [BackupDataDir](#page-70-0) to /var/lib/cluster-data, then all backups are stored under /var/lib/cluster-data/BACKUP. This also means that the effective default backup location is the directory named BACKUP under the location specified by the [FileSystemPath](#page-69-1) parameter.

#### **Data Memory, Index Memory, and String Memory**

[DataMemory](#page-71-0) and [IndexMemory](#page-72-0) are [ndbd] parameters specifying the size of memory segments used to store the actual records and their indexes. In setting values for these, it is important to understand how [DataMemory](#page-71-0) is used, as it usually needs to be updated to reflect actual usage by the cluster.
...
#### <span id="page-71-0"></span>• [DataMemory](#page-71-0)
...
The memory allocated by [DataMemory](#page-71-0) is used to store both the actual records and indexes. There is a 16-byte overhead on each record; an additional amount for each record is incurred because it is stored in a 32KB page with 128 byte page overhead (see below). There is also a small amount wasted per page due to the fact that each record is stored in only one page.

For variable-size table attributes, the data is stored on separate data pages, allocated from [DataMemory](#page-71-0). Variable-length records use a fixed-size part with an extra overhead of 4 bytes to reference the variable-size part. The variable-size part has 2 bytes overhead plus 2 bytes per attribute.
...
The IndexMemory parameter is deprecated (and subject to future removal); any memory assigned to IndexMemory is allocated instead to the same pool as [DataMemory](#page-71-0), which is solely responsible for all resources needed for storing data and indexes in memory. In NDB 8.0, the use of IndexMemory in the cluster configuration file triggers a warning from the management server.
...
Assume that table t1 has 1 million rows and an ordered index named ix1 on two four-byte integers. Assume in addition that [IndexStatSaveSize](#page-160-1) and [IndexStatSaveScale](#page-160-0) are set to their default values (32K and 100, respectively). Using the previous 2 formulas, we can calculate as follows:
...
# <span id="page-73-0"></span>• [StringMemory](#page-73-0)
...
An important point for both [IndexMemory](#page-72-0) and [DataMemory](#page-71-0) is that the total database size is the sum of all data memory and all index memory for each node group. Each node group is used to store replicated information, so if there are four nodes with two fragment replicas, there are two node groups. Thus, the total data memory available is 2 × [DataMemory](#page-71-0) for each data node.

It is highly recommended that [DataMemory](#page-71-0) and [IndexMemory](#page-72-0) be set to the same values for all nodes. Data distribution is even over all nodes in the cluster, so the maximum amount of space available for any node can be no greater than that of the smallest node in the cluster.

[DataMemory](#page-71-0) can be changed, but decreasing it can be risky; doing so can easily lead to a node or even an entire NDB Cluster that is unable to restart due to there being insufficient memory space. Increasing these values should be acceptable, but it is recommended that such upgrades are performed in the same manner as a software upgrade, beginning with an update of the configuration file, and then restarting the management server followed by restarting each data node in turn.

<span id="page-74-0"></span>**MinFreePct.** A proportion (5% by default) of data node resources including [DataMemory](#page-71-0) is kept in reserve to insure that the data node does not exhaust its memory when performing a restart. This can be adjusted using the [MinFreePct](#page-74-0) data node configuration parameter (default 5).
...
<span id="page-74-1"></span>**Transaction parameters.** The next few [ndbd] parameters that we discuss are important because they affect the number of parallel transactions and the sizes of transactions that can be handled by the

system. [MaxNoOfConcurrentTransactions](#page-75-0) sets the number of parallel transactions possible in a node. [MaxNoOfConcurrentOperations](#page-76-0) sets the number of records that can be in update phase or locked simultaneously.

Both of these parameters (especially [MaxNoOfConcurrentOperations](#page-76-0)) are likely targets for users setting specific values and not using the default value. The default value is set for systems using small transactions, to ensure that these do not use excessive memory.

[MaxDMLOperationsPerTransaction](#page-77-0) sets the maximum number of DML operations that can be performed in a given transaction.

<span id="page-75-0"></span>• [MaxNoOfConcurrentTransactions](#page-75-0)
...
In addition, each transaction involves at least one operation; for this reason, the value set for MaxNoOfConcurrentTransactions should always be no more than the value of [MaxNoOfConcurrentOperations](#page-76-0).
...
<span id="page-76-0"></span>• [MaxNoOfConcurrentOperations](#page-76-0)
...
Because each transaction always involves at least one operation, the value of MaxNoOfConcurrentOperations should always be greater than or equal to the value of [MaxNoOfConcurrentTransactions](#page-75-0).
...
By default, this parameter is calculated as 1.1 × [MaxNoOfConcurrentOperations](#page-76-0). This fits systems with many simultaneous transactions, none of them being very large. If there is a need to handle one very large transaction at a time and there are many nodes, it is a good idea to override the default value by explicitly specifying this parameter.

This parameter is deprecated in NDB 8.0, and is subject to removal in a future NDB Cluster release. In addition, this parameter is incompatible with the [TransactionMemory](#page-82-2) parameter; if you try to set values for both parameters in the cluster configuration file (config.ini), the management server refuses to start.

<span id="page-77-0"></span>• [MaxDMLOperationsPerTransaction](#page-77-0)
...
The value of this parameter cannot exceed that set for [MaxNoOfConcurrentOperations](#page-76-0).

<span id="page-78-1"></span>**Transaction temporary storage.** The next set of [ndbd] parameters is used to determine temporary storage when executing a statement that is part of a Cluster transaction. All records are released when the statement is completed and the cluster is waiting for the commit or rollback.

The default values for these parameters are adequate for most situations. However, users with a need to support transactions involving large numbers of rows or operations may need to increase these values to enable better parallelism in the system, whereas users whose applications require relatively small transactions can decrease the values to save memory.

#### <span id="page-78-0"></span>• [MaxNoOfConcurrentIndexOperations](#page-78-0)
...
#### <span id="page-79-0"></span>• [MaxNoOfFiredTriggers](#page-79-0)
...
The default value of [MaxNoOfFiredTriggers](#page-79-0) is 4000, which is sufficient for most situations. In some cases it can even be decreased if the DBA feels certain the need for parallelism in the cluster is not high.
...
#### <span id="page-79-1"></span>• [TransactionBufferMemory](#page-79-1)
...
The default value for [TransactionBufferMemory](#page-79-1) is 1MB.
...
<span id="page-80-1"></span>• [ReservedConcurrentOperations](#page-80-1)
...
<span id="page-81-0"></span>• [ReservedConcurrentScans](#page-81-0)
...
<span id="page-81-1"></span>• [ReservedConcurrentTransactions](#page-81-1)
...
<span id="page-81-2"></span>• [ReservedFiredTriggers](#page-81-2)
...
<span id="page-82-0"></span>• [ReservedLocalScans](#page-82-0)
...
<span id="page-82-1"></span>• [ReservedTransactionBufferMemory](#page-82-1)
...
<span id="page-82-2"></span>• [TransactionMemory](#page-82-2)
...
A number of configuration parameters are incompatible with TransactionMemory; it is not possible to set any of these parameters concurrently with TransactionMemory, and if you attempt to do so, the management server is unable to start (see [Parameters incompatible with](#page-83-1) [TransactionMemory\)](#page-83-1).
...
<span id="page-83-1"></span>**Parameters incompatible with TransactionMemory.** The following parameters cannot be used concurrently with TransactionMemory and are deprecated in NDB 8.0:

- [MaxNoOfConcurrentIndexOperations](#page-78-0)
- [MaxNoOfFiredTriggers](#page-79-0)
- [MaxNoOfLocalOperations](#page-77-1)
- [MaxNoOfLocalScans](#page-85-0)
...
<span id="page-83-0"></span>• [BatchSizePerLocalScan](#page-83-0)
...
BatchSizePerLocalScan has a strong connection to the [BatchSize](#page-167-1) defined in the SQL nodes.

#### Deprecated in NDB 8.0.

<span id="page-84-0"></span>• [LongMessageBuffer](#page-84-0)
...
<span id="page-84-1"></span>• [MaxFKBuildBatchSize](#page-84-1)
...
<span id="page-84-2"></span>• [MaxNoOfConcurrentScans](#page-84-2)
...
This parameter is used to control the number of parallel scans that can be performed in the cluster. Each transaction coordinator can handle the number of parallel scans defined for this parameter. Each scan query is performed by scanning all partitions in parallel. Each partition scan uses a scan record in the node where the partition is located, the number of records being the value of this parameter times the number of nodes. The cluster should be able to sustain [MaxNoOfConcurrentScans](#page-84-2) scans concurrently from all nodes in the cluster.
...
The default value of [MaxNoOfConcurrentScans](#page-84-2) is 256. The maximum value is 500.

#### <span id="page-85-0"></span>• [MaxNoOfLocalScans](#page-85-0)
...
#### <span id="page-85-1"></span>• [MaxParallelCopyInstances](#page-85-1)
...
#### <span id="page-86-0"></span>• [MaxParallelScansPerFragment](#page-86-0)
...
#### <span id="page-86-1"></span>• [MaxReorgBuildBatchSize](#page-86-1)
...
#### <span id="page-86-2"></span>• [MaxUIBuildBatchSize](#page-86-2)
...
# <span id="page-87-1"></span>**Memory Allocation**

#### [MaxAllocate](#page-87-1)
...
#### [NodeGroupTransporters](#page-87-2)
...
#### <span id="page-87-0"></span>**Hash Map Size**

#### [DefaultHashMapSize](#page-87-0)
...
<span id="page-88-2"></span>**Logging and checkpointing.** The following [ndbd] parameters control log and checkpoint behavior.

<span id="page-88-0"></span>• [FragmentLogFileSize](#page-88-0)
...
For more information about fragment log files, see the description for [NoOfFragmentLogFiles](#page-92-0).

<span id="page-88-1"></span>• [InitialNoOfOpenFiles](#page-88-1)
...
<span id="page-89-1"></span>• [InitFragmentLogFiles](#page-89-1)
...
<span id="page-89-0"></span>• [EnablePartialLcp](#page-89-0)
...
#### <span id="page-90-0"></span>• [LcpScanProgressTimeout](#page-90-0)
...
## <span id="page-90-1"></span>• [MaxNoOfOpenFiles](#page-90-1)
...
#### <span id="page-91-1"></span>• [MaxNoOfSavedMessages](#page-91-1)
...
# <span id="page-91-0"></span>• [MaxLCPStartDelay](#page-91-0)
...
The default parameter value is 16, which by default means 16 sets of 4 16MB files for a total of 1024MB. The size of the individual log files is configurable using the [FragmentLogFileSize](#page-88-0) parameter. In scenarios requiring a great many updates, the value for [NoOfFragmentLogFiles](#page-92-0) may need to be set as high as 300 or even higher to provide sufficient space for REDO logs.
...
<span id="page-92-1"></span>• [RecoveryWork](#page-92-1)
...
Percentage of storage overhead for LCP files. This parameter has an effect only when [EnablePartialLcp](#page-89-0) is true, that is, only when partial local checkpoints are enabled. A higher value means:
...
<span id="page-93-0"></span>• [InsertRecoveryWork](#page-93-0)

| Version (or | NDB 8.0.13 |
|-------------|------------|
| later)      |            |
...
Percentage of [RecoveryWork](#page-92-1) used for inserted rows. A higher value increases the number of writes during a local checkpoint, and decreases the total size of the LCP. A lower value decreases the number of writes during an LCP, but results in more space being used for the LCP, which means that recovery takes longer. This parameter has an effect only when [EnablePartialLcp](#page-89-0) is true, that is, only when partial local checkpoints are enabled.

#### <span id="page-94-0"></span>• [EnableRedoControl](#page-94-0)
...
#### <span id="page-95-1"></span>• [MaxNoOfAttributes](#page-95-1)
...
This parameter sets a suggested maximum number of attributes that can be defined in the cluster; like [MaxNoOfTables](#page-95-0), it is not intended to function as a hard upper limit.
...
When setting [MaxNoOfAttributes](#page-95-1), it is important to prepare in advance for any ALTER TABLE statements that you might want to perform in the future. This is due to the fact, during the execution of ALTER TABLE on a Cluster table, 3 times the number of attributes as in the original table are used, and a good practice is to permit double this amount. For example, if the NDB Cluster table having the greatest number of attributes (greatest\_number\_of\_attributes) has 100 attributes, a good starting point for the value of [MaxNoOfAttributes](#page-95-1) would be 6 \* greatest\_number\_of\_attributes = 600.

You should also estimate the average number of attributes per table and multiply this by [MaxNoOfTables](#page-95-0). If this value is larger than the value obtained in the previous paragraph, you should use the larger value instead.

Assuming that you can create all desired tables without any problems, you should also verify that this number is sufficient by trying an actual ALTER TABLE after configuring the parameter. If this is not successful, increase [MaxNoOfAttributes](#page-95-1) by another multiple of [MaxNoOfTables](#page-95-0) and test it again.

#### <span id="page-95-0"></span>• [MaxNoOfTables](#page-95-0)
...
A table object is allocated for each table and for each unique hash index in the cluster. This parameter sets a suggested maximum number of table objects for the cluster as a whole; like [MaxNoOfAttributes](#page-95-1), it is not intended to function as a hard upper limit.
...
The sum of [MaxNoOfTables](#page-95-0), [MaxNoOfOrderedIndexes](#page-96-0), and [MaxNoOfUniqueHashIndexes](#page-97-0) must not exceed 2 <sup>32</sup> − 2 (4294967294).

<span id="page-96-0"></span>• [MaxNoOfOrderedIndexes](#page-96-0)
...
For each ordered index in the cluster, an object is allocated describing what is being indexed and its storage segments. By default, each index so defined also defines an ordered index. Each unique index and primary key has both an ordered index and a hash index. [MaxNoOfOrderedIndexes](#page-96-0) sets the total number of ordered indexes that can be in use in the system at any one time.
...
# <span id="page-97-0"></span>• [MaxNoOfUniqueHashIndexes](#page-97-0)
...
#### <span id="page-97-2"></span>• [MaxNoOfTriggers](#page-97-2)
...
#### • [MaxNoOfSubscriptions](#page-97-1)
...
The default value for [MaxNoOfSubscriptions](#page-97-1) is 0, which is treated as equal to [MaxNoOfTables](#page-95-0). Each subscription consumes 108 bytes.

#### <span id="page-98-1"></span>• [MaxNoOfSubscribers](#page-98-1)
...
#### <span id="page-99-0"></span>• [CompressedLCP](#page-99-0)
...
#### <span id="page-99-1"></span>• [CrashOnCorruptedTuple](#page-99-1)
...
When this parameter is enabled (the default), it forces a data node to shut down whenever it 4270 encounters a corrupted tuple.

# <span id="page-100-1"></span>• [Diskless](#page-100-1)
...
[Diskless](#page-100-1) is disabled by default.

# <span id="page-100-0"></span>• [EncryptedFileSystem](#page-100-0)
...
<span id="page-101-0"></span>• [LateAlloc](#page-101-0)
...
<span id="page-101-1"></span>• [LockPagesInMainMemory](#page-101-1)
...
If the operating system is not configured to permit unprivileged users to lock pages, then the data node process making use of this parameter may have to be run as system root. ([LockPagesInMainMemory](#page-101-1) uses the mlockall function. From Linux kernel 2.6.9, unprivileged users can lock memory as limited by max locked memory. For more information, see ulimit -l and<http://linux.die.net/man/2/mlock>).
...
#### <span id="page-102-0"></span>• [ODirect](#page-102-0)
...
Enabling this parameter causes NDB to attempt using O\_DIRECT writes for LCP, backups, and redo logs, often lowering kswapd and CPU usage. When using NDB Cluster on Linux, enable [ODirect](#page-102-0) if you are using a 2.6 or later kernel.

[ODirect](#page-102-0) is disabled by default.

<span id="page-102-1"></span>• [ODirectSyncFlag](#page-102-1)
...
- [ODirect](#page-102-0) is not enabled.
- InitFragmentLogFiles is set to SPARSE.

#### Disabled by default.

#### <span id="page-103-0"></span>• [RestartOnErrorInsert](#page-103-0)
...
#### <span id="page-103-1"></span>• [StopOnError](#page-103-1)
...
<span id="page-104-0"></span>• [TimeBetweenWatchDogCheck](#page-104-0)
...
<span id="page-104-1"></span>• [TimeBetweenWatchDogCheckInitial](#page-104-1)
...
This is similar to the [TimeBetweenWatchDogCheck](#page-104-0) parameter, except that [TimeBetweenWatchDogCheckInitial](#page-104-1) controls the amount of time that passes between execution checks inside a storage node in the early start phases during which memory is allocated.

The default timeout is 6000 milliseconds (6 seconds).

# <span id="page-105-0"></span>• [StartPartialTimeout](#page-105-0)
...
# <span id="page-105-1"></span>• [StartPartitionedTimeout](#page-105-1)
...
If the cluster is ready to start after waiting for [StartPartialTimeout](#page-105-0) milliseconds but is still possibly in a partitioned state, the cluster waits until this timeout has also passed. If [StartPartitionedTimeout](#page-105-1) is set to 0, the cluster waits indefinitely (2<sup>32</sup> −1 ms, or approximately 49.71 days).

This parameter is overridden when performing an initial start or initial restart of the cluster.

# <span id="page-106-0"></span>• [StartFailureTimeout](#page-106-0)
...
#### <span id="page-106-1"></span>• [StartNoNodeGroupTimeout](#page-106-1)
...
<span id="page-107-1"></span>• [HeartbeatIntervalDbDb](#page-107-1)
...
See also Network communication and latency, as well as the description of the [ConnectCheckIntervalDelay](#page-109-0) configuration parameter.

<span id="page-107-0"></span>• [HeartbeatIntervalDbApi](#page-107-0)
...
until all activities initiated by the previous MySQL instance have been completed. The threeheartbeat criteria for this determination are the same as described for [HeartbeatIntervalDbDb](#page-107-1).
...
#### <span id="page-108-0"></span>• [HeartbeatOrder](#page-108-0)
...
The [HeartbeatOrder](#page-108-0) configuration parameter makes the order of heartbeat transmission userconfigurable. The default value for [HeartbeatOrder](#page-108-0) is zero; allowing the default value to be used on all data nodes causes the order of heartbeat transmission to be determined by NDB. If this parameter is used, it must be set to a nonzero value (maximum 65535) for every data node in the cluster, and this value must be unique for each data node; this causes the heartbeat transmission to proceed from data node to data node in the order of their [HeartbeatOrder](#page-108-0) values from lowest to highest (and then directly from the data node having the highest [HeartbeatOrder](#page-108-0) to the data node having the lowest value, to complete the circle). The values need not be consecutive. For example, to force the heartbeat transmission order A->B->D->C->A in the scenario outlined previously, you could set the [HeartbeatOrder](#page-108-0) values as shown here:
...
To use this parameter to change the heartbeat transmission order in a running NDB Cluster, you must first set [HeartbeatOrder](#page-108-0) for each data node in the cluster in the global configuration (config.ini) file (or files). To cause the change to take effect, you must perform either of the following:
...
<span id="page-109-0"></span>• [ConnectCheckIntervalDelay](#page-109-0)
...
This parameter enables connection checking between data nodes after one of them has failed heartbeat checks for 5 intervals of up to [HeartbeatIntervalDbDb](#page-107-1) milliseconds.
...
<span id="page-110-1"></span>• [TimeBetweenLocalCheckpoints](#page-110-1)
...
All the write operations in the cluster are added together. Setting [TimeBetweenLocalCheckpoints](#page-110-1) to 6 or less means that local checkpoints are executed continuously without pause, independent of the cluster's workload.

<span id="page-110-0"></span>• [TimeBetweenGlobalCheckpoints](#page-110-0)
...
In NDB 8.0, we recommended when you are using solid-state disks (especially those employing NVMe) with Disk Data tables that you reduce this value. In such cases, you should also ensure that [MaxDiskDataLatency](#page-155-0) is set to a proper level.

This parameter defines the interval between global checkpoints. The default is 2000 milliseconds.

<span id="page-111-2"></span>• [TimeBetweenGlobalCheckpointsTimeout](#page-111-2)
...
#### <span id="page-111-0"></span>• [TimeBetweenEpochs](#page-111-0)
...
[TimeBetweenEpochs](#page-111-0) is part of the implementation of "micro-GCPs", which can be used to improve the performance of NDB Cluster Replication.

#### <span id="page-111-1"></span>• [TimeBetweenEpochsTimeout](#page-111-1)
...
[TimeBetweenEpochsTimeout](#page-111-1) is part of the implementation of "micro-GCPs", which can be used to improve the performance of NDB Cluster Replication.
...
#### <span id="page-112-0"></span>• [MaxBufferedEpochs](#page-112-0)
...
#### <span id="page-112-1"></span>• [MaxBufferedEpochBytes](#page-112-1)
...
<span id="page-113-0"></span>• [TimeBetweenInactiveTransactionAbortCheck](#page-113-0)
...
<span id="page-113-2"></span>• [TransactionInactiveTimeout](#page-113-2)
...
<span id="page-113-1"></span>• [TransactionDeadlockDetectionTimeout](#page-113-1)
...
#### <span id="page-114-0"></span>• [DiskSyncSize](#page-114-0)
...
This is the maximum number of bytes to store before flushing data to a local checkpoint file. This is done to prevent write buffering, which can impede performance significantly. This parameter is not intended to take the place of [TimeBetweenLocalCheckpoints](#page-110-1).

![](_page_114_Picture_13.jpeg)

# **Note**

When [ODirect](#page-102-0) is enabled, it is not necessary to set [DiskSyncSize](#page-114-0); in fact, in such cases its value is simply ignored.

The default value is 4M (4 megabytes).

# <span id="page-115-0"></span>• [MaxDiskWriteSpeed](#page-115-0)
...
For setting the maximum rate of disk writes allowed while this data node is restarting, use [MaxDiskWriteSpeedOwnRestart](#page-115-2). For setting the maximum rate of disk writes allowed while other data nodes are restarting, use [MaxDiskWriteSpeedOtherNodeRestart](#page-115-1). The minimum speed for disk writes by all LCPs and backup operations can be adjusted by setting [MinDiskWriteSpeed](#page-116-1).

#### <span id="page-115-1"></span>• [MaxDiskWriteSpeedOtherNodeRestart](#page-115-1)
...
For setting the maximum rate of disk writes allowed while this data node is restarting, use [MaxDiskWriteSpeedOwnRestart](#page-115-2). For setting the maximum rate of disk writes allowed when no data nodes are restarting anywhere in the cluster, use [MaxDiskWriteSpeed](#page-115-0). The minimum speed for disk writes by all LCPs and backup operations can be adjusted by setting [MinDiskWriteSpeed](#page-116-1).

#### <span id="page-115-2"></span>• [MaxDiskWriteSpeedOwnRestart](#page-115-2)
...
For setting the maximum rate of disk writes allowed while other data nodes are restarting, use [MaxDiskWriteSpeedOtherNodeRestart](#page-115-1). For setting the maximum rate of disk writes allowed when no data nodes are restarting anywhere in the cluster, use [MaxDiskWriteSpeed](#page-115-0). The minimum speed for disk writes by all LCPs and backup operations can be adjusted by setting [MinDiskWriteSpeed](#page-116-1).

#### <span id="page-116-1"></span>• [MinDiskWriteSpeed](#page-116-1)
...
The maximum rates of disk writes allowed for LCPs and backups under various conditions are adjustable using the parameters [MaxDiskWriteSpeed](#page-115-0), [MaxDiskWriteSpeedOwnRestart](#page-115-2), and [MaxDiskWriteSpeedOtherNodeRestart](#page-115-1). See the descriptions of these parameters for more information.

#### <span id="page-116-0"></span>• [ApiFailureHandlingTimeout](#page-116-0)

| Version (or<br>later) | NDB 8.0.42                     |
|-----------------------|--------------------------------|
| Type or units         | seconds                        |
| Default               | 600                            |
| Range                 | 0 - 4294967039<br>(0xFFFFFEFF) |
...
<span id="page-117-1"></span>• [ArbitrationTimeout](#page-117-1)
...
<span id="page-117-0"></span>• [Arbitration](#page-117-0)
...
The [Arbitration](#page-117-0) parameter enables a choice of arbitration schemes, corresponding to one of 3 possible values for this parameter:
...
- **WaitExternal.** The [Arbitration](#page-117-0) parameter also makes it possible to configure arbitration in such a way that the cluster waits until after the time determined by [ArbitrationTimeout](#page-117-1) has passed for an external cluster manager application to perform arbitration instead of handling <sup>4288</sup>

arbitration internally. This can be done by setting Arbitration = WaitExternal in the [ndbd default] section of the config.ini file. For best results with the WaitExternal setting, it is recommended that [ArbitrationTimeout](#page-117-1) be 2 times as long as the interval required by the external cluster manager to perform arbitration.

![](_page_118_Picture_2.jpeg)

#### **Important**

This parameter should be used only in the [ndbd default] section of the cluster configuration file. The behavior of the cluster is unspecified when [Arbitration](#page-117-0) is set to different values for individual data nodes.

<span id="page-118-1"></span>• [RestartSubscriberConnectTimeout](#page-118-1)
...
<span id="page-118-0"></span>• [KeepAliveSendInterval](#page-118-0)
...
#### <span id="page-119-2"></span>• [UndoIndexBuffer](#page-119-2)
...
#### <span id="page-119-1"></span>• [UndoDataBuffer](#page-119-1)
...
#### <span id="page-119-0"></span>• [RedoBuffer](#page-119-0)
...
ndbmtd allocates a separate buffer for each LDM thread (see [ThreadConfig](#page-142-0)). For example, with 4 LDM threads, an ndbmtd data node actually has 4 buffers and allocates RedoBuffer bytes to each one, for a total of 4 \* RedoBuffer bytes.

#### <span id="page-120-0"></span>• [EventLogBufferSize](#page-120-0)
...
#### <span id="page-121-1"></span>• [LogLevelStartup](#page-121-1)
...
#### <span id="page-121-0"></span>• [LogLevelShutdown](#page-121-0)
...
# <span id="page-121-2"></span>• [LogLevelStatistic](#page-121-2)
...
<span id="page-122-0"></span>• [LogLevelCheckpoint](#page-122-0)
...
<span id="page-122-2"></span>• [LogLevelNodeRestart](#page-122-2)
...
<span id="page-122-1"></span>• [LogLevelConnection](#page-122-1)
...
#### <span id="page-123-1"></span>• [LogLevelError](#page-123-1)
...
#### <span id="page-123-0"></span>• [LogLevelCongestion](#page-123-0)
...
#### <span id="page-123-2"></span>• [LogLevelInfo](#page-123-2)
...
<span id="page-124-0"></span>• [MemReportFrequency](#page-124-0)
...
Each data node's data memory and index memory usage is logged as both a percentage and a number of 32 KB pages of [DataMemory](#page-71-0), as set in the config.ini file. For example, if [DataMemory](#page-71-0) is equal to 100 MB, and a given data node is using 50 MB for data memory storage, the corresponding line in the cluster log might look like this:

```
2006-12-24 01:18:16 [MgmSrvr] INFO -- Node 2: Data usage is 50%(1280 32K pages of total 2560)
```

[MemReportFrequency](#page-124-0) is not a required parameter. If used, it can be set for all cluster data nodes in the [ndbd default] section of config.ini, and can also be set or overridden for individual data nodes in the corresponding [ndbd] sections of the configuration file. The minimum value which is also the default value—is 0, in which case memory reports are logged only when memory usage reaches certain percentages (80%, 90%, and 100%), as mentioned in the discussion of statistics events in Section 25.6.3.2, "NDB Cluster Log Events".

<span id="page-124-1"></span>• [StartupStatusReportFrequency](#page-124-1)
...
When a data node is started with the --initial, it initializes the redo log file during Start Phase 4 (see Section 25.6.4, "Summary of NDB Cluster Start Phases"). When very large values are set for [NoOfFragmentLogFiles](#page-92-0), [FragmentLogFileSize](#page-88-0), or both, this initialization can take a long time. You can force reports on the progress of this process to be logged periodically, by means of4295 the [StartupStatusReportFrequency](#page-124-1) configuration parameter. In this case, progress is reported in the cluster log, in terms of both the number of files and the amount of space that have been initialized, as shown here:
...
These reports are logged each [StartupStatusReportFrequency](#page-124-1) seconds during Start Phase 4. If [StartupStatusReportFrequency](#page-124-1) is 0 (the default), then reports are written to the cluster log only when at the beginning and at the completion of the redo log file initialization process.
...
#### <span id="page-125-0"></span>• [DictTrace](#page-125-0)
...
# <span id="page-125-1"></span>• [WatchDogImmediateKill](#page-125-1)
...
# <span id="page-126-0"></span>• [BackupDataBufferSize](#page-126-0)
...
In creating a backup, there are two buffers used for sending data to the disk. The backup data buffer is used to fill in data recorded by scanning a node's tables. Once this buffer has been filled to the level specified as [BackupWriteSize](#page-128-2), the pages are sent to disk. While flushing data to disk, the backup process can continue filling this buffer until it runs out of space. When this happens, the backup process pauses the scan and waits until some disk writes have completed freeing up memory so that scanning may continue.

The default value for this parameter is 16MB. The minimum is 512K.

# <span id="page-126-1"></span>• [BackupDiskWriteSpeedPct](#page-126-1)
...
BackupDiskWriteSpeedPct applies only when a backup is single-threaded. With the introduction of multi-threaded backups in NDB 8.0.16, it is usually no longer necessary to adjust this parameter, which has no effect in the multi-threaded case. The discussion that follows is specific to singlethreaded backups.

During normal operation, data nodes attempt to maximize the disk write speed used for local checkpoints and backups while remaining within the bounds set by [MinDiskWriteSpeed](#page-116-1) and [MaxDiskWriteSpeed](#page-115-0). Disk write throttling gives each LDM thread an equal share of the total budget. This allows parallel LCPs to take place without exceeding the disk I/O budget. Because a backup is executed by only one LDM thread, this effectively caused a budget cut, resulting in longer backup completion times, and—if the rate of change is sufficiently high—in failure to complete the backup when the backup log buffer fill rate is higher than the achievable write rate.
...
#### <span id="page-127-0"></span>• [BackupLogBufferSize](#page-127-0)
...
#### <span id="page-127-1"></span>• [BackupMemory](#page-127-1)
...
# <span id="page-128-1"></span>• [BackupReportFrequency](#page-128-1)
...
#### <span id="page-128-0"></span>• [BackupMaxWriteSize](#page-128-0)
...
<span id="page-129-0"></span>• [CompressedBackup](#page-129-0)
...
<span id="page-129-1"></span>• [RequireEncryptedBackup](#page-129-1)

| Version (or<br>later) | NDB 8.0.22                                                                        |
|-----------------------|-----------------------------------------------------------------------------------|
| Type or units         | integer                                                                           |
| Default               | 0                                                                                 |
| Range                 | 0 - 1                                                                             |
...
If set to 1, backups must be encrypted. While it is possible to set this parameter for each data node individually, it is recommended that you set it in the [ndbd default] section of the

config.ini global configuration file. For more information about performing encrypted backups, see Section 25.6.8.2, "Using The NDB Cluster Management Client to Create a Backup".
...
The location of the backup files is determined by the [BackupDataDir](#page-70-0) data node configuration parameter.
...
<span id="page-130-0"></span>• [BuildIndexThreads](#page-130-0)
...
You can enable multithreaded builds during data node initial restarts by setting the [TwoPassInitialNodeRestartCopy](#page-134-0) data node configuration parameter to TRUE.

<span id="page-130-1"></span>• [LockExecuteThreadToCPU](#page-130-1)
...
The number of IDs specified should match the number of execution threads determined by [MaxNoOfExecutionThreads](#page-136-1). However, there is no guarantee that threads are assigned to CPUs in any given order when using this parameter. You can obtain more finely-grained control of this type using [ThreadConfig](#page-142-0).

[LockExecuteThreadToCPU](#page-130-1) has no default value.

#### <span id="page-131-0"></span>• [LockMaintThreadsToCPU](#page-131-0)
...
# <span id="page-131-1"></span>• [Numa](#page-131-1)
...
#### <span id="page-132-0"></span>• [RealtimeScheduler](#page-132-0)
...
#### <span id="page-132-1"></span>• [SchedulerExecutionTimer](#page-132-1)
...
# <span id="page-133-0"></span>• [SchedulerResponsiveness](#page-133-0)
...
#### <span id="page-133-1"></span>• [SchedulerSpinTimer](#page-133-1)
...
Starting with NDB 8.0.20, if [SpinMethod](#page-133-2) is set, any setting for this parameter is ignored.

#### <span id="page-133-2"></span>• [SpinMethod](#page-133-2)

| Version (or<br>later) | NDB 8.0.20                                                                                    |  |
|-----------------------|-----------------------------------------------------------------------------------------------|--|
| Type or units         | enumeration                                                                                   |  |
...
- 1. StaticSpinning (default): Sets EnableAdaptiveSpinning to false and [SchedulerSpinTimer](#page-133-1) to 0. (SetAllowedSpinOverhead is not relevant in this case.)
- 2. CostBasedSpinning: Sets EnableAdaptiveSpinning to true, [SchedulerSpinTimer](#page-133-1) to 100, and SetAllowedSpinOverhead to 200.
- 3. LatencyOptimisedSpinning: Sets EnableAdaptiveSpinning to true, [SchedulerSpinTimer](#page-133-1) to 200, and SetAllowedSpinOverhead to 1000.
- 4. DatabaseMachineSpinning: Sets EnableAdaptiveSpinning to true, [SchedulerSpinTimer](#page-133-1) to 500, and SetAllowedSpinOverhead to 10000. This is intended for use in cases where threads own their own CPUs.

The spin parameters modified by [SpinMethod](#page-133-2) are described in the following list:

- [SchedulerSpinTimer](#page-133-1): This is the same as the data node configuration parameter of that name. The setting applied to this parameter by SpinMethod overrides any value set in the config.ini file.
- EnableAdaptiveSpinning: Enables or disables adaptive spinning. Disabling it causes spinning to be performed without making any checks for CPU resources. This parameter cannot be set directly in the cluster configuration file, and under most circumstances should not need to be, but can be enabled directly using [DUMP 104004 1](https://dev.mysql.com/doc/ndb-internals/en/dump-command-104004.md) or disabled with [DUMP 104004 0](https://dev.mysql.com/doc/ndb-internals/en/dump-command-104004.md) in the ndb\_mgm management client.
...
<span id="page-134-0"></span>• [TwoPassInitialNodeRestartCopy](#page-134-0)
...
You must also set [BuildIndexThreads](#page-130-0) to a nonzero value.

<span id="page-135-2"></span>**Multi-Threading Configuration Parameters (ndbmtd).** ndbmtd runs by default as a singlethreaded process and must be configured to use multiple threads, using either of two methods, both of which require setting configuration parameters in the config.ini file. The first method is simply to set an appropriate value for the [MaxNoOfExecutionThreads](#page-136-1) configuration parameter. A second method makes it possible to set up more complex rules for ndbmtd multithreading using [ThreadConfig](#page-142-0). The next few paragraphs provide information about these parameters and their use with multithreaded data nodes.
...
#### <span id="page-135-0"></span>• [AutomaticThreadConfig](#page-135-0)

| Version (or<br>later) | NDB 8.0.23                                                                                                                                                                                                         |
|-----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Type or units         | boolean                                                                                                                                                                                                            |
| Default               | false                                                                                                                                                                                                              |
| Range                 | true, false                                                                                                                                                                                                        |
...
When set to 1, enables automatic thread configuration employing the number of CPUs available to a data node taking into account any limits set by taskset, numactl, virtual machines, Docker, and other such means of controlling which CPUs are available to a given application (on Windows platforms, automatic thread configuration uses all CPUs which are online); alternatively, you can set [NumCPUs](#page-141-0) to the desired number of CPUs (up to 1024, the maximum number of CPUs that can be handled by automatic thread configuration). Any settings for [ThreadConfig](#page-142-0) and [MaxNoOfExecutionThreads](#page-136-1) are ignored. In addition, enabling this parameter automatically disables [ClassicFragmentation](#page-135-1).

<span id="page-135-1"></span>• [ClassicFragmentation](#page-135-1)
...
For new clusters for which a downgrade to NDB 8.0.22 or earlier is never expected to occur, setting ClassicFragmentation to false when first setting up the cluster is preferable; doing so causes the number of partitions per node to be equal to the value of [PartitionsPerNode](#page-141-1), ensuring that all partitions are spread out evenly between all LDMs.

This parameter and [AutomaticThreadConfig](#page-135-0) are mutually exclusive; enabling AutomaticThreadConfig automatically disables ClassicFragmentation.

# <span id="page-136-0"></span>• EnableMultithreadedBackup
...
#### <span id="page-136-1"></span>• MaxNoOfExecutionThreads
...
Enabling [AutomaticThreadConfig](#page-135-0) causes any setting for this parameter to be ignored.

Setting MaxNoOfExecutionThreads sets the number of threads for each type as determined by a matrix in the file storage/ndb/src/common/mt\_thr\_config.cpp. (Prior to NDB 8.0.30, this was storage/ndb/src/kernel/vm/mt\_thr\_config.cpp.) This table shows these numbers of threads for possible values of MaxNoOfExecutionThreads.
...
[NoOfFragmentLogParts](#page-140-1) should be set equal to the number of LDM threads used by ndbmtd, as determined by the setting for this parameter. This ratio should not be any greater than 4:1; a configuration in which this is the case is specifically disallowed.
...
Adding large tablespaces for Disk Data tables when using more than the default number of LDM threads may cause issues with resource and CPU usage if the disk page buffer is insufficiently large; see the description of the [DiskPageBufferMemory](#page-150-0) configuration parameter, for more information.

The thread types are described later in this section (see [ThreadConfig](#page-142-0)).
...
Changing the number of LDM threads normally requires a system restart, whether it is changed using this parameter or [ThreadConfig](#page-142-0), but it is possible to effect the change using a node initial restart (NI) provided the following two conditions are met:
...
#### <span id="page-140-0"></span>• MaxSendDelay
...
# <span id="page-140-1"></span>• NoOfFragmentLogParts
...
Set the number of log file groups for redo logs belonging to this ndbmtd. The value of this parameter should be set equal to the number of LDM threads used by ndbmtd as determined by the setting for [MaxNoOfExecutionThreads](#page-136-1). A configuration using more than 4 redo log parts per LDM is disallowed.

See the description of [MaxNoOfExecutionThreads](#page-136-1) for more information.

#### <span id="page-141-0"></span>• [NumCPUs](#page-141-0)
...
Cause automatic thread configuration to use only this many CPUs. Has no effect if [AutomaticThreadConfig](#page-135-0) is not enabled.

#### <span id="page-141-1"></span>• [PartitionsPerNode](#page-141-1)
...
While it is possible to set this parameter to different values on different data nodes and there are no known issues with doing so, this is also not likely to be of any advantage; for this reason, it is

recommended simply to set it once, for all data nodes, in the [ndbd default] section of the global config.ini file.

If [ClassicFragmentation](#page-135-1) is enabled, any setting for this parameter is ignored. (Remember that enabling [AutomaticThreadConfig](#page-135-0) disables ClassicFragmentation.)

<span id="page-142-0"></span>• ThreadConfig
...
On Solaris, you can instead specify a set of CPUs to which the threads of the given type are to be bound exclusively. cpubind\_exclusive causes each thread to be bound exclusively to a CPU

in the set; cpuset\_exclsuive means that each thread is bound exclusively to the set of CPUs specified.
...
The default value for spintime is the value of the [SchedulerSpinTimer](#page-133-1) data node configuration parameter.
...
The realtime parameter is set by default to the value of the [RealtimeScheduler](#page-132-0) data node configuration parameter.
...
**Table 25.13 Mapping of thread\_prio to nice values on Linux and FreeBSD**
...
**Table 25.14 Mapping of thread\_prio to FX priority on Solaris**
...
**Table 25.15 Mapping of thread\_prio to Windows thread priority**
...
• ldm: Local query handler ([DBLQH](https://dev.mysql.com/doc/ndb-internals/en/ndb-internals-kernel-blocks-dblqh.md) kernel block) that handles data. The more LDM threads that are used, the more highly partitioned the data becomes. (Beginning with NDB 8.0.23, when [ClassicFragmentation](#page-135-1) is set to 0, the number of partitions is independent of the number of LDM threads, and depends on the value of [PartitionsPerNode](#page-141-1) instead.) Each LDM thread maintains its own sets of data and index partitions, as well as its own redo log. Prior to NDB 8.0.23, the value set for ldm must be one of the values 1, 2, 4, 6, 8, 12, 16, 24, or 32. In NDB 8.0.23 and later, it is possible to set ldm to any value in the range 1 to 332 inclusive; it also

becomes possible to set it to 0, provided that main, rep, and tc are also 0, and that recv is set to 1; doing this causes ndbmtd to emulate ndbd.
...
Changing the number of LDM threads normally requires a system restart to be effective and safe for cluster operations; this requirement is relaxed in certain cases, as explained later in this section. This is also true when this is done using [MaxNoOfExecutionThreads](#page-136-1).

Adding large tablespaces (hundreds of gigabytes or more) for Disk Data tables when using more than the default number of LDMs may cause issues with resource and CPU usage if [DiskPageBufferMemory](#page-150-0) is not sufficiently large.

In NDB 8.0.30 (only), ldm must be included in the ThreadConfig value string. Beginning with NDB 8.0.31, if this is omitted, one ldm thread is created. These changes may affect upgrades from previous releases; see [Section 25.3.7, "Upgrading and Downgrading NDB Cluster"](#page-16-0), for more information.

• query (Added in NDB 8.0.23): A query thread is tied to an LDM and together with it forms an LDM group; acts only on READ COMMITTED queries. The number of query threads must be set to 0, 1, 2, or 3 times the number of LDM threads. Query threads are not used, unless this is overridden by setting query to a nonzero value, or by enabling the [AutomaticThreadConfig](#page-135-0) parameter, in which case LDMs behave as they did prior to NDB 8.0.23.
...
• tc: Transaction coordinator thread ([DBTC](https://dev.mysql.com/doc/ndb-internals/en/ndb-internals-kernel-blocks-dbtc.md) kernel block) containing the state of an ongoing transaction. In NDB 8.0.23 and later, the maximum number of TC threads is 128; previously, this was 32.
...
• main: Data dictionary and transaction coordinator ([DBDIH](https://dev.mysql.com/doc/ndb-internals/en/ndb-internals-kernel-blocks-dbdih.md) and [DBTC](https://dev.mysql.com/doc/ndb-internals/en/ndb-internals-kernel-blocks-dbtc.md) kernel blocks), providing schema management. Prior to NDB 8.0.23, this was always handled by a single dedicated thread, beginning with NDB 8.0.23, it is also possible to specify zero or two main threads.
...
In NDB 8.0.30 (only), main must be included in the ThreadConfig value string. Beginning with NDB 8.0.31, if this is omitted, one main thread is created. These changes may affect upgrades from previous releases; see [Section 25.3.7, "Upgrading and Downgrading NDB Cluster"](#page-16-0), for more information.
...
In NDB 8.0.30 (only), recv must be included in the ThreadConfig value string. Beginning with NDB 8.0.31, if this is omitted, one recv thread is created. These changes may affect upgrades from previous releases; see [Section 25.3.7, "Upgrading and Downgrading NDB Cluster"](#page-16-0), for more information.
...
• rep: Replication thread ([SUMA](https://dev.mysql.com/doc/ndb-internals/en/ndb-internals-kernel-blocks-suma.md) kernel block). Prior to NDB 8.0.23, asynchronous replication operations are always handled by a single, dedicated thread. Beginning with NDB 8.0.23, this thread can be combined with the main thread (see range information).
...
In NDB 8.0.30 (only), rep must be included in the ThreadConfig value string. Beginning with NDB 8.0.31, if this is omitted, one rep thread is created. These changes may affect upgrades from previous releases; see [Section 25.3.7, "Upgrading and Downgrading NDB Cluster"](#page-16-0), for more information.
...
<span id="page-149-1"></span>**Disk Data Configuration Parameters.** Configuration parameters affecting Disk Data behavior include the following:

<span id="page-149-0"></span>• [DiskPageBufferEntries](#page-149-0)
...
#### <span id="page-150-1"></span>• [SharedGlobalMemory](#page-150-1)
...
This parameter determines the number of unbound threads used for Disk Data file access. Before [DiskIOThreadPool](#page-151-0) was introduced, exactly one thread was spawned for each Disk Data file, which could lead to performance issues, particularly when using very large data files. With [DiskIOThreadPool](#page-151-0), you can—for example—access a single large data file using several threads working in parallel.
...
• **Physical distribution of Disk Data files.** You can obtain better performance by placing data files, undo log files, and the data node file system on separate physical disks. If you do this with some or all of these sets of files, then you can (and should) set [DiskIOThreadPool](#page-151-0) higher to enable separate threads to handle the files on each disk.

In NDB 8.0, you should also disable [DiskDataUsingSameDisk](#page-156-0) when using a separate disk or disks for Disk Data files; this increases the rate at which checkpoints of Disk Data tablespaces can be performed.
...
drives can handle many more disk I/O threads than conventional disks, and thus higher values for [DiskIOThreadPool](#page-151-0).

Decreasing [TimeBetweenGlobalCheckpoints](#page-110-0) is also recommended when using solid-state disk drives, in particular those using NVMe. See also [Disk Data latency parameters](#page-155-1).

The default value for this parameter is 2.

- <span id="page-152-1"></span>• **Disk Data file system parameters.** The parameters in the following list make it possible to place NDB Cluster Disk Data files in specific directories without the need for using symbolic links.
  - [FileSystemPathDD](#page-152-1)
...
If this parameter is specified, then NDB Cluster Disk Data data files and undo log files are placed in the indicated directory. This can be overridden for data files, undo log files, or both, by specifying values for [FileSystemPathDataFiles](#page-152-0), [FileSystemPathUndoFiles](#page-153-0), or both, as explained for these parameters. It can also be overridden for data files by specifying a path in the ADD DATAFILE clause of a CREATE TABLESPACE or ALTER TABLESPACE statement, and for undo log files by specifying a path in the ADD UNDOFILE clause of a CREATE LOGFILE GROUP or ALTER LOGFILE GROUP statement. If [FileSystemPathDD](#page-152-1) is not specified, then [FileSystemPath](#page-69-1) is used.

If a [FileSystemPathDD](#page-152-1) directory is specified for a given data node (including the case where the parameter is specified in the [ndbd default] section of the config.ini file), then starting that data node with --initial causes all files in the directory to be deleted.

<span id="page-152-0"></span>• [FileSystemPathDataFiles](#page-152-0)
...
| Range                 |                                                                                                           |  |
| Restart Type          | Initial Node<br>Restart:<br>Requires a<br>rolling restart<br>of the cluster;<br>each data<br>node must be |  |
...
If this parameter is specified, then NDB Cluster Disk Data data files are placed in the indicated directory. This overrides any value set for [FileSystemPathDD](#page-152-1). This parameter can be overridden for a given data file by specifying a path in the ADD DATAFILE clause of a CREATE TABLESPACE or ALTER TABLESPACE statement used to create that data file. If [FileSystemPathDataFiles](#page-152-0) is not specified, then [FileSystemPathDD](#page-152-1) is used (or [FileSystemPath](#page-69-1), if [FileSystemPathDD](#page-152-1) has also not been set).

If a [FileSystemPathDataFiles](#page-152-0) directory is specified for a given data node (including the case where the parameter is specified in the [ndbd default] section of the config.ini file), then starting that data node with --initial causes all files in the directory to be deleted.

<span id="page-153-0"></span>• [FileSystemPathUndoFiles](#page-153-0)
...
If this parameter is specified, then NDB Cluster Disk Data undo log files are placed in the indicated directory. This overrides any value set for [FileSystemPathDD](#page-152-1). This parameter can be overridden for a given data file by specifying a path in the ADD UNDO clause of a CREATE LOGFILE GROUP or ALTER LOGFILE GROUP statement used to create that data file. If [FileSystemPathUndoFiles](#page-153-0) is not specified, then [FileSystemPathDD](#page-152-1) is used (or [FileSystemPath](#page-69-1), if [FileSystemPathDD](#page-152-1) has also not been set).

If a [FileSystemPathUndoFiles](#page-153-0) directory is specified for a given data node (including the case where the parameter is specified in the [ndbd default] section of the config.ini file), then starting that data node with --initial causes all files in the directory to be deleted.

For more information, see Section 25.6.11.1, "NDB Cluster Disk Data Objects".

- <span id="page-153-1"></span>• **Disk Data object creation parameters.** The next two parameters enable you—when starting the cluster for the first time—to cause a Disk Data log file group, tablespace, or both, to be created without the use of SQL statements.
  - [InitialLogFileGroup](#page-153-1)
...
This parameter can be used to specify a log file group that is created when performing an initial start of the cluster. [InitialLogFileGroup](#page-153-1) is specified as shown here:
...
The name of the log file group is optional and defaults to DEFAULT-LG. The undo\_buffer\_size is also optional; if omitted, it defaults to 64M. Each file-specification corresponds to an undo log file, and at least one must be specified in the file-specification-list. Undo log files are placed according to any values that have been set for [FileSystemPath](#page-69-1), [FileSystemPathDD](#page-152-1), and [FileSystemPathUndoFiles](#page-153-0), just as if they had been created as the result of a CREATE LOGFILE GROUP or ALTER LOGFILE GROUP statement.
...
Resources for the initial log file group are added to the global memory pool along with those indicated by the value of [SharedGlobalMemory](#page-150-1).
...
<span id="page-154-0"></span>• [InitialTablespace](#page-154-0)
...
This parameter can be used to specify an NDB Cluster Disk Data tablespace that is created when performing an initial start of the cluster. [InitialTablespace](#page-154-0) is specified as shown here:

```
InitialTablespace = [name=name;] [extent_size=size;] file-specification-list
```

The name of the tablespace is optional and defaults to DEFAULT-TS. The extent\_size is also optional; it defaults to 1M. The file-specification-list uses the same syntax as shown with the [InitialLogfileGroup](#page-153-1) parameter, the only difference being that each file-specification used with [InitialTablespace](#page-154-0) corresponds to a data file. At least one must be specified in the file-specification-list. Data files are placed according to any values that have been set for [FileSystemPath](#page-69-1), [FileSystemPathDD](#page-152-1), and [FileSystemPathDataFiles](#page-152-0), just as if they had been created as the result of a CREATE TABLESPACE or ALTER TABLESPACE statement.

For example, consider the following line specifying [InitialTablespace](#page-154-0) in the [ndbd default] section of the config.ini file (as with [InitialLogfileGroup](#page-153-1), this parameter should always be set in the [ndbd default] section, as the behavior of an NDB Cluster when different values are set on different data nodes is not defined):
...
- <span id="page-155-1"></span><span id="page-155-0"></span>• **Disk Data latency parameters.** The two parameters listed here can be used to improve handling of latency issues with NDB Cluster Disk Data tables.
  - [MaxDiskDataLatency](#page-155-0)
...
#### <span id="page-156-0"></span>• [DiskDataUsingSameDisk](#page-156-0)
...
You can help prevent these errors from occurring by using faster disks, and by placing Disk Data files on a separate disk from the data node file system. Reducing the value of [TimeBetweenGlobalCheckpoints](#page-110-0) tends to decrease the amount of data to be written for each global checkpoint, and so may provide some protection against redo log buffer overflows when trying to write a global checkpoint; however, reducing this value also permits less time in which to write the GCP, so this must be done with caution.

In addition to the considerations given for [DiskPageBufferMemory](#page-150-0) as explained previously, it is also very important that the [DiskIOThreadPool](#page-151-0) configuration parameter be set correctly; having [DiskIOThreadPool](#page-151-0) set too high is very likely to cause GCP stop errors (Bug #37227).

GCP stops can be caused by save or commit timeouts; the [TimeBetweenEpochsTimeout](#page-111-1) data node configuration parameter determines the timeout for commits. However, it is possible to disable both types of timeouts by setting this parameter to 0.
...
while the node was running.) The [TotalSendBufferMemory](#page-157-3) and OverLoadLimit data node configuration parameters permit the setting of limits on this memory allocation. For more information about the use of these parameters (as well as SendBufferMemory), see Section 25.4.3.14, "Configuring NDB Cluster Send Buffer Parameters".

#### <span id="page-157-0"></span>• [ExtraSendBufferMemory](#page-157-0)

This parameter specifies the amount of transporter send buffer memory to allocate in addition to any set using [TotalSendBufferMemory](#page-157-3), SendBufferMemory, or both.

## <span id="page-157-3"></span>• [TotalSendBufferMemory](#page-157-3)
...
This parameter sets an upper limit in seconds for trying to write a given redo log to disk before timing out. The number of times the data node tries to flush this redo log, but takes longer than RedoOverCommitLimit, is kept and compared with [RedoOverCommitCounter](#page-157-1), and when flushing takes too long more times than the value of that parameter, any transactions that were not committed as a result of the flush timeout are aborted. When this occurs, the API node where any of these transactions originated handles the operations making up those transactions according to its [DefaultOperationRedoProblemAction](#page-169-1) setting (it either queues the operations to be re-tried, or aborts them).

**Controlling restart attempts.** It is possible to exercise finely-grained control over restart attempts by data nodes when they fail to start using the [MaxStartFailRetries](#page-159-2) and [StartFailRetryDelay](#page-158-0) data node configuration parameters.

[MaxStartFailRetries](#page-159-2) limits the total number of retries made before giving up on starting the data node, [StartFailRetryDelay](#page-158-0) sets the number of seconds between retry attempts. These parameters are listed here:

<span id="page-158-0"></span>• [StartFailRetryDelay](#page-158-0)
...
Both this parameter and [MaxStartFailRetries](#page-159-2) are ignored unless [StopOnError](#page-103-1) is equal to 0.

# <span id="page-159-2"></span>• [MaxStartFailRetries](#page-159-2)
...
Both this parameter and [StartFailRetryDelay](#page-158-0) are ignored unless [StopOnError](#page-103-1) is equal to 0.

**NDB index statistics parameters.** The parameters in the following list relate to NDB index statistics generation.

#### <span id="page-159-0"></span>• [IndexStatAutoCreate](#page-159-0)
...
| Default               | 0                                                                                 |
| Range                 | 0, 1                                                                              |
| Version (or<br>later) | NDB 8.0.27                                                                        |
| Type or units         | integer                                                                           |
| Default               | 1                                                                                 |
| Range                 | 0, 1                                                                              |
| Restart Type          | Node Restart:<br>Requires a<br>rolling restart<br>of the cluster.<br>(NDB 8.0.13) |
...
#### <span id="page-159-1"></span>• [IndexStatAutoUpdate](#page-159-1)
...
Enable (set equal to 1) or disable (set equal to 0) monitoring of indexes for changes, and trigger automatic statistics updates when these are detected. The degree of change needed to trigger the updates are determined by the settings for the [IndexStatTriggerPct](#page-161-0) and [IndexStatTriggerScale](#page-161-1) options.

#### <span id="page-160-1"></span>• [IndexStatSaveSize](#page-160-1)
...
At least one sample is always produced, regardless of any size limit. This size is scaled by [IndexStatSaveScale](#page-160-0).

The size specified by [IndexStatSaveSize](#page-160-1) is scaled by the value of IndexStatTriggerPct for a large index, times 0.01. This is further multiplied by the logarithm to the base 2 of the index size. Setting IndexStatTriggerPct equal to 0 disables the scaling effect.

#### <span id="page-160-0"></span>• [IndexStatSaveScale](#page-160-0)
...
| Default               | 100                                    |
| Range                 | 0 - 4294967039<br>(0xFFFFFEFF)         |
| Restart Type          | Initial Node<br>Restart:<br>Requires a |
...
#### <span id="page-161-0"></span>• [IndexStatTriggerPct](#page-161-0)
...
Percentage change in updates that triggers an index statistics update. The value is scaled by [IndexStatTriggerScale](#page-161-1). You can disable this trigger altogether by setting IndexStatTriggerPct to 0.

#### <span id="page-161-1"></span>• [IndexStatTriggerScale](#page-161-1)
...
Scale [IndexStatTriggerPct](#page-161-0) by this amount times 0.01 for a large index. A value of 0 disables scaling.

#### <span id="page-162-0"></span>• [IndexStatUpdateDelay](#page-162-0)
...
# <span id="page-162-1"></span>**25.4.3.7 Defining SQL and Other API Nodes in an NDB Cluster**
...
For a discussion of MySQL server options for NDB Cluster, see [MySQL Server](#page-174-1) [Options for NDB Cluster](#page-174-1). For information about MySQL server system variables relating to NDB Cluster, see [NDB Cluster System Variables](#page-187-0).

#### <span id="page-163-1"></span>• Id
...
[NodeId](#page-164-1) is the preferred parameter name to use when identifying API nodes. (Id continues to be supported for backward compatibility, but is now deprecated and generates a warning when used. It is also subject to future removal.)

#### <span id="page-163-0"></span>• ConnectionMap
...
#### <span id="page-164-1"></span>• NodeId
...
This parameter is deprecated, and is subject to removal in a future release. Use the [HostName](#page-165-0) parameter instead.

The node ID for this node can be given out only to connections that explicitly request it. A management server that requests "any" node ID cannot use this one. This parameter can be used when running multiple management servers on the same host, and [HostName](#page-165-0) is not sufficient for distinguishing among processes.

#### <span id="page-165-0"></span>• HostName
...
# <span id="page-165-1"></span>• [LocationDomainId](#page-165-1)
...
# <span id="page-166-1"></span>• ArbitrationRank
...
By setting ArbitrationRank to 0 on all management and SQL nodes, you can disable arbitration completely. You can also control arbitration by overriding this parameter; to do so, set the [Arbitration](#page-117-0) parameter in the [ndbd default] section of the config.ini global configuration file.

# <span id="page-166-0"></span>• ArbitrationDelay
...
#### <span id="page-167-0"></span>• [BatchByteSize](#page-167-0)
...
For queries that are translated into full table scans or range scans on indexes, it is important for best performance to fetch records in properly sized batches. It is possible to set the proper size both in terms of number of records ([BatchSize](#page-167-1)) and in terms of bytes (BatchByteSize). The actual batch size is limited by both parameters.
...
#### <span id="page-167-1"></span>• [BatchSize](#page-167-1)
...
#### <span id="page-167-2"></span>• [ExtraSendBufferMemory](#page-167-2)
...
This parameter specifies the amount of transporter send buffer memory to allocate in addition to any that has been set using [TotalSendBufferMemory](#page-169-2), SendBufferMemory, or both.

<span id="page-168-0"></span>• [HeartbeatThreadPriority](#page-168-0)
...
<span id="page-168-1"></span>• [MaxScanBatchSize](#page-168-1)
...
# <span id="page-169-2"></span>• TotalSendBufferMemory
...
#### <span id="page-169-0"></span>• [AutoReconnect](#page-169-0)
...
#### • [DefaultOperationRedoProblemAction](#page-169-1)
...
This parameter (along with [RedoOverCommitLimit](#page-157-2) and [RedoOverCommitCounter](#page-157-1)) controls the data node's handling of operations when too much time is taken flushing redo logs to disk. This occurs when a given redo log flush takes longer than [RedoOverCommitLimit](#page-157-2) seconds, more than [RedoOverCommitCounter](#page-157-1) times, causing any pending transactions to be aborted.
...
- <span id="page-170-0"></span>• [DefaultHashMapSize](#page-170-0)
...
<span id="page-170-1"></span>• [Wan](#page-170-1)
...
#### <span id="page-171-0"></span>• [ConnectBackoffMaxTime](#page-171-0)
...
| Symbol | Restart Type | Description                                                                                                   |
|--------|--------------|---------------------------------------------------------------------------------------------------------------|
|        |              | Section 25.6.5, "Performing<br>a Rolling Restart of an NDB<br>Cluster")                                       |
| S      | System       | All cluster nodes must be<br>shut down completely, then<br>restarted, to effect a change in<br>this parameter |
| I      | Initial      | Data nodes must be restarted<br>using theinitial option                                                       |