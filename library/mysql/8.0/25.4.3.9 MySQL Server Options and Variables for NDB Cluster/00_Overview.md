...
For NDB Cluster configuration parameters used in the cluster configuration file (usually named config.ini), see [Section 25.4, "Configuration of NDB Cluster".](#page-22-0)

#### <span id="page-174-1"></span>**MySQL Server Options for NDB Cluster**
...
<span id="page-174-0"></span>• [--ndbcluster](#page-174-0)
...
The NDBCLUSTER storage engine is necessary for using NDB Cluster. If a mysqld binary includes support for the NDBCLUSTER storage engine, the engine is disabled by default. Use the [-](#page-174-0) [ndbcluster](#page-174-0) option to enable it. Use --skip-ndbcluster to explicitly disable the engine.
...
<span id="page-175-2"></span>• --ndb-allow-copying-alter-table=[ON|OFF]
...
<span id="page-175-0"></span>• --ndb-applier-allow-skip-epoch
...
<span id="page-175-1"></span>• --ndb-batch-size=#
...
<span id="page-176-0"></span>• --ndb-cluster-connection-pool=#
...
<span id="page-176-1"></span>• --ndb-cluster-connection-pool-nodeids=list
...
Specifies a comma-separated list of node IDs for connections to the cluster used by an SQL node. The number of nodes in this list must be the same as the value set for the [--ndb-cluster](#page-176-0)[connection-pool](#page-176-0) option.

<span id="page-177-0"></span>• --ndb-blob-read-batch-bytes=bytes
...
<span id="page-177-1"></span>• --ndb-blob-write-batch-bytes=bytes
...
<span id="page-178-0"></span>• --ndb-connectstring=connection\_string

| Command-Line Format | ndb-connectstring |
|---------------------|-------------------|
| Type                | String            |

When using the NDBCLUSTER storage engine, this option specifies the management server that distributes cluster configuration data. See [Section 25.4.3.3, "NDB Cluster Connection Strings",](#page-53-0) for syntax.

<span id="page-178-1"></span>• --ndb-default-column-format=[FIXED|DYNAMIC]
...
<span id="page-178-2"></span>• --ndb-deferred-constraints=[0|1]
...
<span id="page-179-2"></span>• --ndb-schema-dist-timeout=#
...
<span id="page-179-0"></span>• --ndb-distribution=[KEYHASH|LINHASH]
...
<span id="page-179-1"></span>• --ndb-log-apply-status
...
This option has no effect unless the MySQL server is started with the [--ndbcluster](#page-174-0) option.

<span id="page-180-0"></span>• --ndb-log-empty-epochs=[ON|OFF]
...
Causes epochs during which there were no changes to be written to the ndb\_apply\_status and ndb\_binlog\_index tables, even when log\_replica\_updates or log\_slave\_updates is enabled.
...
<span id="page-180-1"></span>• --ndb-log-empty-update=[ON|OFF]
...
Causes updates that produced no changes to be written to the ndb\_apply\_status and ndb\_binlog\_index tables, even when log\_replica\_updates or log\_slave\_updates is enabled.
...
<span id="page-181-0"></span>• --ndb-log-exclusive-reads=[0|1]
...
Starting the server with this option causes primary key reads to be logged with exclusive locks, which allows for NDB Cluster Replication conflict detection and resolution based on read conflicts. You can also enable and disable these locks at runtime by setting the value of the [ndb\\_log\\_exclusive\\_reads](#page-197-2) system variable to 1 or 0, respectively. 0 (disable locking) is the default.

For more information, see Read conflict detection and resolution.

<span id="page-181-1"></span>• --ndb-log-fail-terminate
...
<span id="page-181-2"></span>• --ndb-log-orig
...
<span id="page-182-0"></span>• --ndb-log-transaction-dependency
...
<span id="page-182-1"></span>• --ndb-log-transaction-id
...
The deprecated log\_bin\_use\_v1\_row\_events system variable, which defaults to OFF, must not be set to ON when you use --ndb-log-transaction-id=ON.

<span id="page-182-2"></span>• --ndb-log-update-as-write
...
Whether updates on the source are written to the binary log as updates (OFF) or writes (ON). When this option is enabled, and both [--ndb-log-updated-only](#page-183-1) and [--ndb-log-update-minimal](#page-183-0) are disabled, operations of different types are loǵged as described in the following list:
...
<span id="page-183-1"></span>• --ndb-log-updated-only
...
Whether mysqld writes updates only (ON) or complete rows (OFF) to the binary log. When this option is enabled, and both [--ndb-log-update-as-write](#page-182-2) and [--ndb-log-update-minimal](#page-183-0) are disabled, operations of different types are loǵged as described in the following list:
...
<span id="page-183-0"></span>• --ndb-log-update-minimal
...
<span id="page-184-0"></span>• --ndb-mgmd-host=host[:port]
...
Can be used to set the host and port number of a single management server for the program to connect to. If the program requires node IDs or references to multiple management servers (or both) in its connection information, use the [--ndb-connectstring](#page-178-0) option instead.

<span id="page-184-1"></span>• --ndb-nodeid=#
...
The --ndb-nodeid option overrides any node ID set with [--ndb-connectstring](#page-178-0), regardless of the order in which the two options are used.
...
Regardless of how the node ID is determined, its is shown as the value of the global status variable Ndb\_cluster\_node\_id in the output of SHOW STATUS, and as cluster\_node\_id in the connection row of the output of SHOW ENGINE NDBCLUSTER STATUS.

For more information about node IDs for NDB Cluster SQL nodes, see [Section 25.4.3.7, "Defining](#page-162-1) [SQL and Other API Nodes in an NDB Cluster".](#page-162-1)

<span id="page-185-2"></span>• --ndbinfo={ON|OFF|FORCE}
...
<span id="page-185-1"></span>• --ndb-optimization-delay=milliseconds
...
<span id="page-185-0"></span>• --ndb-optimized-node-selection
...
#### <span id="page-186-0"></span>• --ndb-transid-mysql-connection-map=state

| Command-Line Format | ndb-transid-mysql-connection<br>map[=state] |
|---------------------|---------------------------------------------|
| Type                | Enumeration                                 |
| Default Value       | ON                                          |
| Valid Values        | ON                                          |
...
#### <span id="page-186-1"></span>• --ndb-wait-connected=seconds
...
#### <span id="page-186-2"></span>• --ndb-wait-setup=seconds
...
<span id="page-187-3"></span>• --skip-ndbcluster

| Command-Line Format | skip-ndbcluster |
|---------------------|-----------------|
|---------------------|-----------------|

Disable the NDBCLUSTER storage engine. This is the default for binaries that were built with NDBCLUSTER storage engine support; the server allocates memory and other resources for this storage engine only if the [--ndbcluster](#page-174-0) option is given explicitly. See [Section 25.4.1, "Quick Test](#page-22-1) [Setup of NDB Cluster",](#page-22-1) for an example.

#### <span id="page-187-0"></span>**NDB Cluster System Variables**
...
<span id="page-187-1"></span>• [ndb\\_autoincrement\\_prefetch\\_sz](#page-187-1)
...
<span id="page-187-2"></span>• [ndb\\_clear\\_apply\\_status](#page-187-2)
...
By the default, executing RESET SLAVE causes an NDB Cluster replica to purge all rows from its ndb\_apply\_status table. You can disable this by setting ndb\_clear\_apply\_status=OFF.

<span id="page-188-0"></span>• [ndb\\_conflict\\_role](#page-188-0)
...
Determines the role of this SQL node (and NDB Cluster) in a circular ("active-active") replication setup. ndb\_slave\_conflict\_role can take any one of the values PRIMARY, SECONDARY, PASS, or NULL (the default). The replica SQL thread must be stopped before you can change ndb\_slave\_conflict\_role. In addition, it is not possible to change directly between PASS and either of PRIMARY or SECONDARY directly; in such cases, you must ensure that the SQL thread is stopped, then execute SET @@GLOBAL.ndb\_slave\_conflict\_role = 'NONE' first.

This variable replaces ndb\_slave\_conflict\_role, which is deprecated as of NDB 8.0.23.

For more information, see Section 25.7.12, "NDB Cluster Replication Conflict Resolution".

<span id="page-188-1"></span>• [ndb\\_data\\_node\\_neighbour](#page-188-1)
...
<span id="page-189-2"></span>• [ndb\\_dbg\\_check\\_shares](#page-189-2)
...
<span id="page-189-0"></span>• [ndb\\_default\\_column\\_format](#page-189-0)
...
<span id="page-189-1"></span>• [ndb\\_deferred\\_constraints](#page-189-1)
...
#### <span id="page-190-0"></span>• [ndb\\_distribution](#page-190-0)
...
# <span id="page-190-1"></span>• [ndb\\_eventbuffer\\_free\\_percent](#page-190-1)
...
# <span id="page-190-2"></span>• [ndb\\_eventbuffer\\_max\\_alloc](#page-190-2)
...
<span id="page-191-0"></span>• [ndb\\_extra\\_logging](#page-191-0)
...
<span id="page-191-1"></span>• [ndb\\_force\\_send](#page-191-1)
...
• [ndb\\_fully\\_replicated](#page-191-2)
...
#### <span id="page-192-0"></span>• [ndb\\_index\\_stat\\_enable](#page-192-0)
...
#### <span id="page-192-1"></span>• [ndb\\_index\\_stat\\_option](#page-192-1)
...
**Table 25.20 ndb\_index\_stat\_option options and values**
...
<span id="page-194-0"></span>• [ndb\\_join\\_pushdown](#page-194-0)
...
This variable controls whether joins on NDB tables are pushed down to the NDB kernel (data nodes). Previously, a join was handled using multiple accesses of NDB by the SQL node; however, when [ndb\\_join\\_pushdown](#page-194-0) is enabled, a pushable join is sent in its entirety to the data nodes, where it can be distributed among the data nodes and executed in parallel on multiple copies of the data, with a single, merged result being returned to mysqld. This can reduce greatly the number of round trips between an SQL node and the data nodes required to handle such a join.

By default, [ndb\\_join\\_pushdown](#page-194-0) is enabled.
...
<span id="page-196-1"></span>• [ndb\\_log\\_apply\\_status](#page-196-1)
...
A read-only variable which shows whether the server was started with the [--ndb-log-apply](#page-179-1)[status](#page-179-1) option.

<span id="page-196-0"></span>• [ndb\\_log\\_bin](#page-196-0)
...
Causes updates to NDB tables to be written to the binary log. The setting for this variable has no effect if binary logging is not already enabled on the server using log\_bin. In NDB 8.0, ndb\_log\_bin defaults to 0 (FALSE).

<span id="page-196-2"></span>• [ndb\\_log\\_binlog\\_index](#page-196-2)
...
Causes a mapping of epochs to positions in the binary log to be inserted into the ndb\_binlog\_index table. Setting this variable has no effect if binary logging is not already enabled for the server using log\_bin. (In addition, [ndb\\_log\\_bin](#page-196-0) must not be disabled.) ndb\_log\_binlog\_index defaults to 1 (ON); normally, there is never any need to change this value in a production environment.

<span id="page-196-3"></span>• [ndb\\_log\\_cache\\_size](#page-196-3)
...
| Scope                | Global               |
| Dynamic              | Yes                  |
| SET_VAR Hint Applies | 4367<br>No           |
...
<span id="page-197-0"></span>• [ndb\\_log\\_empty\\_epochs](#page-197-0)
...
<span id="page-197-1"></span>• [ndb\\_log\\_empty\\_update](#page-197-1)
...
When this variable is set to ON (1), update transactions with no changes are written to the binary log, even when log\_replica\_updates or log\_slave\_updates is enabled.

<span id="page-197-2"></span>• [ndb\\_log\\_exclusive\\_reads](#page-197-2)
...
<span id="page-197-3"></span>• [ndb\\_log\\_orig](#page-197-3)
...
Shows whether the originating server ID and epoch are logged in the ndb\_binlog\_index table. Set using the [--ndb-log-orig](#page-181-2) server option.

<span id="page-198-0"></span>• [ndb\\_log\\_transaction\\_id](#page-198-0)
...
This read-only, Boolean system variable shows whether a replica mysqld writes NDB transaction IDs in the binary log (required to use "active-active" NDB Cluster Replication with NDB \$EPOCH\_TRANS() conflict detection). To change the setting, use the [--ndb-log-transaction](#page-182-1)[id](#page-182-1) option.

[ndb\\_log\\_transaction\\_id](#page-198-0) is not supported in mainline MySQL Server 8.0.

For more information, see Section 25.7.12, "NDB Cluster Replication Conflict Resolution".

<span id="page-198-1"></span>• [ndb\\_log\\_transaction\\_compression](#page-198-1)
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| +++<br>  binlog_transaction_compression<br>  ON<br>  binlog_transaction_compression_level_zstd   3<br>  ndb_log_transaction_compression<br>  ON<br>  ndb_log_transaction_compression_level_zstd   3<br>+++ |
...
<span id="page-199-0"></span>• [ndb\\_log\\_transaction\\_compression\\_level\\_zstd](#page-199-0)
...
The ZSTD compression level used for writing compressed transactions to the replica's binary log if enabled by [ndb\\_log\\_transaction\\_compression](#page-198-1). Not supported if mysqld was not compiled with support for the NDB storage engine.

See Section 7.4.4.5, "Binary Log Transaction Compression", for more information.

<span id="page-199-1"></span>• [ndb\\_metadata\\_check](#page-199-1)
...
<span id="page-0-0"></span>• [ndb\\_metadata\\_check\\_interval](#page-0-0)
...
<span id="page-0-1"></span>• [ndb\\_metadata\\_sync](#page-0-1)
...
<span id="page-0-2"></span>• [ndb\\_optimized\\_node\\_selection](#page-0-2)
...
- 1. Start with the value set for the [Group](#page-34-0) parameter (default 55).
...
- 3. Setting ndb\_data\_node\_neighbour further decreases the effective Group value by 50, causing this node to be regarded as the nearest node. This is needed only when all data nodes are on hosts other than that hosts the API node and it is desirable to dedicate one of them to the API node. In normal cases, the default adjustment described previously is sufficient.
...
<span id="page-1-0"></span>• [ndb\\_read\\_backup](#page-1-0)
...
<span id="page-2-0"></span>• [ndb\\_recv\\_thread\\_activation\\_threshold](#page-2-0)
...
<span id="page-2-1"></span>• [ndb\\_recv\\_thread\\_cpu\\_mask](#page-2-1)
...
<span id="page-2-2"></span>• [ndb\\_report\\_thresh\\_binlog\\_epoch\\_slip](#page-2-2)
...
#### <span id="page-3-0"></span>• [ndb\\_report\\_thresh\\_binlog\\_mem\\_usage](#page-3-0)
...
#### <span id="page-3-1"></span>• [ndb\\_row\\_checksum](#page-3-1)
...
<span id="page-4-0"></span>• [ndb\\_schema\\_dist\\_lock\\_wait\\_timeout](#page-4-0)
...
<span id="page-4-1"></span>• [ndb\\_schema\\_dist\\_timeout](#page-4-1)
...
<span id="page-4-2"></span>• [ndb\\_schema\\_dist\\_upgrade\\_allowed](#page-4-2)
...
<span id="page-5-0"></span>• [ndb\\_show\\_foreign\\_key\\_mock\\_tables](#page-5-0)
...
<span id="page-5-1"></span>• [ndb\\_slave\\_conflict\\_role](#page-5-1)
...
Deprecated in NDB 8.0.23, and subject to removal in a future release. Use ndb\_conflict\_role instead.

<span id="page-5-2"></span>• [ndb\\_table\\_no\\_logging](#page-5-2)
...
[ndb\\_table\\_no\\_logging](#page-5-2) has no effect on the creation of NDB table schema files; to suppress these, use [ndb\\_table\\_temporary](#page-6-0) instead.

<span id="page-6-0"></span>• [ndb\\_table\\_temporary](#page-6-0)
...
<span id="page-6-1"></span>• [ndb\\_use\\_copying\\_alter\\_table](#page-6-1)
...
<span id="page-6-2"></span>• [ndb\\_use\\_exact\\_count](#page-6-2)
...
<span id="page-7-0"></span>• [ndb\\_use\\_transactions](#page-7-0)
...
<span id="page-7-1"></span>• [ndb\\_version](#page-7-1)
...
<span id="page-7-2"></span>• [ndb\\_version\\_string](#page-7-2)
...
<span id="page-7-3"></span>• [replica\\_allow\\_batching](#page-7-3)
...
Whether or not batched updates are enabled on NDB Cluster replicas. Beginning with NDB 8.0.26, you should use replica\_allow\_batching in place of [slave\\_allow\\_batching](#page-9-0), which is deprecated in that release.

Allowing batched updates on the replica greatly improves performance, particularly when replicating TEXT, BLOB, and JSON columns. For this reason, replica\_allow\_batching is enabled by default in NDB 8.0.30 and later.
...
<span id="page-8-0"></span>• [ndb\\_replica\\_batch\\_size](#page-8-0)
...
<span id="page-8-1"></span>• [ndb\\_replica\\_blob\\_write\\_batch\\_bytes](#page-8-1)
...
sessions. The reason for this is that, when ndb\_replica\_blob\_write\_batch\_bytesis not set,the effective blob batch size (that is, the maximum number of pending bytes to write for blob columns) is determined by the greater of the value of --ndb-blob-write-batch-bytes and 2 MB (the default for ndb\_replica\_blob\_write\_batch\_bytes).
...
<span id="page-9-1"></span>• [server\\_id\\_bits](#page-9-1)
...
When extracting the effective server ID from server\_id for purposes such as detection of replication loops, the server ignores the remaining bits. The [server\\_id\\_bits](#page-9-1) variable is used to mask out any irrelevant bits of server\_id in the I/O and SQL threads when deciding whether an event should be ignored based on the server ID.

This data can be read from the binary log by mysqlbinlog, provided that it is run with its own [server\\_id\\_bits](#page-9-1) variable set to 32 (the default).

If the value of server\_id greater than or equal to 2 to the power of [server\\_id\\_bits](#page-9-1); otherwise, mysqld refuses to start.

This system variable is supported only by NDB Cluster. It is not supported in the standard MySQL 8.0 Server.

<span id="page-9-0"></span>• [slave\\_allow\\_batching](#page-9-0)
...
Whether or not batched updates are enabled on NDB Cluster replicas. Beginning with NDB 8.0.26, this variable is deprecated, and you should use [replica\\_allow\\_batching](#page-7-3) instead.

Allowing batched updates on the replica greatly improves performance, particularly when replicating TEXT, BLOB, and JSON columns. For this reason, replica\_allow\_batching is ON by default in NDB 8.0.30 and later. Also beginning with NDB 8.0.30, a warning is issued whenever this variable is set to OFF.

Setting this variable has an effect only when using replication with the NDB storage engine; in MySQL Server 8.0, it is present but does nothing. For more information, see Section 25.7.6, "Starting NDB Cluster Replication (Single Replication Channel)".

#### <span id="page-10-0"></span>• [transaction\\_allow\\_batching](#page-10-0)
...
When set to 1 or ON, this variable enables batching of statements within the same transaction. To use this variable, autocommit must first be disabled by setting it to 0 or OFF; otherwise, setting [transaction\\_allow\\_batching](#page-10-0) has no effect.
...
[transaction\\_allow\\_batching](#page-10-0) should not be used whenever there is the possibility that the effects of a given statement depend on the outcome of a previous statement within the same transaction.
...
#### <span id="page-10-1"></span>• [ndbinfo\\_database](#page-10-1)
...
#### <span id="page-10-2"></span>• [ndbinfo\\_max\\_bytes](#page-10-2)
...
<span id="page-11-0"></span>• [ndbinfo\\_max\\_rows](#page-11-0)
...
<span id="page-11-1"></span>• [ndbinfo\\_offline](#page-11-1)
...
• [ndbinfo\\_show\\_hidden](#page-11-2)
...
<span id="page-12-0"></span>• [ndbinfo\\_table\\_prefix](#page-12-0)
...
The prefix used in naming the ndbinfo database's base tables (normally hidden, unless exposed by setting [ndbinfo\\_show\\_hidden](#page-11-2)). This is a read-only variable whose default value is ndb\$; the prefix itself is determined at compile time.

<span id="page-12-1"></span>• [ndbinfo\\_version](#page-12-1)
...
<span id="page-12-2"></span>• [Handler\\_discover](#page-12-2)

The MySQL server can ask the NDBCLUSTER storage engine if it knows about a table with a given name. This is called discovery. [Handler\\_discover](#page-12-2) indicates the number of times that tables have been discovered using this mechanism.

<span id="page-12-3"></span>• [Ndb\\_api\\_adaptive\\_send\\_deferred\\_count](#page-12-3)
...
<span id="page-12-4"></span>• [Ndb\\_api\\_adaptive\\_send\\_deferred\\_count\\_session](#page-12-4)

Number of adaptive send calls that were not actually sent.

<span id="page-13-0"></span>• [Ndb\\_api\\_adaptive\\_send\\_deferred\\_count\\_replica](#page-13-0)

Number of adaptive send calls that were not actually sent by this replica.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-13-1"></span>• [Ndb\\_api\\_adaptive\\_send\\_deferred\\_count\\_slave](#page-13-1)
...
[Ndb\\_api\\_adaptive\\_send\\_deferred\\_count\\_replica](#page-13-0) instead.

Number of adaptive send calls that were not actually sent by this replica.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-13-2"></span>• [Ndb\\_api\\_adaptive\\_send\\_forced\\_count](#page-13-2)

Number of adaptive send calls using forced-send sent by this MySQL Server (SQL node).

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-13-3"></span>• [Ndb\\_api\\_adaptive\\_send\\_forced\\_count\\_session](#page-13-3)

Number of adaptive send calls using forced-send sent in this client session.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-13-4"></span>• [Ndb\\_api\\_adaptive\\_send\\_forced\\_count\\_replica](#page-13-4)

Number of adaptive send calls using forced-send sent by this replica.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-13-5"></span>• [Ndb\\_api\\_adaptive\\_send\\_forced\\_count\\_slave](#page-13-5)
...
[Ndb\\_api\\_adaptive\\_send\\_forced\\_count\\_replica](#page-13-4) instead.

Number of adaptive send calls using forced-send sent by this replica.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-13-6"></span>• [Ndb\\_api\\_adaptive\\_send\\_unforced\\_count](#page-13-6)

Number of adaptive send calls without forced-send sent by this MySQL server (SQL node).

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-13-7"></span>• [Ndb\\_api\\_adaptive\\_send\\_unforced\\_count\\_session](#page-13-7)

Number of adaptive send calls without forced-send sent in this client session.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-13-8"></span>• [Ndb\\_api\\_adaptive\\_send\\_unforced\\_count\\_replica](#page-13-8)

Number of adaptive send calls without forced-send sent by this replica.

<span id="page-14-0"></span>• [Ndb\\_api\\_adaptive\\_send\\_unforced\\_count\\_slave](#page-14-0)

![](_page_14_Picture_3.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_adaptive\\_send\\_unforced\\_count\\_replica](#page-13-8) instead.

Number of adaptive send calls without forced-send sent by this replica.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-14-1"></span>• [Ndb\\_api\\_bytes\\_sent\\_count\\_session](#page-14-1)
...
<span id="page-14-2"></span>• [Ndb\\_api\\_bytes\\_sent\\_count\\_replica](#page-14-2)
...
<span id="page-14-3"></span>• [Ndb\\_api\\_bytes\\_sent\\_count\\_slave](#page-14-3)

![](_page_14_Picture_17.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_bytes\\_sent\\_count\\_replica](#page-14-2) instead.
...
<span id="page-14-4"></span>• [Ndb\\_api\\_bytes\\_sent\\_count](#page-14-4)
...
<span id="page-14-5"></span>• [Ndb\\_api\\_bytes\\_received\\_count\\_session](#page-14-5)

Amount of data (in bytes) received from the data nodes in this client session.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it relates to the current session only, and is not affected by any other clients of this mysqld.

<span id="page-15-0"></span>• [Ndb\\_api\\_bytes\\_received\\_count\\_replica](#page-15-0)
...
<span id="page-15-1"></span>• [Ndb\\_api\\_bytes\\_received\\_count\\_slave](#page-15-1)

![](_page_15_Picture_7.jpeg)

### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_bytes\\_received\\_count\\_replica](#page-15-0) instead.
...
<span id="page-15-2"></span>• [Ndb\\_api\\_bytes\\_received\\_count](#page-15-2)
...
<span id="page-15-3"></span>• [Ndb\\_api\\_event\\_data\\_count\\_injector](#page-15-3)
...
<span id="page-15-4"></span>• [Ndb\\_api\\_event\\_data\\_count](#page-15-4)
...
<span id="page-15-5"></span>• [Ndb\\_api\\_event\\_nondata\\_count\\_injector](#page-15-5)

The number of events received, other than row change events, by the NDB binary log injector thread.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope.

<span id="page-16-0"></span>• [Ndb\\_api\\_event\\_nondata\\_count](#page-16-0)
...
<span id="page-16-1"></span>• [Ndb\\_api\\_event\\_bytes\\_count\\_injector](#page-16-1)
...
<span id="page-16-2"></span>• [Ndb\\_api\\_event\\_bytes\\_count](#page-16-2)
...
<span id="page-16-3"></span>• [Ndb\\_api\\_pk\\_op\\_count\\_session](#page-16-3)
...
<span id="page-16-4"></span>• [Ndb\\_api\\_pk\\_op\\_count\\_replica](#page-16-4)
...
<span id="page-17-0"></span>• [Ndb\\_api\\_pk\\_op\\_count\\_slave](#page-17-0)

![](_page_17_Picture_2.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_pk\\_op\\_count\\_replica](#page-16-4) instead.
...
<span id="page-17-1"></span>• [Ndb\\_api\\_pk\\_op\\_count](#page-17-1)
...
<span id="page-17-2"></span>• [Ndb\\_api\\_pruned\\_scan\\_count\\_session](#page-17-2)
...
<span id="page-17-3"></span>• [Ndb\\_api\\_pruned\\_scan\\_count\\_replica](#page-17-3)
...
<span id="page-17-4"></span>• [Ndb\\_api\\_pruned\\_scan\\_count\\_slave](#page-17-4)

![](_page_17_Picture_21.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_pruned\\_scan\\_count\\_replica](#page-17-3) instead.
...
<span id="page-17-5"></span>• [Ndb\\_api\\_pruned\\_scan\\_count](#page-17-5)
...
<span id="page-18-0"></span>• [Ndb\\_api\\_range\\_scan\\_count\\_session](#page-18-0)
...
<span id="page-18-1"></span>• [Ndb\\_api\\_range\\_scan\\_count\\_replica](#page-18-1)
...
<span id="page-18-2"></span>• [Ndb\\_api\\_range\\_scan\\_count\\_slave](#page-18-2)

![](_page_18_Picture_13.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_range\\_scan\\_count\\_replica](#page-18-1) instead.
...
<span id="page-18-3"></span>• [Ndb\\_api\\_range\\_scan\\_count](#page-18-3)
...
<span id="page-18-4"></span>• [Ndb\\_api\\_read\\_row\\_count\\_session](#page-18-4)
...
<span id="page-19-0"></span>• [Ndb\\_api\\_read\\_row\\_count\\_replica](#page-19-0)
...
<span id="page-19-1"></span>• [Ndb\\_api\\_read\\_row\\_count\\_slave](#page-19-1)

![](_page_19_Picture_6.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_read\\_row\\_count\\_replica](#page-19-0) instead.
...
<span id="page-19-2"></span>• [Ndb\\_api\\_read\\_row\\_count](#page-19-2)
...
<span id="page-19-3"></span>• [Ndb\\_api\\_scan\\_batch\\_count\\_session](#page-19-3)
...
<span id="page-19-4"></span>• [Ndb\\_api\\_scan\\_batch\\_count\\_replica](#page-19-4)
...
<span id="page-20-0"></span>• [Ndb\\_api\\_scan\\_batch\\_count\\_slave](#page-20-0)

![](_page_20_Picture_3.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_scan\\_batch\\_count\\_replica](#page-19-4) instead.
...
<span id="page-20-1"></span>• [Ndb\\_api\\_scan\\_batch\\_count](#page-20-1)
...
<span id="page-20-2"></span>• [Ndb\\_api\\_table\\_scan\\_count\\_session](#page-20-2)
...
<span id="page-20-3"></span>• [Ndb\\_api\\_table\\_scan\\_count\\_replica](#page-20-3)
...
<span id="page-20-4"></span>• [Ndb\\_api\\_table\\_scan\\_count\\_slave](#page-20-4)

![](_page_20_Picture_22.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_table\\_scan\\_count\\_replica](#page-20-3) instead.

The number of table scans that have been started by this replica, including scans of internal tables.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope. If this MySQL server does not act as a replica, or does not use NDB tables, this value is always 0.

<span id="page-21-0"></span>• [Ndb\\_api\\_table\\_scan\\_count](#page-21-0)
...
<span id="page-21-1"></span>• [Ndb\\_api\\_trans\\_abort\\_count\\_session](#page-21-1)
...
<span id="page-21-2"></span>• [Ndb\\_api\\_trans\\_abort\\_count\\_replica](#page-21-2)
...
<span id="page-21-3"></span>• [Ndb\\_api\\_trans\\_abort\\_count\\_slave](#page-21-3)

![](_page_21_Picture_14.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_trans\\_abort\\_count\\_replica](#page-21-2) instead.
...
<span id="page-21-4"></span>• [Ndb\\_api\\_trans\\_abort\\_count](#page-21-4)
...
<span id="page-21-5"></span>• [Ndb\\_api\\_trans\\_close\\_count\\_session](#page-21-5)

The number of transactions closed in this client session. This value may be greater than the sum of [Ndb\\_api\\_trans\\_commit\\_count\\_session](#page-22-0) and [Ndb\\_api\\_trans\\_abort\\_count\\_session](#page-21-1), since some transactions may have been rolled back.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it relates to the current session only, and is not affected by any other clients of this mysqld.

<span id="page-22-1"></span>• [Ndb\\_api\\_trans\\_close\\_count\\_replica](#page-22-1)

The number of transactions closed by this replica. This value may be greater than the sum of [Ndb\\_api\\_trans\\_commit\\_count\\_replica](#page-22-2) and [Ndb\\_api\\_trans\\_abort\\_count\\_replica](#page-21-2), since some transactions may have been rolled back.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope. If this MySQL server does not act as a replica, or does not use NDB tables, this value is always 0.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-22-3"></span>• [Ndb\\_api\\_trans\\_close\\_count\\_slave](#page-22-3)

![](_page_22_Picture_6.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_trans\\_close\\_count\\_replica](#page-22-1) instead.
...
<span id="page-22-4"></span>• [Ndb\\_api\\_trans\\_close\\_count](#page-22-4)

The number of transactions closed by this MySQL Server (SQL node). This value may be greater than the sum of [Ndb\\_api\\_trans\\_commit\\_count](#page-23-0) and [Ndb\\_api\\_trans\\_abort\\_count](#page-21-4), since some transactions may have been rolled back.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-22-0"></span>• [Ndb\\_api\\_trans\\_commit\\_count\\_session](#page-22-0)
...
<span id="page-22-2"></span>• [Ndb\\_api\\_trans\\_commit\\_count\\_replica](#page-22-2)

The number of transactions committed by this replica.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope. If this MySQL server does not act as a replica, or does not use NDB tables, this value is always 0.

<span id="page-23-1"></span>• [Ndb\\_api\\_trans\\_commit\\_count\\_slave](#page-23-1)

![](_page_23_Picture_2.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_trans\\_commit\\_count\\_replica](#page-22-2) instead.
...
<span id="page-23-0"></span>• [Ndb\\_api\\_trans\\_commit\\_count](#page-23-0)
...
<span id="page-23-2"></span>• [Ndb\\_api\\_trans\\_local\\_read\\_row\\_count\\_session](#page-23-2)
...
<span id="page-23-3"></span>• [Ndb\\_api\\_trans\\_local\\_read\\_row\\_count\\_replica](#page-23-3)
...
<span id="page-23-4"></span>• [Ndb\\_api\\_trans\\_local\\_read\\_row\\_count\\_slave](#page-23-4)

![](_page_23_Picture_21.jpeg)

### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_trans\\_local\\_read\\_row\\_count\\_replica](#page-23-3) instead.

The total number of rows that have been read by this replica. This includes all rows read by any primary key, unique key, or scan operation made by this replica.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope. If this MySQL server does not act as a replica, or does not use NDB tables, this value is always 0.

<span id="page-24-0"></span>• [Ndb\\_api\\_trans\\_local\\_read\\_row\\_count](#page-24-0)
...
<span id="page-24-1"></span>• [Ndb\\_api\\_trans\\_start\\_count\\_session](#page-24-1)
...
<span id="page-24-2"></span>• [Ndb\\_api\\_trans\\_start\\_count\\_replica](#page-24-2)
...
<span id="page-24-3"></span>• [Ndb\\_api\\_trans\\_start\\_count\\_slave](#page-24-3)

![](_page_24_Picture_14.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_trans\\_start\\_count\\_replica](#page-24-2) instead.
...
<span id="page-24-4"></span>• [Ndb\\_api\\_trans\\_start\\_count](#page-24-4)
...
<span id="page-24-5"></span>• [Ndb\\_api\\_uk\\_op\\_count\\_session](#page-24-5)

The number of operations in this client session based on or using unique keys.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it relates to the current session only, and is not affected by any other clients of this mysqld. <span id="page-25-0"></span>• [Ndb\\_api\\_uk\\_op\\_count\\_replica](#page-25-0)
...
<span id="page-25-1"></span>• [Ndb\\_api\\_uk\\_op\\_count\\_slave](#page-25-1)

![](_page_25_Picture_6.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_uk\\_op\\_count\\_replica](#page-25-0) instead.
...
<span id="page-25-2"></span>• [Ndb\\_api\\_uk\\_op\\_count](#page-25-2)
...
<span id="page-25-3"></span>• [Ndb\\_api\\_wait\\_exec\\_complete\\_count\\_session](#page-25-3)
...
<span id="page-25-4"></span>• [Ndb\\_api\\_wait\\_exec\\_complete\\_count\\_replica](#page-25-4)
...
<span id="page-25-5"></span>• [Ndb\\_api\\_wait\\_exec\\_complete\\_count\\_slave](#page-25-5)

![](_page_25_Picture_25.jpeg)

### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_wait\\_exec\\_complete\\_count\\_replica](#page-25-4) instead. The number of times a thread has been blocked by this replica while waiting for execution of an operation to complete. This includes all [execute\(\)](https://dev.mysql.com/doc/ndbapi/en/ndb-ndbtransaction.md#ndb-ndbtransaction-execute) calls as well as implicit executes for blob and auto-increment operations not visible to clients.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope. If this MySQL server does not act as a replica, or does not use NDB tables, this value is always 0.

For more information, see Section 25.6.15, "NDB API Statistics Counters and Variables".

<span id="page-26-0"></span>• [Ndb\\_api\\_wait\\_exec\\_complete\\_count](#page-26-0)
...
<span id="page-26-1"></span>• [Ndb\\_api\\_wait\\_meta\\_request\\_count\\_session](#page-26-1)
...
<span id="page-26-2"></span>• [Ndb\\_api\\_wait\\_meta\\_request\\_count\\_replica](#page-26-2)
...
<span id="page-26-3"></span>• [Ndb\\_api\\_wait\\_meta\\_request\\_count\\_slave](#page-26-3)

![](_page_26_Picture_17.jpeg)

### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_wait\\_meta\\_request\\_count\\_replica](#page-26-2) instead.

The number of times a thread has been blocked by this replica waiting for a metadata-based signal, such as is expected for DDL requests, new epochs, and seizure of transaction records.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope. If this MySQL server does not act as a replica, or does not use NDB tables, this value is always 0.

<span id="page-27-0"></span>• [Ndb\\_api\\_wait\\_meta\\_request\\_count](#page-27-0)
...
<span id="page-27-1"></span>• [Ndb\\_api\\_wait\\_nanos\\_count\\_session](#page-27-1)
...
<span id="page-27-2"></span>• [Ndb\\_api\\_wait\\_nanos\\_count\\_replica](#page-27-2)
...
<span id="page-27-3"></span>• [Ndb\\_api\\_wait\\_nanos\\_count\\_slave](#page-27-3)

![](_page_27_Picture_14.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_wait\\_nanos\\_count\\_replica](#page-27-2) instead.
...
<span id="page-27-4"></span>• [Ndb\\_api\\_wait\\_nanos\\_count](#page-27-4)

Total time (in nanoseconds) spent by this MySQL Server (SQL node) waiting for any type of signal from the data nodes.

Although this variable can be read using either SHOW GLOBAL STATUS or SHOW SESSION STATUS, it is effectively global in scope.

<span id="page-28-0"></span>• [Ndb\\_api\\_wait\\_scan\\_result\\_count\\_session](#page-28-0)
...
<span id="page-28-1"></span>• [Ndb\\_api\\_wait\\_scan\\_result\\_count\\_replica](#page-28-1)
...
<span id="page-28-2"></span>• [Ndb\\_api\\_wait\\_scan\\_result\\_count\\_slave](#page-28-2)

![](_page_28_Picture_10.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_api\\_wait\\_scan\\_result\\_count\\_replica](#page-28-1) instead.
...
<span id="page-28-3"></span>• [Ndb\\_api\\_wait\\_scan\\_result\\_count](#page-28-3)
...
<span id="page-28-4"></span>• [Ndb\\_cluster\\_node\\_id](#page-28-4)
...
<span id="page-28-5"></span>• [Ndb\\_config\\_from\\_host](#page-28-5)
...
<span id="page-28-6"></span>• [Ndb\\_config\\_from\\_port](#page-28-6)
...
<span id="page-29-0"></span>• [Ndb\\_config\\_generation](#page-29-0)
...
<span id="page-29-1"></span>• [Ndb\\_conflict\\_fn\\_epoch](#page-29-1)
...
<span id="page-29-2"></span>• [Ndb\\_conflict\\_fn\\_epoch\\_trans](#page-29-2)
...
<span id="page-29-3"></span>• [Ndb\\_conflict\\_fn\\_epoch2](#page-29-3)
...
<span id="page-29-4"></span>• [Ndb\\_conflict\\_fn\\_epoch2\\_trans](#page-29-4)
...
<span id="page-29-5"></span>• [Ndb\\_conflict\\_fn\\_max](#page-29-5)
...
<span id="page-29-6"></span>• [Ndb\\_conflict\\_fn\\_max\\_del\\_win](#page-29-6)
...
<span id="page-30-0"></span>• [Ndb\\_conflict\\_fn\\_max\\_del\\_win\\_ins](#page-30-0)
...
<span id="page-30-1"></span>• [Ndb\\_conflict\\_fn\\_max\\_ins](#page-30-1)
...
<span id="page-30-2"></span>• [Ndb\\_conflict\\_fn\\_old](#page-30-2)
...
<span id="page-30-3"></span>• [Ndb\\_conflict\\_last\\_conflict\\_epoch](#page-30-3)

The most recent epoch in which a conflict was detected on this replica. You can compare this value with [Ndb\\_replica\\_max\\_replicated\\_epoch](#page-33-0); if Ndb\_replica\_max\_replicated\_epoch is greater than Ndb\_conflict\_last\_conflict\_epoch, no conflicts have yet been detected.

See Section 25.7.12, "NDB Cluster Replication Conflict Resolution", for more information.

<span id="page-30-4"></span>• [Ndb\\_conflict\\_reflected\\_op\\_discard\\_count](#page-30-4)
...
<span id="page-30-5"></span>• [Ndb\\_conflict\\_reflected\\_op\\_prepare\\_count](#page-30-5)
...
<span id="page-30-6"></span>• [Ndb\\_conflict\\_refresh\\_op\\_count](#page-30-6)
...
<span id="page-30-7"></span>• [Ndb\\_conflict\\_last\\_stable\\_epoch](#page-30-7)

Number of rows found to be in conflict by a transactional conflict function

See Section 25.7.12, "NDB Cluster Replication Conflict Resolution", for more information.

<span id="page-31-0"></span>• [Ndb\\_conflict\\_trans\\_row\\_conflict\\_count](#page-31-0)
...
Currently, the only transactional conflict detection function supported by NDB Cluster is NDB\$EPOCH\_TRANS(), so this status variable is effectively the same as [Ndb\\_conflict\\_fn\\_epoch\\_trans](#page-29-2).

For more information, see Section 25.7.12, "NDB Cluster Replication Conflict Resolution".

<span id="page-31-1"></span>• [Ndb\\_conflict\\_trans\\_row\\_reject\\_count](#page-31-1)

Used in NDB Cluster Replication conflict resolution, this status variable shows the total number of rows realigned due to being determined as conflicting by a transactional conflict detection function. This includes not only [Ndb\\_conflict\\_trans\\_row\\_conflict\\_count](#page-31-0), but any rows in or dependent on conflicting transactions.

For more information, see Section 25.7.12, "NDB Cluster Replication Conflict Resolution".

<span id="page-31-2"></span>• [Ndb\\_conflict\\_trans\\_reject\\_count](#page-31-2)
...
<span id="page-31-3"></span>• [Ndb\\_conflict\\_trans\\_detect\\_iter\\_count](#page-31-3)

Used in NDB Cluster Replication conflict resolution, this shows the number of internal iterations required to commit an epoch transaction. Should be (slightly) greater than or equal to [Ndb\\_conflict\\_trans\\_conflict\\_commit\\_count](#page-31-4).

For more information, see Section 25.7.12, "NDB Cluster Replication Conflict Resolution".

<span id="page-31-4"></span>• [Ndb\\_conflict\\_trans\\_conflict\\_commit\\_count](#page-31-4)
...
<span id="page-31-5"></span>• [Ndb\\_epoch\\_delete\\_delete\\_count](#page-31-5)
...
<span id="page-31-6"></span>• [Ndb\\_execute\\_count](#page-31-6)

Provides the number of round trips to the NDB kernel made by operations.

<span id="page-31-7"></span>• [Ndb\\_fetch\\_table\\_stats](#page-31-7)
...
<span id="page-31-8"></span>• [Ndb\\_last\\_commit\\_epoch\\_server](#page-31-8)

The epoch most recently committed by NDB.

<span id="page-32-0"></span>• [Ndb\\_last\\_commit\\_epoch\\_session](#page-32-0)

The epoch most recently committed by this NDB client.

<span id="page-32-1"></span>• [Ndb\\_metadata\\_detected\\_count](#page-32-1)
...
<span id="page-32-2"></span>• [Ndb\\_metadata\\_excluded\\_count](#page-32-2)
...
Prior to NDB 8.0.22, this variable was named Ndb\_metadata\_blacklist\_size.

<span id="page-32-3"></span>• [Ndb\\_metadata\\_synced\\_count](#page-32-3)

The number of NDB metadata objects which have been synchronized on this SQL node since it was last restarted.

<span id="page-32-4"></span>• [Ndb\\_number\\_of\\_data\\_nodes](#page-32-4)

If the server is part of an NDB Cluster, the value of this variable is the number of data nodes in the cluster.

If the server is not part of an NDB Cluster, then the value of this variable is 0.

<span id="page-32-5"></span>• [Ndb\\_pushed\\_queries\\_defined](#page-32-5)
...
<span id="page-32-6"></span>• [Ndb\\_pushed\\_queries\\_dropped](#page-32-6)

The number of joins that were pushed down to the NDB kernel but that could not be handled there.

<span id="page-32-7"></span>• [Ndb\\_pushed\\_queries\\_executed](#page-32-7)

The number of joins successfully pushed down to NDB and executed there.

<span id="page-32-8"></span>• [Ndb\\_pushed\\_reads](#page-32-8)
...
<span id="page-32-9"></span>• [Ndb\\_pruned\\_scan\\_count](#page-32-9)
...
Using this variable together with [Ndb\\_scan\\_count](#page-33-1) can be helpful in schema design to maximize the ability of the server to prune scans to a single table partition, thereby involving replica only a single data node.

<span id="page-33-0"></span>• [Ndb\\_replica\\_max\\_replicated\\_epoch](#page-33-0)

The most recently committed epoch on this replica. You can compare this value with [Ndb\\_conflict\\_last\\_conflict\\_epoch](#page-30-3); if Ndb\_replica\_max\_replicated\_epoch is the greater of the two, no conflicts have yet been detected.

For more information, see Section 25.7.12, "NDB Cluster Replication Conflict Resolution".

<span id="page-33-1"></span>• [Ndb\\_scan\\_count](#page-33-1)

This variable holds a count of the total number of scans executed by NDBCLUSTER since the NDB Cluster was last started.

<span id="page-33-2"></span>• [Ndb\\_schema\\_participant\\_count](#page-33-2)

Indicates the number of MySQL servers that are participating in NDB schema change distribution.

Added in NDB 8.0.42.

<span id="page-33-3"></span>• [Ndb\\_slave\\_max\\_replicated\\_epoch](#page-33-3)

![](_page_33_Picture_11.jpeg)

#### **Note**

Deprecated in NDB 8.0.23; use [Ndb\\_slave\\_max\\_replicated\\_epoch](#page-33-3) instead.

The most recently committed epoch on this replica. You can compare this value with [Ndb\\_conflict\\_last\\_conflict\\_epoch](#page-30-3); if Ndb\_slave\_max\_replicated\_epoch is the greater of the two, no conflicts have yet been detected.

For more information, see Section 25.7.12, "NDB Cluster Replication Conflict Resolution".

<span id="page-33-4"></span>• [Ndb\\_system\\_name](#page-33-4)
...
<span id="page-33-5"></span>• [Ndb\\_trans\\_hint\\_count\\_session](#page-33-5)

The number of transactions using hints that have been started in the current session. Compare with [Ndb\\_api\\_trans\\_start\\_count\\_session](#page-24-1) to obtain the proportion of all NDB transactions able to use hints.

## <span id="page-33-6"></span>**25.4.3.10 NDB Cluster TCP/IP Connections**
...
For an exception to this rule, see [Section 25.4.3.11, "NDB Cluster TCP/IP](#page-41-0) [Connections Using Direct Connections".](#page-41-0)

To override the default connection parameters, it is necessary to define a connection using one or more [tcp] sections in the config.ini file. Each [tcp] section explicitly defines a TCP/IP connection between two NDB Cluster nodes, and must contain at a minimum the parameters [NodeId1](#page-35-0) and [NodeId2](#page-36-0), as well as any connection parameters to override.
...
#### <span id="page-34-0"></span>• Group

When [ndb\\_optimized\\_node\\_selection](#page-0-2) is enabled, node proximity is used in some cases to select which node to connect to. This parameter can be used to influence proximity by setting it to a lower value, which is interpreted as "closer". See the description of the system variable for more information.

### <span id="page-35-2"></span>• HostName1
...
The HostName1 and [HostName2](#page-35-1) parameters can be used to specify specific network interfaces to be used for a given TCP connection between two nodes. The values used for these parameters can be host names or IP addresses.

#### <span id="page-35-1"></span>• HostName2
...
The [HostName1](#page-35-2) and HostName2 parameters can be used to specify specific network interfaces to be used for a given TCP connection between two nodes. The values used for these parameters can be host names or IP addresses.

#### <span id="page-35-0"></span>• NodeId1
...
To identify a connection between two nodes it is necessary to provide their node IDs in the [tcp] section of the configuration file as the values of [NodeId1](#page-35-0) and NodeId2. These are the same unique Id values for each of these nodes as described in Section 25.4.3.7, "Defining SQL and Other API Nodes in an NDB Cluster".

#### <span id="page-36-1"></span>• [NodeIdServer](#page-36-1)
...
This parameter can be used to determine the amount of unsent data that must be present in the send buffer before the connection is considered overloaded. See [Section 25.4.3.14, "Configuring](#page-52-0) [NDB Cluster Send Buffer Parameters",](#page-52-0) for more information.

### <span id="page-37-0"></span>• [PreferIPVersion](#page-37-0)

| Version (or<br>later) | NDB 8.0.26                                                                                                                                                                                                         |
|-----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
#### <span id="page-37-1"></span>• [PreSendChecksum](#page-37-1)
...
If this parameter and [Checksum](#page-34-1) are both enabled, perform pre-send checksum checks, and check all TCP signals between nodes for errors. Has no effect if Checksum is not also enabled.

#### <span id="page-37-2"></span>• [Proxy](#page-37-2)
...
<span id="page-38-0"></span>• [ReceiveBufferMemory](#page-38-0)
...
<span id="page-38-1"></span>• [SendBufferMemory](#page-38-1)
...
If this parameter is set explicitly, then the memory is not dedicated to each transporter; instead, the value used denotes the hard limit for how much memory (out of the total available memory —that is, TotalSendBufferMemory) that may be used by a single transporter. For more information about configuring dynamic transporter send buffer memory allocation in NDB Cluster, see [Section 25.4.3.14, "Configuring NDB Cluster Send Buffer Parameters"](#page-52-0).
...
#### <span id="page-39-0"></span>• [TcpSpinTime](#page-39-0)
...
#### <span id="page-39-1"></span>• [TCP\\_MAXSEG\\_SIZE](#page-39-1)
...
#### <span id="page-40-0"></span>• [TCP\\_RCV\\_BUF\\_SIZE](#page-40-0)
...
### <span id="page-40-1"></span>• [TCP\\_SND\\_BUF\\_SIZE](#page-40-1)
...
| Symbol | Restart Type | Description                  |
|--------|--------------|------------------------------|
| I      | Initial      | Data nodes must be restarted |
|        |              | using theinitial option      |

## <span id="page-41-0"></span>**25.4.3.11 NDB Cluster TCP/IP Connections Using Direct Connections**
...
The [HostName1](#page-35-2) and [HostName2](#page-35-1) parameters are used only when specifying direct connections.
...
## <span id="page-41-1"></span>**25.4.3.12 NDB Cluster Shared-Memory Connections**
...
• By using [shm] sections in the cluster configuration file, each containing settings for [NodeId1](#page-45-0) and [NodeId2](#page-45-1). This method is described in more detail later in this section.
...
- [ShmSize](#page-47-0): Shared memory size
- [ShmSpinTime](#page-47-1): Time in µs to spin before sleeping
- [SendBufferMemory](#page-46-0): Size of buffer for signals sent from this node, in bytes.
- [SendSignalId](#page-46-1): Indicates that a signal ID is included in each signal sent through the transporter.
- [Checksum](#page-43-0): Indicates that a checksum is included in each signal sent through the transporter.
- [PreSendChecksum](#page-46-2): Checks of the checksum are made prior to sending the signal; Checksum must also be enabled for this to work
...
Parameters affecting all shared memory transporters are set in the [shm default] section; these can be overridden on a per-connection basis in one or more [shm] sections. Each such section must be associated with a given SHM connection using [NodeId1](#page-45-0) and [NodeId2](#page-45-1); the values required for these parameters are the node IDs of the two nodes connected by the transporter. You can also identify the nodes by host name using [HostName1](#page-44-0) and [HostName2](#page-44-1), but these parameters are not required.
...
For optimum performance, you can define a spin time for the SHM transporter ([ShmSpinTime](#page-47-1) parameter); this affects both the data node receiver thread and the poll owner (receive thread or user thread) in NDB.

#### <span id="page-43-0"></span>• Checksum
...
#### <span id="page-44-0"></span>• HostName1
...
The HostName1 and [HostName2](#page-44-1) parameters can be used to specify specific network interfaces to be used for a given SHM connection between two nodes. The values used for these parameters can be host names or IP addresses.

#### <span id="page-44-1"></span>• HostName2
...
The [HostName1](#page-44-0) and HostName2 parameters can be used to specify specific network interfaces to be used for a given SHM connection between two nodes. The values used for these parameters can be host names or IP addresses.

### <span id="page-45-0"></span>• NodeId1
...
To identify a connection between two nodes it is necessary to provide node identifiers for each of them, as NodeId1 and [NodeId2](#page-45-1).

#### <span id="page-45-1"></span>• NodeId2
...
To identify a connection between two nodes it is necessary to provide node identifiers for each of them, as [NodeId1](#page-45-0) and NodeId2.
...
When more than this many unsent bytes are in the send buffer, the connection is considered overloaded. See [Section 25.4.3.14, "Configuring NDB Cluster Send Buffer Parameters"](#page-52-0), and Section 25.6.16.65, "The ndbinfo transporters Table", for more information.

#### <span id="page-46-2"></span>• [PreSendChecksum](#page-46-2)
...
If this parameter and [Checksum](#page-43-0) are both enabled, perform pre-send checksum checks, and check all SHM signals between nodes for errors. Has no effect if Checksum is not also enabled.

### <span id="page-46-0"></span>• [SendBufferMemory](#page-46-0)
...
#### <span id="page-46-1"></span>• SendSignalId
...
#### <span id="page-47-2"></span>• [ShmKey](#page-47-2)
...
#### <span id="page-47-0"></span>• [ShmSize](#page-47-0)
...
Each SHM connection has a shared memory segment where messages between nodes are placed by the sender and read by the reader. The size of this segment is defined by [ShmSize](#page-47-0). The default value is 4MB.

#### • [ShmSpinTime](#page-47-1)
...
#### <span id="page-48-0"></span>• [SigNum](#page-48-0)
