...
- a member joining or leaving, which is covered by Group Replication's [Section 20.5.4, "Distributed](#page-134-0) [Recovery"](#page-134-0) and write protection.
- network failures, which are covered by the fencing modes.
- in single-primary groups, primary failover, which can also be an operation triggered by group\_replication\_set\_as\_primary().
...
#### <span id="page-129-0"></span>**Transaction Synchronization Points**
...
the latest write. This ensures that you do not pay the synchronization cost for every read-only transaction, but only for read/write transactions.
