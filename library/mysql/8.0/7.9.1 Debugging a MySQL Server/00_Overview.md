...
## <span id="page-80-0"></span>**7.9.1.1 Compiling MySQL for Debugging**
...
When you configure MySQL for debugging you automatically enable a lot of extra safety check functions that monitor the health of mysqld. If they find something "unexpected," an entry is written to stderr, which mysqld\_safe directs to the error log! This also means that if you are having some unexpected problems with MySQL and are using a source distribution, the first thing you should do is to configure MySQL for debugging. If you believe that you have found a bug, please use the instructions at Section 1.5, "How to Report Bugs or Problems".

In the Windows MySQL distribution, mysqld.exe is by default compiled with support for trace files.

## <span id="page-80-1"></span>**7.9.1.2 Creating Trace Files**
...
The trace file is made with the DBUG package by Fred Fish. See [Section 7.9.4, "The DBUG Package".](#page-90-0)