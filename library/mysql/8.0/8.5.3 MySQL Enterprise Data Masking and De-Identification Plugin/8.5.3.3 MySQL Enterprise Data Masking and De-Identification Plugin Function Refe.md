...
## <span id="page-163-0"></span>**8.5.3.4 MySQL Enterprise Data Masking and De-Identification Plugin Function Descriptions**
...
- [Data Masking Plugin Functions](#page-164-1)
- [Random Data Generation Plugin Functions](#page-167-2)

### • [Random Data Dictionary-Based Plugin Functions](#page-170-3)
...
For any version, if a string return value should be in a different character set, convert it. The following example shows how to convert the result of [gen\\_rnd\\_email\(\)](#page-168-0) to the utf8mb4 character set:
...
It may also be necessary to convert string arguments, as illustrated in [Using Masked Data for](https://dev.mysql.com/doc/refman/5.7/en/data-masking-usage.md#data-masking-usage-customer-identification) [Customer Identification](https://dev.mysql.com/doc/refman/5.7/en/data-masking-usage.md#data-masking-usage-customer-identification).
...
## <span id="page-164-1"></span>**Data Masking Plugin Functions**

Each plugin function in this section performs a masking operation on its string argument and returns the masked result.

<span id="page-164-0"></span>• [mask\\_inner\(](#page-164-0)str, margin1, margin2 [, mask\_char])
...
<span id="page-165-0"></span>• [mask\\_outer\(](#page-165-0)str, margin1, margin2 [, mask\_char])
...
<span id="page-166-0"></span>• [mask\\_pan\(](#page-166-0)str)
...
<span id="page-166-1"></span>• [mask\\_pan\\_relaxed\(](#page-166-1)str)
...
<span id="page-167-0"></span>• [mask\\_ssn\(](#page-167-0)str)
...
## <span id="page-167-2"></span>**Random Data Generation Plugin Functions**

The plugin functions in this section generate random values for different types of data. When possible, generated values have characteristics reserved for demonstration or test values, to avoid having them mistaken for legitimate data. For example, [gen\\_rnd\\_us\\_phone\(\)](#page-169-1) returns a US phone number that uses the 555 area code, which is not assigned to phone numbers in actual use. Individual function descriptions describe any exceptions to this principle.

<span id="page-167-1"></span>• [gen\\_range\(](#page-167-1)lower, upper)
...
<span id="page-168-0"></span>• [gen\\_rnd\\_email\(\)](#page-168-0)
...
<span id="page-168-1"></span>• [gen\\_rnd\\_pan\(\[](#page-168-1)size])
...
Values returned from [gen\\_rnd\\_pan\(\)](#page-168-1) should be used only for test purposes, and are not suitable for publication. There is no way to guarantee that a given return value is not assigned to a legitimate payment account. Should it be necessary to publish a [gen\\_rnd\\_pan\(\)](#page-168-1) result, consider masking it with [mask\\_pan\(\)](#page-166-0) or [mask\\_pan\\_relaxed\(\)](#page-166-1).
...
<span id="page-169-0"></span>• [gen\\_rnd\\_ssn\(\)](#page-169-0)
...
<span id="page-169-1"></span>• [gen\\_rnd\\_us\\_phone\(\)](#page-169-1)
...
### <span id="page-170-3"></span>**Random Data Dictionary-Based Plugin Functions**
...
<span id="page-170-2"></span>• gen\_blacklist(str, dictionary\_name, [replacement\\_dictionary\\_name](#page-170-2))

Replaces a term present in one dictionary with a term from a second dictionary and returns the replacement term. This masks the original term by substitution. This function is deprecated in MySQL 8.0.23; use [gen\\_blocklist\(\)](#page-170-1) instead.

<span id="page-170-1"></span>• gen\_blocklist(str, dictionary\_name, [replacement\\_dictionary\\_name](#page-170-1))

Replaces a term present in one dictionary with a term from a second dictionary and returns the replacement term. This masks the original term by substitution. This function was added in MySQL 8.0.23; use it instead of [gen\\_blacklist\(\)](#page-170-2).
...
<span id="page-170-0"></span>• [gen\\_dictionary\(](#page-170-0)dictionary\_name)
...
<span id="page-171-0"></span>• [gen\\_dictionary\\_drop\(](#page-171-0)dictionary\_name)
...
<span id="page-172-0"></span>• [gen\\_dictionary\\_load\(](#page-172-0)dictionary\_path, dictionary\_name)
...
Once loaded into the registry, a dictionary is used as is, even if the underlying dictionary file changes. To reload a dictionary, first drop it with [gen\\_dictionary\\_drop\(\)](#page-171-0), then load it again with [gen\\_dictionary\\_load\(\)](#page-172-0).
