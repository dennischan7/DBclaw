...
title: 10.2.5.2 Optimizing UPDATE Statements
---
...
# <span id="page-95-1"></span>**10.3 Optimization and Indexes**
...
# <span id="page-95-2"></span>**10.3.1 How MySQL Uses Indexes**
...
In general, indexes are used as described in the following discussion. Characteristics specific to hash indexes (as used in MEMORY tables) are described in [Section 10.3.9, "Comparison of B-Tree and Hash](#page-102-0) [Indexes"](#page-102-0).

MySQL uses indexes for these operations:

- To find the rows matching a WHERE clause quickly.
- To eliminate rows from consideration. If there is a choice between multiple indexes, MySQL normally uses the index that finds the smallest number of rows (the most selective index).
- If the table has a multiple-column index, any leftmost prefix of the index can be used by the optimizer to look up rows. For example, if you have a three-column index on (col1, col2, col3), you have indexed search capabilities on (col1), (col1, col2), and (col1, col2, col3). For more information, see [Section 10.3.6, "Multiple-Column Indexes"](#page-99-0).
...
- To sort or group a table if the sorting or grouping is done on a leftmost prefix of a usable index (for example, ORDER BY key\_part1, key\_part2). If all key parts are followed by DESC, the key is read in reverse order. (Or, if the index is a descending index, the key is read in forward order.) See [Section 10.2.1.16, "ORDER BY Optimization",](#page-61-1) [Section 10.2.1.17, "GROUP BY Optimization"](#page-65-0), and [Section 10.3.13, "Descending Indexes"](#page-109-0).
...
Indexes are less important for queries on small tables, or big tables where report queries process most or all of the rows. When a query needs to access most of the rows, reading sequentially is faster than working through an index. Sequential reads minimize disk seeks, even if not all the rows are needed for the query. See [Section 10.2.1.23, "Avoiding Full Table Scans"](#page-74-0) for details.

## <span id="page-97-0"></span>**10.3.2 Primary Key Optimization**
...
## <span id="page-97-1"></span>**10.3.3 SPATIAL Index Optimization**
...
# <span id="page-97-2"></span>**10.3.4 Foreign Key Optimization**
...
## <span id="page-98-0"></span>**10.3.5 Column Indexes**
...
- [Index Prefixes](#page-98-1)
- [FULLTEXT Indexes](#page-98-2)
- [Spatial Indexes](#page-99-1)
- [Indexes in the MEMORY Storage Engine](#page-99-2)

### <span id="page-98-1"></span>**Index Prefixes**
...
## <span id="page-98-2"></span>**FULLTEXT Indexes**
...
## <span id="page-99-1"></span>**Spatial Indexes**
...
### <span id="page-99-2"></span>**Indexes in the MEMORY Storage Engine**

The MEMORY storage engine uses HASH indexes by default, but also supports BTREE indexes.

# <span id="page-99-0"></span>**10.3.6 Multiple-Column Indexes**

MySQL can create composite indexes (that is, indexes on multiple columns). An index may consist of up to 16 columns. For certain data types, you can index a prefix of the column (see [Section 10.3.5,](#page-98-0) ["Column Indexes"\)](#page-98-0).
...
If a multiple-column index exists on col1 and col2, the appropriate rows can be fetched directly. If separate single-column indexes exist on col1 and col2, the optimizer attempts to use the Index Merge optimization (see [Section 10.2.1.3, "Index Merge Optimization"\)](#page-33-0), or attempts to find the most restrictive index by deciding which index excludes more rows and using that index to fetch the rows.
...
# <span id="page-100-0"></span>**10.3.7 Verifying Index Usage**

Always check whether all your queries really use the indexes that you have created in the tables. Use the EXPLAIN statement, as described in [Section 10.8.1, "Optimizing Queries with EXPLAIN".](#page-141-0)

## <span id="page-101-0"></span>**10.3.8 InnoDB and MyISAM Index Statistics Collection**
...
- To estimate how many rows must be read for each [ref](#page-146-1) access
...
## <span id="page-102-0"></span>**10.3.9 Comparison of B-Tree and Hash Indexes**
...
- [B-Tree Index Characteristics](#page-102-1)
- [Hash Index Characteristics](#page-103-1)

## <span id="page-102-1"></span>**B-Tree Index Characteristics**
...
### <span id="page-103-1"></span>**Hash Index Characteristics**
...
## <span id="page-103-0"></span>**10.3.10 Use of Index Extensions**
...
The optimizer can use extended secondary indexes for ref, range, and [index\\_merge](#page-161-0) index access, for Loose Index Scan access, for join and sorting optimization, and for MIN()/MAX() optimization.
...
The [use\\_index\\_extensions](#page-161-7) flag of the optimizer\_switch system variable permits control over whether the optimizer takes the primary key columns into account when determining how to use an InnoDB table's secondary indexes. By default, [use\\_index\\_extensions](#page-161-7) is enabled. To check whether disabling use of index extensions can improve performance, use this statement:
...
# <span id="page-106-0"></span>**10.3.11 Optimizer Use of Generated Column Indexes**
...
# <span id="page-107-0"></span>**10.3.12 Invisible Indexes**
...
The [use\\_invisible\\_indexes](#page-161-8) flag of the optimizer\_switch system variable controls whether the optimizer uses invisible indexes for query execution plan construction. If the flag is off (the default), the optimizer ignores invisible indexes (the same behavior as prior to the introduction of this flag). If the flag is on, invisible indexes remain invisible but the optimizer takes them into account for execution plan construction.

Using the [SET\\_VAR](#page-180-0) optimizer hint to update the value of optimizer\_switch temporarily, you can enable invisible indexes for the duration of a single query only, like this:
...
## <span id="page-109-0"></span>**10.3.13 Descending Indexes**
...
# <span id="page-110-0"></span>10.3.14 Indexed Lookups from TIMESTAMP Columns
...
The same lack of one-to-one mapping for time zone conversions in both directions occurs in other contexts as well, such as conversions performed with the FROM\_UNIXTIME() and UNIX\_TIMESTAMP() functions. See Section 14.7, "Date and Time Functions".

# <span id="page-112-0"></span>**10.4 Optimizing Database Structure**
...
# <span id="page-112-1"></span>**10.4.1 Optimizing Data Size**
...
- [Table Columns](#page-113-0)
- [Row Format](#page-113-1)
- [Indexes](#page-113-2)
- [Joins](#page-114-1)
- [Normalization](#page-114-2)

### <span id="page-113-0"></span>**Table Columns**
...
### <span id="page-113-1"></span>**Row Format**
...
### <span id="page-113-2"></span>**Indexes**
...
### <span id="page-114-2"></span>**Normalization**
...
# <span id="page-114-0"></span>**10.4.2 Optimizing MySQL Data Types**

### <span id="page-114-3"></span>**10.4.2.1 Optimizing for Numeric Data**
...
### <span id="page-114-4"></span>**10.4.2.2 Optimizing for Character and String Types**
...
- See [Section 10.4.2.1, "Optimizing for Numeric Data"](#page-114-3) for reasons why a numeric column is usually preferable to an equivalent string column.
...
- See [Section 10.4.2.2, "Optimizing for Character and String Types"](#page-114-4) for reasons why a binary VARCHAR column is sometimes preferable to an equivalent BLOB column.
...
# <span id="page-115-0"></span>**10.4.3 Optimizing for Many Tables**
