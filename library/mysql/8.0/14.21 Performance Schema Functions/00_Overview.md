...
<span id="page-19-0"></span>• [FORMAT\\_BYTES\(](#page-19-0)count)
...
[FORMAT\\_BYTES\(\)](#page-19-0) was added in MySQL 8.0.16. It may be used instead of the sys schema format\_bytes() function, keeping in mind this difference:

- [FORMAT\\_BYTES\(\)](#page-19-0) uses the EiB units indicator. sys.format\_bytes() does not.
- <span id="page-19-1"></span>• [FORMAT\\_PICO\\_TIME\(](#page-19-1)time\_val)
...
mysql> **SELECT FORMAT\_PICO\_TIME(3501), FORMAT\_PICO\_TIME(188732396662000);**
...
[FORMAT\\_PICO\\_TIME\(\)](#page-19-1) was added in MySQL 8.0.16. It may be used instead of the sys schema format\_time() function, keeping in mind these differences:

- To indicate minutes, sys.format\_time() uses the m units indicator, whereas [FORMAT\\_PICO\\_TIME\(\)](#page-19-1) uses min.
- sys.format\_time() uses the w (weeks) units indicator. [FORMAT\\_PICO\\_TIME\(\)](#page-19-1) does not.
- <span id="page-20-0"></span>• [PS\\_CURRENT\\_THREAD\\_ID\(\)](#page-20-0)
...
Performance Schema configuration affects [PS\\_CURRENT\\_THREAD\\_ID\(\)](#page-20-0) the same way as for [PS\\_THREAD\\_ID\(\)](#page-20-1). For details, see the description of that function.
...
[PS\\_CURRENT\\_THREAD\\_ID\(\)](#page-20-0) was added in MySQL 8.0.16. It may be used as a shortcut for invoking the sys schema ps\_thread\_id() function with an argument of NULL or CONNECTION\_ID().

<span id="page-20-1"></span>• [PS\\_THREAD\\_ID\(](#page-20-1)connection\_id)
...
Performance Schema configuration affects [PS\\_THREAD\\_ID\(\)](#page-20-1) operation as follows. (These remarks also apply to [PS\\_CURRENT\\_THREAD\\_ID\(\)](#page-20-0).)

- Disabling the thread\_instrumentation consumer disables statistics from being collected and aggregated at the thread level, but has no effect on [PS\\_THREAD\\_ID\(\)](#page-20-1).
- If performance\_schema\_max\_thread\_instances is not 0, the Performance Schema allocates memory for thread statistics and assigns an internal ID to each thread for which instance memory is available. If there are threads for which

instance memory is not available, [PS\\_THREAD\\_ID\(\)](#page-20-1) returns NULL; in this case, Performance\_schema\_thread\_instances\_lost is nonzero.

- If performance\_schema\_max\_thread\_instances is 0, the Performance Schema allocates no thread memory and [PS\\_THREAD\\_ID\(\)](#page-20-1) returns NULL.
- If the Performance Schema itself is disabled, [PS\\_THREAD\\_ID\(\)](#page-20-1) produces an error.
...
[PS\\_THREAD\\_ID\(\)](#page-20-1) was added in MySQL 8.0.16. It may be used instead of the sys schema ps\_thread\_id() function, keeping in mind this difference:

• With an argument of NULL, sys.ps\_thread\_id() returns the thread ID for the current connection, whereas [PS\\_THREAD\\_ID\(\)](#page-20-1) returns NULL. To obtain the current connection thread ID, use [PS\\_CURRENT\\_THREAD\\_ID\(\)](#page-20-0) instead.