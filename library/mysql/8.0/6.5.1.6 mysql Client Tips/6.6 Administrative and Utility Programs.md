...
title: 6.6 Administrative and Utility Programs
---

This section describes administrative programs and programs that perform miscellaneous utility operations.

## <span id="page-58-0"></span>**6.6.1 ibd2sdi — InnoDB Tablespace SDI Extraction Utility**

[ibd2sdi](#page-58-0) is a utility for extracting serialized dictionary information (SDI) from InnoDB tablespace files. SDI data is present in all persistent InnoDB tablespace files.

[ibd2sdi](#page-58-0) can be run on file-per-table tablespace files (\*.ibd files), general tablespace files (\*.ibd files), system tablespace files (ibdata\* files), and the data dictionary tablespace (mysql.ibd). It is not supported for use with temporary tablespaces or undo tablespaces.

[ibd2sdi](#page-58-0) can be used at runtime or while the server is offline. During DDL operations, ROLLBACK operations, and undo log purge operations related to SDI, there may be a short interval of time when [ibd2sdi](#page-58-0) fails to read SDI data stored in the tablespace.

[ibd2sdi](#page-58-0) performs an uncommitted read of SDI from the specified tablespace. Redo logs and undo logs are not accessed.

Invoke the [ibd2sdi](#page-58-0) utility like this:

```
ibd2sdi [options] file_name1 [file_name2 file_name3 ...]
```

[ibd2sdi](#page-58-0) supports multi-file tablespaces like the InnoDB system tablespace, but it cannot be run on more than one tablespace at a time. For multi-file tablespaces, specify each file:
...
[ibd2sdi](#page-58-0) outputs SDI (containing id, type, and data fields) in JSON format.

## <span id="page-58-1"></span>**ibd2sdi Options**

[ibd2sdi](#page-58-0) supports the following options:
...
<span id="page-59-0"></span>• [--version](#page-59-0), -v
...
<span id="page-59-1"></span>• --debug[=[debug\\_options](#page-59-1)], -# [debug\_options]
...
<span id="page-59-2"></span>• [--dump-file=](#page-59-2), -d
...
## <span id="page-60-0"></span>• [--skip-data](#page-60-0), -s
...
### <span id="page-60-1"></span>• [--id=](#page-60-1)#, -i #
...
## <span id="page-61-0"></span>• [--type=](#page-61-0)#, -t #
...
Due to the way in which InnoDB handles default value metadata, a default value may be present and non-empty in [ibd2sdi](#page-58-0) output for a given table column even if it is not defined using DEFAULT. Consider the two tables created using the following statements, in the database named i:
...
Using [ibd2sdi](#page-58-0), we can see that the default\_value for column c is nonempty and is in fact padded to length in both tables, like this:
...
Examination of [ibd2sdi](#page-58-0) output may be easier using a JSON-aware utility like [jq](https://stedolan.github.io/jq/), as shown here:

"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\nAAAAAAAAAAA=" \$> **ibd2sdi ../data/i/t2.ibd | jq '.[1]["object"]["dd\_object"]["columns"][0]["default\_value"]'** "BlNha2lsYQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\nAAAAAAAAAAA="

For more information, see the [MySQL Internals documentation.](https://dev.mysql.com/doc/dev/mysql-server/latest/)

<span id="page-62-0"></span>• [--strict-check](#page-62-0), -c
...
If you do not specify the [--strict-check](#page-62-0) option, validation is performed against non-strict innodb, crc32 and none checksums.

<span id="page-62-1"></span>• [--no-check](#page-62-1), -n
...
<span id="page-62-2"></span>• [--pretty](#page-62-2), -p
...
## <span id="page-62-3"></span>**6.6.2 innochecksum — Offline InnoDB File Checksum Utility**

[innochecksum](#page-62-3) prints checksums for InnoDB files. This tool reads an InnoDB tablespace file, calculates the checksum for each page, compares the calculated checksum to the stored checksum, and reports mismatches, which indicate damaged pages. It was originally developed to speed up verifying the integrity of tablespace files after power outages but can also be used after file copies. Because checksum mismatches cause InnoDB to deliberately shut down a running server, it may be preferable to use this tool rather than waiting for an in-production server to encounter the damaged pages.

[innochecksum](#page-62-3) cannot be used on tablespace files that the server already has open. For such files, you should use CHECK TABLE to check tables within the tablespace. Attempting to run [innochecksum](#page-62-3) on a tablespace that the server already has open results in an Unable to lock file error.
...
Invoke [innochecksum](#page-62-3) like this:

innochecksum [options] file\_name

## **innochecksum Options**

[innochecksum](#page-62-3) supports the following options. For options that refer to page numbers, the numbers are zero-based.

<span id="page-63-0"></span>• [--help](#page-63-0), -?
...
<span id="page-63-1"></span>• [--info](#page-63-1), -I
...
Synonym for [--help](#page-63-0). Displays command line help. Example usage:

innochecksum --info

<span id="page-63-2"></span>• [--version](#page-63-2), -V
...
<span id="page-63-3"></span>• [--verbose](#page-63-3), -v
...
<span id="page-64-0"></span>• [--count](#page-64-0), -c
...
<span id="page-64-1"></span>• [--start-page=](#page-64-1)num, -s num
...
## <span id="page-65-0"></span>• [--end-page=](#page-65-0)num, -e num
...
<span id="page-65-1"></span>• [--page=](#page-65-1)num, -p num
...
<span id="page-65-2"></span>• [--strict-check](#page-65-2), -C
...
- If you do not specify the [--strict-check](#page-65-2) option, [innochecksum](#page-62-3) validates against innodb, crc32 and none.
...
- <span id="page-66-0"></span>• [--no-check](#page-66-0), -n
...
Ignore the checksum verification when rewriting a checksum. This option may only be used with the [innochecksum](#page-62-3) [--write](#page-67-0) option. If the [--write](#page-67-0) option is not specified, [innochecksum](#page-62-3) terminates.
...
<span id="page-66-1"></span>• [--allow-mismatches](#page-66-1), -a
...
The maximum number of checksum mismatches allowed before [innochecksum](#page-62-3) terminates. The default setting is 0. If --allow-mismatches=N, where N>=0, N mismatches are permitted and [innochecksum](#page-62-3) terminates at N+1. When --allow-mismatches is set to 0, [innochecksum](#page-62-3) terminates on the first checksum mismatch.
...
With --allow-mismatches set to 1, if there is a mismatch at page 600 and another at page 700 on a file with 1000 pages, the checksum is updated for pages 0-599 and 601-699. Because --

allow-mismatches is set to 1, the checksum tolerates the first mismatch and terminates on the second mismatch, leaving page 600 and pages 700-999 unchanged.

<span id="page-67-0"></span>• [--write=](#page-67-0)name, -w num
...
Rewrite a checksum. When rewriting an invalid checksum, the [--no-check](#page-66-0) option must be used together with the --write option. The [--no-check](#page-66-0) option tells [innochecksum](#page-62-3) to ignore verification of the invalid checksum. You do not have to specify the [--no-check](#page-66-0) option if the current checksum is valid.

An algorithm must be specified when using the [--write](#page-67-0) option. Possible values for the --write option are:
...
[innochecksum](#page-62-3) obtains an exclusive lock when the --write option is used.
...
<span id="page-67-1"></span>• [--page-type-summary](#page-67-1), -S
...
<span id="page-68-0"></span>• [--page-type-dump](#page-68-0), -D
...
<span id="page-68-1"></span>• [--log](#page-68-1), -l
...
Log output for the [innochecksum](#page-62-3) tool. A log file name must be provided. Log output contains checksum values for each tablespace page. For uncompressed tables, LSN values are also provided. The [--log](#page-68-1) replaces the --debug option, which was available in earlier releases. Example usage:
...
Specify the - option to read from standard input. If the - option is missing when "read from standard in" is expected, [innochecksum](#page-62-3) prints [innochecksum](#page-62-3) usage information indicating that the "-" option was omitted. Example usages:

```
cat t1.ibd | innochecksum -
```

In this example, [innochecksum](#page-62-3) writes the crc32 checksum algorithm to a.ibd without changing the original t1.ibd file.
...
The following examples demonstrate how to run [innochecksum](#page-62-3) on multiple user-defined tablespace files (.ibd files).

Run [innochecksum](#page-62-3) for all tablespace (.ibd) files in the "test" database:

```
innochecksum ./data/test/*.ibd
```

Run [innochecksum](#page-62-3) for all tablespace files (.ibd files) that have a file name starting with "t":

```
innochecksum ./data/test/t*.ibd
```

Run [innochecksum](#page-62-3) for all tablespace files (.ibd files) in the data directory:
...
Running [innochecksum](#page-62-3) on multiple user-defined tablespace files is not supported on Windows operating systems, as Windows shells such as cmd.exe do not support glob pattern expansion. On Windows systems, [innochecksum](#page-62-3) must be run separately for each user-defined tablespace file. For example:
...
The three files (ibdata1, ibdata2, and ibdata3) form one logical system tablespace. To run [innochecksum](#page-62-3) on multiple files that form one logical system tablespace, [innochecksum](#page-62-3) requires the - option to read tablespace files in from standard input, which is equivalent to concatenating multiple files to create one single file. For the example provided above, the following [innochecksum](#page-62-3) command would be used:

```
cat ibdata* | innochecksum -
```

Refer to the [innochecksum](#page-62-3) options information for more information about the "-" option.

![](_page_69_Picture_17.jpeg)

#### **Note**

Running [innochecksum](#page-62-3) on multiple files in the same tablespace is not supported on Windows operating systems, as Windows shells such as cmd.exe do not support glob pattern expansion. On Windows systems, [innochecksum](#page-62-3) must be run separately for each system tablespace file. For example:
...
## <span id="page-69-0"></span>**6.6.3 myisam\_ftdump — Display Full-Text Index information**

[myisam\\_ftdump](#page-69-0) displays information about FULLTEXT indexes in MyISAM tables. It reads the MyISAM index file directly, so it must be run on the server host where the table is located. Before using [myisam\\_ftdump](#page-69-0), be sure to issue a FLUSH TABLES statement first if the server is running.

[myisam\\_ftdump](#page-69-0) scans and dumps the entire index, which is not particularly fast. On the other hand, the distribution of words changes infrequently, so it need not be run often.

Invoke [myisam\\_ftdump](#page-69-0) like this:

```
myisam_ftdump [options] tbl_name index_num
```

The tbl\_name argument should be the name of a MyISAM table. You can also specify a table by naming its index file (the file with the .MYI suffix). If you do not invoke [myisam\\_ftdump](#page-69-0) in the directory where the table files are located, the table or index file name must be preceded by the path name to the table's database directory. Index numbers begin with 0.
...
The index on id is index 0 and the FULLTEXT index on txt is index 1. If your working directory is the test database directory, invoke [myisam\\_ftdump](#page-69-0) as follows:

```
myisam_ftdump mytexttable 1
```

If the path name to the test database directory is /usr/local/mysql/data/test, you can also specify the table name argument using that path name. This is useful if you do not invoke [myisam\\_ftdump](#page-69-0) in the database directory:

```
myisam_ftdump /usr/local/mysql/data/test/mytexttable 1
```

You can use [myisam\\_ftdump](#page-69-0) to generate a list of index entries in order of frequency of occurrence like this on Unix-like systems:
...
[myisam\\_ftdump](#page-69-0) supports the following options:

<span id="page-70-0"></span>• [--help](#page-70-0), -h -?
...
<span id="page-70-1"></span>• [--count](#page-70-1), -c
...
<span id="page-70-2"></span>• [--dump](#page-70-2), -d
...
<span id="page-70-3"></span>• [--length](#page-70-3), -l
...
<span id="page-71-0"></span>• [--stats](#page-71-0), -s
...
<span id="page-71-1"></span>• [--verbose](#page-71-1), -v

|--|

Verbose mode. Print more output about what the program does.

## <span id="page-71-2"></span>**6.6.4 myisamchk — MyISAM Table-Maintenance Utility**

The [myisamchk](#page-71-2) utility gets information about your database tables or checks, repairs, or optimizes them. [myisamchk](#page-71-2) works with MyISAM tables (tables that have .MYD and .MYI files for storing data and indexes).
...
The use of [myisamchk](#page-71-2) with partitioned tables is not supported.
...
Invoke [myisamchk](#page-71-2) like this:

```
myisamchk [options] tbl_name ...
```

The options specify what you want [myisamchk](#page-71-2) to do. They are described in the following sections. You can also get a list of options by invoking [myisamchk --help](#page-71-2).

With no options, [myisamchk](#page-71-2) simply checks your table as the default operation. To get more information or to tell [myisamchk](#page-71-2) to take corrective action, specify options as described in the following discussion.

tbl\_name is the database table you want to check or repair. If you run [myisamchk](#page-71-2) somewhere other than in the database directory, you must specify the path to the database directory, because [myisamchk](#page-71-2) has no idea where the database is located. In fact, [myisamchk](#page-71-2) does not actually care whether the files you are working on are located in a database directory. You can copy the files that correspond to a database table into some other location and perform recovery operations on them there.

You can name several tables on the [myisamchk](#page-71-2) command line if you wish. You can also specify a table by naming its index file (the file with the .MYI suffix). This enables you to specify all tables in a directory by using the pattern \*.MYI. For example, if you are in a database directory, you can check all the MyISAM tables in that directory like this:
...
myisamchk /path/to/database\_dir/\*.MYI
...
This command assumes that you have more than 64MB free. For more information about memory allocation with [myisamchk](#page-71-2), see [Section 6.6.4.6, "myisamchk Memory Usage".](#page-89-0)

For additional information about using [myisamchk](#page-71-2), see Section 9.6, "MyISAM Table Maintenance and Crash Recovery".

![](_page_72_Picture_9.jpeg)

## **Important**

You must ensure that no other program is using the tables while you are running [myisamchk](#page-71-2). The most effective means of doing so is to shut down the MySQL server while running [myisamchk](#page-71-2), or to lock all tables that [myisamchk](#page-71-2) is being used on.

Otherwise, when you run [myisamchk](#page-71-2), it may display the following error message:
...
If mysqld is running, you must force it to flush any table modifications that are still buffered in memory by using FLUSH TABLES. You should then ensure that no one is using the tables while you are running [myisamchk](#page-71-2)

However, the easiest way to avoid this problem is to use CHECK TABLE instead of [myisamchk](#page-71-2) to check tables. See Section 15.7.3.2, "CHECK TABLE Statement".

[myisamchk](#page-71-2) supports the following options, which can be specified on the command line or in the [myisamchk] group of an option file. For information about option files used by MySQL programs, see Section 6.2.2.2, "Using Option Files".
