...
The process of starting a group for the first time is called bootstrapping. You use the group\_replication\_bootstrap\_group system variable to bootstrap a group. The bootstrap should only be done by a single server, the one that starts the group and only once. This is why the value of the group\_replication\_bootstrap\_group option was not stored in the instance's option file. If it is saved in the option file, upon restart the server automatically bootstraps a second group with the same name. This would result in two distinct groups with the same name. The same reasoning applies to stopping and restarting the plugin with this option set to ON. Therefore to safely bootstrap the group, connect to s1 and issue the following statements:
...
Or if you are providing user credentials for distributed recovery in the START GROUP\_REPLICATION statement (MySQL 8.0.21 and later), issue the following statements:
...
Once the START GROUP\_REPLICATION statement returns, the group has been started. You can check that the group is now created and that there is one member in it:
