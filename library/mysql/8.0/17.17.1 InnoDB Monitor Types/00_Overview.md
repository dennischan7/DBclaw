...
## <span id="page-117-0"></span>**17.17.2 Enabling InnoDB Monitors**
...
To assist with troubleshooting, InnoDB temporarily enables standard InnoDB Monitor output under certain conditions. For more information, see [Section 17.21, "InnoDB Troubleshooting"](#page-162-0).
...
The [innodb\\_status\\_output](#page-66-0) and [innodb\\_status\\_output\\_locks](#page-66-1) system variables are used to enable the standard InnoDB Monitor and InnoDB Lock Monitor.

The PROCESS privilege is required to enable or disable InnoDB Monitors.

## **Enabling the Standard InnoDB Monitor**

Enable the standard InnoDB Monitor by setting the [innodb\\_status\\_output](#page-66-0) system variable to ON.

```
SET GLOBAL innodb_status_output=ON;
```

To disable the standard InnoDB Monitor, set [innodb\\_status\\_output](#page-66-0) to OFF.

When you shut down the server, the [innodb\\_status\\_output](#page-66-0) variable is set to the default OFF value.
...
To enable the InnoDB Lock Monitor, set the [innodb\\_status\\_output\\_locks](#page-66-1) system variable to ON. Both the InnoDB standard Monitor and InnoDB Lock Monitor must be enabled to have InnoDB Lock Monitor data printed periodically:
...
To disable the InnoDB Lock Monitor, set [innodb\\_status\\_output\\_locks](#page-66-1) to OFF. Set [innodb\\_status\\_output](#page-66-0) to OFF to also disable the InnoDB Standard Monitor.

When you shut down the server, the [innodb\\_status\\_output](#page-66-0) and [innodb\\_status\\_output\\_locks](#page-66-1) variables are set to the default OFF value.

![](_page_118_Picture_10.jpeg)

### **Note**

To enable the InnoDB Lock Monitor for SHOW ENGINE INNODB STATUS output, you are only required to enable [innodb\\_status\\_output\\_locks](#page-66-1).
...
## <span id="page-118-0"></span>**17.17.3 InnoDB Standard Monitor and Lock Monitor Output**
...
The number of these threads are controlled by the [innodb\\_read\\_io\\_threads](#page-57-0) and [innodb\\_write\\_io\\_threads](#page-76-0) parameters. See Section 17.14, "InnoDB Startup Options and System Variables".
