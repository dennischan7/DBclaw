...
This section provides some examples that show precision math query results in MySQL. These examples demonstrate the principles described in [Section 14.24.3, "Expression Handling",](#page-38-0) and [Section 14.24.4, "Rounding Behavior".](#page-40-0)
...
| 15.3.4 SAVEPOINT, ROLLBACK TO SAVEPOINT, and RELEASE SAVEPOINT Statements 2881 |      |
...
| 15.4.2 SQL Statements for Controlling Replica Servers 2899                     |      |
| 15.4.3 SQL Statements for Controlling Group Replication 2941                   |      |
| 15.5 Prepared Statements 2943                                                  |      |
| 15.5.1 PREPARE Statement 2946                                                  |      |
| 15.5.2 EXECUTE Statement 2948                                                  |      |
| 15.5.3 DEALLOCATE PREPARE Statement 2948                                       |      |
| 15.6 Compound Statement Syntax 2949                                            |      |
| 15.6.1 BEGIN END Compound Statement 2949                                       |      |
| 15.6.2 Statement Labels 2949                                                   |      |
| 15.6.3 DECLARE Statement 2950                                                  |      |
| 15.6.4 Variables in Stored Programs 2950                                       |      |
| 15.6.5 Flow Control Statements 2952                                            |      |
| 15.6.6 Cursors 2956                                                            |      |
| 15.6.7 Condition Handling 2958                                                 |      |
| 15.6.8 Restrictions on Condition Handling 2984                                 |      |
| 15.7 Database Administration Statements 2985                                   |      |
| 15.7.1 Account Management Statements 2985                                      |      |
| 15.7.2 Resource Group Management Statements 3037                               |      |
| 15.7.3 Table Maintenance Statements 3040                                       |      |
| 15.7.4 Component, Plugin, and Loadable Function Statements 3055                |      |
| 15.7.5 CLONE Statement 3060                                                    |      |
| 15.7.6 SET Statements 3061                                                     |      |
| 15.7.7 SHOW Statements 3066                                                    |      |
| 15.7.8 Other Administrative Statements 3121                                    |      |
| 15.8 Utility Statements                                                        | 3134 |
| 15.8.1 DESCRIBE Statement 3134                                                 |      |
| 15.8.2 EXPLAIN Statement                                                       | 3134 |
| 15.8.3 HELP Statement 3140                                                     |      |
| 15.8.4 USE Statement 3142                                                      |      |

This chapter describes the syntax for the SQL statements supported by MySQL.

# <span id="page-47-0"></span>**15.1 Data Definition Statements**

# <span id="page-47-1"></span>**15.1.1 Atomic Data Definition Statement Support**
...
Atomic DDL is made possible by the introduction of the MySQL data dictionary in MySQL 8.0. In earlier MySQL versions, metadata was stored in metadata files, nontransactional tables, and storage enginespecific dictionaries, which necessitated intermediate commits. Centralized, transactional metadata storage provided by the MySQL data dictionary removed this barrier, making it possible to restructure DDL statement operations to be atomic.

The atomic DDL feature is described under the following topics in this section:

- [Supported DDL Statements](#page-48-0)
- [Atomic DDL Characteristics](#page-48-1)
- [Changes in DDL Statement Behavior](#page-49-0)
- [Storage Engine Support](#page-51-0)
- [Viewing DDL Logs](#page-52-0)

# <span id="page-48-0"></span>**Supported DDL Statements**
...
- [CREATE SERVER](#page-116-0), [ALTER SERVER](#page-64-0), and [DROP SERVER](#page-197-0) statements.

# <span id="page-48-1"></span>**Atomic DDL Characteristics**
...
- The visible behaviour of DDL operations is atomic, which changes the behavior of some DDL statements. See [Changes in DDL Statement Behavior](#page-49-0).

# <span id="page-49-0"></span>**Changes in DDL Statement Behavior**

This section describes changes in DDL statement behavior due to the introduction of atomic DDL support.

• [DROP TABLE](#page-198-0) operations are fully atomic if all named tables use a storage engine which supports atomic DDL. The statement either drops all tables successfully or is rolled back.

[DROP TABLE](#page-198-0) fails with an error if a named table does not exist, and no changes are made, regardless of the storage engine. This change in behavior is demonstrated in the following example, where the [DROP TABLE](#page-198-0) statement fails because a named table does not exist:
...
Prior to the introduction of atomic DDL, [DROP TABLE](#page-198-0) reports an error for the named table that does not exist but succeeds for the named table that does exist:
...
Due to this change in behavior, a partially completed [DROP TABLE](#page-198-0) statement on a MySQL 5.7 replication source server fails when replicated on a MySQL 8.0 replica. To avoid this failure scenario, use IF EXISTS syntax in [DROP](#page-198-0) [TABLE](#page-198-0) statements to prevent errors from occurring for tables that do not exist.

- [DROP DATABASE](#page-194-0) is atomic if all tables use a storage engine which supports atomic DDL. The statement either drops all objects successfully or is rolled back. However, removal of the database directory from the file system occurs last and is not part of the atomic operation. If removal of the database directory fails due to a file system error or server halt, the [DROP DATABASE](#page-194-0) transaction is not rolled back.
- For tables that do not use a storage engine which supports atomic DDL, table deletion occurs outside of the atomic [DROP TABLE](#page-198-0) or [DROP DATABASE](#page-194-0) transaction. Such table deletions are written to the binary log individually, which limits the discrepancy between the storage engine, data dictionary, and binary log to one table at most in the case of an interrupted [DROP TABLE](#page-198-0) or [DROP](#page-194-0)

[DATABASE](#page-194-0) operation. For operations that drop multiple tables, the tables that do not use a storage engine which supports atomic DDL are dropped before tables that do.

- [CREATE TABLE](#page-121-0), [ALTER TABLE](#page-64-1), RENAME TABLE, TRUNCATE TABLE, [CREATE TABLESPACE](#page-181-0), and [DROP TABLESPACE](#page-198-1) operations for tables that use a storage engine which supports atomic DDL are either fully committed or rolled back if the server halts during their operation. In earlier MySQL releases, interruption of these operations could cause discrepancies between the storage engine, data dictionary, and binary log, or leave behind orphan files. RENAME TABLE operations are only atomic if all named tables use a storage engine which supports atomic DDL.
- As of MySQL 8.0.21, on storage engines that support atomic DDL, the [CREATE TABLE ...](#page-149-0) [SELECT](#page-149-0) statement is logged as one transaction in the binary log when row-based replication is in use. Previously, it was logged as two transactions, one to create the table, and the other to insert data. A server failure between the two transactions or while inserting data could result in replication of an empty table. With the introduction of atomic DDL support, [CREATE TABLE ...](#page-149-0) [SELECT](#page-149-0) statements are now safe for row-based replication and permitted for use with GTID-based replication.

On storage engines that support both atomic DDL and foreign key constraints, creation of foreign keys is not permitted in [CREATE TABLE ... SELECT](#page-149-0) statements when row-based replication is in use. Foreign key constraints can be added later using [ALTER TABLE](#page-64-1).

When [CREATE TABLE ... SELECT](#page-149-0) is applied as an atomic operation, a metadata lock is held on the table while data is inserted, which prevents concurrent access to the table for the duration of the operation.
...
# <span id="page-51-0"></span>**Storage Engine Support**
...
To view DDL logs that are written to the mysql.innodb\_ddl\_log table during a DDL operation, enable the innodb\_print\_ddl\_logs configuration option. For more information, see [Viewing DDL](#page-52-0) [Logs.](#page-52-0)
...
The InnoDB storage engine executes DDL operations in phases. DDL operations such as [ALTER](#page-64-1) [TABLE](#page-64-1) may perform the Prepare and Perform phases multiple times prior to the Commit phase.
...
# <span id="page-52-0"></span>**Viewing DDL Logs**
...
InnoDB writes DDL logs to the mysql.innodb\_ddl\_log table to support redo and rollback of DDL operations. The mysql.innodb\_ddl\_log table is a hidden data dictionary table that resides in the mysql.ibd data dictionary tablespace. Like other hidden data dictionary tables, the mysql.innodb\_ddl\_log table cannot be accessed directly in non-debug versions of MySQL. (See Section 16.1, "Data Dictionary Schema".) The structure of the mysql.innodb\_ddl\_log table corresponds to this definition:
...
# <span id="page-53-0"></span>**15.1.2 ALTER DATABASE Statement**
...
[ALTER DATABASE](#page-53-0) enables you to change the overall characteristics of a database. These characteristics are stored in the data dictionary. This statement requires the ALTER privilege on the database. [ALTER SCHEMA](#page-53-0) is a synonym for [ALTER DATABASE](#page-53-0).
...
- [Character Set and Collation Options](#page-54-0)
- [Encryption Option](#page-54-1)
- [Read Only Option](#page-54-2)

# <span id="page-54-0"></span>**Character Set and Collation Options**
...
A stored routine that uses the database defaults when the routine is created includes those defaults as part of its definition. (In a stored routine, variables with character data types use the database defaults if the character set or collation are not specified explicitly. See [Section 15.1.17, "CREATE](#page-110-0) [PROCEDURE and CREATE FUNCTION Statements".](#page-110-0)) If you change the default character set or collation for a database, any stored routines that are to use the new defaults must be dropped and recreated.

# <span id="page-54-1"></span>**Encryption Option**
...
# <span id="page-54-2"></span>**Read Only Option**
...
In [ALTER DATABASE](#page-53-0) statements, the READ ONLY option interacts with other instances of itself and with other options as follows:

- An error occurs if multiple instances of READ ONLY conflict (for example, READ ONLY = 1 READ ONLY = 0).
- An [ALTER DATABASE](#page-53-0) statement that contains only (nonconflicting) READ ONLY options is permitted even for a read-only database.
...
- The database itself:
  - [CREATE DATABASE](#page-89-1)
  - [ALTER DATABASE](#page-53-0) (except to change the READ ONLY option)
  - [DROP DATABASE](#page-194-0)
- Views:
  - [CREATE VIEW](#page-190-0)
  - [ALTER VIEW](#page-89-0)
  - DROP VIEW
...
- Stored routines:
  - [CREATE PROCEDURE](#page-110-0)
  - [DROP PROCEDURE](#page-196-1)
  - CALL (of procedures with side effects)
  - [CREATE FUNCTION](#page-95-0)
  - [DROP FUNCTION](#page-195-1)
...
- Triggers:
  - [CREATE TRIGGER](#page-188-0)
  - DROP TRIGGER
  - Trigger invocation.
- Events:
  - [CREATE EVENT](#page-90-0)
  - [ALTER EVENT](#page-58-0)
  - [DROP EVENT](#page-195-0)
  - Event execution:
...
- [CREATE TABLE](#page-121-0)
- [ALTER TABLE](#page-64-1)
- [CREATE INDEX](#page-95-1)
- [DROP INDEX](#page-195-2)
- RENAME TABLE
- TRUNCATE TABLE
- [DROP TABLE](#page-198-0)
- DELETE
- INSERT
- IMPORT TABLE
- LOAD DATA
...
READ ONLY is permitted for [ALTER DATABASE](#page-53-0), but not for [CREATE DATABASE](#page-89-1). However, for a readonly database, the statement produced by SHOW CREATE DATABASE does include READ ONLY=1 within a comment to indicate its read-only status:
...
If the server executes a [CREATE DATABASE](#page-89-1) statement containing such a comment, the server ignores the comment and the READ ONLY option is not processed. This has implications for mysqldump and mysqlpump, which use SHOW CREATE DATABASE to produce [CREATE DATABASE](#page-89-1) statements in dump output:

- In a dump file, the [CREATE DATABASE](#page-89-1) statement for a read-only database contains the commented READ ONLY option.
- The dump file can be restored as usual, but because the server ignores the commented READ ONLY option, the restored database is not read only. If the database is to be read only after being restored, you must execute [ALTER DATABASE](#page-53-0) manually to make it so.
...
A restore operation later must be followed by [ALTER DATABASE](#page-53-0) if mydb should still be read only:
...
# <span id="page-58-0"></span>**15.1.3 ALTER EVENT Statement**
...
The [ALTER EVENT](#page-58-0) statement changes one or more of the characteristics of an existing event without the need to drop and recreate it. The syntax for each of the DEFINER, ON SCHEDULE, ON COMPLETION, COMMENT, ENABLE / DISABLE, and DO clauses is exactly the same as when used with [CREATE EVENT](#page-90-0). (See [Section 15.1.13, "CREATE EVENT Statement"](#page-90-0).)

Any user can alter an event defined on a database for which that user has the EVENT privilege. When a user executes a successful [ALTER EVENT](#page-58-0) statement, that user becomes the definer for the affected event.

[ALTER EVENT](#page-58-0) works only with an existing event:
...
Specify the options in an [ALTER EVENT](#page-58-0) statement only for those characteristics that you want to change; omitted options keep their existing values. This includes any default values for [CREATE](#page-90-0) [EVENT](#page-90-0) such as ENABLE.

To disable myevent, use this [ALTER EVENT](#page-58-0) statement:
...
The ON SCHEDULE clause may use expressions involving built-in MySQL functions and user variables to obtain any of the timestamp or interval values which it contains. You cannot use stored routines or loadable functions in such expressions, and you cannot use any table references; however, you can use SELECT FROM DUAL. This is true for both [ALTER EVENT](#page-58-0) and [CREATE EVENT](#page-90-0) statements. References to stored routines, loadable functions, and tables in such cases are specifically not permitted, and fail with an error (see Bug #22830).

Although an [ALTER EVENT](#page-58-0) statement that contains another [ALTER EVENT](#page-58-0) statement in its DO clause appears to succeed, when the server attempts to execute the resulting scheduled event, the execution fails with an error.

To rename an event, use the [ALTER EVENT](#page-58-0) statement's RENAME TO clause. This statement renames the event myevent to yourevent:
...
# <span id="page-60-0"></span>**15.1.4 ALTER FUNCTION Statement**
...
This statement can be used to change the characteristics of a stored function. More than one change may be specified in an [ALTER FUNCTION](#page-60-0) statement. However, you cannot change the parameters or body of a stored function using this statement; to make such changes, you must drop and re-create the function using [DROP FUNCTION](#page-195-1) and [CREATE FUNCTION](#page-95-0).

You must have the ALTER ROUTINE privilege for the function. (That privilege is granted automatically to the function creator.) If binary logging is enabled, the [ALTER FUNCTION](#page-60-0) statement might also require the SUPER privilege, as described in Section 27.7, "Stored Program Binary Logging".

# <span id="page-60-1"></span>**15.1.5 ALTER INSTANCE Statement**
...
An [ALTER INSTANCE \[ENABLE|DISABLE\] INNODB REDO\\_LOG](#page-60-1) operation requires an exclusive backup lock, which prevents other [ALTER INSTANCE](#page-60-1) operations from executing concurrently. Other [ALTER INSTANCE](#page-60-1) operations must wait for the lock to be released before executing.
...
By default, the statement reloads the TLS context for the main connection interface. If the FOR CHANNEL clause (available as of MySQL 8.0.21) is given, the statement reloads the TLS context for the named channel: mysql\_main for the main connection interface, mysql\_admin for the administrative connection interface. For information about the different interfaces, see Section 7.1.12.1, "Connection Interfaces". The updated TLS context properties are exposed in the Performance Schema tls\_channel\_status table. See Section 29.12.21.9, "The tls\_channel\_status Table".
...
This statement adds an UNDO file named 'file\_name' to an existing log file group logfile\_group. An [ALTER LOGFILE GROUP](#page-62-0) statement has one and only one ADD UNDOFILE clause. No DROP UNDOFILE clause is currently supported.
...
Here is an example, which assumes that the log file group lg\_3 has already been created using [CREATE LOGFILE GROUP](#page-109-0) (see [Section 15.1.16, "CREATE LOGFILE GROUP Statement"](#page-109-0)):
...
When [ALTER LOGFILE GROUP](#page-62-0) is used with ENGINE = NDBCLUSTER (alternatively, ENGINE = NDB), an UNDO log file is created on each NDB Cluster data node. You can verify that the UNDO files were created and obtain information about them by querying the Information Schema FILES table. For example:
...
[ALTER LOGFILE GROUP](#page-62-0) is useful only with Disk Data storage for NDB Cluster. For more information, see Section 25.6.11, "NDB Cluster Disk Data Tables".

# <span id="page-63-0"></span>**15.1.7 ALTER PROCEDURE Statement**
...
This statement can be used to change the characteristics of a stored procedure. More than one change may be specified in an [ALTER PROCEDURE](#page-63-0) statement. However, you cannot change the parameters or body of a stored procedure using this statement; to make such changes, you must drop and re-create the procedure using [DROP PROCEDURE](#page-196-1) and [CREATE PROCEDURE](#page-110-0).
...
# <span id="page-64-0"></span>**15.1.8 ALTER SERVER Statement**
...
Alters the server information for server\_name, adjusting any of the options permitted in the [CREATE](#page-116-0) [SERVER](#page-116-0) statement. The corresponding fields in the mysql.servers table are updated accordingly. This statement requires the SUPER privilege.
...
# <span id="page-64-1"></span>**15.1.9 ALTER TABLE Statement**
...
[ALTER TABLE](#page-64-1) changes the structure of a table. For example, you can add or delete columns, create or destroy indexes, change the type of existing columns, or rename columns or the table itself. You can also change characteristics such as the storage engine used for the table or the table comment.

- To use [ALTER TABLE](#page-64-1), you need ALTER, CREATE, and INSERT privileges for the table. Renaming a table requires ALTER and DROP on the old table, ALTER, CREATE, and INSERT on the new table.
- Following the table name, specify the alterations to be made. If none are given, [ALTER TABLE](#page-64-1) does nothing.
- The syntax for many of the permissible alterations is similar to clauses of the [CREATE TABLE](#page-121-0) statement. column\_definition clauses use the same syntax for ADD and CHANGE as for [CREATE](#page-121-0) [TABLE](#page-121-0). For more information, see [Section 15.1.20, "CREATE TABLE Statement".](#page-121-0)
...
- Multiple ADD, ALTER, DROP, and CHANGE clauses are permitted in a single [ALTER TABLE](#page-64-1) statement, separated by commas. This is a MySQL extension to standard SQL, which permits only one of each clause per [ALTER TABLE](#page-64-1) statement. For example, to drop multiple columns in a single statement, do this:

```
ALTER TABLE t2 DROP COLUMN c, DROP COLUMN d;
```

- If a storage engine does not support an attempted [ALTER TABLE](#page-64-1) operation, a warning may result. Such warnings can be displayed with SHOW WARNINGS. See Section 15.7.7.42, "SHOW WARNINGS Statement". For information on troubleshooting [ALTER TABLE](#page-64-1), see Section B.3.6.1, "Problems with ALTER TABLE".
- For information about generated columns, see [Section 15.1.9.2, "ALTER TABLE and Generated](#page-84-0) [Columns".](#page-84-0)
- For usage examples, see [Section 15.1.9.3, "ALTER TABLE Examples".](#page-85-0)
- InnoDB in MySQL 8.0.17 and later supports addition of multi-valued indexes on JSON columns using a key\_part specification can take the form (CAST json\_path AS type ARRAY). See [Multi-Valued Indexes](#page-100-0), for detailed information regarding multi-valued index creation and usage of, as well as restrictions and limitations on multi-valued indexes.
- With the [mysql\\_info\(\)](https://dev.mysql.com/doc/c-api/8.0/en/mysql-info.md) C API function, you can find out how many rows were copied by [ALTER](#page-64-1) [TABLE](#page-64-1). See [mysql\\_info\(\).](https://dev.mysql.com/doc/c-api/8.0/en/mysql-info.md)

There are several additional aspects to the ALTER TABLE statement, described under the following topics in this section:

• [Table Options](#page-67-0)

- [Performance and Space Requirements](#page-69-0)
- [Concurrency Control](#page-72-0)
- [Adding and Dropping Columns](#page-73-0)
- [Renaming, Redefining, and Reordering Columns](#page-73-1)
- [Primary Keys and Indexes](#page-75-0)
- [Foreign Keys and Other Constraints](#page-76-0)
- [Changing the Character Set](#page-78-0)
- [Importing InnoDB Tables](#page-79-0)
- [Row Order for MyISAM Tables](#page-79-1)
- [Partitioning Options](#page-79-2)

# <span id="page-67-0"></span>**Table Options**

table\_options signifies table options of the kind that can be used in the [CREATE TABLE](#page-121-0) statement, such as ENGINE, AUTO\_INCREMENT, AVG\_ROW\_LENGTH, MAX\_ROWS, ROW\_FORMAT, or TABLESPACE.

For descriptions of all table options, see [Section 15.1.20, "CREATE TABLE Statement"](#page-121-0). However, [ALTER TABLE](#page-64-1) ignores DATA DIRECTORY and INDEX DIRECTORY when given as table options. [ALTER TABLE](#page-64-1) permits them only as partitioning options, and requires that you have the FILE privilege.

Use of table options with [ALTER TABLE](#page-64-1) provides a convenient way of altering single table characteristics. For example:
...
- When you specify an ENGINE clause, [ALTER TABLE](#page-64-1) rebuilds the table. This is true even if the table already has the specified storage engine.
- Running ALTER TABLE tbl\_name [ENGINE=INNODB](#page-64-1) on an existing InnoDB table performs a "null" [ALTER TABLE](#page-64-1) operation, which can be used to defragment an InnoDB table, as described in Section 17.11.4, "Defragmenting a Table". Running [ALTER TABLE](#page-64-1) tbl\_name FORCE on an InnoDB table performs the same function.
- ALTER TABLE tbl\_name [ENGINE=INNODB](#page-64-1) and [ALTER TABLE](#page-64-1) tbl\_name FORCE use online DDL. For more information, see Section 17.12, "InnoDB and Online DDL".
...
- To prevent inadvertent loss of data, [ALTER TABLE](#page-64-1) cannot be used to change the storage engine of a table to MERGE or BLACKHOLE.
...
See also [Changing the Character Set.](#page-78-0)
...
- Use ALTER TABLE with the TABLESPACE option to move InnoDB tables between existing general tablespaces, file-per-table tablespaces, and the system tablespace. See Moving Tables Between Tablespaces Using ALTER TABLE.
  ...
  - The DATA DIRECTORY clause, which is supported with [CREATE TABLE ... TABLESPACE](#page-121-0), is not supported with ALTER TABLE ... TABLESPACE, and is ignored if specified.
  - For more information about the capabilities and limitations of the TABLESPACE option, see [CREATE](#page-121-0) [TABLE](#page-121-0).
- MySQL NDB Cluster 8.0 supports setting NDB\_TABLE options for controlling a table's partition balance (fragment count type), read-from-any-replica capability, full replication, or any combination

of these, as part of the table comment for an ALTER TABLE statement in the same manner as for [CREATE TABLE](#page-121-0), as shown in this example:
...
Setting the blob inline size in this fashion is supported by NDB 8.0.30 and later. Bear in mind that ALTER TABLE ... COMMENT ... discards any existing comment for the table. See [Setting](#page-134-0) [NDB\\_TABLE options](#page-134-0), for additional information and examples.

• ENGINE\_ATTRIBUTE and SECONDARY\_ENGINE\_ATTRIBUTE options (available as of MySQL 8.0.21) are used to specify table, column, and index attributes for primary and secondary storage engines. The options are reserved for future use. Index attributes cannot be altered. An index must be dropped and added back with the desired change, which can be performed in a single [ALTER](#page-64-1) [TABLE](#page-64-1) statement.
...
# <span id="page-69-0"></span>**Performance and Space Requirements**

[ALTER TABLE](#page-64-1) operations are processed using one of the following algorithms:
...
The ALGORITHM clause is optional. If the ALGORITHM clause is omitted, MySQL uses ALGORITHM=INSTANT for storage engines and [ALTER TABLE](#page-64-1) clauses that support it. Otherwise, ALGORITHM=INPLACE is used. If ALGORITHM=INPLACE is not supported, ALGORITHM=COPY is used.

![](_page_70_Picture_1.jpeg)

#### **Note**

After adding a column to a partitioned table using ALGORITHM=INSTANT, it is no longer possible to perform [ALTER TABLE ... EXCHANGE PARTITION](#page-80-0) on the table.
...
[ALTER TABLE](#page-64-1) operations that use the COPY algorithm wait for other operations that are modifying the table to complete. After alterations are applied to the table copy, data is copied over, the original table is deleted, and the table copy is renamed to the name of the original table. While the [ALTER TABLE](#page-64-1) operation executes, the original table is readable by other sessions (with the exception noted shortly). Updates and writes to the table started after the [ALTER TABLE](#page-64-1) operation begins are stalled until the new table is ready, then are automatically redirected to the new table. The temporary copy of the table is created in the database directory of the original table unless it is a RENAME TO operation that moves the table to a database that resides in a different directory.

The exception referred to earlier is that [ALTER TABLE](#page-64-1) blocks reads (not just writes) at the point where it is ready to clear outdated table structures from the table and table definition caches. At this point, it must acquire an exclusive lock. To do so, it waits for current readers to finish, and blocks new reads and writes.

An [ALTER TABLE](#page-64-1) operation that uses the COPY algorithm prevents concurrent DML operations. Concurrent queries are still allowed. That is, a table-copying operation always includes at least the concurrency restrictions of LOCK=SHARED (allow queries but not DML). You can further restrict concurrency for operations that support the LOCK clause by specifying LOCK=EXCLUSIVE, which prevents DML and queries. For more information, see [Concurrency Control.](#page-72-0)

To force use of the COPY algorithm for an [ALTER TABLE](#page-64-1) operation that would otherwise not use it, specify ALGORITHM=COPY or enable the old\_alter\_table system variable. If there is a conflict between the old\_alter\_table setting and an ALGORITHM clause with a value other than DEFAULT, the ALGORITHM clause takes precedence.

For InnoDB tables, an [ALTER TABLE](#page-64-1) operation that uses the COPY algorithm on a table that resides in a shared tablespace can increase the amount of space used by the tablespace. Such operations require as much additional space as the data in the table plus indexes. For a table residing in a shared tablespace, the additional space used during the operation is not released back to the operating system as it is for a table that resides in a file-per-table tablespace.

For information about space requirements for online DDL operations, see Section 17.12.3, "Online DDL Space Requirements".

[ALTER TABLE](#page-64-1) operations that support the INPLACE algorithm include:
...
The WITHOUT VALIDATION and WITH VALIDATION clauses affect whether [ALTER TABLE](#page-64-1) performs an in-place operation for virtual generated column modifications. See [Section 15.1.9.2, "ALTER TABLE](#page-84-0) [and Generated Columns"](#page-84-0).
...
# <span id="page-73-0"></span>**Adding and Dropping Columns**
...
If a table contains only one column, the column cannot be dropped. If what you intend is to remove the table, use the [DROP TABLE](#page-198-0) statement instead.
...
# <span id="page-73-1"></span>**Renaming, Redefining, and Reordering Columns**
...
enable strict SQL mode before using [ALTER TABLE](#page-64-1) (see Section 7.1.11, "Server SQL Modes").
...
- Generated column and partition expressions that refer to the renamed column. You must use CHANGE to redefine such expressions in the same [ALTER TABLE](#page-64-1) statement as the one that renames the column.
...
As of MySQL 8.0.23, ALTER ... SET VISIBLE and ALTER ... SET INVISIBLE enable column visibility to be changed. See [Section 15.1.20.10, "Invisible Columns".](#page-168-0)

# <span id="page-75-0"></span>**Primary Keys and Indexes**
...
[DROP INDEX](#page-195-2) removes an index. This is a MySQL extension to standard SQL. See [Section 15.1.27,](#page-195-2) ["DROP INDEX Statement"](#page-195-2). To determine index names, use SHOW INDEX FROM tbl\_name.

Some storage engines permit you to specify an index type when creating an index. The syntax for the index\_type specifier is USING type\_name. For details about USING, see [Section 15.1.15,](#page-95-1) ["CREATE INDEX Statement".](#page-95-1) The preferred position is after the column list. Expect support for use of the option before the column list to be removed in a future MySQL release.

index\_option values specify additional options for an index. USING is one such option. For details about permissible index\_option values, see [Section 15.1.15, "CREATE INDEX Statement"](#page-95-1).

RENAME INDEX old\_index\_name TO new\_index\_name renames an index. This is a MySQL extension to standard SQL. The content of the table remains unchanged. old\_index\_name must be the name of an existing index in the table that is not dropped by the same [ALTER TABLE](#page-64-1) statement. new\_index\_name is the new index name, which cannot duplicate the name of an index in the resulting table after changes have been applied. Neither index name can be PRIMARY.

If you use [ALTER TABLE](#page-64-1) on a MyISAM table, all nonunique indexes are created in a separate batch (as for REPAIR TABLE). This should make [ALTER TABLE](#page-64-1) much faster when you have many indexes. For MyISAM tables, key updating can be controlled explicitly. Use ALTER TABLE ... DISABLE KEYS to tell MySQL to stop updating nonunique indexes. Then use ALTER TABLE ... ENABLE KEYS to re-create missing indexes. MyISAM does this with a special algorithm that is much faster than inserting keys one by one, so disabling keys before performing bulk insert operations should give a considerable speedup. Using ALTER TABLE ... DISABLE KEYS requires the INDEX privilege in addition to the privileges mentioned earlier.
...
After an [ALTER TABLE](#page-64-1) statement, it may be necessary to run ANALYZE TABLE to update index cardinality information. See Section 15.7.7.22, "SHOW INDEX Statement".
...
# <span id="page-76-0"></span>**Foreign Keys and Other Constraints**

The FOREIGN KEY and REFERENCES clauses are supported by the InnoDB and NDB storage engines, which implement ADD [CONSTRAINT [symbol]] FOREIGN KEY [index\_name] (...) REFERENCES ... (...). See [Section 15.1.20.5, "FOREIGN KEY Constraints".](#page-153-0) For other storage engines, the clauses are parsed but ignored.

For [ALTER TABLE](#page-64-1), unlike [CREATE TABLE](#page-121-0), ADD FOREIGN KEY ignores index\_name if given and uses an automatically generated foreign key name. As a workaround, include the CONSTRAINT clause to specify the foreign key name:
...
MySQL Server and NDB Cluster both support the use of [ALTER TABLE](#page-64-1) to drop foreign keys:

ALTER TABLE tbl\_name DROP FOREIGN KEY fk\_symbol;

Adding and dropping a foreign key in the same [ALTER TABLE](#page-64-1) statement is supported for [ALTER](#page-64-1) [TABLE ... ALGORITHM=INPLACE](#page-64-1) but not for [ALTER TABLE ... ALGORITHM=COPY](#page-64-1).

The server prohibits changes to foreign key columns that have the potential to cause loss of referential integrity. A workaround is to use [ALTER TABLE ... DROP FOREIGN KEY](#page-64-1) before changing the column definition and [ALTER TABLE ... ADD FOREIGN KEY](#page-64-1) afterward. Examples of prohibited changes include:
...
Prior to MySQL 8.0.16, [ALTER TABLE](#page-64-1) permits only the following limited version of CHECK constraintadding syntax, which is parsed and ignored:

```
ADD CHECK (expr)
```

As of MySQL 8.0.16, [ALTER TABLE](#page-64-1) permits CHECK constraints for existing tables to be added, dropped, or altered:
...
The meaning of constraint syntax elements is the same as for [CREATE TABLE](#page-121-0). See [Section 15.1.20.6, "CHECK Constraints"](#page-160-0).
...
As of MySQL 8.0.19, [ALTER TABLE](#page-64-1) permits more general (and SQL standard) syntax for dropping and altering existing constraints of any type, where the constraint type is determined from the constraint name:
...
# <span id="page-78-0"></span>**Changing the Character Set**
...
# <span id="page-79-0"></span>**Importing InnoDB Tables**
...
# <span id="page-79-1"></span>**Row Order for MyISAM Tables**
...
# <span id="page-79-2"></span>**Partitioning Options**
...
It is possible for an [ALTER TABLE](#page-64-1) statement to contain a PARTITION BY or REMOVE PARTITIONING clause in an addition to other alter specifications, but the PARTITION BY or REMOVE PARTITIONING clause must be specified last after any other specifications. The ADD PARTITION, DROP PARTITION, DISCARD PARTITION, IMPORT PARTITION, COALESCE PARTITION, REORGANIZE PARTITION, EXCHANGE PARTITION, ANALYZE PARTITION, CHECK PARTITION, and REPAIR PARTITION options cannot be combined with other alter specifications in a single ALTER TABLE, since the options just listed act on individual partitions.

For more information about partition options, see [Section 15.1.20, "CREATE TABLE Statement",](#page-121-0) and [Section 15.1.9.1, "ALTER TABLE Partition Operations"](#page-80-0). For information about and examples of ALTER TABLE ... EXCHANGE PARTITION statements, see Section 26.3.3, "Exchanging Partitions and Subpartitions with Tables".

# <span id="page-80-0"></span>**15.1.9.1 ALTER TABLE Partition Operations**

Partitioning-related clauses for [ALTER TABLE](#page-64-1) can be used with partitioned tables for repartitioning, to add, drop, discard, import, merge, and split partitions, and to perform partitioning maintenance.

• Simply using a partition\_options clause with [ALTER TABLE](#page-64-1) on a partitioned table repartitions the table according to the partitioning scheme defined by the partition\_options. This clause always begins with PARTITION BY, and follows the same syntax and other rules as apply to the partition\_options clause for [CREATE TABLE](#page-121-0) (for more detailed information, see [Section 15.1.20, "CREATE TABLE Statement"](#page-121-0)), and can also be used to partition an existing table that is not already partitioned. For example, consider a (nonpartitioned) table defined as shown here:
...
When running [DISCARD PARTITION ... TABLESPACE](#page-64-1) and [IMPORT PARTITION ...](#page-64-1) [TABLESPACE](#page-64-1) on subpartitioned tables, both partition and subpartition names are allowed. When a partition name is specified, subpartitions of that partition are included.
...
- REMOVE PARTITIONING enables you to remove a table's partitioning without otherwise affecting the table or its data. This option can be combined with other [ALTER TABLE](#page-64-1) options such as those used to add, drop, or rename columns or indexes.
- Using the ENGINE option with [ALTER TABLE](#page-64-1) changes the storage engine used by the table without affecting the partitioning. The target storage engine must provide its own partitioning handler. Only the InnoDB and NDB storage engines have native partitioning handlers; NDB is not currently supported in MySQL 8.0.

It is possible for an [ALTER TABLE](#page-64-1) statement to contain a PARTITION BY or REMOVE PARTITIONING clause in an addition to other alter specifications, but the PARTITION BY or REMOVE PARTITIONING clause must be specified last after any other specifications.
...
# <span id="page-84-0"></span>**15.1.9.2 ALTER TABLE and Generated Columns**
...
- For non-partitioned tables, ADD COLUMN and DROP COLUMN are in-place operations for virtual columns. However, adding or dropping a virtual column cannot be performed in place in combination with other [ALTER TABLE](#page-64-1) operations.

For partitioned tables, ADD COLUMN and DROP COLUMN are not in-place operations for virtual columns.

- InnoDB supports secondary indexes on virtual generated columns. Adding or dropping a secondary index on a virtual generated column is an in-place operation. For more information, see [Section 15.1.20.9, "Secondary Indexes and Generated Columns"](#page-165-0).
...
WITHOUT VALIDATION and WITH VALIDATION are permitted only with ADD COLUMN, CHANGE COLUMN, and MODIFY COLUMN operations. Otherwise, an [ER\\_WRONG\\_USAGE](https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.md#error_er_wrong_usage) error occurs.

- If expression evaluation causes truncation or provides incorrect input to a function, the [ALTER](#page-64-1) [TABLE](#page-64-1) statement terminates with an error and the DDL operation is rejected.
- An [ALTER TABLE](#page-64-1) statement that changes the default value of a column col\_name may also change the value of a generated column expression that refers to the column using col\_name, which may change the value of a generated column expression that refers to the column using [DEFAULT\(](#page-25-1)col\_name). For this reason, [ALTER TABLE](#page-64-1) operations that change the definition of a column cause a table rebuild if any generated column expression uses [DEFAULT\(\)](#page-25-1).

# <span id="page-85-0"></span>**15.1.9.3 ALTER TABLE Examples**
...
To convert this table to disk-based storage, you can use the following [ALTER TABLE](#page-64-1) statement:
...
It is not necessary that the tablespace was referenced when the table was originally created; however, the tablespace must be referenced by the [ALTER TABLE](#page-64-1):
...
To change the storage type of an individual column, you can use ALTER TABLE ... MODIFY [COLUMN]. For example, suppose you create an NDB Cluster Disk Data table with two columns, using this [CREATE TABLE](#page-121-0) statement:
...
Column c1 uses disk-based storage, since this is the default for the table (determined by the tablelevel STORAGE DISK clause in the [CREATE TABLE](#page-121-0) statement). However, column c2 uses in-memory storage, as can be seen here in the output of SHOW [CREATE TABLE](#page-121-0):
...
When you add an AUTO\_INCREMENT column, column values are filled in with sequence numbers automatically. For MyISAM tables, you can set the first sequence number by executing SET INSERT\_ID=value before [ALTER TABLE](#page-64-1) or by using the AUTO\_INCREMENT=value table option.
...
# <span id="page-87-0"></span>**15.1.10 ALTER TABLESPACE Statement**
...
INITIAL\_SIZE is rounded, explicitly, as for [CREATE TABLESPACE](#page-181-0).
...
As of MySQL 8.0.16, if the table\_encryption\_privilege\_check variable is enabled, the TABLE\_ENCRYPTION\_ADMIN privilege is required to alter a general tablespace with an ENCRYPTION clause setting that differs from the default\_table\_encryption setting.

Enabling encryption for a general tablespace fails if any table in the tablespace belongs to a schema defined with [DEFAULT ENCRYPTION='N'](#page-89-1). Similarly, disabling encryption fails if any table in the general tablespace belongs to a schema defined with [DEFAULT ENCRYPTION='Y'](#page-89-1). The [DEFAULT](#page-89-1) [ENCRYPTION](#page-89-1) schema option was introduced in MySQL 8.0.16.

If an [ALTER TABLESPACE](#page-87-0) statement executed on a general tablespace does not include an ENCRYPTION clause, the tablespace retains its current encryption status, regardless of the default\_table\_encryption setting.
...
# <span id="page-89-0"></span>**15.1.11 ALTER VIEW Statement**
...
This statement changes the definition of a view, which must exist. The syntax is similar to that for [CREATE VIEW](#page-190-0) see [Section 15.1.23, "CREATE VIEW Statement"\)](#page-190-0). This statement requires the CREATE VIEW and DROP privileges for the view, and some privilege for each column referred to in the SELECT statement. [ALTER VIEW](#page-89-0) is permitted only to the definer or users with the SET\_USER\_ID privilege (or the deprecated SUPER privilege).

# <span id="page-89-1"></span>**15.1.12 CREATE DATABASE Statement**
...
[CREATE DATABASE](#page-89-1) creates a database with the given name. To use this statement, you need the CREATE privilege for the database. [CREATE SCHEMA](#page-89-1) is a synonym for [CREATE DATABASE](#page-89-1).

An error occurs if the database exists and you did not specify IF NOT EXISTS.

[CREATE DATABASE](#page-89-1) is not permitted within a session that has an active LOCK TABLES statement.
...
A database in MySQL is implemented as a directory containing files that correspond to tables in the database. Because there are no tables in a database when it is initially created, the [CREATE](#page-89-1) [DATABASE](#page-89-1) statement creates only a directory under the MySQL data directory. Rules for permissible database names are given in Section 11.2, "Schema Object Names". If a database name contains special characters, the name for the database directory contains encoded versions of those characters as described in Section 11.2.4, "Mapping of Identifiers to File Names".
...
# <span id="page-90-0"></span>**15.1.13 CREATE EVENT Statement**
...
[CREATE EVENT](#page-90-0) requires the EVENT privilege for the schema in which the event is to be created. If the DEFINER clause is present, the privileges required depend on the user value, as discussed in Section 27.6, "Stored Object Access Control".

The minimum requirements for a valid [CREATE EVENT](#page-90-0) statement are as follows:

- The keywords [CREATE EVENT](#page-90-0) plus an event name, which uniquely identifies the event in a database schema.
- An ON SCHEDULE clause, which determines when and how often the event executes.
- A DO clause, which contains the SQL statement to be executed by an event.

This is an example of a minimal [CREATE EVENT](#page-90-0) statement:
...
If the DEFINER clause is omitted, the default definer is the user who executes the [CREATE EVENT](#page-90-0) statement. This is the same as specifying DEFINER = CURRENT\_USER explicitly.
...
IF NOT EXISTS has the same meaning for [CREATE EVENT](#page-90-0) as for [CREATE TABLE](#page-121-0): If an event named event\_name already exists in the same schema, no action is taken, and no error results. (However, a warning is generated in such cases.)
...
[CREATE EVENT](#page-90-0) statements which are themselves invalid—for whatever reason—fail with an error.
...
You can create an event but prevent it from being active using the DISABLE keyword. Alternatively, you can use ENABLE to make explicit the default status, which is active. This is most useful in conjunction with [ALTER EVENT](#page-58-0) (see [Section 15.1.3, "ALTER EVENT Statement"\)](#page-58-0).
...
A [CREATE EVENT](#page-90-0) statement that contains an [ALTER EVENT](#page-58-0) statement in its DO clause appears to succeed; however, when the server attempts to execute the resulting scheduled event, the execution fails with an error.
...
# <span id="page-95-0"></span>**15.1.14 CREATE FUNCTION Statement**

The [CREATE FUNCTION](#page-95-0) statement is used to create stored functions and loadable functions:

- For information about creating stored functions, see [Section 15.1.17, "CREATE PROCEDURE and](#page-110-0) [CREATE FUNCTION Statements".](#page-110-0)
- For information about creating loadable functions, see Section 15.7.4.1, "CREATE FUNCTION Statement for Loadable Functions".

# <span id="page-95-1"></span>**15.1.15 CREATE INDEX Statement**
...
Normally, you create all indexes on a table at the time the table itself is created with [CREATE TABLE](#page-121-0). See [Section 15.1.20, "CREATE TABLE Statement".](#page-121-0) This guideline is especially important for InnoDB tables, where the primary key determines the physical layout of rows in the data file. [CREATE INDEX](#page-95-1) enables you to add indexes to existing tables.

[CREATE INDEX](#page-95-1) is mapped to an [ALTER TABLE](#page-64-1) statement to create indexes. See [Section 15.1.9,](#page-64-1) ["ALTER TABLE Statement".](#page-64-1) [CREATE INDEX](#page-95-1) cannot be used to create a PRIMARY KEY; use [ALTER](#page-64-1) [TABLE](#page-64-1) instead. For more information about indexes, see Section 10.3.1, "How MySQL Uses Indexes".

InnoDB supports secondary indexes on virtual columns. For more information, see [Section 15.1.20.9,](#page-165-0) ["Secondary Indexes and Generated Columns".](#page-165-0)
...
Beginning with MySQL 8.0.17, the expr for a key\_part specification can take the form (CAST json\_expression AS type ARRAY) to create a multi-valued index on a JSON column. See [Multi-](#page-100-0)[Valued Indexes.](#page-100-0)
...
A key\_part specification can end with ASC or DESC to specify whether index values are stored in ascending or descending order. The default is ascending if no order specifier is given. ASC and DESC are not permitted for HASH indexes. ASC and DESC are also not supported for multi-valued indexes. As of MySQL 8.0.12, ASC and DESC are not permitted for SPATIAL indexes.

The following sections describe different aspects of the [CREATE INDEX](#page-95-1) statement:

- [Column Prefix Key Parts](#page-96-0)
- [Functional Key Parts](#page-97-0)
- [Unique Indexes](#page-100-1)
- [Full-Text Indexes](#page-100-2)
- [Multi-Valued Indexes](#page-100-0)
- [Spatial Indexes](#page-105-0)
- [Index Options](#page-106-0)
- [Table Copying and Locking Options](#page-109-1)

# <span id="page-96-0"></span>**Column Prefix Key Parts**
...
- Prefix limits are measured in bytes. However, prefix lengths for index specifications in [CREATE](#page-121-0) [TABLE](#page-121-0), [ALTER TABLE](#page-64-1), and [CREATE INDEX](#page-95-1) statements are interpreted as number of characters for nonbinary string types (CHAR, VARCHAR, TEXT) and number of bytes for binary string types (BINARY, VARBINARY, BLOB). Take this into account when specifying a prefix length for a nonbinary string column that uses a multibyte character set.
...
If a specified index prefix exceeds the maximum column data type size, [CREATE INDEX](#page-95-1) handles the index as follows:
...
# <span id="page-97-0"></span>**Functional Key Parts**
...
For [CREATE TABLE ... LIKE](#page-148-0), the destination table preserves functional key parts from the original table.
...
For more information about applicable restrictions, see [Section 15.1.20.8, "CREATE TABLE and](#page-162-0) [Generated Columns",](#page-162-0) and [Section 15.1.9.2, "ALTER TABLE and Generated Columns".](#page-84-0)
...
# <span id="page-100-1"></span>**Unique Indexes**
...
# <span id="page-100-2"></span>**Full-Text Indexes**
...
# <span id="page-100-0"></span>**Multi-Valued Indexes**
...
You can create a multi-valued index in a [CREATE TABLE](#page-121-0), [ALTER TABLE](#page-64-1), or [CREATE INDEX](#page-95-1) statement. This requires using CAST(... AS ... ARRAY) in the index definition, which casts sametyped scalar values in a JSON array to an SQL data type array. A virtual column is then generated transparently with the values in the SQL data type array; finally, a functional index (also referred to as a virtual index) is created on the virtual column. It is the functional index defined on the virtual column of values from the SQL data type array that forms the multi-valued index.
...
- Online creation of a multi-value index is not supported, which means the operation uses ALGORITHM=COPY. See [Performance and Space Requirements.](#page-69-0)
...
# <span id="page-105-0"></span>**Spatial Indexes**
...
# <span id="page-106-0"></span>**Index Options**
...
KEY\_BLOCK\_SIZE is not supported at the index level for InnoDB tables. See [Section 15.1.20,](#page-121-0) ["CREATE TABLE Statement"](#page-121-0).
...
The index\_type clause cannot be used for FULLTEXT INDEX or (prior to MySQL 8.0.12) SPATIAL INDEX specifications. Full-text index implementation is storage engine dependent. Spatial indexes are implemented as R-tree indexes.
...
| 2678 | Unique      | HASH       | Yes                   | Yes                                | Index                | Index                    |
...
This option can be used only with FULLTEXT indexes. It associates a parser plugin with the index if full-text indexing and searching operations need special handling. InnoDB and MyISAM support fulltext parser plugins. If you have a MyISAM table with an associated full-text parser plugin, you can convert the table to InnoDB using ALTER TABLE. See [Full-Text Parser Plugins](https://dev.mysql.com/doc/extending-mysql/8.0/en/plugin-types.md#full-text-plugin-type) and [Writing Full-Text](https://dev.mysql.com/doc/extending-mysql/8.0/en/writing-full-text-plugins.md) [Parser Plugins](https://dev.mysql.com/doc/extending-mysql/8.0/en/writing-full-text-plugins.md) for more information.

• COMMENT 'string'

Index definitions can include an optional comment of up to 1024 characters.

The MERGE\_THRESHOLD for index pages can be configured for individual indexes using the index\_option COMMENT clause of the [CREATE INDEX](#page-95-1) statement. For example:
...
MERGE\_THRESHOLD can also be defined at the index level and table level using [CREATE TABLE](#page-121-0) and [ALTER TABLE](#page-64-1) statements. For more information, see Section 17.8.11, "Configuring the Merge Threshold for Index Pages".
...
• ENGINE\_ATTRIBUTE and SECONDARY\_ENGINE\_ATTRIBUTE options (available as of MySQL 8.0.21) are used to specify index attributes for primary and secondary storage engines. The options are reserved for future use.
...
# <span id="page-109-1"></span>**Table Copying and Locking Options**
...
# <span id="page-109-0"></span>**15.1.16 CREATE LOGFILE GROUP Statement**
...
This statement creates a new log file group named logfile\_group having a single UNDO file named 'undo\_file'. A [CREATE LOGFILE GROUP](#page-109-0) statement has one and only one ADD UNDOFILE clause. For rules covering the naming of log file groups, see Section 11.2, "Schema Object Names".
...
Memory used for UNDO\_BUFFER\_SIZE comes from the global pool whose size is determined by the value of the SharedGlobalMemory data node configuration parameter. This includes any default
...
The ENGINE option determines the storage engine to be used by this log file group, with engine\_name being the name of the storage engine. In MySQL 8.0, this must be NDB (or NDBCLUSTER). If ENGINE is not set, MySQL tries to use the engine specified by the default\_storage\_engine server system variable (formerly storage\_engine). In any case, if the engine is not specified as NDB or NDBCLUSTER, the CREATE LOGFILE GROUP statement appears to succeed but actually fails to create the log file group, as shown here:
...
REDO\_BUFFER\_SIZE, NODEGROUP, WAIT, and COMMENT are parsed but ignored, and so have no effect in MySQL 8.0. These options are intended for future expansion.
...
[CREATE LOGFILE GROUP](#page-109-0) is useful only with Disk Data storage for NDB Cluster. See Section 25.6.11, "NDB Cluster Disk Data Tables".

# <span id="page-110-0"></span>**15.1.17 CREATE PROCEDURE and CREATE FUNCTION Statements**
...
[CREATE PROCEDURE](#page-110-0) and [CREATE FUNCTION](#page-95-0) require the CREATE ROUTINE privilege. If the DEFINER clause is present, the privileges required depend on the user value, as discussed in Section 27.6, "Stored Object Access Control". If binary logging is enabled, [CREATE FUNCTION](#page-95-0) might require the SUPER privilege, as discussed in Section 27.7, "Stored Program Binary Logging".
...
If the DEFINER clause is omitted, the default definer is the user who executes the [CREATE](#page-110-0) [PROCEDURE](#page-110-0) or [CREATE FUNCTION](#page-95-0) statement. This is the same as specifying DEFINER = CURRENT\_USER explicitly.
...
# <span id="page-116-0"></span>**15.1.18 CREATE SERVER Statement**
...
# <span id="page-117-0"></span>**15.1.19 CREATE SPATIAL REFERENCE SYSTEM Statement**
...
- SRID 0 is a valid SRID but cannot be used with [CREATE SPATIAL REFERENCE SYSTEM](#page-117-0).
...
Here is an example [CREATE SPATIAL REFERENCE SYSTEM](#page-117-0) statement. The DEFINITION value is reformatted across multiple lines for readability. (For the statement to be legal, the value actually must be given on a single line.)
...
If an SRS definition specifies an authority code for the projection (which is recommended), an error occurs if the definition is missing mandatory parameters. In this case, the error message indicates what the problem is. The projection methods and mandatory parameters that MySQL supports are shown in [Table 15.7, "Supported Spatial Reference System Projection Methods"](#page-119-0) and [Table 15.8, "Spatial](#page-120-0) [Reference System Projection Parameters"](#page-120-0).

For additional information about writing SRS definitions for MySQL, see [Geographic Spatial Reference](https://mysqlserverteam.com/geographic-spatial-reference-systems-in-mysql-8-0/) [Systems in MySQL 8.0](https://mysqlserverteam.com/geographic-spatial-reference-systems-in-mysql-8-0/) and [Projected Spatial Reference Systems in MySQL 8.0](https://mysqlserverteam.com/projected-spatial-reference-systems-in-mysql-8-0/)
...
# <span id="page-121-0"></span>**15.1.20 CREATE TABLE Statement**
...
[CREATE TABLE](#page-121-0) creates a table with the given name. You must have the CREATE privilege for the table.
...
For information about the physical representation of a table, see [Section 15.1.20.1, "Files Created by](#page-147-0) [CREATE TABLE"](#page-147-0).

There are several aspects to the [CREATE TABLE](#page-121-0) statement, described under the following topics in this section:

- [Table Name](#page-124-0)
- [Temporary Tables](#page-124-1)
- [Table Cloning and Copying](#page-125-0)
- [Column Data Types and Attributes](#page-125-1)
- [Indexes, Foreign Keys, and CHECK Constraints](#page-128-0)
- [Table Options](#page-132-0)
- [Table Partitioning](#page-140-0)

# <span id="page-124-0"></span>**Table Name**
...
Prevents an error from occurring if the table exists. However, there is no verification that the existing table has a structure identical to that indicated by the [CREATE TABLE](#page-121-0) statement.

# <span id="page-124-1"></span>**Temporary Tables**

You can use the TEMPORARY keyword when creating a table. A TEMPORARY table is visible only within the current session, and is dropped automatically when the session is closed. For more information, see [Section 15.1.20.2, "CREATE TEMPORARY TABLE Statement"](#page-147-1).

# <span id="page-125-0"></span>**Table Cloning and Copying**
...
For more information, see [Section 15.1.20.3, "CREATE TABLE ... LIKE Statement"](#page-148-0).

• [AS] query\_expression

To create one table from another, add a SELECT statement at the end of the [CREATE TABLE](#page-121-0) statement:

```
CREATE TABLE new_tbl AS SELECT * FROM orig_tbl;
```

For more information, see [Section 15.1.20.4, "CREATE TABLE ... SELECT Statement"](#page-149-0).
...
# <span id="page-125-1"></span>**Column Data Types and Attributes**
...
• Some attributes do not apply to all data types. AUTO\_INCREMENT applies only to integer and floating-point types. Using AUTO\_INCREMENT with FLOAT or DOUBLE columns is deprecated as of MySQL 8.0.17; expect support for it to be removed in a future version of MySQL.
...
• For CHAR, VARCHAR, BINARY, and VARBINARY columns, indexes can be created that use only the leading part of column values, using col\_name(length) syntax to specify an index prefix length. BLOB and TEXT columns also can be indexed, but a prefix length must be given. Prefix lengths are given in characters for nonbinary string types and in bytes for binary string types. That is, index entries consist of the first length characters of each column value for CHAR, VARCHAR, and TEXT columns, and the first length bytes of each column value for BINARY, VARBINARY, and BLOB

columns. Indexing only a prefix of column values like this can make the index file much smaller. For additional information about index prefixes, see [Section 15.1.15, "CREATE INDEX Statement".](#page-95-1)
...
If a specified index prefix exceeds the maximum column data type size, [CREATE TABLE](#page-121-0) handles the index as follows:

- For a nonunique index, either an error occurs (if strict SQL mode is enabled), or the index length is reduced to lie within the maximum column data type size and a warning is produced (if strict SQL mode is not enabled).
- For a unique index, an error occurs regardless of SQL mode because reducing the index length might enable insertion of nonunique entries that do not meet the specified uniqueness requirement.
- JSON columns cannot be indexed. You can work around this restriction by creating an index on a generated column that extracts a scalar value from the JSON column. See [Indexing a Generated](#page-166-0) [Column to Provide a JSON Column Index,](#page-166-0) for a detailed example.
...
If the NO\_ZERO\_DATE or NO\_ZERO\_IN\_DATE SQL mode is enabled and a date-valued default is not correct according to that mode, [CREATE TABLE](#page-121-0) produces a warning if strict SQL mode is not enabled and an error if strict mode is enabled. For example, with NO\_ZERO\_IN\_DATE enabled, c1 DATE DEFAULT '2010-00-00' produces a warning.

## • VISIBLE, INVISIBLE

Specify column visibility. The default is VISIBLE if neither keyword is present. A table must have at least one visible column. Attempting to make all columns invisible produces an error. For more information, see [Section 15.1.20.10, "Invisible Columns".](#page-168-0)
...
Used to specify a generated column expression. For information about generated columns, see [Section 15.1.20.8, "CREATE TABLE and Generated Columns".](#page-162-0)

Stored generated columns can be indexed. InnoDB supports secondary indexes on virtual generated columns. See [Section 15.1.20.9, "Secondary Indexes and Generated Columns".](#page-165-0)

# <span id="page-128-0"></span>**Indexes, Foreign Keys, and CHECK Constraints**

Several keywords apply to creation of indexes, foreign keys, and CHECK constraints. For general background in addition to the following descriptions, see [Section 15.1.15, "CREATE INDEX](#page-95-1) [Statement",](#page-95-1) [Section 15.1.20.5, "FOREIGN KEY Constraints"](#page-153-0), and [Section 15.1.20.6, "CHECK](#page-160-0) [Constraints".](#page-160-0)
...
If a table has a PRIMARY KEY or UNIQUE NOT NULL index that consists of a single column that has an integer type, you can use \_rowid to refer to the indexed column in SELECT statements, as described in [Unique Indexes](#page-100-1).

In MySQL, the name of a PRIMARY KEY is PRIMARY. For other indexes, if you do not assign a name, the index is assigned the same name as the first indexed column, with an optional suffix (\_2, \_3, ...) to make it unique. You can see index names for a table using SHOW INDEX FROM tbl\_name. See Section 15.7.7.22, "SHOW INDEX Statement".
...
MySQL supports foreign keys, which let you cross-reference related data across tables, and foreign key constraints, which help keep this spread-out data consistent. For definition and option information, see [reference\\_definition](#page-131-0), and [reference\\_option](#page-132-1).
...
The CHECK clause enables the creation of constraints to be checked for data values in table rows. See [Section 15.1.20.6, "CHECK Constraints".](#page-160-0)
...
- Beginning with MySQL 8.0.17, the expr for a key\_part specification can take the form (CAST json\_path AS type ARRAY) to create a multi-valued index on a JSON column. [Multi-Valued](#page-100-0) [Indexes](#page-100-0), provides detailed information regarding creation of, usage of, and restrictions and limitations on multi-valued indexes.
- index\_type
...
For information about the table-level KEY\_BLOCK\_SIZE attribute, see [Table Options](#page-132-0).
...
For more information about permissible index\_option values, see [Section 15.1.15, "CREATE](#page-95-1) [INDEX Statement"](#page-95-1). For more information about indexes, see Section 10.3.1, "How MySQL Uses Indexes".

<span id="page-131-0"></span>• reference\_definition

For reference\_definition syntax details and examples, see [Section 15.1.20.5, "FOREIGN KEY](#page-153-0) [Constraints".](#page-153-0)
...
For other storage engines, MySQL Server parses and ignores the FOREIGN KEY syntax in [CREATE](#page-121-0) [TABLE](#page-121-0) statements.
...
#### <span id="page-132-1"></span>• reference\_option

For information about the RESTRICT, CASCADE, SET NULL, NO ACTION, and SET DEFAULT options, see [Section 15.1.20.5, "FOREIGN KEY Constraints"](#page-153-0).

# <span id="page-132-0"></span>**Table Options**

Table options are used to optimize the behavior of the table. In most cases, you do not have to specify any of them. These options apply to all storage engines unless otherwise indicated. Options that do not apply to a given storage engine may be accepted and remembered as part of the table definition. Such options then apply if you later use [ALTER TABLE](#page-64-1) to convert the table to use a different storage engine.
...
| NDB            | Clustered, fault-tolerant, memory-based tables,<br>supporting transactions and foreign keys. Also<br>known as NDBCLUSTER. See Chapter 25,<br>MySQL NDB Cluster 8.0. |
...
The initial AUTO\_INCREMENT value for the table. In MySQL 8.0, this works for MyISAM, MEMORY, InnoDB, and ARCHIVE tables. To set the first auto-increment value for engines that do not support the AUTO\_INCREMENT table option, insert a "dummy" row with a value one less than the desired value after creating the table, and then delete the dummy row.

For engines that support the AUTO\_INCREMENT table option in [CREATE TABLE](#page-121-0) statements, you can also use ALTER TABLE tbl\_name AUTO\_INCREMENT = N to reset the AUTO\_INCREMENT value. The value cannot be set lower than the maximum value currently in the column.

# • AVG\_ROW\_LENGTH

An approximation of the average row length for your table. You need to set this only for large tables with variable-size rows.

When you create a MyISAM table, MySQL uses the product of the MAX\_ROWS and AVG\_ROW\_LENGTH options to decide how big the resulting table is. If you don't specify either option, the maximum size for MyISAM data and index files is 256TB by default. (If your operating system does not support files that large, table sizes are constrained by the file size limit.) If you want to keep down the pointer sizes to make the index smaller and faster and you don't really need big files, you can decrease the default pointer size by setting the myisam\_data\_pointer\_size system variable. (See Section 7.1.8, "Server System Variables".) If you want all your tables to be able
...
This comment syntax is also supported with [ALTER TABLE](#page-64-1) statements for NDB tables. Keep in mind that a table comment used with ALTER TABLE replaces any existing comment which the table might have had previously.

Setting the MERGE\_THRESHOLD option in table comments is not supported for NDB tables (it is ignored).

For complete syntax information and examples, see [Section 15.1.20.12, "Setting NDB Comment](#page-174-0) [Options".](#page-174-0)
...
• ENGINE\_ATTRIBUTE and SECONDARY\_ENGINE\_ATTRIBUTE options (available as of MySQL 8.0.21) are used to specify table attributes for primary and secondary storage engines. The options are reserved for future use.
...
The use of MAX\_ROWS with NDB tables to control the number of table partitions is deprecated. It remains supported in later versions for backward compatibility, but is subject to removal in a future release. Use PARTITION\_BALANCE instead; see [Setting NDB\\_TABLE options.](#page-134-0)
...
- To enable compression for InnoDB tables, specify ROW\_FORMAT=COMPRESSED. The ROW\_FORMAT=COMPRESSED option is not supported when creating temporary tables. See Section 17.9, "InnoDB Table and Page Compression" for requirements associated with the COMPRESSED row format.
...
Support for TABLESPACE = innodb\_file\_per\_table and TABLESPACE = innodb\_temporary clauses with [CREATE TEMPORARY TABLE](#page-121-0) is deprecated as of MySQL 8.0.13; expect it to be removed in a future version of MySQL.
...
# <span id="page-140-0"></span>**Table Partitioning**

partition\_options can be used to control partitioning of the table created with [CREATE TABLE](#page-121-0).
...
Partitions can be modified, merged, added to tables, and dropped from tables. For basic information about the MySQL statements to accomplish these tasks, see [Section 15.1.9, "ALTER TABLE](#page-64-1) [Statement".](#page-64-1) For more detailed descriptions and examples, see Section 26.3, "Partition Management".
...
Hashes one or more columns to create a key for placing and locating rows. expr is an expression using one or more table columns. This can be any valid MySQL expression (including MySQL functions) that yields a single integer value. For example, these are both valid [CREATE TABLE](#page-121-0) statements using PARTITION BY HASH:
...
MySQL 5.5 or later server which can be used on a MySQL 5.1 server. For more information, see [Section 15.1.9.1, "ALTER TABLE Partition Operations"](#page-80-0).
...
A table implementing such a partitioning scheme can be realized by the [CREATE TABLE](#page-121-0) statement shown here:
...
DATA DIRECTORY and INDEX DIRECTORY behave in the same way as in the [CREATE TABLE](#page-121-0) statement's table\_option clause as used for MyISAM tables.
...
# <span id="page-147-0"></span>**15.1.20.1 Files Created by CREATE TABLE**
...
# <span id="page-147-1"></span>**15.1.20.2 CREATE TEMPORARY TABLE Statement**

You can use the TEMPORARY keyword when creating a table. A TEMPORARY table is visible only within the current session, and is dropped automatically when the session is closed. This means that two different sessions can use the same temporary table name without conflicting with each other or with an existing non-TEMPORARY table of the same name. (The existing table is hidden until the temporary table is dropped.)

InnoDB does not support compressed temporary tables. When innodb\_strict\_mode is enabled (the default), [CREATE TEMPORARY TABLE](#page-121-0) returns an error if ROW\_FORMAT=COMPRESSED or KEY\_BLOCK\_SIZE is specified. If innodb\_strict\_mode is disabled, warnings are issued and the temporary table is created using a non-compressed row format. The innodb\_file\_per-table option does not affect the creation of InnoDB temporary tables.

[CREATE TABLE](#page-121-0) causes an implicit commit, except when used with the TEMPORARY keyword. See Section 15.3.3, "Statements That Cause an Implicit Commit".
...
To create a temporary table, you must have the CREATE TEMPORARY TABLES privilege. After a session has created a temporary table, the server performs no further privilege checks on the table. The creating session can perform any operation on the table, such as [DROP TABLE](#page-198-0), INSERT, UPDATE, or SELECT.
...
# <span id="page-148-0"></span>**15.1.20.3 CREATE TABLE ... LIKE Statement**

Use CREATE TABLE ... LIKE to create an empty table based on the definition of another table, including any column attributes and indexes defined in the original table:

CREATE TABLE new\_tbl LIKE orig\_tbl;
...
![](_page_148_Picture_17.jpeg)

#### **Important**

You cannot execute CREATE TABLE or CREATE TABLE ... LIKE while a LOCK TABLES statement is in effect.

[CREATE TABLE ... LIKE](#page-121-0) makes the same checks as [CREATE TABLE](#page-121-0). This means that if the current SQL mode is different from the mode in effect when the original table was created, the table definition might be considered invalid for the new mode and cause the statement to fail.
...
# <span id="page-149-0"></span>**15.1.20.4 CREATE TABLE ... SELECT Statement**

You can create one table from another by adding a SELECT statement at the end of the [CREATE](#page-121-0) [TABLE](#page-121-0) statement:
...
This creates an InnoDB table with three columns, a, b, and c. The ENGINE option is part of the [CREATE TABLE](#page-121-0) statement, and should not be used following the SELECT; this would result in a syntax error. The same is true for other [CREATE TABLE](#page-121-0) options such as CHARSET.
...
In a table resulting from [CREATE TABLE ... SELECT](#page-121-0), columns named only in the [CREATE TABLE](#page-121-0) part come first. Columns named in both parts or only in the SELECT part come after that. The data type of SELECT columns can be overridden by also specifying the column in the [CREATE TABLE](#page-121-0) part.

If errors occur while copying data to the table, the table is automatically dropped and not created. However, prior to MySQL 8.0.21, when row-based replication is in use, a [CREATE TABLE ...](#page-149-0) [SELECT](#page-149-0) statement is recorded in the binary log as two transactions, one to create the table, and the other to insert data. When the statement applied from the binary log, a failure between the two transactions or while copying data can result in replication of an empty table. That limitation is removed in MySQL 8.0.21. On storage engines that support atomic DDL, [CREATE TABLE ... SELECT](#page-149-0) is now recorded and applied as one transaction when row-based replication is in use. For more information, see [Section 15.1.1, "Atomic Data Definition Statement Support"](#page-47-1).

As of MySQL 8.0.21, on storage engines that support both atomic DDL and foreign key constraints, creation of foreign keys is not permitted in [CREATE TABLE ... SELECT](#page-149-0) statements when row-based replication is in use. Foreign key constraints can be added later using [ALTER TABLE](#page-64-1).
...
[CREATE TABLE ... SELECT](#page-121-0) does not automatically create any indexes for you. This is done intentionally to make the statement as flexible as possible. If you want to have indexes in the created table, you should specify these before the SELECT statement:
...
When creating a table with [CREATE TABLE ... SELECT](#page-149-0), make sure to alias any function calls or expressions in the query. If you do not, the CREATE statement might fail or result in undesirable column names.
...
For [CREATE TABLE ... SELECT](#page-121-0), if IF NOT EXISTS is given and the target table exists, nothing is inserted into the destination table, and the statement is not logged.

To ensure that the binary log can be used to re-create the original tables, MySQL does not permit concurrent inserts during [CREATE TABLE ... SELECT](#page-121-0). However, prior to MySQL 8.0.21, when a [CREATE TABLE ... SELECT](#page-121-0) operation is applied from the binary log when row-based replication is in use, concurrent inserts are permitted on the replicated table while copying data. That limitation is removed in MySQL 8.0.21 on storage engines that support atomic DDL. For more information, see [Section 15.1.1, "Atomic Data Definition Statement Support"](#page-47-1).

You cannot use FOR UPDATE as part of the SELECT in a statement such as [CREATE TABLE](#page-149-0) new\_table [SELECT ... FROM](#page-149-0) old\_table .... If you attempt to do so, the statement fails.

[CREATE TABLE ... SELECT](#page-149-0) operations apply ENGINE\_ATTRIBUTE and SECONDARY\_ENGINE\_ATTRIBUTE values to columns only. Table and index ENGINE\_ATTRIBUTE and SECONDARY\_ENGINE\_ATTRIBUTE values are not applied to the new table unless specified explicitly.

# <span id="page-153-0"></span>**15.1.20.5 FOREIGN KEY Constraints**

MySQL supports foreign keys, which permit cross-referencing related data across tables, and foreign key constraints, which help keep the related data consistent.

A foreign key relationship involves a parent table that holds the initial column values, and a child table with column values that reference the parent column values. A foreign key constraint is defined on the child table.

The essential syntax for a defining a foreign key constraint in a [CREATE TABLE](#page-121-0) or [ALTER TABLE](#page-64-1) statement includes the following:
...
- [Identifiers](#page-153-1)
- [Conditions and Restrictions](#page-154-0)
- [Referential Actions](#page-155-0)
- [Foreign Key Constraint Examples](#page-156-0)
- [Adding Foreign Key Constraints](#page-156-1)
- [Dropping Foreign Key Constraints](#page-157-0)
- [Foreign Key Checks](#page-157-1)
- [Locking](#page-158-0)
- [Foreign Key Definitions and Metadata](#page-158-1)
- [Foreign Key Errors](#page-159-0)

## <span id="page-153-1"></span>**Identifiers**
...
Prior to MySQL 8.0.16, if the CONSTRAINT symbol clause was not defined, or a symbol was not included following the CONSTRAINT keyword, both InnoDB and NDB storage engines would use the FOREIGN\_KEY index\_name if defined. In MySQL 8.0.16 and higher, the FOREIGN\_KEY index\_name is ignored.
...
- NDB Cluster stores foreign names using the same lettercase with which they are created. Prior to version 8.0.20, when processing SELECT and other SQL statements, NDB compared the names of foreign keys in such statements with the names as stored in a case-sensitive fashion when lower\_case\_table\_names was equal to 0. In NDB 8.0.20 and later, this value no longer has any effect on how such comparisons are made, and they are always done without regard to lettercase. (Bug #30512043)
...
# <span id="page-154-0"></span>**Conditions and Restrictions**
...
#### <span id="page-155-0"></span>**Referential Actions**
...
## <span id="page-156-0"></span>**Foreign Key Constraint Examples**
...
#### <span id="page-156-1"></span>**Adding Foreign Key Constraints**

You can add a foreign key constraint to an existing table using the following [ALTER TABLE](#page-64-1) syntax:
...
The foreign key can be self referential (referring to the same table). When you add a foreign key constraint to a table using [ALTER TABLE](#page-64-1), remember to first create an index on the column(s) referenced by the foreign key.

## <span id="page-157-0"></span>**Dropping Foreign Key Constraints**

You can drop a foreign key constraint using the following [ALTER TABLE](#page-64-1) syntax:
...
Adding and dropping a foreign key in the same [ALTER TABLE](#page-64-1) statement is supported for [ALTER](#page-64-1) [TABLE ... ALGORITHM=INPLACE](#page-64-1). It is not supported for [ALTER TABLE ... ALGORITHM=COPY](#page-64-1).

#### <span id="page-157-1"></span>**Foreign Key Checks**
...
- Performing an [ALTER TABLE](#page-64-1) operation on a table that has a foreign key relationship.
...
## <span id="page-158-0"></span>**Locking**
...
#### <span id="page-158-1"></span>**Foreign Key Definitions and Metadata**
...
#### <span id="page-159-0"></span>**Foreign Key Errors**
...
[ER\\_NO\\_REFERENCED\\_ROW\\_2](https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.md#error_er_no_referenced_row_2) and [ER\\_ROW\\_IS\\_REFERENCED\\_2](https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.md#error_er_row_is_referenced_2) error messages for foreign key operations expose information about parent tables. If a user does not have table-level privileges for all parent tables, more generic error messages are displayed instead ([ER\\_NO\\_REFERENCED\\_ROW](https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.md#error_er_no_referenced_row) and [ER\\_ROW\\_IS\\_REFERENCED](https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.md#error_er_row_is_referenced)).
...
# <span id="page-160-0"></span>**15.1.20.6 CHECK Constraints**

Prior to MySQL 8.0.16, [CREATE TABLE](#page-121-0) permits only the following limited version of table CHECK constraint syntax, which is parsed and ignored:

```
CHECK (expr)
```

As of MySQL 8.0.16, [CREATE TABLE](#page-121-0) permits the core features of table and column CHECK constraints, for all storage engines. [CREATE TABLE](#page-121-0) permits the following CHECK constraint syntax, for both table constraints and column constraints:
