...
This section describes how to use the openssl command to set up the RSA key files that enable MySQL to support secure password exchange over unencrypted connections for accounts authenticated by the sha256\_password and caching\_sha2\_password plugins.

![](_page_33_Picture_3.jpeg)

### **Note**

There are easier alternatives to generating the files required for RSA than the procedure described here: Let the server autogenerate them or use the mysql\_ssl\_rsa\_setup program (deprecated as of MySQL 8.0.34). See [Section 8.3.3.1, "Creating SSL and RSA Certificates and Keys using MySQL"](#page-24-1).
...
# <span id="page-33-0"></span>**8.3.4 Connecting to MySQL Remotely from Windows with SSH**
