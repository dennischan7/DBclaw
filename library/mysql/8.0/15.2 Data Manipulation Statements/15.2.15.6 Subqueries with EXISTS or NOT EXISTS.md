...
title: 15.2.15.6 Subqueries with EXISTS or NOT EXISTS
---
...
Traditionally, an EXISTS subquery starts with SELECT \*, but it could begin with SELECT 5 or SELECT column1 or anything at all. MySQL ignores the [SELECT](#page-49-0) list in such a subquery, so it makes no difference.
...
In MySQL 8.0.19 and later, you can also use NOT EXISTS or NOT EXISTS with [TABLE](#page-85-0) in the subquery, like this:
...
## <span id="page-75-0"></span>**15.2.15.7 Correlated Subqueries**
...
Beginning with MySQL 8.0.24, the optimizer can transform a correlated scalar subquery to a derived table when the subquery\_to\_derived flag of the optimizer\_switch variable is enabled. Consider the query shown here:
...
In the rewritten query, REJECT\_IF() represents an internal function which tests a given condition (here, the comparison ct > 1) and raises a given error (in this case, [ER\\_SUBQUERY\\_NO\\_1\\_ROW](https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.md#error_er_subquery_no_1_row)) if the condition is true. This reflects the cardinality check that the optimizer performs as part of evaluating the JOIN or WHERE clause, prior to evaluating any lifted predicate, which is done only if the subquery does not return more than one row.

This type of transformation can be performed, provided the following conditions are met:

- The subquery can be part of a [SELECT](#page-49-0) list, WHERE condition, or HAVING condition, but cannot be part of a [JOIN](#page-58-0) condition, and cannot contain a LIMIT or OFFSET clause. In addition, the subquery cannot contain any set operations such as [UNION](#page-91-0).
...
See also [Section 15.2.15.8, "Derived Tables".](#page-76-0)

## <span id="page-76-0"></span>**15.2.15.8 Derived Tables**

This section discusses general characteristics of derived tables. For information about lateral derived tables preceded by the LATERAL keyword, see [Section 15.2.15.9, "Lateral Derived Tables"](#page-80-0).

A derived table is an expression that generates a table within the scope of a query FROM clause. For example, a subquery in a [SELECT](#page-49-0) statement FROM clause is a derived table:
...
- A derived table cannot contain references to other tables of the same [SELECT](#page-49-0) (use a LATERAL derived table for that; see [Section 15.2.15.9, "Lateral Derived Tables"](#page-80-0)).
...
This is because the [SELECT](#page-49-0) statement did not reference any tables, as can be seen in the table and Extra columns of the output. This is also true of the following nested [SELECT](#page-49-0):
...
However, if the outer [SELECT](#page-49-0) references any tables, the optimizer executes the statement in the subquery as well, with the result that t2 is modified:
...
The derived table optimization can also be employed with many correlated (scalar) subqueries (MySQL 8.0.24 and later). For more information and examples, see [Section 15.2.15.7, "Correlated Subqueries".](#page-75-0)

## <span id="page-80-0"></span>**15.2.15.9 Lateral Derived Tables**
...
Nonlateral derived tables are specified using the syntax discussed in [Section 15.2.15.8, "Derived](#page-76-0) [Tables"](#page-76-0). The syntax for a lateral derived table is the same as for a nonlateral derived table except that the keyword LATERAL is specified before the derived table specification. The LATERAL keyword must precede each table to be used as a lateral derived table.
...
- In accordance with the SQL standard, MySQL always treats a join with a table function such as JSON\_TABLE() as though LATERAL had been used. This is true regardless of MySQL release version, which is why it is possible to join against this function even in MySQL versions prior to 8.0.14. In MySQL 8.0.14 and later, the LATERAL keyword is implicit, and is not allowed before JSON\_TABLE(). This is also according to the SQL standard.
...
Second approach to solving the problem: A different solution could be used if a subquery in the [SELECT](#page-49-0) list could return multiple columns:
...
## <span id="page-82-0"></span>**15.2.15.10 Subquery Errors**
...
You may use a subquery that returns multiple columns, if the purpose is row comparison. In other contexts, the subquery must be a scalar operand. See [Section 15.2.15.5, "Row Subqueries".](#page-73-0)
...
You can use a common table expression or derived table to work around this. See [Section 15.2.15.12, "Restrictions on Subqueries"](#page-84-0).

In MySQL 8.0.19 and later, all of the errors described in this section also apply when using [TABLE](#page-85-0) in subqueries.
