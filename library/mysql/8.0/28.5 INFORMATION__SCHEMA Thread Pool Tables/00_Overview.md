...
As of MySQL 8.0.14, the INFORMATION\_SCHEMA thread pool tables are also available as Performance Schema tables. (See Section 29.12.16, "Performance Schema Thread Pool Tables".) The INFORMATION\_SCHEMA tables are deprecated; expect them be removed in a future version of MySQL. Applications should transition away from the old tables to the new tables. For example, if an application uses this query:
...
- [TP\\_THREAD\\_GROUP\\_STATE](#page-102-2): Information about thread pool thread group states
- [TP\\_THREAD\\_GROUP\\_STATS](#page-102-0): Thread group statistics
- [TP\\_THREAD\\_STATE](#page-102-1): Information about thread pool thread states
