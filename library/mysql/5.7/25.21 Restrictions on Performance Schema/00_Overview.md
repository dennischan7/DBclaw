...
If you save event information in another table, you should not assume that the original events are still available later. For example, if you select events from a performance\_schema table into a temporary table, intending to join that table with the original table later, there might be no matches.
...
Results for queries that refer to tables in the performance\_schema database are not saved in the query cache.
...
The types of timers might vary per platform. The [performance\\_timers](#page-96-0) table shows which event timers are available. If the values in this table for a given timer name are NULL, that timer is not supported on your platform.
...
# <span id="page-136-1"></span>Chapter 26 MySQL sys Schema

# **Table of Contents**

| 26.1 Prerequisites for Using the sys Schema 4109 |  |
|--------------------------------------------------|--|
| 26.2 Using the sys Schema 4110                   |  |
| 26.3 sys Schema Progress Reporting 4111          |  |
| 26.4 sys Schema Object Reference 4112            |  |
| 26.4.1 sys Schema Object Index 4112              |  |
| 26.4.2 sys Schema Tables and Triggers 4116       |  |
| 26.4.3 sys Schema Views 4119                     |  |
| 26.4.4 sys Schema Stored Procedures 4158         |  |
| 26.4.5 sys Schema Stored Functions 4176          |  |

MySQL 5.7 includes the [sys](#page-136-1) schema, a set of objects that helps DBAs and developers interpret data collected by the Performance Schema. [sys](#page-136-1) schema objects can be used for typical tuning and diagnosis use cases. Objects in this schema include:
...
For new installations, the [sys](#page-136-1) schema is installed by default during data directory initialization if you use mysqld with the --initialize or --initialize-insecure option. You can drop the [sys](#page-136-1) schema manually after initialization if it is unneeded.

For upgrades, mysql\_upgrade installs the [sys](#page-136-1) schema if it is not installed, and upgrades it to the current version otherwise. To permit this behavior to be suppressed, mysql\_upgrade has a --skipsys-schema option.

mysql\_upgrade returns an error if a [sys](#page-136-1) schema exists but has no [version](#page-182-0) view, on the assumption that absence of this view indicates a user-created sys schema. To upgrade in this case, remove or rename the existing [sys](#page-136-1) schema first.

[sys](#page-136-1) schema objects have a DEFINER of 'mysql.sys'@'localhost'. Use of the dedicated mysql.sys account avoids problems that occur if a DBA renames or removes the root account.

# <span id="page-136-0"></span>**26.1 Prerequisites for Using the sys Schema**

Before using the [sys](#page-136-1) schema, the prerequisites described in this section must be satisfied.

Because the [sys](#page-136-1) schema provides an alternative means of accessing the Performance Schema, the Performance Schema must be enabled for the [sys](#page-136-1) schema to work. See Section 25.3, "Performance Schema Startup Configuration".

For full access to the [sys](#page-136-1) schema, a user must have these privileges:

- SELECT on all [sys](#page-136-1) tables and views
- EXECUTE on all [sys](#page-136-1) stored procedures and functions
- INSERT and UPDATE for the [sys\\_config](#page-144-0) table, if changes are to be made to it
- Additional privileges for certain [sys](#page-136-1) schema stored procedures and functions, as noted in their descriptions (for example, the [ps\\_setup\\_save\(\)](#page-192-0) procedure)

It is also necessary to have privileges for the objects underlying the [sys](#page-136-1) schema objects:

- SELECT on any Performance Schema tables accessed by [sys](#page-136-1) schema objects, and UPDATE for any tables to be updated using [sys](#page-136-1) schema objects
- PROCESS for the INFORMATION\_SCHEMA INNODB\_BUFFER\_PAGE table

Certain Performance Schema instruments and consumers must be enabled and (for instruments) timed to take full advantage of [sys](#page-136-1) schema capabilities:
...
You can use the [sys](#page-136-1) schema itself to enable all of the additional instruments and consumers:
...
# <span id="page-137-0"></span>**26.2 Using the sys Schema**

You can make the [sys](#page-136-1) schema the default schema so that references to its objects need not be qualified with the schema name:
...
(The [version](#page-182-0) view shows the [sys](#page-136-1) schema and MySQL server versions.)

To access [sys](#page-136-1) schema objects while a different schema is the default (or simply to be explicit), qualify object references with the schema name:
...
The [x\\$host\\_summary\\_by\\_file\\_io](#page-147-0) view summarizes the same data but displays unformatted picosecond latencies:
...
The view without the x\$ prefix is intended to provide output that is more user friendly and easier for humans to read. The view with the x\$ prefix that displays the same values in raw form is intended more for use with other tools that perform their own processing on the data. For additional information about the differences between non-x\$ and x\$ views, see [Section 26.4.3, "sys Schema Views".](#page-146-0)

To examine [sys](#page-136-1) schema object definitions, use the appropriate SHOW statement or INFORMATION\_SCHEMA query. For example, to examine the definitions of the [session](#page-172-0) view and format\_bytes() function, use these statements:
...
However, those statements display the definitions in relatively unformatted form. To view object definitions with more readable formatting, access the individual .sql files found under the scripts/ sys\_schema in MySQL source distributions. Prior to MySQL 5.7.28, the sources are maintained in a separate distribution available from the [sys](#page-136-1) schema development website at [https://github.com/mysql/](https://github.com/mysql/mysql-sys) [mysql-sys](https://github.com/mysql/mysql-sys).

Neither mysqldump nor mysqlpump dump the [sys](#page-136-1) schema by default. To generate a dump file, name the [sys](#page-136-1) schema explicitly on the command line using either of these commands:
...
# <span id="page-138-0"></span>**26.3 sys Schema Progress Reporting**

The following [sys](#page-136-1) schema views provide progress reporting for long-running transactions:
...
# <span id="page-139-0"></span>**26.4 sys Schema Object Reference**

The [sys](#page-136-1) schema includes tables and triggers, views, and stored procedures and functions. The following sections provide details for each of these objects.

# <span id="page-139-1"></span>**26.4.1 sys Schema Object Index**

The following tables list [sys](#page-136-1) schema objects and provide a short description of each one.
...
| innodb_buffer_stats_by_schema,<br>x<br>\$innodb_buffer_stats_by_schema | InnoDB buffer information,<br>grouped by schema                   |            |
...
| io_global_by_wait_by_bytes,<br>x                       | Global I/O consumers, grouped<br>by bytes            |            |
...
| x<br>\$memory_by_user_by_current_bytes                 |                                                      |            |
...
| processlist, x<br>\$processlist                        | Processlist information                              |            |
...
| schema_index_statistics,<br>x                          | Index statistics                                     |            |
| \$schema_index_statistics                              |                                                      |            |
| schema_object_overview                                 | Types of objects within each<br>schema               |            |
| schema_redundant_indexes                               | Duplicate or redundant indexes                       |            |
| schema_table_lock_waits,<br>x                          | Sessions waiting for metadata<br>locks               |            |
| \$schema_table_lock_waits                              |                                                      |            |
| schema_table_statistics,<br>x                          | Table statistics                                     |            |
...
| statements_with_errors_or_warnings,               | Statements that have produced                         |            |
| x<br>\$statements_with_errors_or_warnings         | errors or warnings                                    |            |
| statements_with_full_table_scans,                 | Statements that have done full                        |            |
| x                                                 | table scans                                           |            |
...
| user_summary_by_file_io,<br>x                     | File I/O, grouped by user                             |            |
| \$user_summary_by_file_io                         |                                                       |            |
| user_summary_by_file_io_type,                     | File I/O, grouped by user and                         |            |
| x<br>\$user_summary_by_file_io_type               | event                                                 |            |
| user_summary_by_stages, x                         | Stage events, grouped by user                         |            |
| \$user_summary_by_stages                          |                                                       |            |
| user_summary_by_statement_latency,<br>x           | Statement statistics, grouped by<br>user              |            |
...
| wait_classes_global_by_avg_latency,<br>x          | Wait class average latency,<br>grouped by event class |            |
| \$wait_classes_global_by_avg_latency              |                                                       |            |

| View Name                             | Description                       | Deprecated |
|---------------------------------------|-----------------------------------|------------|
| wait_classes_global_by_latency,       | Wait class total latency, grouped |            |
| x                                     | by event class                    |            |
| \$wait_classes_global_by_latency      |                                   |            |
| waits_by_host_by_latency,             | Wait events, grouped by host      |            |
| x                                     | and event                         |            |
...
| format_path()                      | Replace directories in path name with symbolic<br>system variable names    |
...
| ps_is_account_enabled()            | Whether Performance Schema instrumentation for<br>account is enabled       |
| ps_is_consumer_enabled()           | Whether Performance Schema consumer is<br>enabled                          |
| ps_is_instrument_default_enabled() | Whether Performance Schema instrument is<br>enabled by default             |
| ps_is_instrument_default_timed()   | Whether Performance Schema instrument is timed<br>by default               |
| ps_is_thread_instrumented()        | Whether Performance Schema instrumentation for<br>connection ID is enabled |
| ps_thread_account()                | Account associated with Performance Schema<br>thread ID                    |
| ps_thread_id()                     | Performance Schema thread ID associated with<br>connection ID              |
...
# <span id="page-143-0"></span>**26.4.2 sys Schema Tables and Triggers**

The following sections describe [sys](#page-136-1) schema tables and triggers.

## <span id="page-144-0"></span>**26.4.2.1 The sys\_config Table**

This table contains [sys](#page-136-1) schema configuration options, one row per option. Configuration changes made by updating this table persist across client sessions and server restarts.

The [sys\\_config](#page-144-0) table has these columns:
...
The account that made the most recent modification to the row. The value is NULL if the row has not been changed since the [sys](#page-136-1) schema was installed.

As an efficiency measure to minimize the number of direct reads from the [sys\\_config](#page-144-0) table, [sys](#page-136-1) schema functions that use a value from this table check for a user-defined variable with a corresponding name, which is the user-defined variable having the same name plus a @sys. prefix. (For example, the variable corresponding to the diagnostics.include\_raw option is @sys.diagnostics.include\_raw.) If the user-defined variable exists in the current session and is non-NULL, the function uses its value in preference to the value in the [sys\\_config](#page-144-0) table. Otherwise, the function reads and uses the value from the table. In the latter case, the calling function conventionally also sets the corresponding user-defined variable to the table value so that further references to the configuration option within the same session use the variable and need not read the table again.
...
The conventional relationship just described between options in the [sys\\_config](#page-144-0) table and userdefined variables can be exploited to make temporary configuration changes that end when your session ends. However, if you set a user-defined variable and then subsequently change the corresponding table value within the same session, the changed table value is not used in that session as long as the user-defined variable exists and is not NULL. (The changed table value is used in other sessions that do not have the user-defined variable assigned.)

The following list describes the options in the [sys\\_config](#page-144-0) table and the corresponding user-defined variables:

• diagnostics.allow\_i\_s\_tables, @sys.diagnostics.allow\_i\_s\_tables

If this option is ON, the [diagnostics\(\)](#page-186-0) procedure is permitted to perform table scans on the Information Schema TABLES table. This can be expensive if there are many tables. The default is OFF.

• diagnostics.include\_raw, @sys.diagnostics.include\_raw

If this option is ON, the [diagnostics\(\)](#page-186-0) procedure includes the raw output from querying the [metrics](#page-160-2) view. The default is OFF.
...
The maximum number of rows to return for views that have no built-in limit. (For example, the [statements\\_with\\_runtimes\\_in\\_95th\\_percentile](#page-176-0) view has a built-in limit in the sense that it returns only statements with average execution time in the 95th percentile.) The default is 100.

• statement\_performance\_analyzer.view, @sys.statement\_performance\_analyzer.view

The custom query or view to be used by the [statement\\_performance\\_analyzer\(\)](#page-199-1) procedure (which is itself invoked by the [diagnostics\(\)](#page-186-0) procedure). If the option value contains a space, it is interpreted as a query. Otherwise, it must be the name of an existing view that queries the Performance Schema [events\\_statements\\_summary\\_by\\_digest](#page-78-0) table. There cannot be any LIMIT clause in the query or view definition if the statement\_performance\_analyzer.limit configuration option is greater than 0. The default is NULL (no custom view defined).
...
Other options can be added to the [sys\\_config](#page-144-0) table. For example, the [diagnostics\(\)](#page-186-0) and [execute\\_prepared\\_stmt\(\)](#page-188-0) procedures use the debug option if it exists, but this option is not part of the [sys\\_config](#page-144-0) table by default because debug output normally is enabled only temporarily, by setting the corresponding @sys.debug user-defined variable. To enable debug output without having to set that variable in individual sessions, add the option to the table:
...
## <span id="page-146-1"></span>**26.4.2.2 The sys\_config\_insert\_set\_user Trigger**

For rows added to the [sys\\_config](#page-144-0) table by INSERT statements, the [sys\\_config\\_insert\\_set\\_user](#page-146-1) trigger sets the set\_by column to the current user.

## <span id="page-146-2"></span>**26.4.2.3 The sys\_config\_update\_set\_user Trigger**

The [sys\\_config\\_update\\_set\\_user](#page-146-2) trigger for the [sys\\_config](#page-144-0) table is similar to the [sys\\_config\\_insert\\_set\\_user](#page-146-1) trigger, but for UPDATE statements.

# <span id="page-146-0"></span>**26.4.3 sys Schema Views**

The following sections describe [sys](#page-136-1) schema views.
...
## <span id="page-147-1"></span>**26.4.3.1 The host\_summary and x\$host\_summary Views**

These views summarize statement activity, file I/O, and connections, grouped by host.

The [host\\_summary](#page-147-1) and [x\\$host\\_summary](#page-147-1) views have these columns:
...
## <span id="page-147-0"></span>**26.4.3.2 The host\_summary\_by\_file\_io and x\$host\_summary\_by\_file\_io Views**

These views summarize file I/O, grouped by host. By default, rows are sorted by descending total file I/ O latency.

The [host\\_summary\\_by\\_file\\_io](#page-147-0) and [x\\$host\\_summary\\_by\\_file\\_io](#page-147-0) views have these columns:
...
## <span id="page-148-0"></span>**26.4.3.3 The host\_summary\_by\_file\_io\_type and x\$host\_summary\_by\_file\_io\_type Views**
...
The [host\\_summary\\_by\\_file\\_io\\_type](#page-148-0) and [x\\$host\\_summary\\_by\\_file\\_io\\_type](#page-148-0) views have these columns:
...
## <span id="page-148-1"></span>**26.4.3.4 The host\_summary\_by\_stages and x\$host\_summary\_by\_stages Views**

These views summarize statement stages, grouped by host. By default, rows are sorted by host and descending total latency.

The [host\\_summary\\_by\\_stages](#page-148-1) and [x\\$host\\_summary\\_by\\_stages](#page-148-1) views have these columns:
...
## <span id="page-149-0"></span>**26.4.3.5 The host\_summary\_by\_statement\_latency and x \$host\_summary\_by\_statement\_latency Views**
...
The [host\\_summary\\_by\\_statement\\_latency](#page-149-0) and [x\\$host\\_summary\\_by\\_statement\\_latency](#page-149-0) views have these columns:
...
## <span id="page-149-1"></span>**26.4.3.6 The host\_summary\_by\_statement\_type and x \$host\_summary\_by\_statement\_type Views**
...
The [host\\_summary\\_by\\_statement\\_type](#page-149-1) and [x\\$host\\_summary\\_by\\_statement\\_type](#page-149-1) views have these columns:
...
# <span id="page-150-0"></span>**26.4.3.7 The innodb\_buffer\_stats\_by\_schema and x\$innodb\_buffer\_stats\_by\_schema Views**
...
The [innodb\\_buffer\\_stats\\_by\\_schema](#page-150-0) and [x\\$innodb\\_buffer\\_stats\\_by\\_schema](#page-150-0) views have these columns:
...
## <span id="page-151-0"></span>**26.4.3.8 The innodb\_buffer\_stats\_by\_table and x\$innodb\_buffer\_stats\_by\_table Views**
...
The [innodb\\_buffer\\_stats\\_by\\_table](#page-151-0) and [x\\$innodb\\_buffer\\_stats\\_by\\_table](#page-151-0) views have these columns:
...
## <span id="page-152-0"></span>**26.4.3.9 The innodb\_lock\_waits and x\$innodb\_lock\_waits Views**
...
The [innodb\\_lock\\_waits](#page-152-0) and [x\\$innodb\\_lock\\_waits](#page-152-0) views have these columns:
...
## <span id="page-154-0"></span>**26.4.3.10 The io\_by\_thread\_by\_latency and x\$io\_by\_thread\_by\_latency Views**
...
The [io\\_by\\_thread\\_by\\_latency](#page-154-0) and [x\\$io\\_by\\_thread\\_by\\_latency](#page-154-0) views have these columns:
...
## <span id="page-154-1"></span>**26.4.3.11 The io\_global\_by\_file\_by\_bytes and x\$io\_global\_by\_file\_by\_bytes Views**
...
The [io\\_global\\_by\\_file\\_by\\_bytes](#page-154-1) and [x\\$io\\_global\\_by\\_file\\_by\\_bytes](#page-154-1) views have these columns:
...
## <span id="page-155-0"></span>**26.4.3.12 The io\_global\_by\_file\_by\_latency and x\$io\_global\_by\_file\_by\_latency Views**
...
The [io\\_global\\_by\\_file\\_by\\_latency](#page-155-0) and [x\\$io\\_global\\_by\\_file\\_by\\_latency](#page-155-0) views have these columns:
...
## <span id="page-156-0"></span>**26.4.3.13 The io\_global\_by\_wait\_by\_bytes and x\$io\_global\_by\_wait\_by\_bytes Views**
...
The [io\\_global\\_by\\_wait\\_by\\_bytes](#page-156-0) and [x\\$io\\_global\\_by\\_wait\\_by\\_bytes](#page-156-0) views have these columns:
...
## <span id="page-156-1"></span>**26.4.3.14 The io\_global\_by\_wait\_by\_latency and x\$io\_global\_by\_wait\_by\_latency Views**
...
The [io\\_global\\_by\\_wait\\_by\\_latency](#page-156-1) and [x\\$io\\_global\\_by\\_wait\\_by\\_latency](#page-156-1) views have these columns:
...
## <span id="page-157-0"></span>**26.4.3.15 The latest\_file\_io and x\$latest\_file\_io Views**

These views summarize file I/O activity, grouped by file and thread. By default, rows are sorted with most recent I/O first.

The [latest\\_file\\_io](#page-157-0) and [x\\$latest\\_file\\_io](#page-157-0) views have these columns:
...
## <span id="page-158-0"></span>**26.4.3.16 The memory\_by\_host\_by\_current\_bytes and x \$memory\_by\_host\_by\_current\_bytes Views**

These views summarize memory use, grouped by host. By default, rows are sorted by descending amount of memory used.

The [memory\\_by\\_host\\_by\\_current\\_bytes](#page-158-0) and [x\\$memory\\_by\\_host\\_by\\_current\\_bytes](#page-158-0) views have these columns:
...
# <span id="page-158-1"></span>**26.4.3.17 The memory\_by\_thread\_by\_current\_bytes and x \$memory\_by\_thread\_by\_current\_bytes Views**

These views summarize memory use, grouped by thread. By default, rows are sorted by descending amount of memory used.

The [memory\\_by\\_thread\\_by\\_current\\_bytes](#page-158-1) and [x\\$memory\\_by\\_thread\\_by\\_current\\_bytes](#page-158-1) views have these columns:
...
## <span id="page-159-0"></span>**26.4.3.18 The memory\_by\_user\_by\_current\_bytes and x \$memory\_by\_user\_by\_current\_bytes Views**

These views summarize memory use, grouped by user. By default, rows are sorted by descending amount of memory used.

The [memory\\_by\\_user\\_by\\_current\\_bytes](#page-159-0) and [x\\$memory\\_by\\_user\\_by\\_current\\_bytes](#page-159-0) views have these columns:
...
## <span id="page-160-0"></span>**26.4.3.19 The memory\_global\_by\_current\_bytes and x \$memory\_global\_by\_current\_bytes Views**
...
The [memory\\_global\\_by\\_current\\_bytes](#page-160-0) and [x\\$memory\\_global\\_by\\_current\\_bytes](#page-160-0) views have these columns:
...
## <span id="page-160-1"></span>**26.4.3.20 The memory\_global\_total and x\$memory\_global\_total Views**

These views summarize total memory use within the server.

The [memory\\_global\\_total](#page-160-1) and [x\\$memory\\_global\\_total](#page-160-1) views have these columns:

• total\_allocated

The total bytes of memory allocated within the server.

## <span id="page-160-2"></span>**26.4.3.21 The metrics View**
...
The [metrics](#page-160-2) view includes this information:

- Global status variables from the Performance Schema [global\\_status](#page-72-0) table
...
There is some duplication of information between the [global\\_status](#page-72-0) and INNODB\_METRICS tables, which the [metrics](#page-160-2) view eliminates.

The [metrics](#page-160-2) view has these columns:

• Variable\_name

The metric name. The metric type determines the source from which the name is taken:

- For global status variables: The VARIABLE\_NAME column of the [global\\_status](#page-72-0) table
...
- For global status variables: The VARIABLE\_VALUE column of the [global\\_status](#page-72-0) table
- For InnoDB metrics: The COUNT column of the INNODB\_METRICS table
- For memory metrics: The relevant column from the Performance Schema [memory\\_summary\\_global\\_by\\_event\\_name](#page-88-0) table
...
## <span id="page-161-0"></span>**26.4.3.22 The processlist and x\$processlist Views**

The MySQL process list indicates the operations currently being performed by the set of threads executing within the server. The [processlist](#page-161-0) and [x\\$processlist](#page-161-0) views summarize process information. They provide more complete information than the SHOW PROCESSLIST statement and the INFORMATION\_SCHEMA PROCESSLIST table, and are also nonblocking. By default, rows are sorted by descending process time and descending wait time. For a comparison of process information sources, see Sources of Process Information.

The column descriptions here are brief. For additional information, see the description of the Performance Schema [threads](#page-100-0) table at [Section 25.12.16.4, "The threads Table".](#page-100-0)

The [processlist](#page-161-0) and [x\\$processlist](#page-161-0) views have these columns:
...
The percentage of work completed for stages that support progress reporting. See [Section 26.3, "sys](#page-138-0) [Schema Progress Reporting".](#page-138-0)
...
## <span id="page-163-0"></span>**26.4.3.23 The ps\_check\_lost\_instrumentation View**
...
The [ps\\_check\\_lost\\_instrumentation](#page-163-0) view has these columns:
...
## <span id="page-164-0"></span>**26.4.3.24 The schema\_auto\_increment\_columns View**
...
The [schema\\_auto\\_increment\\_columns](#page-164-0) view has these columns:
...
## <span id="page-164-1"></span>**26.4.3.25 The schema\_index\_statistics and x\$schema\_index\_statistics Views**

These views provide index statistics. By default, rows are sorted by descending total index latency.

The [schema\\_index\\_statistics](#page-164-1) and [x\\$schema\\_index\\_statistics](#page-164-1) views have these columns:
...
## <span id="page-166-0"></span>**26.4.3.27 The schema\_redundant\_indexes and x\$schema\_flattened\_keys Views**

The [schema\\_redundant\\_indexes](#page-166-0) view displays indexes that duplicate other indexes or are made redundant by them. The [x\\$schema\\_flattened\\_keys](#page-166-0) view is a helper view for [schema\\_redundant\\_indexes](#page-166-0).

In the following column descriptions, the dominant index is the one that makes the redundant index redundant.

The [schema\\_redundant\\_indexes](#page-166-0) view has these columns:
...
The [x\\$schema\\_flattened\\_keys](#page-166-0) view has these columns:
...
## <span id="page-167-0"></span>**26.4.3.28 The schema\_table\_lock\_waits and x\$schema\_table\_lock\_waits Views**

These views display which sessions are blocked waiting on metadata locks, and what is blocking them.

The column descriptions here are brief. For additional information, see the description of the Performance Schema [metadata\\_locks](#page-67-0) table at [Section 25.12.12.1, "The metadata\\_locks Table"](#page-67-0).

The [schema\\_table\\_lock\\_waits](#page-167-0) and [x\\$schema\\_table\\_lock\\_waits](#page-167-0) views have these columns:
...
## <span id="page-168-0"></span>**26.4.3.29 The schema\_table\_statistics and x\$schema\_table\_statistics Views**
...
The [schema\\_table\\_statistics](#page-168-0) and [x\\$schema\\_table\\_statistics](#page-168-0) views have these columns:
...
## <span id="page-169-0"></span>**26.4.3.30 The schema\_table\_statistics\_with\_buffer and x \$schema\_table\_statistics\_with\_buffer Views**
...
The [schema\\_table\\_statistics\\_with\\_buffer](#page-169-0) and [x](#page-169-0) [\\$schema\\_table\\_statistics\\_with\\_buffer](#page-169-0) views have these columns:
...
# <span id="page-171-0"></span>**26.4.3.31 The schema\_tables\_with\_full\_table\_scans and x \$schema\_tables\_with\_full\_table\_scans Views**
...
The [schema\\_tables\\_with\\_full\\_table\\_scans](#page-171-0) and [x](#page-171-0) [\\$schema\\_tables\\_with\\_full\\_table\\_scans](#page-171-0) views have these columns:
...
## <span id="page-172-1"></span>**26.4.3.32 The schema\_unused\_indexes View**
...
The [schema\\_unused\\_indexes](#page-172-1) view has these columns:
...
## <span id="page-172-0"></span>**26.4.3.33 The session and x\$session Views**

These views are similar to [processlist](#page-161-0) and [x\\$processlist](#page-161-0), but they filter out background processes to display only user sessions. For descriptions of the columns, see [Section 26.4.3.22, "The](#page-161-0) [processlist and x\\$processlist Views"](#page-161-0).

## <span id="page-172-2"></span>**26.4.3.34 The session\_ssl\_status View**

For each connection, this view displays the SSL version, cipher, and count of reused SSL sessions.

The [session\\_ssl\\_status](#page-172-2) view has these columns:
...
## <span id="page-172-3"></span>**26.4.3.35 The statement\_analysis and x\$statement\_analysis Views**
...
The [statement\\_analysis](#page-172-3) and [x\\$statement\\_analysis](#page-172-3) views have these columns:
...
## <span id="page-174-0"></span>**26.4.3.36 The statements\_with\_errors\_or\_warnings and x \$statements\_with\_errors\_or\_warnings Views**
...
The [statements\\_with\\_errors\\_or\\_warnings](#page-174-0) and [x](#page-174-0) [\\$statements\\_with\\_errors\\_or\\_warnings](#page-174-0) views have these columns:
...
## <span id="page-175-0"></span>**26.4.3.37 The statements\_with\_full\_table\_scans and x \$statements\_with\_full\_table\_scans Views**
...
The [statements\\_with\\_full\\_table\\_scans](#page-175-0) and [x\\$statements\\_with\\_full\\_table\\_scans](#page-175-0) views have these columns:
...
## <span id="page-176-0"></span>**26.4.3.38 The statements\_with\_runtimes\_in\_95th\_percentile and x \$statements\_with\_runtimes\_in\_95th\_percentile Views**
...
The [statements\\_with\\_runtimes\\_in\\_95th\\_percentile](#page-176-0) and [x](#page-176-0) [\\$statements\\_with\\_runtimes\\_in\\_95th\\_percentile](#page-176-0) views have these columns:
...
## <span id="page-177-0"></span>**26.4.3.39 The statements\_with\_sorting and x\$statements\_with\_sorting Views**

These views list normalized statements that have performed sorts. By default, rows are sorted by descending total latency.

The [statements\\_with\\_sorting](#page-177-0) and [x\\$statements\\_with\\_sorting](#page-177-0) views have these columns:
...
## <span id="page-178-0"></span>**26.4.3.40 The statements\_with\_temp\_tables and x\$statements\_with\_temp\_tables Views**
...
The [statements\\_with\\_temp\\_tables](#page-178-0) and [x\\$statements\\_with\\_temp\\_tables](#page-178-0) views have these columns:
...
## <span id="page-179-0"></span>**26.4.3.41 The user\_summary and x\$user\_summary Views**
...
The [user\\_summary](#page-179-0) and [x\\$user\\_summary](#page-179-0) views have these columns:
...
## <span id="page-179-1"></span>**26.4.3.42 The user\_summary\_by\_file\_io and x\$user\_summary\_by\_file\_io Views**

These views summarize file I/O, grouped by user. By default, rows are sorted by descending total file I/ O latency.

The [user\\_summary\\_by\\_file\\_io](#page-179-1) and [x\\$user\\_summary\\_by\\_file\\_io](#page-179-1) views have these columns:
...
## <span id="page-180-0"></span>**26.4.3.43 The user\_summary\_by\_file\_io\_type and x\$user\_summary\_by\_file\_io\_type Views**
...
The [user\\_summary\\_by\\_file\\_io\\_type](#page-180-0) and [x\\$user\\_summary\\_by\\_file\\_io\\_type](#page-180-0) views have these columns:
...
## <span id="page-180-1"></span>**26.4.3.44 The user\_summary\_by\_stages and x\$user\_summary\_by\_stages Views**

These views summarize stages, grouped by user. By default, rows are sorted by user and descending total stage latency.

The [user\\_summary\\_by\\_stages](#page-180-1) and [x\\$user\\_summary\\_by\\_stages](#page-180-1) views have these columns:
...
## <span id="page-181-0"></span>**26.4.3.45 The user\_summary\_by\_statement\_latency and x \$user\_summary\_by\_statement\_latency Views**
...
The [user\\_summary\\_by\\_statement\\_latency](#page-181-0) and [x\\$user\\_summary\\_by\\_statement\\_latency](#page-181-0) views have these columns:
...
## <span id="page-181-1"></span>**26.4.3.46 The user\_summary\_by\_statement\_type and x \$user\_summary\_by\_statement\_type Views**
...
The [user\\_summary\\_by\\_statement\\_type](#page-181-1) and [x\\$user\\_summary\\_by\\_statement\\_type](#page-181-1) views have these columns:
...
## <span id="page-182-0"></span>**26.4.3.47 The version View**

This view provides the current [sys](#page-136-1) schema and MySQL server versions.

![](_page_182_Picture_24.jpeg)
...
The [version](#page-182-0) view has these columns:

• sys\_version

The [sys](#page-136-1) schema version.

• mysql\_version

The MySQL server version.

## <span id="page-183-0"></span>**26.4.3.48 The wait\_classes\_global\_by\_avg\_latency and x \$wait\_classes\_global\_by\_avg\_latency Views**
...
The [wait\\_classes\\_global\\_by\\_avg\\_latency](#page-183-0) and [x](#page-183-0) [\\$wait\\_classes\\_global\\_by\\_avg\\_latency](#page-183-0) views have these columns:
...
## <span id="page-183-1"></span>**26.4.3.49 The wait\_classes\_global\_by\_latency and x\$wait\_classes\_global\_by\_latency Views**
...
The [wait\\_classes\\_global\\_by\\_latency](#page-183-1) and [x\\$wait\\_classes\\_global\\_by\\_latency](#page-183-1) views have these columns:
...
## <span id="page-184-0"></span>**26.4.3.50 The waits\_by\_host\_by\_latency and x\$waits\_by\_host\_by\_latency Views**
...
The [waits\\_by\\_host\\_by\\_latency](#page-184-0) and [x\\$waits\\_by\\_host\\_by\\_latency](#page-184-0) views have these columns:
...
## <span id="page-184-1"></span>**26.4.3.51 The waits\_by\_user\_by\_latency and x\$waits\_by\_user\_by\_latency Views**
...
The [waits\\_by\\_user\\_by\\_latency](#page-184-1) and [x\\$waits\\_by\\_user\\_by\\_latency](#page-184-1) views have these columns:
...
## <span id="page-185-1"></span>**26.4.3.52 The waits\_global\_by\_latency and x\$waits\_global\_by\_latency Views**
...
The [waits\\_global\\_by\\_latency](#page-185-1) and [x\\$waits\\_global\\_by\\_latency](#page-185-1) views have these columns:
...
# <span id="page-185-0"></span>**26.4.4 sys Schema Stored Procedures**

The following sections describe [sys](#page-136-1) schema stored procedures.

## <span id="page-185-2"></span>**26.4.4.1 The create\_synonym\_db() Procedure**
...
## <span id="page-186-0"></span>**26.4.4.2 The diagnostics() Procedure**

Creates a report of the current server status for diagnostic purposes.

This procedure disables binary logging during its execution by manipulating the session value of the sql\_log\_bin system variable. That is a restricted operation, so the procedure requires privileges sufficient to set restricted session variables. See Section 5.1.8.1, "System Variable Privileges".

Data collected for [diagnostics\(\)](#page-186-0) includes this information:

- Information from the [metrics](#page-160-2) view (see [Section 26.4.3.21, "The metrics View"\)](#page-160-2)
...
- The initial view is the content of the view at the start of the [diagnostics\(\)](#page-186-0) procedure. This output is the same as the start values used for the delta view. The initial view is included if the diagnostics.include\_raw configuration option is ON.
- The overall view is the content of the view at the end of the [diagnostics\(\)](#page-186-0) procedure. This output is the same as the end values used for the delta view. The overall view is always included.
- The delta view is the difference from the beginning to the end of procedure execution. The minimum and maximum values are the minimum and maximum values from the end view, respectively. They do not necessarily reflect the minimum and maximum values in the monitored period. Except for the [metrics](#page-160-2) view, the delta is calculated only between the first and last outputs.
...
[diagnostics\(\)](#page-186-0) operation can be modified using the following configuration options or their corresponding user-defined variables (see [Section 26.4.2.1, "The sys\\_config Table"](#page-144-0)):
...
If this option is ON, the [diagnostics\(\)](#page-186-0) procedure output includes the raw output from querying the [metrics](#page-160-2) view. The default is OFF.
...
## <span id="page-188-0"></span>**26.4.4.3 The execute\_prepared\_stmt() Procedure**
...
• in\_query LONGTEXT CHARACTER SET utf8: The statement string to execute.

### **Configuration Options**

[execute\\_prepared\\_stmt\(\)](#page-188-0) operation can be modified using the following configuration options or their corresponding user-defined variables (see [Section 26.4.2.1, "The sys\\_config Table"](#page-144-0)):
...
## <span id="page-188-1"></span>**26.4.4.4 The ps\_setup\_disable\_background\_threads() Procedure**
...
## <span id="page-188-2"></span>**26.4.4.5 The ps\_setup\_disable\_consumer() Procedure**
...
# <span id="page-189-0"></span>**26.4.4.6 The ps\_setup\_disable\_instrument() Procedure**
...
## <span id="page-189-1"></span>**26.4.4.7 The ps\_setup\_disable\_thread() Procedure**
...
• in\_connection\_id BIGINT: The connection ID. This is a value of the type given in the PROCESSLIST\_ID column of the Performance Schema [threads](#page-100-0) table or the Id column of SHOW PROCESSLIST output.
...
## <span id="page-190-0"></span>**26.4.4.8 The ps\_setup\_enable\_background\_threads() Procedure**
...
## <span id="page-190-1"></span>**26.4.4.9 The ps\_setup\_enable\_consumer() Procedure**
...
## <span id="page-190-2"></span>**26.4.4.10 The ps\_setup\_enable\_instrument() Procedure**
...
## <span id="page-191-0"></span>**26.4.4.11 The ps\_setup\_enable\_thread() Procedure**
...
## <span id="page-191-1"></span>**26.4.4.12 The ps\_setup\_reload\_saved() Procedure**

Reloads a Performance Schema configuration saved earlier within the same session using [ps\\_setup\\_save\(\)](#page-192-0). For more information, see the description of [ps\\_setup\\_save\(\)](#page-192-0).
...
## <span id="page-192-1"></span>**26.4.4.13 The ps\_setup\_reset\_to\_default() Procedure**
...
# <span id="page-192-0"></span>**26.4.4.14 The ps\_setup\_save() Procedure**

Saves the current Performance Schema configuration. This enables you to alter the configuration temporarily for debugging or other purposes, then restore it to the previous state by invoking the [ps\\_setup\\_reload\\_saved\(\)](#page-191-1) procedure.

To prevent other simultaneous calls to save the configuration, [ps\\_setup\\_save\(\)](#page-192-0) acquires an advisory lock named sys.ps\_setup\_save by calling the GET\_LOCK() function. [ps\\_setup\\_save\(\)](#page-192-0) takes a timeout parameter to indicate how many seconds to wait if the lock already exists (which indicates that some other session has a saved configuration outstanding). If the timeout expires without obtaining the lock, [ps\\_setup\\_save\(\)](#page-192-0) fails.

It is intended you call [ps\\_setup\\_reload\\_saved\(\)](#page-191-1) later within the same session as [ps\\_setup\\_save\(\)](#page-192-0) because the configuration is saved in TEMPORARY tables. [ps\\_setup\\_save\(\)](#page-192-0) drops the temporary tables and releases the lock. If you end your session without invoking [ps\\_setup\\_save\(\)](#page-192-0), the tables and lock disappear automatically.
...
## <span id="page-193-0"></span>**26.4.4.15 The ps\_setup\_show\_disabled() Procedure**
...
## <span id="page-193-1"></span>**26.4.4.16 The ps\_setup\_show\_disabled\_consumers() Procedure**
...
## <span id="page-194-0"></span>**26.4.4.17 The ps\_setup\_show\_disabled\_instruments() Procedure**
...
## <span id="page-194-1"></span>**26.4.4.18 The ps\_setup\_show\_enabled() Procedure**
...
## <span id="page-195-0"></span>**26.4.4.19 The ps\_setup\_show\_enabled\_consumers() Procedure**
...
# <span id="page-195-1"></span>**26.4.4.20 The ps\_setup\_show\_enabled\_instruments() Procedure**
...
## <span id="page-195-2"></span>**26.4.4.21 The ps\_statement\_avg\_latency\_histogram() Procedure**

Displays a textual histogram graph of the average latency values across all normalized statements tracked within the Performance Schema [events\\_statements\\_summary\\_by\\_digest](#page-78-0) table.
...
## <span id="page-196-0"></span>**26.4.4.22 The ps\_trace\_statement\_digest() Procedure**

Traces all Performance Schema instrumentation for a specific statement digest.

If you find a statement of interest within the Performance Schema [events\\_statements\\_summary\\_by\\_digest](#page-78-0) table, specify its DIGEST column MD5 value to this procedure and indicate the polling duration and interval. The result is a report of all statistics tracked within Performance Schema for that digest for the interval.
...
- in\_start\_fresh BOOLEAN: Whether to truncate the Performance Schema [events\\_statements\\_history\\_long](#page-39-1) and [events\\_stages\\_history\\_long](#page-31-1) tables before starting.
- in\_auto\_enable BOOLEAN: Whether to automatically enable required consumers.

### **Example**

mysql> **CALL sys.ps\_trace\_statement\_digest('891ec6860f98ba46d89dd20b0c03652c', 10, 0.1, TRUE, TRUE);**
...
## <span id="page-197-0"></span>**26.4.4.23 The ps\_trace\_thread() Procedure**
...
## <span id="page-199-0"></span>**26.4.4.24 The ps\_truncate\_all\_tables() Procedure**
...
## <span id="page-199-1"></span>**26.4.4.25 The statement\_performance\_analyzer() Procedure**
...
- in\_action ENUM('snapshot', 'overall', 'delta', 'create\_tmp', 'create\_table', 'save', 'cleanup'): The action to take. These values are permitted:
  - snapshot: Store a snapshot. The default is to make a snapshot of the current content of the Performance Schema [events\\_statements\\_summary\\_by\\_digest](#page-78-0) table. By setting in\_table, this can be overwritten to copy the content of the specified table. The snapshot is stored in the [sys](#page-136-1) schema tmp\_digests temporary table.
  ...
  - delta: Generate a delta analysis. The delta is calculated between the reference table specified by in\_table and the snapshot, which must exist. This action uses the [sys](#page-136-1) schema tmp\_digests\_delta temporary table. The in\_views parameter and the statement\_performance\_analyzer.limit configuration option affect the operation of this procedure.
...
statement\_performance\_analyzer() operation can be modified using the following configuration options or their corresponding user-defined variables (see Section 26.4.2.1, "The sys\_config Table"):
...
- in\_db VARCHAR(64): The name of the database in which to check for table existance.
