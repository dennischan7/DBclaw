...
- Certain MySQL features help or do not help performance depending on the workload. For completeness, always test performance with those features turned on and turned off. The two most important features to try with each workload are the [MySQL query cache,](#page-106-0) and the adaptive hash index for InnoDB tables.
