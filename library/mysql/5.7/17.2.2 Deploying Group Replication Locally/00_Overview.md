...
Do not use -initialize-insecure in production environments, it is only used here to simplify the tutorial. For more information on security settings, see [Section 17.6, "Group Replication Security".](#page-26-0)
...
Then each member needs to be able to connect to the other members on their [group\\_replication\\_local\\_address](#page-42-0). For example in the option file of member s1 add:
