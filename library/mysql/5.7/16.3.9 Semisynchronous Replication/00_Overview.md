...
If your goal is to implement a fault-tolerant replication topology where all the servers receive the same transactions in the same order, and a server that crashes can rejoin the group and be brought up to date automatically, you can use Group Replication to achieve this. For information, see [Chapter 17,](#page-186-0) Group [Replication](#page-186-0).
...
The [rpl\\_semi\\_sync\\_master\\_wait\\_point](#page-43-0) system variable controls the point at which a semisynchronous replication source waits for replica acknowledgment of transaction receipt before returning a status to the client that committed the transaction. These values are permitted:
