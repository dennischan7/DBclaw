...
- Alternatively, for transactions that consist only of a single SELECT statement, turning on AUTOCOMMIT helps InnoDB to recognize read-only transactions and optimize them. See [Section 8.5.3, "Optimizing InnoDB Read-Only Transactions"](#page-56-0) for requirements.
...
To get rid of a runaway rollback once it occurs, increase the buffer pool so that the rollback becomes CPU-bound and runs fast, or kill the server and restart with innodb\_force\_recovery=3, as explained in Section 14.19.2, "InnoDB Recovery".
...
• If you can afford the loss of some of the latest committed transactions if an unexpected exit occurs, you can set the innodb\_flush\_log\_at\_trx\_commit parameter to 0. InnoDB tries to flush the log once per second anyway, although the flush is not guaranteed. Also, set the value of innodb\_support\_xa to 0, which reduces the number of disk flushes due to synchronizing on disk data and the binary log.

![](_page_55_Picture_15.jpeg)

#### **Note**

innodb\_support\_xa is deprecated; expect it to be removed in a future release. As of MySQL 5.7.10, InnoDB support for two-phase commit in XA transactions is always enabled and disabling innodb\_support\_xa is no longer permitted.
...
## <span id="page-56-0"></span>**8.5.3 Optimizing InnoDB Read-Only Transactions**
