...
title: 16.4.1.38 Replication and Views
---
...
# <span id="page-180-0"></span>**16.4.2 Replication Compatibility Between MySQL Versions**
...
This also has significant implications for upgrading replication servers; see [Section 16.4.3,](#page-181-0) ["Upgrading a Replication Topology"](#page-181-0), for more information.

- For more information about row-based replication, see [Section 16.2.1, "Replication Formats".](#page-110-1)
...
For more information on potential replication issues, see [Section 16.4.1, "Replication Features and](#page-156-1) [Issues"](#page-156-1).

# <span id="page-181-0"></span>**16.4.3 Upgrading a Replication Topology**
...
- 1. Upgrade the replicas first. On each replica instance:
  - Carry out the preliminary checks and steps described in [Preparing Your Installation for Upgrade](https://dev.mysql.com/doc/refman/8.0/en/upgrade-prerequisites.md).
  - Shut down MySQL Server.
  - Upgrade the MySQL Server binaries or packages.
  - Restart MySQL Server.
  - If you have upgraded to a release earlier than MySQL 8.0.16, invoke mysql\_upgrade manually to upgrade the system tables and schemas. When the server is running with global transaction identifiers (GTIDs) enabled ([gtid\\_mode=ON](#page-101-0)), do not enable binary logging by mysql\_upgrade (so do not use the --write-binlog option). Then shut down and restart the server.
...
- Restart replication using a [START REPLICA](https://dev.mysql.com/doc/refman/8.0/en/start-replica.md) or START SLAVE statement.
- 2. When all the replicas have been upgraded, follow the same steps to upgrade and restart the source server, with the exception of the [START REPLICA](https://dev.mysql.com/doc/refman/8.0/en/start-replica.md) or START SLAVE statement. If you made a temporary change to row-based logging or to the default character set, you can revert the change now.
...
- 1. Stop all the replicas and upgrade the binaries or packages. Restart them with the [--skip-slave](#page-53-0)[start](#page-53-0) option, or from MySQL 8.0.24, the [skip\\_slave\\_start](https://dev.mysql.com/doc/refman/8.0/en/replication-options-replica.md#sysvar_skip_slave_start) system variable, so that they do not connect to the source. Perform any table repair or rebuilding operations needed to re-create database objects, such as use of REPAIR TABLE or ALTER TABLE, or dumping and reloading tables or triggers.
- 2. Disable the binary log on the source. To do this without restarting the source, execute a SET sql\_log\_bin = OFF statement. Alternatively, stop the source and restart it with the [--skip](#page-77-0)[log-bin](#page-77-0) option. If you restart the source, you might also want to disallow client connections. For example, if all clients connect using TCP/IP, enable the skip\_networking system variable when you restart the source.
...
- 4. Re-enable the binary log on the source. If you set [sql\\_log\\_bin](#page-95-0) to OFF earlier, execute a SET sql\_log\_bin = ON statement. If you restarted the source to disable the binary log, restart it without [--skip-log-bin](#page-77-0), and without enabling the skip\_networking system variable so that clients and replicas can connect.
- 5. Restart the replicas, this time without the [--skip-slave-start](#page-53-0) option or [skip\\_slave\\_start](https://dev.mysql.com/doc/refman/8.0/en/replication-options-replica.md#sysvar_skip_slave_start) system variable.