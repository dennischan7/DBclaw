...
title: 12.17.3 Functions That Search JSON Values
---
...
<span id="page-170-0"></span>• [JSON\\_CONTAINS\(](#page-170-0)target, candidate[, path])
...
To check only whether any data exists at the path, use [JSON\\_CONTAINS\\_PATH\(\)](#page-171-0) instead.

The following rules define containment:

- A candidate scalar is contained in a target scalar if and only if they are comparable and are equal. Two scalar values are comparable if they have the same [JSON\\_TYPE\(\)](#page-189-0) types, with the exception that values of types INTEGER and DECIMAL are also comparable to each other.
...
<span id="page-171-0"></span>• [JSON\\_CONTAINS\\_PATH\(](#page-171-0)json\_doc, one\_or\_all, path[, path] ...)
...
To check for a specific value at a path, use [JSON\\_CONTAINS\(\)](#page-170-0) instead.
...
<span id="page-171-1"></span>• [JSON\\_EXTRACT\(](#page-171-1)json\_doc, path[, path] ...)
...
MySQL 5.7.9 and later supports the [->](#page-172-0) operator as shorthand for this function as used with 2 arguments where the left hand side is a JSON column identifier (not an expression) and the right hand side is the JSON path to be matched within the column.

### <span id="page-172-0"></span>• [column](#page-172-0)->path

In MySQL 5.7.9 and later, the [->](#page-172-0) operator serves as an alias for the [JSON\\_EXTRACT\(\)](#page-171-1) function when used with two arguments, a column identifier on the left and a JSON path (a string literal) on the right that is evaluated against the JSON document (the column value). You can use such expressions in place of column references wherever they occur in SQL statements.
...
<span id="page-174-0"></span>• [column](#page-174-0)->>path
...
- [JSON\\_UNQUOTE\(](#page-186-1) [JSON\\_EXTRACT\(](#page-171-1)column, path) )
- JSON\_UNQUOTE(column [->](#page-172-0) path)
- column->>path
...
As with [->](#page-172-0), the ->> operator is always expanded in the output of EXPLAIN, as the following example demonstrates:
...
This is similar to how MySQL expands the [->](#page-172-0) operator in the same circumstances.

The ->> operator was added in MySQL 5.7.13.

<span id="page-176-0"></span>• [JSON\\_KEYS\(](#page-176-0)json\_doc[, path])
...
<span id="page-176-1"></span>• [JSON\\_SEARCH\(](#page-176-1)json\_doc, one\_or\_all, search\_str[, escape\_char[, path] ...])
...
Within the search\_str search string argument, the % and \_ characters work as for the [LIKE](#page-52-0) operator: % matches any number of characters (including zero characters), and \_ matches exactly one character.
...
For more information about matching and escape character behavior, see the description of [LIKE](#page-52-0) in [Section 12.8.1, "String Comparison Functions and Operators"](#page-51-0). For escape character handling, a difference from the [LIKE](#page-52-0) behavior is that the escape character for [JSON\\_SEARCH\(\)](#page-176-1) must evaluate to a constant at compile time, not just at execution time. For example, if [JSON\\_SEARCH\(\)](#page-176-1) is used

in a prepared statement and the escape\_char argument is supplied using a ? parameter, the parameter value might be constant at execution time, but is not at compile time.

search\_str and path are always interpeted as utf8mb4 strings, regardless of their actual encoding. This is a known issue which is fixed in MySQL 8.0 ( Bug #32449181).
...
# <span id="page-178-0"></span>**12.17.4 Functions That Modify JSON Values**
...
Appends values to the end of the indicated arrays within a JSON document and returns the result. This function was renamed to [JSON\\_ARRAY\\_APPEND\(\)](#page-179-0) in MySQL 5.7.9; the alias JSON\_APPEND() is now deprecated in MySQL 5.7, and is removed in MySQL 8.0.
...
<span id="page-179-1"></span>• [JSON\\_ARRAY\\_INSERT\(](#page-179-1)json\_doc, path, val[, path, val] ...)
...
Earlier modifications affect the positions of the following elements in the array, so subsequent paths in the same [JSON\\_ARRAY\\_INSERT\(\)](#page-179-1) call should take this into account. In the final example, the second path inserts nothing because the path no longer matches anything after the first insert.

<span id="page-181-0"></span>• [JSON\\_INSERT\(](#page-181-0)json\_doc, path, val[, path, val] ...)
...
For a comparison of [JSON\\_INSERT\(\)](#page-181-0), [JSON\\_REPLACE\(\)](#page-185-1), and [JSON\\_SET\(\)](#page-186-0), see the discussion of [JSON\\_SET\(\)](#page-186-0).
...
<span id="page-181-1"></span>• [JSON\\_MERGE\(](#page-181-1)json\_doc, json\_doc[, json\_doc] ...)

Merges two or more JSON documents. Synonym for JSON\_MERGE\_PRESERVE(); deprecated in MySQL 5.7.22 and subject to removal in a future release.
...
For additional examples, see the entry for [JSON\\_MERGE\\_PRESERVE\(\)](#page-184-0).

<span id="page-182-0"></span>• [JSON\\_MERGE\\_PATCH\(](#page-182-0)json\_doc, json\_doc[, json\_doc] ...)
...
For an explanation and example of the differences between this function and JSON\_MERGE\_PRESERVE(), see [JSON\\_MERGE\\_PATCH\(\) compared with](#page-183-0) [JSON\\_MERGE\\_PRESERVE\(\).](#page-183-0)
...
JSON\_MERGE\_PATCH() is supported in MySQL 5.7.22 and later.

<span id="page-183-0"></span>**JSON\_MERGE\_PATCH() compared with JSON\_MERGE\_PRESERVE().** The behavior of JSON\_MERGE\_PATCH() is the same as that of [JSON\\_MERGE\\_PRESERVE\(\)](#page-184-0), with the following two exceptions:
...
<span id="page-184-0"></span>• [JSON\\_MERGE\\_PRESERVE\(](#page-184-0)json\_doc, json\_doc[, json\_doc] ...)
...
This function was added in MySQL 5.7.22 as a synonym for [JSON\\_MERGE\(\)](#page-181-1). The JSON\_MERGE() function is now deprecated, and is subject to removal in a future release of MySQL.

This function is similar to but differs from [JSON\\_MERGE\\_PATCH\(\)](#page-182-0) in significant respects; see [JSON\\_MERGE\\_PATCH\(\) compared with JSON\\_MERGE\\_PRESERVE\(\)](#page-183-0), for more information. <span id="page-185-0"></span>• [JSON\\_REMOVE\(](#page-185-0)json\_doc, path[, path] ...)
...
<span id="page-185-1"></span>• [JSON\\_REPLACE\(](#page-185-1)json\_doc, path, val[, path, val] ...)
...
<span id="page-186-0"></span>• [JSON\\_SET\(](#page-186-0)json\_doc, path, val[, path, val] ...)
...
The [JSON\\_SET\(\)](#page-186-0), [JSON\\_INSERT\(\)](#page-181-0), and [JSON\\_REPLACE\(\)](#page-185-1) functions are related:

- [JSON\\_SET\(\)](#page-186-0) replaces existing values and adds nonexisting values.
- [JSON\\_INSERT\(\)](#page-181-0) inserts values without replacing existing values.
- [JSON\\_REPLACE\(\)](#page-185-1) replaces only existing values.
...
<span id="page-186-1"></span>• [JSON\\_UNQUOTE\(](#page-186-1)json\_val)
...
Within a string, certain sequences have special meaning unless the NO\_BACKSLASH\_ESCAPES SQL mode is enabled. Each of these sequences begins with a backslash (\), known as the escape character. MySQL recognizes the escape sequences shown in [Table 12.23, "JSON\\_UNQUOTE\(\)](#page-187-0) [Special Character Escape Sequences".](#page-187-0) For all other escape sequences, backslash is ignored. That is, the escaped character is interpreted as if it was not escaped. For example, \x is just x. These

sequences are case-sensitive. For example, \b is interpreted as a backspace, but \B is interpreted as B.
...
## <span id="page-188-0"></span>**12.17.5 Functions That Return JSON Value Attributes**

The functions in this section return attributes of JSON values.

• [JSON\\_DEPTH\(](#page-188-0)json\_doc)
...
<span id="page-188-1"></span>• [JSON\\_LENGTH\(](#page-188-1)json\_doc[, path])
...
<span id="page-189-0"></span>• [JSON\\_TYPE\(](#page-189-0)json\_val)
...
[JSON\\_TYPE\(\)](#page-189-0) returns NULL if the argument is NULL:
...
For a non-NULL, non-error result, the following list describes the possible [JSON\\_TYPE\(\)](#page-189-0) return values:
...
- All other types:
  - OPAQUE (raw bits)
- <span id="page-190-1"></span>• [JSON\\_VALID\(](#page-190-1)val)
...
# <span id="page-190-2"></span>**12.17.6 JSON Utility Functions**

This section documents utility functions that act on JSON values, or strings that can be parsed as JSON values. [JSON\\_PRETTY\(\)](#page-190-0) prints out a JSON value in a format that is easy to read. [JSON\\_STORAGE\\_SIZE\(\)](#page-192-0) shows the amount of storage space used by a given JSON value.

<span id="page-190-0"></span>• [JSON\\_PRETTY\(](#page-190-0)json\_val)
...
- Special characters in string scalars and key names are escaped employing the same rules used by the [JSON\\_QUOTE\(\)](#page-169-2) function.
...
According to the output of JSON\_STORAGE\_SIZE(), the JSON document inserted into the column takes up 47 bytes. Following an update, the function shows the storage used for the newly-set value:
