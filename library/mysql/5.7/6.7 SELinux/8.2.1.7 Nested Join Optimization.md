...
title: 8.2.1.7 Nested Join Optimization
---
...
Queries with nested outer joins are executed in the same pipeline manner as queries with inner joins. More exactly, a variation of the nested-loop join algorithm is exploited. Recall the algorithm by which the nested-loop join executes a query (see [Section 8.2.1.6, "Nested-Loop Join Algorithms"\)](#page-184-0). Suppose that a join query over 3 tables T1,T2,T3 has this form:
