...
title: 8.14.7 Replication Replica SQL Thread States
---
...
Using the multithreaded replica (slave\_parallel\_workers is greater than 1), one of the replica worker threads is waiting for an event from the coordinator thread.
...
This waiting action occurs when the total size of events being processed by Workers exceeds the size of the slave\_pending\_jobs\_size\_max system variable. The Coordinator resumes scheduling when the size drops below this limit. This state occurs only when slave\_parallel\_workers is set greater than 0.
...
The SQL thread has read an event but is waiting for the replica delay to lapse. This delay is set with the MASTER\_DELAY option of CHANGE MASTER TO.
