...
title: 17.6.2 Group Replication Secure Socket Layer (SSL) Support
---
...
|----------------------|------------------------------------------------------------------------------------------------|
| ssl_key              | Path of key file. To be used as client and server<br>certificate.                              |
| ssl_cert             | Path of certificate file. To be used as client and<br>server certificate.                      |
| ssl_ca               | Path of file with SSL Certificate Authorities that are<br>trusted.                             |
| ssl_capath           | Path of directory containing certificates for SSL<br>Certificate Authorities that are trusted. |
| ssl_crl              | Path of file containing the certificate revocation<br>lists.                                   |
| ssl_crlpath          | Path of directory containing revoked certificate<br>lists.                                     |
| ssl_cipher           | Permitted ciphers to use while encrypting data<br>over the connection.                         |

| Server Configuration | Plugin Configuration Description               |
|----------------------|------------------------------------------------|
| tls_version          | Secure communication uses this version and its |
|                      | protocols.                                     |
...
• [group\\_replication\\_ssl\\_mode](#page-47-1) - specifies the security state of the connection between Group Replication members.
...
The only plugin specific configuration option that is listed is [group\\_replication\\_ssl\\_mode](#page-47-1). This option activates the SSL communication between members of the group, by configuring the SSL framework with the ssl\_\* parameters that are provided to the server.