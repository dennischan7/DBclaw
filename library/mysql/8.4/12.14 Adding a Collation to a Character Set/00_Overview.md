...
MySQL supports several collation implementations, as discussed in [Section 12.14.1, "Collation](#page-16-0) [Implementation Types".](#page-16-0) Some of these can be added to MySQL without recompiling:
...
The instructions here cover only user-defined collations that can be added without recompiling MySQL. To add a collation that does require recompiling (as implemented by means of functions in a C source file), use the instructions in [Section 12.13, "Adding a Character Set"](#page-12-0). However, instead of adding all the information required for a complete character set, just modify the appropriate files for an existing character set. That is, based on what is already present for the character set's current collations, add data structures, functions, and configuration information for the new collation.
...
# <span id="page-16-0"></span>**12.14.1 Collation Implementation Types**
...
For implementation instructions, see [Section 12.14.3, "Adding a Simple Collation to an 8-Bit Character](#page-20-0) [Set"](#page-20-0).

#### **Complex collations for 8-bit character sets**

This kind of collation is implemented using functions in a C source file that define how to order characters, as described in [Section 12.13, "Adding a Character Set".](#page-12-0)
...
For implementation instructions, see [Section 12.13, "Adding a Character Set".](#page-12-0)
...
Non-UCA collations have a one-to-one mapping from character code to weight. In MySQL, such collations are case-insensitive and accent-insensitive. utf8mb4\_general\_ci is an example: 'a', 'A', 'À', and 'á' each have different character codes but all have a weight of 0x0041 and compare as equal.
...
For implementation instructions, for a non-UCA collation, see [Section 12.13, "Adding a Character Set".](#page-12-0) For a UCA collation, see [Section 12.14.4, "Adding a UCA Collation to a Unicode Character Set"](#page-21-0).

### **Miscellaneous collations**

There are also a few collations that do not fall into any of the previous categories.

## <span id="page-19-0"></span>**12.14.2 Choosing a Collation ID**
...
- The number member of the MY\_CHARSET\_INFO data structure returned by the [mysql\\_get\\_character\\_set\\_info\(\)](https://dev.mysql.com/doc/c-api/8.4/en/mysql-get-character-set-info.md) C API function.
...
## <span id="page-20-0"></span>**12.14.3 Adding a Simple Collation to an 8-Bit Character Set**
...
- 1. Choose a collation ID, as shown in [Section 12.14.2, "Choosing a Collation ID".](#page-19-0) The following steps use an ID of 1024.
...
## <span id="page-21-0"></span>**12.14.4 Adding a UCA Collation to a Unicode Character Set**
...
To add a UCA collation for a Unicode character set without recompiling MySQL, use the following procedure. If you are unfamiliar with the LDML rules used to describe the collation's sort characteristics, see [Section 12.14.4.2, "LDML Syntax Supported in MySQL"](#page-23-0).

The example adds a collation named utf8mb4\_phone\_ci to the utf8mb4 character set. The collation is designed for a scenario involving a Web application for which users post their names and phone numbers. Phone numbers can be given in very different formats:
...
- 1. Choose a collation ID, as shown in [Section 12.14.2, "Choosing a Collation ID".](#page-19-0) The following steps use an ID of 1029.
...
### <span id="page-23-0"></span>**12.14.4.2 LDML Syntax Supported in MySQL**
...
The MySQL server generates diagnostics when it finds problems while parsing the Index.xml file. See [Section 12.14.4.3, "Diagnostics During Index.xml Parsing".](#page-27-0)
...
## <span id="page-27-0"></span>**12.14.4.3 Diagnostics During Index.xml Parsing**
