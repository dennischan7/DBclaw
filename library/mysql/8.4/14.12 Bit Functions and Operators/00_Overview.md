...
<span id="page-64-0"></span>• [|](#page-64-0)
...
<span id="page-65-0"></span>• [&](#page-65-0)
...
<span id="page-65-1"></span>• [^](#page-65-1)
...
<span id="page-66-1"></span>• [<<](#page-66-1)
...
<span id="page-66-0"></span>• [>>](#page-66-0)
...
<span id="page-67-1"></span>• [~](#page-67-1)
...
<span id="page-67-0"></span>• [BIT\\_COUNT\(](#page-67-0)N)
...
Bit functions and operators comprise [BIT\\_COUNT\(\)](#page-67-0), BIT\_AND(), BIT\_OR(), BIT\_XOR(), [&](#page-65-0), [|](#page-64-0), [^](#page-65-1), [~](#page-67-1), [<<](#page-66-1), and [>>](#page-66-0). (The BIT\_AND(), BIT\_OR(), and BIT\_XOR() aggregate functions are described in Section 14.19.1, "Aggregate Function Descriptions".)
...
- [Bit Operations](#page-68-0)
- [Binary String Bit-Operation Examples](#page-69-0)
- [Bitwise AND, OR, and XOR Operations](#page-71-0)
- [Bitwise Complement and Shift Operations](#page-72-0)
- [BIT\\_COUNT\(\) Operations](#page-72-1)
- [BIT\\_AND\(\), BIT\\_OR\(\), and BIT\\_XOR\(\) Operations](#page-73-0)
- [Special Handling of Hexadecimal Literals, Bit Literals, and NULL Literals](#page-73-1)

## <span id="page-68-0"></span>**Bit Operations**
...
In binary-string context, bitwise operation arguments must have the same length or an [ER\\_INVALID\\_BITWISE\\_OPERANDS\\_SIZE](https://dev.mysql.com/doc/mysql-errors/8.4/en/server-error-reference.md#error_er_invalid_bitwise_operands_size) error occurs:
...
Padding or stripping can also be accomplished using functions such as LPAD(), RPAD(), SUBSTR(), or [CAST\(\)](#page-42-0). In such cases, the expression arguments are no longer all literals and \_binary becomes unnecessary. Examples:
...
See also [Special Handling of Hexadecimal Literals, Bit Literals, and NULL Literals](#page-73-1).

# <span id="page-69-0"></span>**Binary String Bit-Operation Examples**
...
The preceding example uses these bit operations: right shift ([>>](#page-66-0)) and bitwise AND ([&](#page-65-0)).
...
To produce the complementary value (all ones), use the [~](#page-67-1) operator to invert the bits:
...
The preceding example uses these bit operations: Complement ([~](#page-67-1)), left shift ([<<](#page-66-1)), and bitwise AND ([&](#page-65-0)).
...
## <span id="page-71-0"></span>**Bitwise AND, OR, and XOR Operations**

For [&](#page-65-0), [|](#page-64-0), and [^](#page-65-1) bit operations, the result type depends on whether the arguments are evaluated as binary strings or numbers:
...
## <span id="page-72-0"></span>**Bitwise Complement and Shift Operations**

For [~](#page-67-1), [<<](#page-66-1), and [>>](#page-66-0) bit operations, the result type depends on whether the bit argument is evaluated as a binary string or number:
...
# <span id="page-72-1"></span>**BIT\_COUNT() Operations**

The [BIT\\_COUNT\(\)](#page-67-0) function always returns an unsigned 64-bit integer, or NULL if the argument is NULL.
...
## <span id="page-73-0"></span>**BIT\_AND(), BIT\_OR(), and BIT\_XOR() Operations**

For the BIT\_AND(), BIT\_OR(), and BIT\_XOR() bit functions, the result type depends on whether the function argument values are evaluated as binary strings or numbers:
...
NULL values do not affect the result unless all values are NULL. In that case, the result is a neutral value having the same length as the length of the argument values (all bits 1 for BIT\_AND(), all bits 0 for BIT\_OR(), and BIT\_XOR()).
