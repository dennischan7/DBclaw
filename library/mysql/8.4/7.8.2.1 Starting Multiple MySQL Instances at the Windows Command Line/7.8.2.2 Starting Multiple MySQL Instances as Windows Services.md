...
For the following instructions, suppose that you want to run the mysqld server from two different versions of MySQL that are installed at C:\mysql-5.7.9 and C:\mysql-8.4.8, respectively. (This might be the case if you are running 5.7.9 as your production server, but also want to conduct tests using 8.4.8.)
...
C:\> C:\mysql-8.4.8\bin\mysqld --install mysqld2
```
...
For the 8.4.8 mysqld, create a file C:\my-opts2.cnf that looks like this:
