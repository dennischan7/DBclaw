...
The INNODB\_TABLESPACES and INNODB\_DATAFILES tables also provide metadata about InnoDB tablespaces, but data is limited to file-per-table, general, and undo tablespaces.
...
This query retrieves the FILE\_ID (equivalent to the space ID) and the FILE\_NAME (which includes path information) for InnoDB file-per-table and general tablespaces. File-per-table and general tablespaces have a .ibd file extension.
...
This query retrieves the FILE\_ID and FILE\_NAME for the InnoDB global temporary tablespace. Global temporary tablespace file names are prefixed by ibtmp.
...
Similarly, InnoDB undo tablespace file names are prefixed by undo. The following query returns the FILE\_ID and FILE\_NAME for InnoDB undo tablespaces.
