...
## <span id="page-115-0"></span>**7.6.7.13 Clone System Variables**
...
Setting a global system variable runtime value normally requires the [SYSTEM\\_VARIABLES\\_ADMIN](#page-186-1) privilege (or the deprecated [SUPER](#page-177-1) privilege). For more information, see Section 7.1.9.1, "System Variable Privileges".

Clone variables are configured on the recipient MySQL server instance where the cloning operation is executed.

<span id="page-115-1"></span>• [clone\\_autotune\\_concurrency](#page-115-1)
...
When [clone\\_autotune\\_concurrency](#page-115-1) is enabled (the default), additional threads for remote cloning operations are spawned dynamically to optimize data transfer speed. The setting is applicable to recipient MySQL server instance only.
...
The [clone\\_max\\_concurrency](#page-118-1) variable defines the maximum number of threads that can be spawned.

If [clone\\_autotune\\_concurrency](#page-115-1) is disabled, [clone\\_max\\_concurrency](#page-118-1) defines the number of threads spawned for a remote cloning operation.

<span id="page-116-1"></span>• [clone\\_buffer\\_size](#page-116-1)
...
<span id="page-116-0"></span>• [clone\\_block\\_ddl](#page-116-0)
...
Enables an exclusive backup lock on the donor MySQL Server instance during a cloning operation, which blocks concurrent DDL operations on the donor. See [Section 7.6.7.4, "Cloning and Concurrent](#page-106-0) [DDL"](#page-106-0).

<span id="page-117-1"></span>• [clone\\_delay\\_after\\_data\\_drop](#page-117-1)
...
<span id="page-117-0"></span>• [clone\\_ddl\\_timeout](#page-117-0)
...
Concurrent DDL is permitted on the donor during a cloning operation if [clone\\_block\\_ddl](#page-116-0) is set to OFF (the default). In this case, the cloning operation does not have to wait for a backup lock on the donor. See [Section 7.6.7.4, "Cloning and Concurrent DDL"](#page-106-0).

<span id="page-118-0"></span>• [clone\\_donor\\_timeout\\_after\\_network\\_failure](#page-118-0)
...
Defines the amount of time in minutes the donor allows for the recipient to reconnect and restart a cloning operation after a network failure. For more information, see [Section 7.6.7.9, "Remote Cloning](#page-109-0) [Operation Failure Handling".](#page-109-0)

This variable is set on the donor MySQL server instance. Setting it on the recipient MySQL server instance has no effect.

<span id="page-118-2"></span>• [clone\\_enable\\_compression](#page-118-2)
...
<span id="page-118-1"></span>• [clone\\_max\\_concurrency](#page-118-1)

| Command-Line Format | clone-max-concurrency         |  |
|---------------------|-------------------------------|--|
| System Variable     | clone_max_concurrency<br>1089 |  |
...
If [clone\\_autotune\\_concurrency](#page-115-1) is enabled (the default), [clone\\_max\\_concurrency](#page-118-1) is the maximum number of threads that can be dynamically spawned for a remote cloning operation. If [clone\\_autotune\\_concurrency](#page-115-1) is disabled, [clone\\_max\\_concurrency](#page-118-1) defines the number of threads spawned for a remote cloning operation.

A minimum data transfer rate of 1 mebibyte (MiB) per thread is recommended for remote cloning operations. The data transfer rate for a remote cloning operation is controlled by the [clone\\_max\\_data\\_bandwidth](#page-119-0) variable.

<span id="page-119-0"></span>• [clone\\_max\\_data\\_bandwidth](#page-119-0)
...
The minimum data transfer rate is 1 MiB per second, per thread. For example, if there are 8 threads, the minimum transfer rate is 8 MiB per second. The [clone\\_max\\_concurrency](#page-118-1) variable controls the maximum number threads spawned for a remote cloning operation.

The requested data transfer rate specified by [clone\\_max\\_data\\_bandwidth](#page-119-0) may differ from the actual data transfer rate reported by the DATA\_SPEED column in the performance\_schema.clone\_progress table. If your cloning operation is not achieving the desired data transfer rate and you have available bandwidth, check I/O usage on the recipient and donor. If there is underutilized bandwidth, I/O is the next mostly likely bottleneck.

<span id="page-119-1"></span>• [clone\\_max\\_network\\_bandwidth](#page-119-1)
...
<span id="page-120-0"></span>• [clone\\_ssl\\_ca](#page-120-0)
...
<span id="page-120-1"></span>• [clone\\_ssl\\_cert](#page-120-1)
...
<span id="page-120-2"></span>• [clone\\_ssl\\_key](#page-120-2)
...
<span id="page-121-0"></span>• [clone\\_valid\\_donor\\_list](#page-121-0)
...
The [clone\\_valid\\_donor\\_list](#page-121-0) variable adds a layer of security by providing control over the sources of cloned data. The privilege required to configure [clone\\_valid\\_donor\\_list](#page-121-0) is different from the privilege required to execute remote cloning operations, which permits assigning those responsibilities to different roles. Configuring [clone\\_valid\\_donor\\_list](#page-121-0) requires the [SYSTEM\\_VARIABLES\\_ADMIN](#page-186-1) privilege, whereas executing a remote cloning operation requires the [CLONE\\_ADMIN](#page-181-0) privilege.
