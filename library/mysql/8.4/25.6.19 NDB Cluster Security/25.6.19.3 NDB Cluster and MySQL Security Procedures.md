...
title: 25.6.19.3 NDB Cluster and MySQL Security Procedures
---
...
Never run mysqld as the system root user. Doing so means that potentially any file on the system can be read by MySQL, and thus—should MySQL be compromised—by an attacker.
 ...
 As mentioned in the previous section (see [Section 25.6.19.2, "NDB Cluster and MySQL Privileges"\)](#page-82-0), you should always set a root password for the MySQL Server as soon as you have it running. You should also delete the anonymous user account that is installed by default. You can accomplish these tasks using the following statements:
