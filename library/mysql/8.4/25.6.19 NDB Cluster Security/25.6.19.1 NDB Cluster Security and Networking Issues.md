...
title: 25.6.19.1 NDB Cluster Security and Networking Issues
---
...
By default, no encryption or similar security measures are used in communications between nodes in the cluster; encrypted connections are supported, but must be enabled using the information and instructions found in [Section 25.6.19.5, "TLS Link Encryption for NDB Cluster"](#page-88-0).
...
This means that, if there are any empty [mysqld] or [api] sections in the config.ini file, then any API nodes (including SQL nodes) that know the management server's host name (or IP address) and port can connect to the cluster and access its data without restriction. (See [Section 25.6.19.2,](#page-82-0) ["NDB Cluster and MySQL Privileges",](#page-82-0) for more information about this and related issues.)
...
You can require TLS for connections by starting the management server with --ndb-mgmtls=strict. See [Using TLS Connections](#page-89-0), for details.
...
## <span id="page-82-0"></span>**25.6.19.2 NDB Cluster and MySQL Privileges**
