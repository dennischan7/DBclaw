...
Before using MySQL Enterprise Data Masking and De-Identification, install it according to the instructions provided at [Section 8.5.3.1, "MySQL Enterprise Data Masking and De-Identification Plugin](#page-6-1) [Installation"](#page-6-1).

To use MySQL Enterprise Data Masking and De-Identification in applications, invoke the functions that are appropriate for the operations you wish to perform. For detailed function descriptions, see [Section 8.5.3.4, "MySQL Enterprise Data Masking and De-Identification Plugin Function Descriptions".](#page-13-0) This section demonstrates how to use the functions to carry out some representative tasks. It first
...
- [Masking Data to Remove Identifying Characteristics](#page-8-0)
- [Generating Random Data with Specific Characteristics](#page-9-0)
- [Generating Random Data Using Dictionaries](#page-10-0)
- [Using Masked Data for Customer Identification](#page-11-0)
- [Creating Views that Display Masked Data](#page-12-0)

### <span id="page-8-0"></span>**Masking Data to Remove Identifying Characteristics**
...
[mask\\_inner\(\)](#page-14-0) and [mask\\_outer\(\)](#page-14-1) are general-purpose functions that mask parts of arbitrary strings based on position within the string:

• [mask\\_inner\(\)](#page-14-0) masks the interior of its string argument, leaving the ends unmasked. Other arguments specify the sizes of the unmasked ends.
...
• [mask\\_outer\(\)](#page-14-1) does the reverse, masking the ends of its string argument, leaving the interior unmasked. Other arguments specify the sizes of the masked ends.
...
By default, [mask\\_inner\(\)](#page-14-0) and [mask\\_outer\(\)](#page-14-1) use 'X' as the masking character, but permit an optional masking-character argument:
...
The examples here supply function arguments using the random value generation functions that return the appropriate type of value. For more information about generation functions, see [Generating Random Data with](#page-9-0) [Specific Characteristics](#page-9-0).
...
• [mask\\_pan\(\)](#page-15-0) masks all but the last four digits of the number:
...
• [mask\\_pan\\_relaxed\(\)](#page-16-0) is similar but does not mask the first six digits that indicate the payment card issuer unmasked:
...
**US Social Security number masking.** [mask\\_ssn\(\)](#page-16-1) masks all but the last four digits of the number:
...
## <span id="page-9-0"></span>**Generating Random Data with Specific Characteristics**

Several functions generate random values. These values can be used for testing, simulation, and so forth.

[gen\\_range\(\)](#page-17-0) returns a random integer selected from a given range:
...
[gen\\_rnd\\_email\(\)](#page-17-1) returns a random email address in the example.com domain:
...
[gen\\_rnd\\_pan\(\)](#page-17-2) returns a random payment card Primary Account Number:

```
mysql> SELECT gen_rnd_pan();
```

(The [gen\\_rnd\\_pan\(\)](#page-17-2) function result is not shown because its return values should be used only for testing purposes, and not for publication. It cannot be guaranteed the number is not assigned to a legitimate payment account.)

[gen\\_rnd\\_ssn\(\)](#page-19-0) returns a random US Social Security number with the first and second parts each chosen from a range not used for legitimate numbers:
...
[gen\\_rnd\\_us\\_phone\(\)](#page-19-1) returns a random US phone number in the 555 area code not used for legitimate numbers:
...
### <span id="page-10-0"></span>**Generating Random Data Using Dictionaries**
...
Assume that the secure\_file\_priv system variable is set to /usr/local/mysql/mysql-files. In that case, copy the dictionary files to that directory so that the MySQL server can access them. Then use [gen\\_dictionary\\_load\(\)](#page-22-0) to load the dictionaries into the dictionary registry and assign them names:
...
To select a random term from a dictionary, use [gen\\_dictionary\(\)](#page-20-0):
...
The [gen\\_blocklist\(\)](#page-19-2) function enables a term from one dictionary to be replaced by a term from another dictionary, which effects masking by substitution. Its arguments are the term to replace, the dictionary in which the term appears, and the dictionary from which to choose a replacement. For example, to substitute a US city for a German city, or vice versa, use [gen\\_blocklist\(\)](#page-19-2) like this:
...
If the term to replace is not in the first dictionary, [gen\\_blocklist\(\)](#page-19-2) returns it unchanged:
...
### <span id="page-11-0"></span>**Using Masked Data for Customer Identification**
...
Why was the CONVERT() function used for the argument to [mask\\_ssn\(\)](#page-16-1)? Because [mask\\_ssn\(\)](#page-16-1) requires an argument of length 11. Thus, even though ssn is defined as VARCHAR(11), if the ssn column has a multibyte character set, it may appear to be longer than 11 bytes when passed to a loadable function, and an error occurs. Converting the value to a binary string ensures that the function sees an argument of length 11.
...
### <span id="page-12-0"></span>**Creating Views that Display Masked Data**
