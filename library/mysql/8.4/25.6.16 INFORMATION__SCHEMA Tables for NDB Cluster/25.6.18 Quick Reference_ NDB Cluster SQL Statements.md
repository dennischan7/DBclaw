...
title: 25.6.18 Quick Reference_ NDB Cluster SQL Statements
---
...
• SELECT \* FROM INFORMATION SCHEMA.PLUGINS WHERE PLUGIN NAME LIKE 'NDB%'
...
```
mysgl> SELECT * FROM INFORMATION SCHEMA.PLUGINS
   > WHERE PLUGIN NAME LIKE 'NDB%'\G
          ************* 1. row ****
        PLUGIN NAME: ndbcluster
       PLUGIN VERSION: 1.0
        PLUGIN STATUS: ACTIVE
         PLUGIN TYPE: STORAGE ENGINE
  PLUGIN TYPE VERSION: 80408.0
       PLUGIN LIBRARY: NULL
PLUGIN LIBRARY VERSION: NULL
        PLUGIN AUTHOR: Oracle Corporation
   PLUGIN DESCRIPTION: Clustered, fault-tolerant tables
       PLUGIN LICENSE: GPL
         LOAD OPTION: ON
--------------------------------------
        PLUGIN NAME: ndbinfo
...
PLUGIN LIBRARY VERSION: NULL
        PLUGIN AUTHOR: Oracle Corporation
   PLUGIN DESCRIPTION: MySQL Cluster system information storage engine
       PLUGIN LICENSE: GPL
         LOAD OPTION: ON
                       *** 3. row ********
         PLUGIN NAME: ndb transid mysql connection map
       PLUGIN VERSION: 0.1
        PLUGIN STATUS: ACTIVE
         PLUGIN TYPE: INFORMATION SCHEMA
  PLUGIN TYPE VERSION: 80408.0
       PLUGIN LIBRARY: NULL
PLUGIN LIBRARY VERSION: NULL
        PLUGIN AUTHOR: Oracle Corporation
   PLUGIN DESCRIPTION: Map between MySOL connection ID and NDB transaction ID
       PLUGIN LICENSE: GPL
          LOAD OPTION: ON
```
