...
- A CSV file produced by mysqldump or other export program. Such files can be imported into NDB using LOAD DATA INFILE in the mysql client, or with the [ndb\\_import](#page-1-0) utility provided with the NDB Cluster distribution. For more information about the latter, see [Section 25.5.13, "ndb\\_import —](#page-1-0) [Import CSV Data Into NDB"](#page-1-0).
- A native NDB backup produced using [START BACKUP](#page-151-1) in the NDB management client. To import a native backup, you must use the [ndb\\_restore](#page-35-7) program that comes as part of NDB Cluster. See [Section 25.5.23, "ndb\\_restore — Restore an NDB Cluster Backup",](#page-35-7) for more about using this program.
