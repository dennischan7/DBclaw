...
These steps assume you have enabled GTIDs for transactions on the sources using [gtid\\_mode=ON](#page-99-0), created a replication user, ensured that the replica is using TABLE based replication applier metadata repositories, and provisioned the replica with data from the sources if appropriate.

Use CHANGE REPLICATION SOURCE TO to configure a replication channel for each source on the replica (see [Section 19.2.2, "Replication Channels"](#page-118-0)). The FOR CHANNEL clause is used to specify the channel. For GTID-based replication, GTID auto-positioning is used to synchronize with the source (see Section 19.1.3.3, "GTID Auto-Positioning"). The SOURCE\_AUTO\_POSITION option is set to specify the use of auto-positioning.
