...
- A MySQL instance using a particular InnoDB page size ([innodb\\_page\\_size](#page-41-0)) cannot use data files or log files from an instance that uses a different page size.
...
This chapter covers use cases for special-purpose MySQL storage engines. It does not cover the default InnoDB storage engine or the NDB storage engine which are covered in Chapter 17, *The InnoDB Storage Engine* and Chapter 25, *MySQL NDB Cluster 8.4*. For advanced users, it also contains a description of the pluggable storage engine architecture (see Section 18.11, "Overview of MySQL Storage Engine Architecture").
...
For answers to commonly asked questions about MySQL storage engines, see Section A.2, "MySQL 8.4 FAQ: Storage Engines".

# **MySQL 8.4 Supported Storage Engines**
...
- [Blackhole](#page-144-0): The Blackhole storage engine accepts but does not store data, similar to the Unix / dev/null device. Queries always return an empty set. These tables can be used in replication configurations where DML statements are sent to replica servers, but the source server does not keep its own copy of the data.
...
- [Merge](#page-146-0): Enables a MySQL DBA or developer to logically group a series of identical MyISAM tables and reference them as one object. Good for VLDB environments such as data warehousing.
- [Federated](#page-151-0): Offers the ability to link separate MySQL servers to create one logical database from many physical servers. Very good for distributed or data mart environments.
- [Example](#page-156-1): This engine serves as an example in the MySQL source code that illustrates how to begin writing new storage engines. It is primarily of interest to developers. The storage engine is a "stub" that does nothing. You can create tables with this engine, but no data can be stored in them or retrieved from them.
...
# <span id="page-127-0"></span>**18.1 Setting the Storage Engine**
...
When you omit the ENGINE option, the default storage engine is used. The default engine is InnoDB in MySQL 8.4. You can specify the default engine by using the --default-storage-engine server startup option, or by setting the default-storage-engine option in the my.cnf configuration file.
...
# <span id="page-128-0"></span>**18.2 The MyISAM Storage Engine**
...
In MySQL 8.4, it is normally necessary to use ENGINE to specify the MyISAM storage engine because InnoDB is the default engine.
...
# <span id="page-131-0"></span>**18.2.1 MyISAM Startup Options**
...
# <span id="page-132-0"></span>**18.2.2 Space Needed for Keys**
...
# <span id="page-132-1"></span>**18.2.3 MyISAM Table Storage Formats**
