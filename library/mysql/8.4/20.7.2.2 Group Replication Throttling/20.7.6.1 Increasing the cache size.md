...
title: 20.7.6.1 Increasing the cache size
---
...
In this situation, consider increasing the [group\\_replication\\_message\\_cache\\_size](#page-151-0) limit with reference to the expected volume of messages in the time period specified by the [group\\_replication\\_member\\_expel\\_timeout](#page-149-0) system variable plus the 5-second detection period, so that the cache contains all the missed messages required for members to return successfully. You can also consider increasing the cache size limit temporarily if you expect a member to become unreachable for an unusual period of time.