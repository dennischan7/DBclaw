...
NOT IN is not an alias for <> ANY, but for <> ALL. See [Section 15.2.15.4, "Subqueries with ALL".](#page-93-0)
...
You can use [TABLE](#page-105-0) in a scalar IN, ANY, or SOME subquery provided the table contains only a single column. If t2 has only one column, the statements shown previously in this section can be written as shown here, in each case substituting TABLE t2 for SELECT s1 FROM t2:
...
## <span id="page-93-0"></span>**15.2.15.4 Subqueries with ALL**
...
As with IN, ANY, and SOME, you can use [TABLE](#page-105-0) with ALL and NOT IN provided that the following two conditions are met:
...
# <span id="page-94-0"></span>**15.2.15.5 Row Subqueries**
