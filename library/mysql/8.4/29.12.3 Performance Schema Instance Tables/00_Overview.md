...
- [cond\\_instances](#page-20-0): Condition synchronization object instances
- [file\\_instances](#page-21-0): File instances
- [mutex\\_instances](#page-21-1): Mutex synchronization object instances
- [rwlock\\_instances](#page-22-0): Lock synchronization object instances
- [socket\\_instances](#page-23-0): Active connection instances

These tables list instrumented synchronization objects, files, and connections. There are three types of synchronization objects: cond, mutex, and rwlock. Each instance table has an EVENT\_NAME or NAME column to indicate the instrument associated with each row. Instrument names may have multiple parts and form a hierarchy, as discussed in Section 29.6, "Performance Schema Instrument Naming Conventions".

The mutex\_instances.LOCKED\_BY\_THREAD\_ID and rwlock\_instances.WRITE\_LOCKED\_BY\_THREAD\_ID columns are extremely important for investigating performance bottlenecks or deadlocks. For examples of how to use them for this purpose, see [Section 29.19, "Using the Performance Schema to Diagnose Problems"](#page-192-0)

### <span id="page-20-0"></span>**29.12.3.1 The cond\_instances Table**

The [cond\\_instances](#page-20-0) table lists all the conditions seen by the Performance Schema while the server executes. A condition is a synchronization mechanism used in the code to signal that a specific event has happened, so that a thread waiting for this condition can resume work.
...
The [cond\\_instances](#page-20-0) table has these columns:
...
The [cond\\_instances](#page-20-0) table has these indexes:

- Primary key on (OBJECT\_INSTANCE\_BEGIN)
- Index on (NAME)

TRUNCATE TABLE is not permitted for the [cond\\_instances](#page-20-0) table.

### <span id="page-21-0"></span>**29.12.3.2 The file\_instances Table**

The [file\\_instances](#page-21-0) table lists all the files seen by the Performance Schema when executing file I/O instrumentation. If a file on disk has never been opened, it is not shown in [file\\_instances](#page-21-0). When a file is deleted from the disk, it is also removed from the [file\\_instances](#page-21-0) table.

The [file\\_instances](#page-21-0) table has these columns:
...
The [file\\_instances](#page-21-0) table has these indexes:

- Primary key on (FILE\_NAME)
- Index on (EVENT\_NAME)

TRUNCATE TABLE is not permitted for the [file\\_instances](#page-21-0) table.

### <span id="page-21-1"></span>**29.12.3.3 The mutex\_instances Table**

The [mutex\\_instances](#page-21-1) table lists all the mutexes seen by the Performance Schema while the server executes. A mutex is a synchronization mechanism used in the code to enforce that only one thread at a given time can have access to some common resource. The resource is said to be "protected" by the mutex.
...
The [mutex\\_instances](#page-21-1) table has these columns:
...
The [mutex\\_instances](#page-21-1) table has these indexes:
...
TRUNCATE TABLE is not permitted for the [mutex\\_instances](#page-21-1) table.
...
- When some code creates a mutex, a row is added to the [mutex\\_instances](#page-21-1) table. The OBJECT\_INSTANCE\_BEGIN column is a property that uniquely identifies the mutex.
- When a thread attempts to lock a mutex, the [events\\_waits\\_current](#page-27-0) table shows a row for that thread, indicating that it is waiting on a mutex (in the EVENT\_NAME column), and indicating which mutex is waited on (in the OBJECT\_INSTANCE\_BEGIN column).
- When a thread succeeds in locking a mutex:
  - [events\\_waits\\_current](#page-27-0) shows that the wait on the mutex is completed (in the TIMER\_END and TIMER\_WAIT columns)
  - The completed wait event is added to the [events\\_waits\\_history](#page-29-0) and [events\\_waits\\_history\\_long](#page-30-0) tables
  - [mutex\\_instances](#page-21-1) shows that the mutex is now owned by the thread (in the THREAD\_ID column).
- When a thread unlocks a mutex, [mutex\\_instances](#page-21-1) shows that the mutex now has no owner (the THREAD\_ID column is NULL).
- When a mutex object is destroyed, the corresponding row is removed from [mutex\\_instances](#page-21-1).
...
The [rwlock\\_instances](#page-22-0) table has these columns:
...
The [rwlock\\_instances](#page-22-0) table has these indexes:

- Primary key on (OBJECT\_INSTANCE\_BEGIN)
- Index on (NAME)
- Index on (WRITE\_LOCKED\_BY\_THREAD\_ID)

TRUNCATE TABLE is not permitted for the [rwlock\\_instances](#page-22-0) table.
...
- [events\\_waits\\_current](#page-27-0), to see what rwlock a thread is waiting for
- [rwlock\\_instances](#page-22-0), to see which other thread currently owns an rwlock

There is a limitation: The [rwlock\\_instances](#page-22-0) can be used only to identify the thread holding a write lock, but not the threads holding a read lock.

### <span id="page-23-0"></span>**29.12.3.5 The socket\_instances Table**

The [socket\\_instances](#page-23-0) table provides a real-time snapshot of the active connections to the MySQL server. The table contains one row per TCP/IP or Unix socket file connection. Information available in this table provides a real-time snapshot of the active connections to the server. (Additional information is available in socket summary tables, including network activity such as socket operations and number of bytes transmitted and received; see [Section 29.12.20.9, "Socket Summary Tables"](#page-133-0)).
...
- 3. When a connection terminates, the row in [socket\\_instances](#page-23-0) corresponding to it is deleted.

The [socket\\_instances](#page-23-0) table has these columns:

• EVENT\_NAME

The name of the wait/io/socket/\* instrument that produced the event. This is a NAME value from the [setup\\_instruments](#page-13-0) table. Instrument names may have multiple parts and form a hierarchy, as discussed in Section 29.6, "Performance Schema Instrument Naming Conventions".
...
A socket is idle if it is waiting for a request from the client. When a socket becomes idle, the event row in [socket\\_instances](#page-23-0) that is tracking the socket switches from a status of ACTIVE to IDLE. The EVENT\_NAME value remains wait/io/socket/\*, but timing for the instrument is suspended. Instead, an event is generated in the [events\\_waits\\_current](#page-27-0) table with an EVENT\_NAME value of idle.
...
The [socket\\_instances](#page-23-0) table has these indexes:
...
TRUNCATE TABLE is not permitted for the [socket\\_instances](#page-23-0) table.
...
# <span id="page-25-0"></span>**29.12.4 Performance Schema Wait Event Tables**
...
- [events\\_waits\\_current](#page-27-0): The current wait event for each thread.
- [events\\_waits\\_history](#page-29-0): The most recent wait events that have ended per thread.
- [events\\_waits\\_history\\_long](#page-30-0): The most recent wait events that have ended globally (across all threads).

The following sections describe the wait event tables. There are also summary tables that aggregate information about wait events; see [Section 29.12.20.1, "Wait Event Summary Tables".](#page-116-0)

For more information about the relationship between the three wait event tables, see [Section 29.9,](#page-0-0) ["Performance Schema Tables for Current and Historical Events"](#page-0-0).
...
### <span id="page-27-0"></span>**29.12.4.1 The events\_waits\_current Table**

The [events\\_waits\\_current](#page-27-0) table contains current wait events. The table stores one row per thread showing the current status of the thread's most recent monitored wait event, so there is no system variable for configuring the table size.

Of the tables that contain wait event rows, [events\\_waits\\_current](#page-27-0) is the most fundamental. Other tables that contain wait event rows are logically derived from the current events. For example, the [events\\_waits\\_history](#page-29-0) and [events\\_waits\\_history\\_long](#page-30-0) tables are collections of the most recent wait events that have ended, up to a maximum number of rows per thread and globally across all threads, respectively.

For more information about the relationship between the three wait event tables, see [Section 29.9,](#page-0-0) ["Performance Schema Tables for Current and Historical Events"](#page-0-0).

For information about configuring whether to collect wait events, see [Section 29.12.4, "Performance](#page-25-0) [Schema Wait Event Tables".](#page-25-0)

The [events\\_waits\\_current](#page-27-0) table has these columns:
...
The name of the instrument that produced the event. This is a NAME value from the [setup\\_instruments](#page-13-0) table. Instrument names may have multiple parts and form a hierarchy, as discussed in Section 29.6, "Performance Schema Instrument Naming Conventions".
...
The [events\\_waits\\_current](#page-27-0) table has these indexes:

• Primary key on (THREAD\_ID, EVENT\_ID)

TRUNCATE TABLE is permitted for the [events\\_waits\\_current](#page-27-0) table. It removes the rows.

### <span id="page-29-0"></span>**29.12.4.2 The events\_waits\_history Table**

The [events\\_waits\\_history](#page-29-0) table contains the N most recent wait events that have ended per thread. Wait events are not added to the table until they have ended. When the table contains the maximum number of rows for a given thread, the oldest thread row is discarded when a new row for that thread is added. When a thread ends, all its rows are discarded.

The Performance Schema autosizes the value of N during server startup. To set the number of rows per thread explicitly, set the [performance\\_schema\\_events\\_waits\\_history\\_size](#page-173-0) system variable at server startup.

The [events\\_waits\\_history](#page-29-0) table has the same columns and indexing as [events\\_waits\\_current](#page-27-0). See [Section 29.12.4.1, "The events\\_waits\\_current Table"](#page-27-0).

TRUNCATE TABLE is permitted for the [events\\_waits\\_history](#page-29-0) table. It removes the rows.

For more information about the relationship between the three wait event tables, see [Section 29.9,](#page-0-0) ["Performance Schema Tables for Current and Historical Events"](#page-0-0).

For information about configuring whether to collect wait events, see [Section 29.12.4, "Performance](#page-25-0) [Schema Wait Event Tables".](#page-25-0)

# <span id="page-30-0"></span>**29.12.4.3 The events\_waits\_history\_long Table**

The [events\\_waits\\_history\\_long](#page-30-0) table contains N the most recent wait events that have ended globally, across all threads. Wait events are not added to the table until they have ended. When the table becomes full, the oldest row is discarded when a new row is added, regardless of which thread generated either row.

The Performance Schema autosizes the value of N during server startup. To set the table size explicitly, set the [performance\\_schema\\_events\\_waits\\_history\\_long\\_size](#page-173-1) system variable at server startup.

The [events\\_waits\\_history\\_long](#page-30-0) table has the same columns as [events\\_waits\\_current](#page-27-0). See [Section 29.12.4.1, "The events\\_waits\\_current Table".](#page-27-0) Unlike [events\\_waits\\_current](#page-27-0), [events\\_waits\\_history\\_long](#page-30-0) has no indexing.

TRUNCATE TABLE is permitted for the [events\\_waits\\_history\\_long](#page-30-0) table. It removes the rows.

For more information about the relationship between the three wait event tables, see [Section 29.9,](#page-0-0) ["Performance Schema Tables for Current and Historical Events"](#page-0-0).

For information about configuring whether to collect wait events, see [Section 29.12.4, "Performance](#page-25-0) [Schema Wait Event Tables".](#page-25-0)

# <span id="page-30-1"></span>**29.12.5 Performance Schema Stage Event Tables**
...
- [events\\_stages\\_current](#page-34-0): The current stage event for each thread.
- [events\\_stages\\_history](#page-35-0): The most recent stage events that have ended per thread.
- [events\\_stages\\_history\\_long](#page-35-1): The most recent stage events that have ended globally (across all threads).

The following sections describe the stage event tables. There are also summary tables that aggregate information about stage events; see [Section 29.12.20.2, "Stage Summary Tables"](#page-118-0).

For more information about the relationship between the three stage event tables, see [Section 29.9,](#page-0-0) ["Performance Schema Tables for Current and Historical Events"](#page-0-0).

- [Configuring Stage Event Collection](#page-31-0)
- [Stage Event Progress Information](#page-32-0)

## <span id="page-31-0"></span>**Configuring Stage Event Collection**
...
### <span id="page-32-0"></span>**Stage Event Progress Information**
...
By querying the [events\\_stages\\_current](#page-34-0) table for the monitored session, a monitoring application can report how much work has been performed so far, but cannot report whether the stage is near completion. Currently, no stages are instrumented like this.

• Bounded progress instrumentation

The WORK\_COMPLETED and WORK\_ESTIMATED columns are both meaningful.

This type of progress indicator is appropriate for an operation with a defined completion criterion, such as the table-copy instrument described later. By querying the [events\\_stages\\_current](#page-34-0) table for the monitored session, a monitoring application can report how much work has been performed so far, and can report the overall completion percentage for the stage, by computing the WORK\_COMPLETED / WORK\_ESTIMATED ratio.
...
To see the progress of an ongoing ALTER TABLE statement, select from the [events\\_stages\\_current](#page-34-0) table.

### <span id="page-34-0"></span>**29.12.5.1 The events\_stages\_current Table**

The [events\\_stages\\_current](#page-34-0) table contains current stage events. The table stores one row per thread showing the current status of the thread's most recent monitored stage event, so there is no system variable for configuring the table size.

Of the tables that contain stage event rows, [events\\_stages\\_current](#page-34-0) is the most fundamental. Other tables that contain stage event rows are logically derived from the current events. For example, the [events\\_stages\\_history](#page-35-0) and [events\\_stages\\_history\\_long](#page-35-1) tables are collections of the most recent stage events that have ended, up to a maximum number of rows per thread and globally across all threads, respectively.

For more information about the relationship between the three stage event tables, see [Section 29.9,](#page-0-0) ["Performance Schema Tables for Current and Historical Events"](#page-0-0).

For information about configuring whether to collect stage events, see [Section 29.12.5, "Performance](#page-30-1) [Schema Stage Event Tables"](#page-30-1).

The [events\\_stages\\_current](#page-34-0) table has these columns:
...
The [events\\_stages\\_current](#page-34-0) table has these indexes:

• Primary key on (THREAD\_ID, EVENT\_ID)

TRUNCATE TABLE is permitted for the [events\\_stages\\_current](#page-34-0) table. It removes the rows.

## <span id="page-35-0"></span>**29.12.5.2 The events\_stages\_history Table**

The [events\\_stages\\_history](#page-35-0) table contains the N most recent stage events that have ended per thread. Stage events are not added to the table until they have ended. When the table contains the maximum number of rows for a given thread, the oldest thread row is discarded when a new row for that thread is added. When a thread ends, all its rows are discarded.

The Performance Schema autosizes the value of N during server startup. To set the number of rows per thread explicitly, set the [performance\\_schema\\_events\\_stages\\_history\\_size](#page-171-0) system variable at server startup.

The [events\\_stages\\_history](#page-35-0) table has the same columns and indexing as [events\\_stages\\_current](#page-34-0). See [Section 29.12.5.1, "The events\\_stages\\_current Table"](#page-34-0).

TRUNCATE TABLE is permitted for the [events\\_stages\\_history](#page-35-0) table. It removes the rows.

For more information about the relationship between the three stage event tables, see [Section 29.9,](#page-0-0) ["Performance Schema Tables for Current and Historical Events"](#page-0-0).

For information about configuring whether to collect stage events, see [Section 29.12.5, "Performance](#page-30-1) [Schema Stage Event Tables"](#page-30-1).

### <span id="page-35-1"></span>**29.12.5.3 The events\_stages\_history\_long Table**

The [events\\_stages\\_history\\_long](#page-35-1) table contains the N most recent stage events that have ended globally, across all threads. Stage events are not added to the table until they have ended. When the table becomes full, the oldest row is discarded when a new row is added, regardless of which thread generated either row.

The Performance Schema autosizes the value of N during server startup. To set the table size explicitly, set the [performance\\_schema\\_events\\_stages\\_history\\_long\\_size](#page-170-1) system variable at server startup.

The [events\\_stages\\_history\\_long](#page-35-1) table has the same columns as [events\\_stages\\_current](#page-34-0). See [Section 29.12.5.1, "The events\\_stages\\_current Table".](#page-34-0) Unlike [events\\_stages\\_current](#page-34-0), [events\\_stages\\_history\\_long](#page-35-1) has no indexing.

TRUNCATE TABLE is permitted for the [events\\_stages\\_history\\_long](#page-35-1) table. It removes the rows.

For more information about the relationship between the three stage event tables, see [Section 29.9,](#page-0-0) ["Performance Schema Tables for Current and Historical Events"](#page-0-0).

For information about configuring whether to collect stage events, see [Section 29.12.5, "Performance](#page-30-1) [Schema Stage Event Tables"](#page-30-1).

# <span id="page-36-0"></span>**29.12.6 Performance Schema Statement Event Tables**
...
- [events\\_statements\\_current](#page-39-0): The current statement event for each thread.
- [events\\_statements\\_history](#page-43-0): The most recent statement events that have ended per thread.
- [events\\_statements\\_history\\_long](#page-43-1): The most recent statement events that have ended globally (across all threads).
- [prepared\\_statements\\_instances](#page-44-0): Prepared statement instances and statistics

The following sections describe the statement event tables. There are also summary tables that aggregate information about statement events; see [Section 29.12.20.3, "Statement Summary Tables"](#page-120-0).

For more information about the relationship between the three events\_statements\_xxx event tables, see [Section 29.9, "Performance Schema Tables for Current and Historical Events".](#page-0-0)

- [Configuring Statement Event Collection](#page-36-1)
- [Statement Monitoring](#page-37-0)

### <span id="page-36-1"></span>**Configuring Statement Event Collection**
...
### <span id="page-37-0"></span>**Statement Monitoring**
...
### <span id="page-39-0"></span>**29.12.6.1 The events\_statements\_current Table**

The [events\\_statements\\_current](#page-39-0) table contains current statement events. The table stores one row per thread showing the current status of the thread's most recent monitored statement event, so there is no system variable for configuring the table size.

Of the tables that contain statement event rows, [events\\_statements\\_current](#page-39-0) is the most fundamental. Other tables that contain statement event rows are logically derived from the current events. For example, the [events\\_statements\\_history](#page-43-0) and [events\\_statements\\_history\\_long](#page-43-1) tables are collections of the most recent statement events that have ended, up to a maximum number of rows per thread and globally across all threads, respectively.

For more information about the relationship between the three events\_statements\_xxx event tables, see [Section 29.9, "Performance Schema Tables for Current and Historical Events".](#page-0-0)

For information about configuring whether to collect statement events, see [Section 29.12.6,](#page-36-0) ["Performance Schema Statement Event Tables".](#page-36-0)

The [events\\_statements\\_current](#page-39-0) table has these columns:
...
The name of the instrument from which the event was collected. This is a NAME value from the [setup\\_instruments](#page-13-0) table. Instrument names may have multiple parts and form a hierarchy, as discussed in Section 29.6, "Performance Schema Instrument Naming Conventions".

For SQL statements, the EVENT\_NAME value initially is statement/com/Query until the statement is parsed, then changes to a more appropriate value, as described in [Section 29.12.6, "Performance](#page-36-0) [Schema Statement Event Tables".](#page-36-0)
...
The statement digest SHA-256 value as a string of 64 hexadecimal characters, or NULL if the statements\_digest consumer is no. For more information about statement digesting, see [Section 29.10, "Performance Schema Statement Digests and Sampling"](#page-2-1).

• DIGEST\_TEXT

The normalized statement digest text, or NULL if the statements\_digest consumer is no. For more information about statement digesting, see [Section 29.10, "Performance Schema Statement](#page-2-1) [Digests and Sampling".](#page-2-1)

The [performance\\_schema\\_max\\_digest\\_length](#page-174-0) system variable determines the maximum number of bytes available per session for digest value storage. However, the display length of statement digests may be longer than the available buffer size due to encoding of statement elements such as keywords and literal values in digest buffer. Consequently, values selected from the DIGEST\_TEXT column of statement event tables may appear to exceed the [performance\\_schema\\_max\\_digest\\_length](#page-174-0) value.
...
The number of rows affected by the statement. For a description of the meaning of "affected," see [mysql\\_affected\\_rows\(\).](https://dev.mysql.com/doc/c-api/8.4/en/mysql-affected-rows.md)
...
The [events\\_statements\\_current](#page-39-0) table has these indexes:

• Primary key on (THREAD\_ID, EVENT\_ID)

TRUNCATE TABLE is permitted for the [events\\_statements\\_current](#page-39-0) table. It removes the rows.

### <span id="page-43-0"></span>**29.12.6.2 The events\_statements\_history Table**

The [events\\_statements\\_history](#page-43-0) table contains the N most recent statement events that have ended per thread. Statement events are not added to the table until they have ended. When the table contains the maximum number of rows for a given thread, the oldest thread row is discarded when a new row for that thread is added. When a thread ends, all its rows are discarded.

The Performance Schema autosizes the value of N during server startup. To set the number of rows per thread explicitly, set the [performance\\_schema\\_events\\_statements\\_history\\_size](#page-171-1) system variable at server startup.

The [events\\_statements\\_history](#page-43-0) table has the same columns and indexing as [events\\_statements\\_current](#page-39-0). See [Section 29.12.6.1, "The events\\_statements\\_current Table".](#page-39-0)

TRUNCATE TABLE is permitted for the [events\\_statements\\_history](#page-43-0) table. It removes the rows.

For more information about the relationship between the three events\_statements\_xxx event tables, see [Section 29.9, "Performance Schema Tables for Current and Historical Events".](#page-0-0)

For information about configuring whether to collect statement events, see [Section 29.12.6,](#page-36-0) ["Performance Schema Statement Event Tables".](#page-36-0)

## <span id="page-43-1"></span>**29.12.6.3 The events\_statements\_history\_long Table**

The [events\\_statements\\_history\\_long](#page-43-1) table contains the N most recent statement events that have ended globally, across all threads. Statement events are not added to the table until they have ended. When the table becomes full, the oldest row is discarded when a new row is added, regardless of which thread generated either row.

The value of N is autosized at server startup. To set the table size explicitly, set the [performance\\_schema\\_events\\_statements\\_history\\_long\\_size](#page-171-2) system variable at server startup.

The [events\\_statements\\_history\\_long](#page-43-1) table has the same columns as [events\\_statements\\_current](#page-39-0). See [Section 29.12.6.1, "The events\\_statements\\_current Table".](#page-39-0) Unlike [events\\_statements\\_current](#page-39-0), [events\\_statements\\_history\\_long](#page-43-1) has no indexing.

TRUNCATE TABLE is permitted for the [events\\_statements\\_history\\_long](#page-43-1) table. It removes the rows.

For more information about the relationship between the three events\_statements\_xxx event tables, see [Section 29.9, "Performance Schema Tables for Current and Historical Events".](#page-0-0)

For information about configuring whether to collect statement events, see [Section 29.12.6,](#page-36-0) ["Performance Schema Statement Event Tables".](#page-36-0)

### <span id="page-44-0"></span>**29.12.6.4 The prepared\_statements\_instances Table**
...
Information about prepared statements is available in the [prepared\\_statements\\_instances](#page-44-0) table. This table enables inspection of prepared statements used in the server and provides aggregated statistics about them. To control the size of this table, set the [performance\\_schema\\_max\\_prepared\\_statements\\_instances](#page-179-0) system variable at server startup.
...
The Performance Schema manages the contents of the [prepared\\_statements\\_instances](#page-44-0) table as follows:

#### • Statement preparation

A COM\_STMT\_PREPARE or SQLCOM\_PREPARE command creates a prepared statement in the server. If the statement is successfully instrumented, a new row is added to the [prepared\\_statements\\_instances](#page-44-0) table. If the statement cannot be instrumented, [Performance\\_schema\\_prepared\\_statements\\_lost](#page-189-0) status variable is incremented.

• Prepared statement execution

Execution of a COM\_STMT\_EXECUTE or SQLCOM\_PREPARE command for an instrumented prepared statement instance updates the corresponding [prepared\\_statements\\_instances](#page-44-0) table row.

• Prepared statement deallocation

Execution of a COM\_STMT\_CLOSE or SQLCOM\_DEALLOCATE\_PREPARE command for an instrumented prepared statement instance removes the corresponding [prepared\\_statements\\_instances](#page-44-0) table row. To avoid resource leaks, removal occurs even if the prepared statement instruments described previously are disabled.

The [prepared\\_statements\\_instances](#page-44-0) table has these columns:
...
The remaining SUM\_xxx columns are the same as for the statement summary tables (see [Section 29.12.20.3, "Statement Summary Tables"\)](#page-120-0).
...
The [prepared\\_statements\\_instances](#page-44-0) table has these indexes:
...
TRUNCATE TABLE resets the statistics columns of the [prepared\\_statements\\_instances](#page-44-0) table.

# <span id="page-46-0"></span>**29.12.7 Performance Schema Transaction Tables**
...
- [events\\_transactions\\_current](#page-50-0): The current transaction event for each thread.
- [events\\_transactions\\_history](#page-53-0): The most recent transaction events that have ended per thread.

• [events\\_transactions\\_history\\_long](#page-53-1): The most recent transaction events that have ended globally (across all threads).

The following sections describe the transaction event tables. There are also summary tables that aggregate information about transaction events; see [Section 29.12.20.5, "Transaction Summary](#page-126-0) [Tables"](#page-126-0).

For more information about the relationship between the three transaction event tables, see [Section 29.9, "Performance Schema Tables for Current and Historical Events".](#page-0-0)

- [Configuring Transaction Event Collection](#page-47-0)
- [Transaction Boundaries](#page-48-0)
- [Transaction Instrumentation](#page-49-0)
- [Transactions and Nested Events](#page-49-1)
- [Transactions and Stored Programs](#page-50-1)
- [Transactions and Savepoints](#page-50-2)
- [Transactions and Errors](#page-50-3)

## <span id="page-47-0"></span>**Configuring Transaction Event Collection**
...
### <span id="page-48-0"></span>**Transaction Boundaries**
...
## <span id="page-49-0"></span>**Transaction Instrumentation**
...
### <span id="page-49-1"></span>**Transactions and Nested Events**
...
## <span id="page-50-1"></span>**Transactions and Stored Programs**
...
### <span id="page-50-2"></span>**Transactions and Savepoints**
...
### <span id="page-50-3"></span>**Transactions and Errors**
...
### <span id="page-50-0"></span>**29.12.7.1 The events\_transactions\_current Table**

The [events\\_transactions\\_current](#page-50-0) table contains current transaction events. The table stores one row per thread showing the current status of the thread's most recent monitored transaction event, so there is no system variable for configuring the table size. For example:
...
Of the tables that contain transaction event rows, [events\\_transactions\\_current](#page-50-0) is the most fundamental. Other tables that contain transaction event rows are logically derived from the current events. For example, the [events\\_transactions\\_history](#page-53-0) and [events\\_transactions\\_history\\_long](#page-53-1) tables are collections of the most recent transaction events that have ended, up to a maximum number of rows per thread and globally across all threads, respectively.

For more information about the relationship between the three transaction event tables, see [Section 29.9, "Performance Schema Tables for Current and Historical Events".](#page-0-0)

For information about configuring whether to collect transaction events, see [Section 29.12.7,](#page-46-0) ["Performance Schema Transaction Tables"](#page-46-0).

The [events\\_transactions\\_current](#page-50-0) table has these columns:
...
On a replica, the same XA transaction can appear in the [events\\_transactions\\_current](#page-50-0) table with different states on different threads. This is because immediately after the XA transaction is prepared, it is detached from the replica's applier thread, and can be committed or rolled back by any thread on the replica. The [events\\_transactions\\_current](#page-50-0) table displays the current status of the most recent monitored transaction event on the thread, and does not update this status when the thread is idle. So the XA transaction can still be displayed in the PREPARED state for the original applier thread, after it has been processed by another thread. To positively identify XA transactions that are still in the PREPARED state and need to be recovered, use the XA RECOVER statement rather than the Performance Schema transaction tables.
...
The [events\\_transactions\\_current](#page-50-0) table has these indexes:

• Primary key on (THREAD\_ID, EVENT\_ID)

TRUNCATE TABLE is permitted for the [events\\_transactions\\_current](#page-50-0) table. It removes the rows.

### <span id="page-53-0"></span>**29.12.7.2 The events\_transactions\_history Table**

The [events\\_transactions\\_history](#page-53-0) table contains the N most recent transaction events that have ended per thread. Transaction events are not added to the table until they have ended. When the table contains the maximum number of rows for a given thread, the oldest thread row is discarded when a new row for that thread is added. When a thread ends, all its rows are discarded.

The Performance Schema autosizes the value of N during server startup. To set the number of rows per thread explicitly, set the [performance\\_schema\\_events\\_transactions\\_history\\_size](#page-172-0) system variable at server startup.

The [events\\_transactions\\_history](#page-53-0) table has the same columns and indexing as [events\\_transactions\\_current](#page-50-0). See [Section 29.12.7.1, "The events\\_transactions\\_current Table".](#page-50-0)

TRUNCATE TABLE is permitted for the [events\\_transactions\\_history](#page-53-0) table. It removes the rows.

For more information about the relationship between the three transaction event tables, see [Section 29.9, "Performance Schema Tables for Current and Historical Events".](#page-0-0)

For information about configuring whether to collect transaction events, see [Section 29.12.7,](#page-46-0) ["Performance Schema Transaction Tables"](#page-46-0).

### <span id="page-53-1"></span>**29.12.7.3 The events\_transactions\_history\_long Table**

The [events\\_transactions\\_history\\_long](#page-53-1) table contains the N most recent transaction events that have ended globally, across all threads. Transaction events are not added to the table until they have ended. When the table becomes full, the oldest row is discarded when a new row is added, regardless of which thread generated either row.

The Performance Schema autosizes the value of N is autosized at server startup. To set the table size explicitly, set the [performance\\_schema\\_events\\_transactions\\_history\\_long\\_size](#page-172-1) system variable at server startup.

The [events\\_transactions\\_history\\_long](#page-53-1) table has the same columns as [events\\_transactions\\_current](#page-50-0). See [Section 29.12.7.1, "The events\\_transactions\\_current Table".](#page-50-0) Unlike [events\\_transactions\\_current](#page-50-0), [events\\_transactions\\_history\\_long](#page-53-1) has no indexing.

TRUNCATE TABLE is permitted for the [events\\_transactions\\_history\\_long](#page-53-1) table. It removes the rows.

For more information about the relationship between the three transaction event tables, see [Section 29.9, "Performance Schema Tables for Current and Historical Events".](#page-0-0)

For information about configuring whether to collect transaction events, see [Section 29.12.7,](#page-46-0) ["Performance Schema Transaction Tables"](#page-46-0).

# <span id="page-54-0"></span>**29.12.8 Performance Schema Connection Tables**
...
- [accounts](#page-56-0): Connection statistics per client account
- [hosts](#page-56-1): Connection statistics per client host name
- [users](#page-57-0): Connection statistics per client user name
...
- The [accounts](#page-56-0) table has four rows, for the user1/hosta, user1/hostb, user2/hosta, and user2/hostb account values, each row counting one connection per account.
- The [hosts](#page-56-1) table has two rows, for hosta and hostb, each row counting two connections per host name.
- The [users](#page-57-0) table has two rows, for user1 and user2, each row counting two connections per user name.
...
- Wait event summaries: [Section 29.12.20.1, "Wait Event Summary Tables"](#page-116-0)
- Stage event summaries: [Section 29.12.20.2, "Stage Summary Tables"](#page-118-0)
- Statement event summaries: [Section 29.12.20.3, "Statement Summary Tables"](#page-120-0)
- Transaction event summaries: [Section 29.12.20.5, "Transaction Summary Tables"](#page-126-0)
- Memory event summaries: [Section 29.12.20.10, "Memory Summary Tables"](#page-134-0)
- Error event summaries: [Section 29.12.20.11, "Error Summary Tables"](#page-138-0)
...
Truncating a \_summary\_global summary table also implicitly truncates its corresponding connection and thread summary tables. For example, truncating [events\\_waits\\_summary\\_global\\_by\\_event\\_name](#page-116-0) implicitly truncates the wait event summary tables that are aggregated by account, host, user, or thread.

## <span id="page-56-0"></span>**29.12.8.1 The accounts Table**

The [accounts](#page-56-0) table contains a row for each account that has connected to the MySQL server. For each account, the table counts the current and total number of connections. The table size is autosized at server startup. To set the table size explicitly, set the [performance\\_schema\\_accounts\\_size](#page-169-0) system variable at server startup. To disable account statistics, set this variable to 0.

The [accounts](#page-56-0) table has the following columns. For a description of how the Performance Schema maintains rows in this table, including the effect of TRUNCATE TABLE, see [Section 29.12.8,](#page-54-0) ["Performance Schema Connection Tables".](#page-54-0)
...
The [accounts](#page-56-0) table has these indexes:

• Primary key on (USER, HOST)

### <span id="page-56-1"></span>**29.12.8.2 The hosts Table**

The [hosts](#page-56-1) table contains a row for each host from which clients have connected to the MySQL server. For each host name, the table counts the current and total number of connections. The table size is autosized at server startup. To set the table size explicitly, set the [performance\\_schema\\_hosts\\_size](#page-173-2) system variable at server startup. To disable host statistics, set this variable to 0.

The [hosts](#page-56-1) table has the following columns. For a description of how the Performance Schema maintains rows in this table, including the effect of TRUNCATE TABLE, see [Section 29.12.8,](#page-54-0) ["Performance Schema Connection Tables".](#page-54-0)
...
The [hosts](#page-56-1) table has these indexes:

• Primary key on (HOST)

### <span id="page-57-0"></span>**29.12.8.3 The users Table**

The [users](#page-57-0) table contains a row for each user who has connected to the MySQL server. For each user name, the table counts the current and total number of connections. The table size is autosized at server startup. To set the table size explicitly, set the [performance\\_schema\\_users\\_size](#page-187-0) system variable at server startup. To disable user statistics, set this variable to 0.

The [users](#page-57-0) table has the following columns. For a description of how the Performance Schema maintains rows in this table, including the effect of TRUNCATE TABLE, see [Section 29.12.8,](#page-54-0) ["Performance Schema Connection Tables".](#page-54-0)
...
The [users](#page-57-0) table has these indexes:

• Primary key on (USER)

# <span id="page-58-1"></span>**29.12.9 Performance Schema Connection Attribute Tables**

Connection attributes are key-value pairs that application programs can pass to the server at connect time. For applications based on the C API implemented by the libmysqlclient client library, the [mysql\\_options\(\)](https://dev.mysql.com/doc/c-api/8.4/en/mysql-options.md) and [mysql\\_options4\(\)](https://dev.mysql.com/doc/c-api/8.4/en/mysql-options4.md) functions define the connection attribute set. Other MySQL Connectors may provide their own attribute-definition methods.

These Performance Schema tables expose attribute information:

- [session\\_account\\_connect\\_attrs](#page-61-0): Connection attributes for the current session, and other sessions associated with the session account
- [session\\_connect\\_attrs](#page-61-1): Connection attributes for all sessions
...
- [Available Connection Attributes](#page-58-0)
- [Connection Attribute Limits](#page-60-0)

### <span id="page-58-0"></span>**Available Connection Attributes**
...
- \_connector\_version: The connector version (for example, 8.4.8) (c-ext only).
...
## <span id="page-60-0"></span>**Connection Attribute Limits**
...
- For accepted connections, the Performance Schema checks aggregate attribute size against the value of the [performance\\_schema\\_session\\_connect\\_attrs\\_size](#page-185-0) system variable. If attribute size exceeds this value, these actions take place:
  - The Performance Schema truncates the attribute data and increments the [Performance\\_schema\\_session\\_connect\\_attrs\\_lost](#page-190-1) status variable, which indicates the number of connections for which attribute truncation occurred.
...
### <span id="page-61-0"></span>**29.12.9.1 The session\_account\_connect\_attrs Table**

Application programs can provide key-value connection attributes to be passed to the server at connect time. For descriptions of common attributes, see [Section 29.12.9, "Performance Schema Connection](#page-58-1) [Attribute Tables"](#page-58-1).

The [session\\_account\\_connect\\_attrs](#page-61-0) table contains connection attributes only for the current session, and other sessions associated with the session account. To see connection attributes for all sessions, use the [session\\_connect\\_attrs](#page-61-1) table.

The [session\\_account\\_connect\\_attrs](#page-61-0) table has these columns:
...
The [session\\_account\\_connect\\_attrs](#page-61-0) table has these indexes:

• Primary key on (PROCESSLIST\_ID, ATTR\_NAME)

TRUNCATE TABLE is not permitted for the [session\\_account\\_connect\\_attrs](#page-61-0) table.

### <span id="page-61-1"></span>**29.12.9.2 The session\_connect\_attrs Table**

Application programs can provide key-value connection attributes to be passed to the server at connect time. For descriptions of common attributes, see [Section 29.12.9, "Performance Schema Connection](#page-58-1) [Attribute Tables"](#page-58-1).

The [session\\_connect\\_attrs](#page-61-1) table contains connection attributes for all sessions. To see connection attributes only for the current session, and other sessions associated with the session account, use the [session\\_account\\_connect\\_attrs](#page-61-0) table.

The [session\\_connect\\_attrs](#page-61-1) table has these columns:
...
The [session\\_connect\\_attrs](#page-61-1) table has these indexes:

• Primary key on (PROCESSLIST\_ID, ATTR\_NAME)

TRUNCATE TABLE is not permitted for the [session\\_connect\\_attrs](#page-61-1) table.

# <span id="page-62-0"></span>**29.12.10 Performance Schema User-Defined Variable Tables**

The Performance Schema provides a [user\\_variables\\_by\\_thread](#page-62-0) table that exposes user-defined variables. These are variables defined within a specific session and include a @ character preceding the name; see Section 11.4, "User-Defined Variables".

The [user\\_variables\\_by\\_thread](#page-62-0) table has these columns:
...
The [user\\_variables\\_by\\_thread](#page-62-0) table has these indexes:

• Primary key on (THREAD\_ID, VARIABLE\_NAME)

TRUNCATE TABLE is not permitted for the [user\\_variables\\_by\\_thread](#page-62-0) table.