...
my\_row\_id is an invisible column, which means it is not shown in the output of SELECT \* or TABLE; the column must be selected explicitly by name. See [Section 15.1.20.10, "Invisible Columns"](#page-191-0).

When GIPKs are enabled, a generated primary key cannot be altered other than to switch it between VISIBLE and INVISIBLE. To make the generated invisible primary key on auto\_1 visible, execute this [ALTER TABLE](#page-88-1) statement:
...
The setting for sql\_generate\_invisible\_primary\_key is not replicated, and is ignored by replication applier threads. This means that the setting of this variable on the source has no effect on the replica. You can cause the replica to add a GIPK for tables replicated without primary keys on a given replication channel using REQUIRE\_TABLE\_PRIMARY\_KEY\_CHECK = GENERATE as part of a CHANGE REPLICATION SOURCE TO statement.

GIPKs work with row-based replication of [CREATE TABLE ... SELECT](#page-172-0); the information written to the binary log for this statement in such cases includes the GIPK definition, and thus is replicated correctly. Statement-based replication of CREATE TABLE ... SELECT is not supported with sql\_generate\_invisible\_primary\_key = ON.

When creating or importing backups of installations where GIPKs are in use, it is possible to exclude generated invisible primary key columns and values. The --skip-generated-invisibleprimary-key option for mysqldump causes GIPK information to be excluded in the program's output.

# <span id="page-197-0"></span>**15.1.20.12 Setting NDB Comment Options**

- [NDB\\_COLUMN Options](#page-197-1)
- NDB\_TABLE Options
...
# <span id="page-197-1"></span>**NDB\_COLUMN Options**

 In NDB Cluster, a column comment in a CREATE TABLE or [ALTER TABLE](#page-88-1) statement can also be used to specify an NDB\_COLUMN option. NDB supports two column comment options BLOB\_INLINE\_SIZE and MAX\_BLOB\_PART\_SIZE. Syntax for these options is shown here:
...
BLOB\_INLINE\_SIZE has no effect on TINYBLOB columns. In NDB 8.4.4 and later, it is disallowed with TINYBLOB, and causes a warning if used.
...
## **NDB\_TABLE Options**
 ...
 For an NDB Cluster table, the table comment in a CREATE TABLE or ALTER TABLE statement can also be used to specify an NDB\_TABLE option, which consists of one or more name-value pairs, separated by commas if need be, following the string NDB\_TABLE=. Complete syntax for names and values syntax is shown here:
...
The TABLE\_COMMENT column displays the comment that is required to re-create the table as it is following the ALTER TABLE statement, like this:
...
# <span id="page-3-0"></span>**15.1.21 CREATE TABLESPACE Statement**
...
- [Considerations for InnoDB](#page-4-0)
- [Considerations for NDB Cluster](#page-4-1)
- [Options](#page-5-0)
- [Notes](#page-8-0)
- [InnoDB Examples](#page-9-0)
- [NDB Example](#page-10-0)

# <span id="page-4-0"></span>**Considerations for InnoDB**

[CREATE TABLESPACE](#page-3-0) syntax is used to create general tablespaces or undo tablespaces. The UNDO keyword must be specified to create an undo tablespace.
...
After creating an InnoDB general tablespace, use CREATE TABLE tbl\_name ... TABLESPACE [=] tablespace\_name or ALTER TABLE tbl\_name TABLESPACE [=] tablespace\_name to add tables to the tablespace. For more information, see Section 17.6.3.3, "General Tablespaces".
...
## <span id="page-4-1"></span>**Considerations for NDB Cluster**
...
When [CREATE TABLESPACE](#page-3-0) is used with ENGINE [=] NDB, a tablespace and associated data file are created on each Cluster data node. You can verify that the data files were created and obtain information about them by querying the Information Schema FILES table. (See the example later in this section.)

(See Section 28.3.15, "The INFORMATION\_SCHEMA FILES Table".)

# <span id="page-5-0"></span>**Options**
...
- FILE\_BLOCK\_SIZE: This option—which is specific to InnoDB general tablespaces, and is ignored by NDB—defines the block size for the tablespace data file. Values can be specified in bytes or kilobytes. For example, an 8 kilobyte file block size can be specified as 8192 or 8K. If
...
The INITIAL\_SIZE parameter sets the total size in bytes of the data file that was specific using ADD DATATFILE. Once this file has been created, its size cannot be changed; however, you can add more data files to the tablespace using ALTER TABLESPACE ... ADD DATAFILE.
...
If the ENCRYPTION clause is not specified, the default\_table\_encryption setting controls whether encryption is enabled. The ENCRYPTION clause overrides the default\_table\_encryption setting. However, if the table\_encryption\_privilege\_check variable is enabled, the TABLE\_ENCRYPTION\_ADMIN privilege is required to use an ENCRYPTION clause setting that differs from the default\_table\_encryption setting.
...
- General tablespaces support the addition of tables of any row format using CREATE TABLE ... TABLESPACE. innodb\_file\_per\_table does not need to be enabled.

- innodb\_strict\_mode is not applicable to general tablespaces. Tablespace management rules are strictly enforced independently of innodb\_strict\_mode. If CREATE TABLESPACE parameters are incorrect or incompatible, the operation fails regardless of the innodb\_strict\_mode setting. When a table is added to a general tablespace using CREATE TABLE ... TABLESPACE or ALTER TABLE ... TABLESPACE, innodb\_strict\_mode is ignored but the statement is evaluated as if innodb\_strict\_mode is enabled.
- Use DROP TABLESPACE to remove a tablespace. All tables must be dropped from a tablespace using [DROP TABLE](#page-20-0) prior to dropping the tablespace. Before dropping an NDB Cluster tablespace you must also remove all its data files using one or more ALTER TABLESPACE ... DROP DATATFILE statements. See Section 25.6.11.1, "NDB Cluster Disk Data Objects".
...
- mysqldump does not dump InnoDB [CREATE TABLESPACE](#page-3-0) statements.

## <span id="page-9-0"></span>**InnoDB Examples**
...
## <span id="page-10-0"></span>**NDB Example**
...
# <span id="page-10-1"></span>**15.1.22 CREATE TRIGGER Statement**
...
This section describes [CREATE TRIGGER](#page-10-1) syntax. For additional discussion, see Section 27.3.1, "Trigger Syntax and Examples".

[CREATE TRIGGER](#page-10-1) requires the TRIGGER privilege for the table associated with the trigger. If the DEFINER clause is present, the privileges required depend on the user value, as discussed in Section 27.6, "Stored Object Access Control". If binary logging is enabled, [CREATE TRIGGER](#page-10-1) might require the SUPER privilege, as discussed in Section 27.7, "Stored Program Binary Logging".
...
- [INSERT](#page-36-0): The trigger activates whenever a new row is inserted into the table (for example, through [INSERT](#page-36-0), [LOAD DATA](#page-47-0), and [REPLACE](#page-67-0) statements).
- [UPDATE](#page-108-0): The trigger activates whenever a row is modified (for example, through [UPDATE](#page-108-0) statements).
- [DELETE](#page-27-0): The trigger activates whenever a row is deleted from the table (for example, through [DELETE](#page-27-0) and [REPLACE](#page-67-0) statements). [DROP TABLE](#page-20-0) and [TRUNCATE TABLE](#page-24-0) statements on the table do not activate this trigger, because they do not use [DELETE](#page-27-0). Dropping a partition does not activate [DELETE](#page-27-0) triggers, either.

The trigger\_event does not represent a literal type of SQL statement that activates the trigger so much as it represents a type of table operation. For example, an [INSERT](#page-36-0) trigger activates not only for [INSERT](#page-36-0) statements but also [LOAD DATA](#page-47-0) statements because both statements insert rows into a table.
...
trigger\_body is the statement to execute when the trigger activates. To execute multiple statements, use the [BEGIN ... END](#page-177-0) compound statement construct. This also enables you to use the same statements that are permitted within stored routines. See [Section 15.6.1, "BEGIN ... END](#page-177-0) [Compound Statement".](#page-177-0) Some statements are not permitted in triggers; see Section 27.8, "Restrictions on Stored Programs".
...
Triggers cannot use NEW.col\_name or use OLD.col\_name to refer to generated columns. For information about generated columns, see Section 15.1.20.8, "CREATE TABLE and Generated Columns".
...
If the DEFINER clause is omitted, the default definer is the user who executes the [CREATE TRIGGER](#page-10-1) statement. This is the same as specifying DEFINER = CURRENT\_USER explicitly.

MySQL takes the DEFINER user into account when checking trigger privileges as follows:

- At [CREATE TRIGGER](#page-10-1) time, the user who issues the statement must have the TRIGGER privilege.
...
If you use [LOCK TABLES](#page-131-0) to lock a table that has triggers, the tables used within the trigger are also locked, as described in [LOCK TABLES and Triggers.](#page-134-0)

For additional discussion of trigger use, see Section 27.3.1, "Trigger Syntax and Examples".

# <span id="page-13-0"></span>**15.1.23 CREATE VIEW Statement**
...
The [CREATE VIEW](#page-13-0) statement creates a new view, or replaces an existing view if the OR REPLACE clause is given. If the view does not exist, [CREATE OR REPLACE VIEW](#page-13-0) is the same as [CREATE VIEW](#page-13-0). If the view does exist, [CREATE OR REPLACE VIEW](#page-13-0) replaces it.

For information about restrictions on view use, see Section 27.9, "Restrictions on Views".

The select\_statement is a [SELECT](#page-70-0) statement that provides the definition of the view. (Selecting from the view selects, in effect, using the [SELECT](#page-70-0) statement.) The select\_statement can select from base tables or from other views. The [SELECT](#page-70-0) statement can use a [VALUES](#page-112-0) statement as its source, or can be replaced with a [TABLE](#page-105-0) statement, as with CREATE TABLE ... SELECT.
...
Unqualified table or view names in the [SELECT](#page-70-0) statement are also interpreted with respect to the default database. A view can refer to tables or views in other databases by qualifying the table or view name with the appropriate database name.

Within a database, base tables and views share the same namespace, so a base table and a view cannot have the same name.

Columns retrieved by the [SELECT](#page-70-0) statement can be simple references to table columns, or expressions that use functions, constant values, operators, and so forth.

A view must have unique column names with no duplicates, just like a base table. By default, the names of the columns retrieved by the [SELECT](#page-70-0) statement are used for the view column names.

To define explicit names for the view columns, specify the optional column\_list clause as a list of comma-separated identifiers. The number of names in column\_list must be the same as the number of columns retrieved by the [SELECT](#page-70-0) statement.

A view can be created from many kinds of [SELECT](#page-70-0) statements. It can refer to base tables or other views. It can use joins, [UNION](#page-111-0), and subqueries. The [SELECT](#page-70-0) need not even refer to any tables:
...
- The [SELECT](#page-70-0) statement cannot refer to system variables or user-defined variables.
- Within a stored program, the [SELECT](#page-70-0) statement cannot refer to program parameters or local variables.
- The [SELECT](#page-70-0) statement cannot refer to prepared statement parameters.
...
- Aliases for column names in the [SELECT](#page-70-0) statement are checked against the maximum column length of 64 characters (not the maximum alias length of 256 characters).
...
For other options or clauses in the definition, they are added to the options or clauses of the statement that references the view, but the effect is undefined. For example, if a view definition includes a LIMIT clause, and you select from the view using a statement that has its own LIMIT clause, it is undefined which limit applies. This same principle applies to options such as ALL, DISTINCT, or SQL\_SMALL\_RESULT that follow the [SELECT](#page-70-0) keyword, and to clauses such as INTO, FOR UPDATE, FOR SHARE, LOCK IN SHARE MODE, and PROCEDURE.
...
If the DEFINER clause is omitted, the default definer is the user who executes the [CREATE VIEW](#page-13-0) statement. This is the same as specifying DEFINER = CURRENT\_USER explicitly.
...
Some views are updatable. That is, you can use them in statements such as [UPDATE](#page-108-0), [DELETE](#page-27-0), or [INSERT](#page-36-0) to update the contents of the underlying table. For a view to be updatable, there must be a one-to-one relationship between the rows in the view and the rows in the underlying table. There are also certain other constructs that make a view nonupdatable.
...
# <span id="page-16-0"></span>**15.1.24 DROP DATABASE Statement**

```
DROP {DATABASE | SCHEMA} [IF EXISTS] db_name
```

[DROP DATABASE](#page-16-0) drops all tables in the database and deletes the database. Be very careful with this statement! To use [DROP DATABASE](#page-16-0), you need the DROP privilege on the database. [DROP SCHEMA](#page-16-0) is a synonym for [DROP DATABASE](#page-16-0).
...
If you use [DROP DATABASE](#page-16-0) on a symbolically linked database, both the link and the original database are deleted.

[DROP DATABASE](#page-16-0) returns the number of tables that were removed.

The [DROP DATABASE](#page-16-0) statement removes from the given database directory those files and directories that MySQL itself may create during normal operation. This includes all files with the extensions shown in the following list:
...
If other files or directories remain in the database directory after MySQL removes those just listed, the database directory cannot be removed. In this case, you must remove any remaining files or directories manually and issue the [DROP DATABASE](#page-16-0) statement again.
...
# <span id="page-17-1"></span>**15.1.25 DROP EVENT Statement**
...
# <span id="page-17-0"></span>**15.1.26 DROP FUNCTION Statement**

The [DROP FUNCTION](#page-17-0) statement is used to drop stored functions and loadable functions:

• For information about dropping stored functions, see [Section 15.1.29, "DROP PROCEDURE and](#page-18-0) [DROP FUNCTION Statements"](#page-18-0).

• For information about dropping loadable functions, see Section 15.7.4.2, "DROP FUNCTION Statement for Loadable Functions".

# <span id="page-18-1"></span>**15.1.27 DROP INDEX Statement**
...
[DROP INDEX](#page-18-1) drops the index named index\_name from the table tbl\_name. This statement is mapped to an ALTER TABLE statement to drop the index. See Section 15.1.9, "ALTER TABLE Statement".
...
# <span id="page-18-2"></span>**15.1.28 DROP LOGFILE GROUP Statement**
...
This statement drops the log file group named logfile\_group. The log file group must already exist or an error results. (For information on creating log file groups, see Section 15.1.16, "CREATE LOGFILE GROUP Statement".)
...
The required ENGINE clause provides the name of the storage engine used by the log file group to be dropped. The only permitted values for engine\_name are NDB and NDBCLUSTER.

[DROP LOGFILE GROUP](#page-18-2) is useful only with Disk Data storage for NDB Cluster. See Section 25.6.11, "NDB Cluster Disk Data Tables".

# <span id="page-18-0"></span>**15.1.29 DROP PROCEDURE and DROP FUNCTION Statements**
...
In addition, if the definer of the routine has the SYSTEM\_USER privilege, the user dropping it must also have this privilege.
...
[DROP FUNCTION](#page-17-0) is also used to drop loadable functions (see Section 15.7.4.2, "DROP FUNCTION Statement for Loadable Functions").

# <span id="page-19-1"></span>**15.1.30 DROP SERVER Statement**
...
DROP SERVER causes an implicit commit. See [Section 15.3.3, "Statements That Cause an Implicit](#page-128-0) [Commit".](#page-128-0)

DROP SERVER is not written to the binary log, regardless of the logging format that is in use.

# <span id="page-19-0"></span>**15.1.31 DROP SPATIAL REFERENCE SYSTEM Statement**
...
• SRID 0 is a valid SRID but cannot be used with [DROP SPATIAL REFERENCE SYSTEM](#page-19-0).
...
# <span id="page-20-0"></span>**15.1.32 DROP TABLE Statement**
...
[DROP TABLE](#page-20-0) removes one or more tables. You must have the DROP privilege for each table.
...
[DROP TABLE](#page-20-0) causes an implicit commit, except when used with the TEMPORARY keyword. See [Section 15.3.3, "Statements That Cause an Implicit Commit"](#page-128-0).
...
If any tables named in the argument list do not exist, [DROP TABLE](#page-20-0) behavior depends on whether the IF EXISTS clause is given:
...
[DROP TABLE](#page-20-0) is not supported with all innodb\_force\_recovery settings. See Section 17.20.3, "Forcing InnoDB Recovery".

# <span id="page-21-0"></span>**15.1.33 DROP TABLESPACE Statement**

```
DROP [UNDO] TABLESPACE tablespace_name
```

This statement drops a tablespace that was previously created using [CREATE TABLESPACE](#page-3-0). It is supported by the NDB and InnoDB storage engines.

The UNDO keyword must be specified to drop an undo tablespace. Only undo tablespaces created using [CREATE UNDO TABLESPACE](#page-3-0) syntax can be dropped. An undo tablespace must be in an empty state before it can be dropped. For more information, see Section 17.6.3.4, "Undo Tablespaces".
...
- A [DROP DATABASE](#page-16-0) operation can drop tables that belong to a general tablespace but it cannot drop the tablespace, even if the operation drops all tables that belong to the tablespace. The tablespace must be dropped explicitly using DROP TABLESPACE tablespace\_name.
- Similar to the system tablespace, truncating or dropping tables stored in a general tablespace creates free space internally in the general tablespace .ibd data file which can only be used for new InnoDB data. Space is not released back to the operating system as it is for file-per-table tablespaces.
...
# <span id="page-22-0"></span>**15.1.34 DROP TRIGGER Statement**

```
DROP TRIGGER [IF EXISTS] [schema_name.]trigger_name
```

This statement drops a trigger. The schema (database) name is optional. If the schema is omitted, the trigger is dropped from the default schema. [DROP TRIGGER](#page-22-0) requires the TRIGGER privilege for the table associated with the trigger.
...
# <span id="page-22-1"></span>**15.1.35 DROP VIEW Statement**
...
[DROP VIEW](#page-22-1) removes one or more views. You must have the DROP privilege for each view.
...
In MySQL 8.3 and earlier, [DROP VIEW](#page-22-1) returns an error if any views named in the argument list do not exist, but also drops all views in the list that do exist. Due to the change in behavior in MySQL 8.4, a partially completed [DROP VIEW](#page-22-1) operation on a MySQL 8.3 replication source server fails when replicated on a MySQL 8.4 replica. To avoid this failure scenario, use IF EXISTS syntax in [DROP VIEW](#page-22-1) statements to prevent an error from occurring for views that do not exist. For more information, see Section 15.1.1, "Atomic Data Definition Statement Support".
...
# <span id="page-22-2"></span>**15.1.36 RENAME TABLE Statement**
...
[RENAME TABLE](#page-22-2) renames one or more tables. You must have ALTER and DROP privileges for the original table, and CREATE and INSERT privileges for the new table.
...
You can rename tables locked with a [LOCK TABLES](#page-131-0) statement, provided that they are locked with a WRITE lock or are the product of renaming WRITE-locked tables from earlier steps in a multiple-table rename operation. For example, this is permitted:
...
If a table has triggers, attempts to rename the table into a different database fail with a Trigger in wrong schema ([ER\\_TRG\\_IN\\_WRONG\\_SCHEMA](https://dev.mysql.com/doc/mysql-errors/8.4/en/server-error-reference.md#error_er_trg_in_wrong_schema)) error.
...
# <span id="page-24-0"></span>**15.1.37 TRUNCATE TABLE Statement**

```
TRUNCATE [TABLE] tbl_name
```

[TRUNCATE TABLE](#page-24-0) empties a table completely. It requires the DROP privilege. Logically, [TRUNCATE](#page-24-0) [TABLE](#page-24-0) is similar to a [DELETE](#page-27-0) statement that deletes all rows, or a sequence of [DROP TABLE](#page-20-0) and CREATE TABLE statements.

To achieve high performance, [TRUNCATE TABLE](#page-24-0) bypasses the DML method of deleting data. Thus, it does not cause ON DELETE triggers to fire, it cannot be performed for InnoDB tables with parent-child foreign key relationships, and it cannot be rolled back like a DML operation. However, TRUNCATE TABLE operations on tables that use a storage engine which supports atomic DDL are either fully committed or rolled back if the server halts during their operation. For more information, see Section 15.1.1, "Atomic Data Definition Statement Support".

Although [TRUNCATE TABLE](#page-24-0) is similar to [DELETE](#page-27-0), it is classified as a DDL statement rather than a DML statement. It differs from [DELETE](#page-27-0) in the following ways:

- Truncate operations drop and re-create the table, which is much faster than deleting rows one by one, particularly for large tables.
- Truncate operations cause an implicit commit, and so cannot be rolled back. See [Section 15.3.3,](#page-128-0) ["Statements That Cause an Implicit Commit".](#page-128-0)
- Truncation operations cannot be performed if the session holds an active table lock.

- [TRUNCATE TABLE](#page-24-0) fails for an InnoDB table or NDB table if there are any FOREIGN KEY constraints from other tables that reference the table. Foreign key constraints between columns of the same table are permitted.
- Truncation operations do not return a meaningful value for the number of deleted rows. The usual result is "0 rows affected," which should be interpreted as "no information."
- As long as the table definition is valid, the table can be re-created as an empty table with [TRUNCATE](#page-24-0) [TABLE](#page-24-0), even if the data or index files have become corrupted.
