...
# <span id="page-7-0"></span>**14.19.1 Aggregate Function Descriptions**
...
If you use an aggregate function in a statement containing no GROUP BY clause, it is equivalent to grouping on all rows. For more information, see [Section 14.19.3, "MySQL Handling of GROUP BY"](#page-23-0).

Most aggregate functions can be used as window functions. Those that can be used this way are signified in their syntax description by [over\_clause], representing an optional OVER clause. over\_clause is described in [Section 14.20.2, "Window Function Concepts and Syntax",](#page-36-0) which also includes other information about window function usage.

For numeric arguments, the variance and standard deviation functions return a DOUBLE value. The [SUM\(\)](#page-16-4) and [AVG\(\)](#page-8-0) functions return a DECIMAL value for exact-value arguments (integer or DECIMAL), and a DOUBLE value for approximate-value arguments (FLOAT or DOUBLE).

The [SUM\(\)](#page-16-4) and [AVG\(\)](#page-8-0) aggregate functions do not work with temporal values. (They convert the values to numbers, losing everything after the first nonnumeric character.) To work around this problem, convert to numeric units, perform the aggregate operation, and convert back to a temporal value. Examples:
...
Functions such as [SUM\(\)](#page-16-4) or [AVG\(\)](#page-8-0) that expect a numeric argument cast the argument to a number if necessary. For SET or ENUM values, the cast operation causes the underlying numeric value to be used.

The [BIT\\_AND\(\)](#page-8-1), [BIT\\_OR\(\)](#page-9-0), and [BIT\\_XOR\(\)](#page-9-1) aggregate functions perform bit operations.
...
<span id="page-8-0"></span>• [AVG\(\[DISTINCT\]](#page-8-0) expr) [over\_clause]

Returns the average value of expr. The DISTINCT option can be used to return the average of the distinct values of expr.

If there are no matching rows, [AVG\(\)](#page-8-0) returns NULL. The function also returns NULL if expr is NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax";](#page-36-0) it cannot be used with DISTINCT.
...
<span id="page-8-1"></span>• BIT\_AND(expr) [[over\\_clause](#page-8-1)]
...
- Binary-string evaluation produces a binary string of the same length as the argument values. If argument values have unequal lengths, an [ER\\_INVALID\\_BITWISE\\_OPERANDS\\_SIZE](https://dev.mysql.com/doc/mysql-errors/8.4/en/server-error-reference.md#error_er_invalid_bitwise_operands_size) error occurs. If the argument size exceeds 511 bytes, an [ER\\_INVALID\\_BITWISE\\_AGGREGATE\\_OPERANDS\\_SIZE](https://dev.mysql.com/doc/mysql-errors/8.4/en/server-error-reference.md#error_er_invalid_bitwise_aggregate_operands_size) error occurs. Numeric evaluation produces an unsigned 64-bit integer.

If there are no matching rows, [BIT\\_AND\(\)](#page-8-1) returns a neutral value (all bits set to 1) having the same length as the argument values.
...
If [BIT\\_AND\(\)](#page-8-1) is invoked from within the mysql client, binary string results display using hexadecimal notation, depending on the value of the --binary-as-hex. For more information about that option, see Section 6.5.1, "mysql — The MySQL Command-Line Client".

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-9-0"></span>• BIT\_OR(expr) [[over\\_clause](#page-9-0)]
...
If there are no matching rows, [BIT\\_OR\(\)](#page-9-0) returns a neutral value (all bits set to 0) having the same length as the argument values.

NULL values do not affect the result unless all values are NULL. In that case, the result is a neutral value having the same length as the argument values.

For more information discussion about argument evaluation and result types, see the introductory discussion in Section 14.12, "Bit Functions and Operators".

If [BIT\\_OR\(\)](#page-9-0) is invoked from within the mysql client, binary string results display using hexadecimal notation, depending on the value of the --binary-as-hex. For more information about that option, see Section 6.5.1, "mysql — The MySQL Command-Line Client".

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-9-1"></span>• BIT\_XOR(expr) [[over\\_clause](#page-9-1)]
...
- Binary-string evaluation occurs when the argument values have a binary string type, and the argument is not a hexadecimal literal, bit literal, or NULL literal. Numeric evaluation occurs otherwise, with argument value conversion to unsigned 64-bit integers as necessary.
- Binary-string evaluation produces a binary string of the same length as the argument values. If argument values have unequal lengths, an [ER\\_INVALID\\_BITWISE\\_OPERANDS\\_SIZE](https://dev.mysql.com/doc/mysql-errors/8.4/en/server-error-reference.md#error_er_invalid_bitwise_operands_size) error occurs. If the argument size exceeds 511 bytes,

an [ER\\_INVALID\\_BITWISE\\_AGGREGATE\\_OPERANDS\\_SIZE](https://dev.mysql.com/doc/mysql-errors/8.4/en/server-error-reference.md#error_er_invalid_bitwise_aggregate_operands_size) error occurs. Numeric evaluation produces an unsigned 64-bit integer.

If there are no matching rows, [BIT\\_XOR\(\)](#page-9-1) returns a neutral value (all bits set to 0) having the same length as the argument values.

NULL values do not affect the result unless all values are NULL. In that case, the result is a neutral value having the same length as the argument values.

For more information discussion about argument evaluation and result types, see the introductory discussion in Section 14.12, "Bit Functions and Operators".

If [BIT\\_XOR\(\)](#page-9-1) is invoked from within the mysql client, binary string results display using hexadecimal notation, depending on the value of the --binary-as-hex. For more information about that option, see Section 6.5.1, "mysql — The MySQL Command-Line Client".

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-10-0"></span>• COUNT(expr) [[over\\_clause](#page-10-0)]

Returns a count of the number of non-NULL values of expr in the rows retrieved by a SELECT statement. The result is a BIGINT value.

If there are no matching rows, [COUNT\(\)](#page-10-0) returns 0. COUNT(NULL) returns 0.
...
[COUNT\(\\*\)](#page-10-0) is somewhat different in that it returns a count of the number of rows retrieved, whether or not they contain NULL values.

For transactional storage engines such as InnoDB, storing an exact row count is problematic. Multiple transactions may be occurring at the same time, each of which may affect the count.

InnoDB does not keep an internal count of rows in a table because concurrent transactions might "see" different numbers of rows at the same time. Consequently, SELECT COUNT(\*) statements only count rows visible to the current transaction.

SELECT COUNT(\*) FROM tbl\_name query performance for InnoDB tables is optimized for singlethreaded workloads if there are no extra clauses such as WHERE or GROUP BY.
...
For MyISAM tables, [COUNT\(\\*\)](#page-10-0) is optimized to return very quickly if the SELECT retrieves from one table, no other columns are retrieved, and there is no WHERE clause. For example:
...
<span id="page-11-0"></span>• [COUNT\(DISTINCT](#page-10-0) expr,[expr...])

Returns a count of the number of rows with different non-NULL expr values.

If there are no matching rows, [COUNT\(DISTINCT\)](#page-10-0) returns 0.

```
mysql> SELECT COUNT(DISTINCT results) FROM student;
```

In MySQL, you can obtain the number of distinct expression combinations that do not contain NULL by giving a list of expressions. In standard SQL, you would have to do a concatenation of all expressions inside [COUNT\(DISTINCT ...\)](#page-10-0).

<span id="page-11-1"></span>• [GROUP\\_CONCAT\(](#page-11-1)expr)
...
The result is truncated to the maximum length that is given by the group\_concat\_max\_len system variable, which has a default value of 1024. The value can be set higher, although the effective maximum length of the return value is constrained by the value of max\_allowed\_packet. The

syntax to change the value of group\_concat\_max\_len at runtime is as follows, where val is an unsigned integer:

SET [GLOBAL | SESSION] group\_concat\_max\_len = val;
...
- Estimate the maximum length required for [GROUP\\_CONCAT\(\)](#page-11-1) output and set the value accordingly.
...
If [GROUP\\_CONCAT\(\)](#page-11-1) is invoked from within the mysql client, binary string results display using hexadecimal notation, depending on the value of the --binary-as-hex. For more information about that option, see Section 6.5.1, "mysql — The MySQL Command-Line Client".

See also CONCAT() and CONCAT\_WS(): Section 14.8, "String Functions and Operators".

<span id="page-12-0"></span>• [JSON\\_ARRAYAGG\(](#page-12-0)col\_or\_expr) [over\_clause]
...
<span id="page-12-1"></span>• [JSON\\_OBJECTAGG\(](#page-12-1)key, value) [over\_clause]
...
<span id="page-15-0"></span>• [MAX\(\[DISTINCT\]](#page-15-0) expr) [over\_clause]

Returns the maximum value of expr. [MAX\(\)](#page-15-0) may take a string argument; in such cases, it returns the maximum string value. See Section 10.3.1, "How MySQL Uses Indexes". The DISTINCT keyword can be used to find the maximum of the distinct values of expr, however, this produces the same result as omitting DISTINCT.

If there are no matching rows, or if expr is NULL, [MAX\(\)](#page-15-0) returns NULL.
...
For [MAX\(\)](#page-15-0), MySQL currently compares ENUM and SET columns by their string value rather than by the string's relative position in the set. This differs from how ORDER BY compares them.

<span id="page-15-1"></span>• [MIN\(\[DISTINCT\]](#page-15-1) expr) [over\_clause]

Returns the minimum value of expr. [MIN\(\)](#page-15-1) may take a string argument; in such cases, it returns the minimum string value. See Section 10.3.1, "How MySQL Uses Indexes". The DISTINCT keyword can be used to find the minimum of the distinct values of expr, however, this produces the same result as omitting DISTINCT.

If there are no matching rows, or if expr is NULL, [MIN\(\)](#page-15-1) returns NULL.
...
For [MIN\(\)](#page-15-1), MySQL currently compares ENUM and SET columns by their string value rather than by the string's relative position in the set. This differs from how ORDER BY compares them.

<span id="page-16-0"></span>• STD(expr) [[over\\_clause](#page-16-0)]

Returns the population standard deviation of expr. [STD\(\)](#page-16-0) is a synonym for the standard SQL function [STDDEV\\_POP\(\)](#page-16-2), provided as a MySQL extension.

If there are no matching rows, or if expr is NULL, [STD\(\)](#page-16-0) returns NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-16-1"></span>• STDDEV(expr) [[over\\_clause](#page-16-1)]

Returns the population standard deviation of expr. [STDDEV\(\)](#page-16-1) is a synonym for the standard SQL function [STDDEV\\_POP\(\)](#page-16-2), provided for compatibility with Oracle.

If there are no matching rows, or if expr is NULL, [STDDEV\(\)](#page-16-1) returns NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-16-2"></span>• [STDDEV\\_POP\(](#page-16-2)expr) [over\_clause]

Returns the population standard deviation of expr (the square root of [VAR\\_POP\(\)](#page-17-0)). You can also use [STD\(\)](#page-16-0) or [STDDEV\(\)](#page-16-1), which are equivalent but not standard SQL.

If there are no matching rows, or if expr is NULL, [STDDEV\\_POP\(\)](#page-16-2) returns NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-16-3"></span>• [STDDEV\\_SAMP\(](#page-16-3)expr) [over\_clause]

Returns the sample standard deviation of expr (the square root of [VAR\\_SAMP\(\)](#page-17-1).

If there are no matching rows, or if expr is NULL, [STDDEV\\_SAMP\(\)](#page-16-3) returns NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-16-4"></span>• [SUM\(\[DISTINCT\]](#page-16-4) expr) [over\_clause]

Returns the sum of expr. If the return set has no rows, [SUM\(\)](#page-16-4) returns NULL. The DISTINCT keyword can be used to sum only the distinct values of expr.

If there are no matching rows, or if expr is NULL, [SUM\(\)](#page-16-4) returns NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax";](#page-36-0) it cannot be used with DISTINCT.

<span id="page-17-0"></span>• VAR\_POP(expr) [[over\\_clause](#page-17-0)]

Returns the population standard variance of expr. It considers rows as the whole population, not as a sample, so it has the number of rows as the denominator. You can also use [VARIANCE\(\)](#page-17-2), which is equivalent but is not standard SQL.

If there are no matching rows, or if expr is NULL, [VAR\\_POP\(\)](#page-17-0) returns NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-17-1"></span>• VAR\_SAMP(expr) [[over\\_clause](#page-17-1)]

Returns the sample variance of expr. That is, the denominator is the number of rows minus one.

If there are no matching rows, or if expr is NULL, [VAR\\_SAMP\(\)](#page-17-1) returns NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

<span id="page-17-2"></span>• VARIANCE(expr) [[over\\_clause](#page-17-2)]

Returns the population standard variance of expr. [VARIANCE\(\)](#page-17-2) is a synonym for the standard SQL function [VAR\\_POP\(\)](#page-17-0), provided as a MySQL extension.

If there are no matching rows, or if expr is NULL, [VARIANCE\(\)](#page-17-2) returns NULL.

This function executes as a window function if over\_clause is present. over\_clause is as described in [Section 14.20.2, "Window Function Concepts and Syntax".](#page-36-0)

# <span id="page-17-3"></span>**14.19.2 GROUP BY Modifiers**
...
The NULL values do appear as NULL on the client side and can be tested as such using any MySQL client programming interface. However, at this point, you cannot distinguish whether a NULL represents a regular grouped value or a super-aggregate value. To test the distinction, use the [GROUPING\(\)](#page-52-0) function, described later.

For GROUP BY ... WITH ROLLUP queries, to test whether NULL values in the result represent super-aggregate values, the [GROUPING\(\)](#page-52-0) function is available for use in the select list, HAVING clause, and ORDER BY clause. For example, [GROUPING\(year\)](#page-52-0) returns 1 when NULL in the year column occurs in a super-aggregate row, and 0 otherwise. Similarly, [GROUPING\(country\)](#page-52-0) and [GROUPING\(product\)](#page-52-0) return 1 for super-aggregate NULL values in the country and product columns, respectively:
...
Instead of displaying the [GROUPING\(\)](#page-52-0) results directly, you can use [GROUPING\(\)](#page-52-0) to substitute labels for super-aggregate NULL values:
...
With multiple expression arguments, [GROUPING\(\)](#page-52-0) returns a result representing a bitmask that combines the results for each expression, with the lowest-order bit corresponding to the result for the rightmost expression. For example, [GROUPING\(year, country, product\)](#page-52-0) is evaluated like this:
...
The result of such a [GROUPING\(\)](#page-52-0) is nonzero if any of the expressions represents a super-aggregate NULL, so you can return only the super-aggregate rows and filter out the regular grouped rows like this:
...
Using [GROUPING\(\)](#page-52-0) to substitute labels for the super-aggregate NULL values makes the result easier to interpret:
...
ORDER BY and ROLLUP can be used together, which enables the use of ORDER BY and [GROUPING\(\)](#page-52-0) to achieve a specific sort order of grouped results. For example:
...
# <span id="page-23-0"></span>**14.19.3 MySQL Handling of GROUP BY**
...
You can achieve the same effect without disabling ONLY\_FULL\_GROUP\_BY by using [ANY\\_VALUE\(\)](#page-50-0) to refer to the nonaggregated column.
...
If you know that, for a given data set, each name value in fact uniquely determines the address value, address is effectively functionally dependent on name. To tell MySQL to accept the query, you can use the [ANY\\_VALUE\(\)](#page-50-0) function:
...
Without GROUP BY, there is a single group and it is nondeterministic which name value to choose for the group. Here, too, [ANY\\_VALUE\(\)](#page-50-0) can be used, if it is immaterial which name value MySQL chooses:
...
# <span id="page-26-0"></span>**14.19.4 Detection of Functional Dependence**
...
- [Functional Dependencies Derived from Keys](#page-27-0)
- [Functional Dependencies Derived from Multiple-Column Keys and from Equalities](#page-27-1)
- [Functional Dependency Special Cases](#page-28-0)
- [Functional Dependencies and Views](#page-29-0)
- [Combinations of Functional Dependencies](#page-29-1)

# <span id="page-27-0"></span>**Functional Dependencies Derived from Keys**
...
# <span id="page-27-1"></span>**Functional Dependencies Derived from Multiple-Column Keys and from Equalities**
...
# <span id="page-28-0"></span>**Functional Dependency Special Cases**
...
# <span id="page-29-0"></span>**Functional Dependencies and Views**
...
# <span id="page-29-1"></span>**Combinations of Functional Dependencies**
