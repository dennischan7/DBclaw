...
The functions in this section perform search or comparison operations on JSON values to extract data from them, report whether data exists at a location within them, or report the path to data within them. The [MEMBER OF\(\)](#page-162-0) operator is also documented herein.

<span id="page-149-0"></span>• [JSON\\_CONTAINS\(](#page-149-0)target, candidate[, path])
...
To check only whether any data exists at the path, use [JSON\\_CONTAINS\\_PATH\(\)](#page-150-0) instead.

The following rules define containment:

- A candidate scalar is contained in a target scalar if and only if they are comparable and are equal. Two scalar values are comparable if they have the same [JSON\\_TYPE\(\)](#page-173-0) types, with the exception that values of types INTEGER and DECIMAL are also comparable to each other.
...
Queries using JSON\_CONTAINS() on InnoDB tables can be optimized using multi-valued indexes; see Multi-Valued Indexes, for more information.
...
<span id="page-150-0"></span>• [JSON\\_CONTAINS\\_PATH\(](#page-150-0)json\_doc, one\_or\_all, path[, path] ...)
...
To check for a specific value at a path, use [JSON\\_CONTAINS\(\)](#page-149-0) instead.
...
<span id="page-150-1"></span>• [JSON\\_EXTRACT\(](#page-150-1)json\_doc, path[, path] ...)
...
MySQL supports the [->](#page-151-0) operator as shorthand for this function as used with 2 arguments where the left hand side is a JSON column identifier (not an expression) and the right hand side is the JSON path to be matched within the column.

#### <span id="page-151-0"></span>• [column](#page-151-0)->path

The [->](#page-151-0) operator serves as an alias for the [JSON\\_EXTRACT\(\)](#page-150-1) function when used with two arguments, a column identifier on the left and a JSON path (a string literal) on the right that is evaluated against the JSON document (the column value). You can use such expressions in place of column references wherever they occur in SQL statements.
...
<span id="page-153-0"></span>• [column](#page-153-0)->>path
...
- [JSON\\_UNQUOTE\(](#page-170-1) [JSON\\_EXTRACT\(](#page-150-1)column, path) )
- JSON\_UNQUOTE(column [->](#page-151-0) path)
- column->>path
...
As with [->](#page-151-0), the ->> operator is always expanded in the output of EXPLAIN, as the following example demonstrates:
...
This is similar to how MySQL expands the [->](#page-151-0) operator in the same circumstances.

<span id="page-154-0"></span>• [JSON\\_KEYS\(](#page-154-0)json\_doc[, path])
...
json\_doc argument is not an object, or path, if given, does not locate an object. An error occurs if the json\_doc argument is not a valid JSON document or the path argument is not a valid path expression or contains a \* or \*\* wildcard.
...
<span id="page-155-0"></span>• [JSON\\_OVERLAPS\(](#page-155-0)json\_doc1, json\_doc2)
...
This function serves as counterpart to [JSON\\_CONTAINS\(\)](#page-149-0), which requires all elements of the array searched for to be present in the array searched in. Thus, JSON\_CONTAINS() performs an AND operation on search keys, while JSON\_OVERLAPS() performs an OR operation.
...
<span id="page-157-0"></span>• [JSON\\_SEARCH\(](#page-157-0)json\_doc, one\_or\_all, search\_str[, escape\_char[, path] ...])
...
Within the search\_str search string argument, the % and \_ characters work as for the [LIKE](#page-1-0) operator: % matches any number of characters (including zero characters), and \_ matches exactly one character.
...
For more information about matching and escape character behavior, see the description of [LIKE](#page-1-0) in [Section 14.8.1, "String Comparison Functions and Operators"](#page-1-1). For escape character handling, a difference from the [LIKE](#page-1-0) behavior is that the escape character for [JSON\\_SEARCH\(\)](#page-157-0) must evaluate to a constant at compile time, not just at execution time. For example, if [JSON\\_SEARCH\(\)](#page-157-0) is used in a prepared statement and the escape\_char argument is supplied using a ? parameter, the parameter value might be constant at execution time, but is not at compile time.
...
| JSON SEARCH(@j, 'all', '%b%') |
| ["$[0]", "$[2].x", "$[3].y"] |
mysql> SELECT JSON SEARCH(@j, 'all', '%b%', NULL, '$[0]');
| JSON SEARCH(@j, 'all', '%b%', NULL, '$[0]') |
| "$[0]"
mysql> SELECT JSON SEARCH(@j, 'all', '%b%', NULL, '$[2]');
| JSON SEARCH(@j, 'all', '%b%', NULL, '$[2]') |
   .....
| "$[2].x"
```
...
<span id="page-159-0"></span>• [JSON\\_VALUE\(](#page-159-0)json\_doc, path)
...
The types just listed are the same as the (non-array) types supported by the [CAST\(\)](#page-42-0) function.
...
<span id="page-162-0"></span>• value [MEMBER OF\(](#page-162-0)json\_array)
...
To use this operator with a value which is itself an array, it is necessary to cast it explicitly as a JSON array. You can do this with [CAST\(... AS JSON\)](#page-42-0):
...
It is also possible to perform the necessary cast using the [JSON\\_ARRAY\(\)](#page-148-0) function, like this:
