...
<span id="page-164-0"></span>• [JSON\\_ARRAY\\_INSERT\(](#page-164-0)json\_doc, path, val[, path, val] ...)
...
Earlier modifications affect the positions of the following elements in the array, so subsequent paths in the same [JSON\\_ARRAY\\_INSERT\(\)](#page-164-0) call should take this into account. In the final example, the second path inserts nothing because the path no longer matches anything after the first insert.
...
For a comparison of [JSON\\_INSERT\(\)](#page-165-0), [JSON\\_REPLACE\(\)](#page-169-1), and [JSON\\_SET\(\)](#page-170-0), see the discussion of [JSON\\_SET\(\)](#page-170-0).
...
<span id="page-165-1"></span>• [JSON\\_MERGE\(](#page-165-1)json\_doc, json\_doc[, json\_doc] ...)

Deprecated synonym for JSON\_MERGE\_PRESERVE().

<span id="page-166-0"></span>• [JSON\\_MERGE\\_PATCH\(](#page-166-0)json\_doc, json\_doc[, json\_doc] ...)
...
For an explanation and example of the differences between this function and JSON\_MERGE\_PRESERVE(), see [JSON\\_MERGE\\_PATCH\(\) compared with](#page-167-0) [JSON\\_MERGE\\_PRESERVE\(\).](#page-167-0)
...
<span id="page-167-0"></span>**JSON\_MERGE\_PATCH() compared with JSON\_MERGE\_PRESERVE().** The behavior of JSON\_MERGE\_PATCH() is the same as that of [JSON\\_MERGE\\_PRESERVE\(\)](#page-168-0), with the following two exceptions:
...
<span id="page-168-0"></span>• [JSON\\_MERGE\\_PRESERVE\(](#page-168-0)json\_doc, json\_doc[, json\_doc] ...)
...
This function is similar to but differs from [JSON\\_MERGE\\_PATCH\(\)](#page-166-0) in significant respects; see [JSON\\_MERGE\\_PATCH\(\) compared with JSON\\_MERGE\\_PRESERVE\(\)](#page-167-0), for more information. <span id="page-169-0"></span>• [JSON\\_REMOVE\(](#page-169-0)json\_doc, path[, path] ...)
...
<span id="page-169-1"></span>• [JSON\\_REPLACE\(](#page-169-1)json\_doc, path, val[, path, val] ...)
...
<span id="page-170-0"></span>• [JSON\\_SET\(](#page-170-0)json\_doc, path, val[, path, val] ...)
...
The optimizer can perform a partial, in-place update of a JSON column instead of removing the old document and writing the new document in its entirety to the column. This optimization can be performed for an update statement that uses the [JSON\\_SET\(\)](#page-170-0) function and meets the conditions outlined in Partial Updates of JSON Values.

The [JSON\\_SET\(\)](#page-170-0), [JSON\\_INSERT\(\)](#page-165-0), and [JSON\\_REPLACE\(\)](#page-169-1) functions are related:

- [JSON\\_SET\(\)](#page-170-0) replaces existing values and adds nonexisting values.
- [JSON\\_INSERT\(\)](#page-165-0) inserts values without replacing existing values.
- [JSON\\_REPLACE\(\)](#page-169-1) replaces only existing values.
...
<span id="page-170-1"></span>• [JSON\\_UNQUOTE\(](#page-170-1)json\_val)
...
## <span id="page-172-0"></span>**14.17.5 Functions That Return JSON Value Attributes**

The functions in this section return attributes of JSON values.

• [JSON\\_DEPTH\(](#page-172-0)json\_doc)
...
<span id="page-172-1"></span>• [JSON\\_LENGTH\(](#page-172-1)json\_doc[, path])
...
<span id="page-173-0"></span>• [JSON\\_TYPE\(](#page-173-0)json\_val)
...
[JSON\\_TYPE\(\)](#page-173-0) returns NULL if the argument is NULL:
...
For a non-NULL, non-error result, the following list describes the possible [JSON\\_TYPE\(\)](#page-173-0) return values:
...
- All other types:
  - OPAQUE (raw bits)
- <span id="page-174-1"></span>• [JSON\\_VALID\(](#page-174-1)val)
