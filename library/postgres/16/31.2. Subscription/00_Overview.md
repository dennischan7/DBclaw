...
Each subscription will receive changes via one replication slot (see [Section 27.2.6](#page-19-0)). Additional replication slots may be required for the initial data synchronization of pre-existing table data and those will be dropped at the end of data synchronization.

A logical replication subscription can be a standby for synchronous replication (see [Section 27.2.8](#page-20-0)). The standby name is by default the subscription name. An alternative name can be specified as application\_name in the connection information of the subscription.
...
## <span id="page-104-0"></span>**31.2.1. Replication Slot Management**
...
## <span id="page-104-1"></span>**31.2.2. Examples: Set Up Logical Replication**
...
Create publications for the tables. The publications pub2 and pub3a disallow some publish operations. The publication pub3b has a row filter (see [Section 31.3](#page-109-0)).
