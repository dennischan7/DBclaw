...
LDAP connection parameter lookup uses the connection service file pg\_service.conf (see [Sec](#page-15-0)[tion 34.17](#page-15-0)). A line in a pg\_service.conf stanza that starts with ldap:// will be recognized as an LDAP URL and an LDAP query will be performed. The result must be a list of keyword = value pairs which will be used to set connection options. The URL must conform to [RFC 1959](https://datatracker.ietf.org/doc/html/rfc1959)<sup>4</sup> and be of the form

ldap://[hostname[:port]]/search\_base?attribute?search\_scope?filter
