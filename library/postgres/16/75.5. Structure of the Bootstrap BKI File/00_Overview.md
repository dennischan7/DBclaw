...
The open command cannot be used until the tables it uses exist and have entries for the table that is to be opened. (These minimum tables are pg\_class, pg\_attribute, pg\_proc, and pg\_type.) To allow those tables themselves to be filled, create with the bootstrap option implicitly opens the created table for data insertion.
