...
SQL defines some string functions that use key words, rather than commas, to separate arguments. Details are in [Table 9.9.](#page-68-0) PostgreSQL also provides versions of these functions that use the regular function invocation syntax (see [Table 9.10\)](#page-70-0).

### **Note**

The string concatenation operator (||) will accept non-string input, so long as at least one input is of string type, as shown in [Table 9.9](#page-68-0). For other cases, inserting an explicit coercion to text can be used to have non-string input accepted.

### <span id="page-68-0"></span>**Table 9.9. SQL String Functions and Operators**
...
Additional string manipulation functions and operators are available and are listed in [Table 9.10.](#page-70-0) (Some of these are used internally to implement the SQL-standard string functions listed in [Table 9.9](#page-68-0).) There are also pattern-matching operators, which are described in [Section 9.7](#page-83-0), and operators for fulltext search, which are described in Chapter 12.

#### <span id="page-70-0"></span>**Table 9.10. Other String Functions and Operators**
...
Formats arguments according to a format string; see [Section 9.4.1.](#page-76-0) This function is similar to the C function sprintf.
...
Computes the MD5 [hash](#page-80-0) of the argument, with the result written in hexadecimal.
...
<span id="page-74-0"></span>string\_to\_array('xx~~yy~~zz', '~~', 'yy') → {xx,NULL,zz}
...
See also the aggregate function string\_agg in [Section 9.21,](#page-192-0) and the functions for converting between strings and the bytea type in [Table 9.13](#page-81-0).

## <span id="page-76-0"></span>**9.4.1. format**
