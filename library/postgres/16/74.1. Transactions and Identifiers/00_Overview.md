...
When a top-level transaction with a (non-virtual) xid commits, it is marked as committed in the pg\_xact directory. Additional information is recorded in the pg\_commit\_ts directory if track\_commit\_timestamp is enabled.

In addition to vxid and xid, prepared transactions are also assigned Global Transaction Identifiers (GID). GIDs are string literals up to 200 bytes long, which must be unique amongst other currently prepared transactions. The mapping of GID to xid is shown in pg\_prepared\_xacts.