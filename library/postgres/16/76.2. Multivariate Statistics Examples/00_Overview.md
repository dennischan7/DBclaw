...
### <span id="page-187-0"></span>**76.2.1. Functional Dependencies**
...
As explained in Section 14.2, the planner can determine cardinality of t using the number of pages and rows obtained from pg\_class:
...
As explained in [Section 76.2.1](#page-187-0), functional dependencies are very cheap and efficient type of statistics, but their main limitation is their global nature (only tracking dependencies at the column level, not between individual column values).

This section introduces multivariate variant of MCV (most-common values) lists, a straightforward extension of the per-column statistics described in [Section 76.1](#page-182-0). These statistics address the limitation by storing individual values, but it is naturally more expensive, both in terms of building the statistics in ANALYZE, storage and planning time.

Let's look at the query from [Section 76.2.1](#page-187-0) again, but this time with a MCV list created on the same set of columns (be sure to drop the functional dependencies, to make sure the planner uses the newly created statistics).
...
Inspecting the MCV list is possible using pg\_mcv\_list\_items set-returning function.
...
When estimating the selectivity, the planner applies all the conditions on items in the MCV list, and then sums the frequencies of the matching ones. See mcv\_clauselist\_selectivity in src/ backend/statistics/mcv.c for details.
...
-------------------------------------------------------------------
--------
 Seq Scan on t (cost=0.00..195.00 rows=1 width=8) (actual rows=0
 loops=1)
 Filter: ((a = 1) AND (b = 10))
...
```
EXPLAIN (ANALYZE, TIMING OFF) SELECT * FROM t WHERE a <= 49 AND b >
 49;
 QUERY PLAN
-------------------------------------------------------------------
--------
