...
# <span id="page-101-0"></span>**20.11.1. Statement Behavior**

```
client_min_messages (enum)
```

Controls which [message levels](#page-89-0) are sent to the client. Valid values are DEBUG5, DEBUG4, DE-BUG3, DEBUG2, DEBUG1, LOG, NOTICE, WARNING, and ERROR. Each level includes all the levels that follow it. The later the level, the fewer messages are sent. The default is NOTICE. Note that LOG has a different rank here than in [log\\_min\\_messages.](#page-87-2)
...
This variable is not used for temporary tables; for them, [temp\\_tablespaces](#page-103-0) is consulted instead.
...
For more information on tablespaces, see [Section 23.6](#page-158-0).
...
See also [default\\_tablespace.](#page-103-1)
...
This parameter reflects the current transaction's isolation level. At the beginning of each transaction, it is set to the current value of [default\\_transaction\\_isolation](#page-104-0). Any subsequent attempt to change it is equivalent to a SET TRANSACTION command.

```
transaction_read_only (boolean)
```

This parameter reflects the current transaction's read-only status. At the beginning of each transaction, it is set to the current value of [default\\_transaction\\_read\\_only](#page-104-1). Any subsequent attempt to change it is equivalent to a SET TRANSACTION command.

```
transaction_deferrable (boolean)
```

This parameter reflects the current transaction's deferrability status. At the beginning of each transaction, it is set to the current value of [default\\_transaction\\_deferrable.](#page-104-2) Any subsequent attempt to change it is equivalent to a SET TRANSACTION command.
...
This option can be used to ensure that idle sessions do not hold locks for an unreasonable amount of time. Even when no significant locks are held, an open transaction prevents vacuuming away recently-dead tuples that may be visible only to this transaction; so remaining idle for a long time can contribute to table bloat. See [Section 25.1](#page-185-0) for more details.
...
VACUUM performs an aggressive scan if the table's pg\_class.relfrozenxid field has reached the age specified by this setting. An aggressive scan differs from a regular VACUUM in that it visits every page that might contain unfrozen XIDs or MXIDs, not just those that might contain dead tuples. The default is 150 million transactions. Although users can set this value anywhere from zero to two billion, VACUUM will silently limit the effective value to 95% of [au](#page-101-1)[tovacuum\\_freeze\\_max\\_age,](#page-101-1) so that a periodic manual VACUUM has a chance to run before an anti-wraparound autovacuum is launched for the table. For more information see [Section 25.1.5.](#page-188-0)

```
vacuum_freeze_min_age (integer)
```

Specifies the cutoff age (in transactions) that VACUUM should use to decide whether to trigger freezing of pages that have an older XID. The default is 50 million transactions. Although users can set this value anywhere from zero to one billion, VACUUM will silently limit the effective value to half the value of [autovacuum\\_freeze\\_max\\_age](#page-101-1), so that there is not an unreasonably short time between forced autovacuums. For more information see [Section 25.1.5.](#page-188-0)
...
The default is 1.6 billion transactions. Although users can set this value anywhere from zero to 2.1 billion, VACUUM will silently adjust the effective value to no less than 105% of [autovacu](#page-101-1)[um\\_freeze\\_max\\_age](#page-101-1).

```
vacuum_multixact_freeze_table_age (integer)
```

VACUUM performs an aggressive scan if the table's pg\_class.relminmxid field has reached the age specified by this setting. An aggressive scan differs from a regular VACUUM in that it visits every page that might contain unfrozen XIDs or MXIDs, not just those that might contain dead tuples. The default is 150 million multixacts. Although users can set this value anywhere from zero to two billion, VACUUM will silently limit the effective value to 95% of [autovacuum\\_multix](#page-101-2)[act\\_freeze\\_max\\_age](#page-101-2), so that a periodic manual VACUUM has a chance to run before an anti-wraparound is launched for the table. For more information see [Section 25.1.5.1.](#page-192-1)

```
vacuum_multixact_freeze_min_age (integer)
```

Specifies the cutoff age (in multixacts) that VACUUM should use to decide whether to trigger freezing of pages with an older multixact ID. The default is 5 million multixacts. Although users can set this value anywhere from zero to one billion, VACUUM will silently limit the effective value to half the value of [autovacuum\\_multixact\\_freeze\\_max\\_age,](#page-101-2) so that there is not an unreasonably short time between forced autovacuums. For more information see [Section 25.1.5.1.](#page-192-1)
...
The default is 1.6 billion multixacts. Although users can set this value anywhere from zero to 2.1 billion, VACUUM will silently adjust the effective value to no less than 105% of [autovacuum\\_mul](#page-101-2)[tixact\\_freeze\\_max\\_age.](#page-101-2)
...
## <span id="page-108-3"></span><span id="page-108-2"></span>**20.11.2. Locale and Formatting**
...
Sets the client-side encoding (character set). The default is to use the database encoding. The character sets supported by the PostgreSQL server are described in [Section 24.3.1](#page-175-1).

```
lc_messages (string)
```

Sets the language in which messages are displayed. Acceptable values are system-dependent; see [Section 24.1](#page-161-0) for more information. If this variable is set to the empty string (which is the default) then the value is inherited from the execution environment of the server in a system-dependent way.
...
Sets the locale to use for formatting monetary amounts, for example with the to\_char family of functions. Acceptable values are system-dependent; see [Section 24.1](#page-161-0) for more information. If this variable is set to the empty string (which is the default) then the value is inherited from the execution environment of the server in a system-dependent way.
...
When ICU locale validation problems are encountered, controls which [message level](#page-89-0) is used to report the problem. Valid values are DISABLED, DEBUG5, DEBUG4, DEBUG3, DEBUG2, DEBUG1, INFO, NOTICE, WARNING, ERROR, and LOG.
