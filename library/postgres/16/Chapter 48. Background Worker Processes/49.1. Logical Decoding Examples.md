...
title: 49.1. Logical Decoding Examples
---
...
The following examples shows how logical decoding is controlled over the streaming replication protocol, using the program pg\_recvlogical included in the PostgreSQL distribution. This requires that client authentication is set up to allow replication connections (see Section 27.2.5.1) and that max\_wal\_senders is set sufficiently high to allow an additional connection. The second example shows how to stream two-phase transactions. Before you use two-phase commands, you must set max\_prepared\_transactions to at least 1.
