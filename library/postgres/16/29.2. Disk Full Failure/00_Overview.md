...
- On Windows, if wal\_sync\_method is open\_datasync (the default), write caching can be disabled by unchecking My Computer\Open\disk drive\Properties\Hardware\Properties\Policies\Enable write caching on the disk. Alternative-

<sup>1</sup> <http://sg.danny.cz/sg/sdparm.html>

ly, set wal\_sync\_method to fdatasync (NTFS only), fsync or fsync\_writethrough, which prevent write caching.

• On macOS, write caching can be prevented by setting wal\_sync\_method to fsync\_writethrough.
