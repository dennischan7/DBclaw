...
The sorted method is only available if each of the opclasses used by the index provides a sortsupport function, as described in [Section 68.3](#page-112-0). If they do, this method is usually the best, so it is used by default.
...
If sorting is not possible, then by default a GiST index build switches to the buffering method when the index size reaches effective\_cache\_size. Buffering can be manually forced or prevented by the buffering parameter to the CREATE INDEX command. The default behavior is good for most cases, but turning buffering off might speed up the build somewhat if the input data is ordered.