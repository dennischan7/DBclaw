...
When changing this value, consider also adjusting [max\\_parallel\\_workers,](#page-41-1) [max\\_parallel\\_mainte](#page-41-2)[nance\\_workers,](#page-41-2) and [max\\_parallel\\_workers\\_per\\_gather.](#page-41-3)

```
max_parallel_workers_per_gather (integer)
```

Sets the maximum number of workers that can be started by a single Gather or Gather Merge node. Parallel workers are taken from the pool of processes established by [max\\_worker\\_process](#page-41-0)[es](#page-41-0), limited by [max\\_parallel\\_workers.](#page-41-1) Note that the requested number of workers may not actually be available at run time. If this occurs, the plan will run with fewer workers than expected, which may be inefficient. The default value is 2. Setting this value to 0 disables parallel query execution.

Note that parallel queries may consume very substantially more resources than non-parallel queries, because each worker process is a completely separate process which has roughly the same impact on the system as an additional user session. This should be taken into account when choosing a value for this setting, as well as when configuring other settings that control resource utilization, such as [work\\_mem](#page-35-0). Resource limits such as work\_mem are applied individually to each worker, which means the total utilization may be much higher across all processes than it would normally be for any single process. For example, a parallel query using 4 workers may use up to 5 times as much CPU time, memory, I/O bandwidth, and so forth as a query which uses no workers at all.
...
Sets the maximum number of parallel workers that can be started by a single utility command. Currently, the parallel utility commands that support the use of parallel workers are CREATE INDEX only when building a B-tree index, and VACUUM without FULL option. Parallel workers are taken from the pool of processes established by [max\\_worker\\_processes](#page-41-0), limited by [max\\_par](#page-41-1)[allel\\_workers](#page-41-1). Note that the requested number of workers may not actually be available at run time. If this occurs, the utility operation will run with fewer workers than expected. The default value is 2. Setting this value to 0 disables the use of parallel workers by utility commands.
...
Sets the maximum number of workers that the system can support for parallel operations. The default value is 8. When increasing or decreasing this value, consider also adjusting [max\\_paral](#page-41-2)[lel\\_maintenance\\_workers](#page-41-2) and [max\\_parallel\\_workers\\_per\\_gather](#page-41-3). Also, note that a setting for this value which is higher than [max\\_worker\\_processes](#page-41-0) will have no effect, since parallel workers are taken from the pool of worker processes established by that setting.
