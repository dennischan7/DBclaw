...
# <span id="page-49-1"></span>**8.19. Object Identifier Types**

Object identifiers (OIDs) are used internally by PostgreSQL as primary keys for various system tables. Type oid represents an object identifier. There are also several alias types for oid, each named regsomething. [Table 8.26](#page-49-0) shows an overview.
...
| regconfig     | pg_ts_config | text search configura   | english         |
|               |              | tion                    |                 |
| regdictionary | pg_ts_dict   | text search dictionary  | simple          |
| regnamespace  | pg_namespace | namespace name          | pg_catalog      |
| regoper       | pg_operator  | operator name           | +               |
| regoperator   | pg_operator  | operator with argument  | *(integer,inte  |
|               |              | types                   | ger) or -(NONE, |
|               |              |                         | integer)        |
...
The to\_regclass() function and its siblings can also be used to perform run-time lookups. See Table 9.71.
