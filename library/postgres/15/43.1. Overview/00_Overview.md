...
PL/pgSQL functions can also be declared to accept and return the polymorphic types described in Section 38.2.5, thus allowing the actual data types handled by the function to vary from call to call. Examples appear in [Section 43.3.1.](#page-113-0)
...
Specific examples appear in [Section 43.3.1](#page-113-0) and [Section 43.6.1](#page-128-0).

# <span id="page-111-0"></span>**43.2. Structure of PL/pgSQL**
...
There is actually a hidden "outer block" surrounding the body of any PL/pgSQL function. This block provides the declarations of the function's parameters (if any), as well as some special variables such as FOUND (see [Section 43.5.5\)](#page-127-0). The outer block is labeled with the function's name, meaning that parameters and special variables can be qualified with the function's name.

It is important not to confuse the use of BEGIN/END for grouping statements in PL/pgSQL with the similarly-named SQL commands for transaction control. PL/pgSQL's BEGIN/END are only for grouping; they do not start or end a transaction. See [Section 43.8](#page-149-0) for information on managing transactions in PL/pgSQL. Also, a block containing an EXCEPTION clause effectively forms a subtransaction that can be rolled back without affecting the outer transaction. For more about that see [Section 43.6.8.](#page-139-0)