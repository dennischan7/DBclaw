...
# <span id="page-179-0"></span>**9.19. Array Functions and Operators**

[Table 9.52](#page-179-1) shows the specialized operators available for array types. In addition to those, the usual comparison operators shown in [Table 9.1](#page-55-0) are available for arrays. The comparison operators compare the array contents element-by-element, using the default B-tree comparison function for the element data type, and sort based on the first difference. In multidimensional arrays the elements are visited in row-major order (last subscript varies most rapidly). If the contents of two arrays are equal but the dimensionality is different, the first difference in the dimensionality information determines the sort order.

#### <span id="page-179-1"></span>**Table 9.52. Array Operators**

### **Operator Description Example(s)** anyarray @> anyarray → boolean Does the first array contain the second, that is, does each element appearing in the second array equal some element of the first array? (Duplicates are not treated specially, thus ARRAY[1] and ARRAY[1,1] are each considered to contain the other.) ARRAY[1,4,3] @> ARRAY[3,1,3] → t anyarray <@ anyarray → boolean Is the first array contained by the second? ARRAY[2,2,7] <@ ARRAY[1,7,4,2,6] → t anyarray && anyarray → boolean Do the arrays overlap, that is, have any elements in common? ARRAY[1,4,3] && ARRAY[2,1] → t
...
See [Section 8.15](#page-26-2) for more details about array operator behavior. See Section 11.2 for more details about which operators support indexed operations.

[Table 9.53](#page-180-0) shows the functions available for use with array types. See [Section 8.15](#page-26-2) for more information and examples of the use of these functions.

#### <span id="page-180-0"></span>**Table 9.53. Array Functions**
...
array\_cat ( anycompatiblearray, anycompatiblearray ) → anycompatiblearray
...
See also [Section 9.21](#page-188-0) about the aggregate function array\_agg for use with arrays.