...
title: 64.1. Basic API Structure for Indexes
---
...
## <span id="page-35-0"></span>**64.2. Index Access Method Functions**
...
Estimate the costs of an index scan. This function is described fully in [Section 64.6,](#page-44-0) below.
...
The purpose of an index, of course, is to support scans for tuples matching an indexable WHERE condition, often called a *qualifier* or *scan key*. The semantics of index scanning are described more fully in [Section 64.3,](#page-40-0) below. An index access method can support "plain" index scans, "bitmap" index scans, or both. The scan-related functions that an index access method must or may provide are:
...
# <span id="page-40-0"></span>**64.3. Index Scanning**
...
## <span id="page-42-0"></span>**64.4. Index Locking Considerations**
...
- A new heap entry is made before making its index entries. (Therefore a concurrent index scan is likely to fail to see the heap entry. This is okay because the index reader would be uninterested in an uncommitted row anyway. But see [Section 64.5.](#page-43-0))
...
## <span id="page-43-0"></span>**64.5. Index Uniqueness Checks**
...
## <span id="page-44-0"></span>**64.6. Index Cost Estimation Functions**
...
# <span id="page-47-0"></span>**Chapter 65. Generic WAL Records**
...
In comparison with [Custom WAL Resource Managers](#page-49-0), Generic WAL is simpler for an extension to implement and does not require the extension library to be loaded in order to apply the records.
...
# <span id="page-49-0"></span>**Chapter 66. Custom WAL Resource Managers**
...
An extension, especially a [Table Access Method](#page-31-0) or [Index Access Method,](#page-32-0) may need to use WAL for recovery, replication, and/or Logical Decoding. Custom resource managers are a more flexible alternative to [Generic WAL](#page-47-0) (which does not support logical decoding), but more complex for an extension to implement.
