...
The function noddl raises an exception each time it is called. The event trigger definition associated the function with the ddl\_command\_start event. The effect is that all DDL commands (with the exceptions mentioned in [Section 40.1](#page-72-0)) are prevented from running.
...
ALTER EVENT TRIGGER noddl DISABLE; CREATE TABLE foo (id serial); ALTER EVENT TRIGGER noddl ENABLE;