...
To set up the standby server, restore the base backup taken from primary server (see [Section 26.3.4](#page-181-0)). Create a file [standby.signal](#page-190-1) in the standby's cluster data directory. Set [restore\\_command](#page-50-1) to a simple command to copy files from the WAL archive. If you plan to have multiple standby servers for high availability purposes, make sure that recovery\_target\_timeline is set to latest (the default), to make the standby server follow the timeline change that occurs at failover to another standby.

### **Note**

[restore\\_command](#page-50-1) should return immediately if the file does not exist; the server will retry the command again if necessary.

If you want to use streaming replication, fill in [primary\\_conninfo](#page-55-1) with a libpq connection string, including the host name (or IP address) and any additional details needed to connect to the primary server. If the primary needs a password for authentication, the password needs to be specified in [pri](#page-55-1)[mary\\_conninfo](#page-55-1) as well.
...
If you're using a WAL archive, its size can be minimized using the [archive\\_cleanup\\_command](#page-50-0) parameter to remove files that are no longer required by the standby server. The pg\_archivecleanup utility is designed specifically to be used with archive\_cleanup\_command in typical single-standby configurations, see pg\_archivecleanup. Note however, that if you're using the archive for backup purposes, you need to retain files needed to recover from at least the latest base backup, even if they're no longer needed by the standby.
...
## <span id="page-192-0"></span>**27.2.5. Streaming Replication**
...
To use streaming replication, set up a file-based log-shipping standby server as described in [Sec](#page-189-0)[tion 27.2.](#page-189-0) The step that turns a file-based log-shipping standby into streaming replication standby is setting the primary\_conninfo setting to point to the primary server. Set [listen\\_addresses](#page-27-2) and authentication options (see pg\_hba.conf) on the primary so that the standby server can connect to the replication pseudo-database on the primary server (see [Section 27.2.5.1\)](#page-192-1).

On systems that support the keepalive socket option, setting [tcp\\_keepalives\\_idle,](#page-29-0) [tcp\\_keepalives\\_in](#page-29-1)[terval](#page-29-1) and [tcp\\_keepalives\\_count](#page-29-2) helps the primary promptly notice a broken connection.

Set the maximum number of concurrent connections from the standby servers (see [max\\_wal\\_senders](#page-53-0) for details).
...
### <span id="page-192-1"></span>**27.2.5.1. Authentication**
...
The host name and port number of the primary, connection user name, and password are specified in the [primary\\_conninfo](#page-55-1). The password can also be set in the ~/.pgpass file on the standby (specify replication in the database field). For example, if the primary is running on host IP 192.168.1.50, port 5432, the account name for replication is foo, and the password is foopass, the administrator can add the following line to the postgresql.conf file on the standby:
...
## <span id="page-193-0"></span>**27.2.6. Replication Slots**
...
## <span id="page-194-0"></span>**27.2.7. Cascading Replication**
...
Cascading replication is currently asynchronous. Synchronous replication (see [Section 27.2.8\)](#page-194-1) settings have no effect on cascading replication at present.
...
To use cascading replication, set up the cascading standby so that it can accept replication connections (that is, set [max\\_wal\\_senders](#page-53-0) and [hot\\_standby](#page-56-0), and configure [host-based authentication](#page-105-1)). You will also need to set primary\_conninfo in the downstream standby to point to the cascading standby.

## <span id="page-194-1"></span>**27.2.8. Synchronous Replication**
...
Once streaming replication has been configured, configuring synchronous replication requires only one additional configuration step: [synchronous\\_standby\\_names](#page-54-0) must be set to a non-empty value. synchronous\_commit must also be set to on, but since this is the default value, typically no change is required. (See [Section 20.5.1](#page-42-1) and [Section 20.6.2.](#page-54-1)) This configuration will cause each commit to wait for confirmation that the standby has written the commit record to durable storage. synchronous\_commit can be set by individual users, so it can be configured in the configuration file, for particular users or databases, or dynamically by applications, in order to control the durability guarantee on a per-transaction basis.
...
## <span id="page-198-0"></span>**27.2.9. Continuous Archiving in Standby**
