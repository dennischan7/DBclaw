...
Internally this function uses the [PGTYPESdate\\_today](#page-39-1) function.
...
Internally the function uses the [PGTYPESdate\\_julmdy](#page-38-0) function.
...
Internally this function is implemented to use the [PGTYPESdate\\_defmt\\_asc](#page-40-1) function. See the reference there for a table of example input.
...
Internally this function uses the [PGTYPESdate\\_fmt\\_asc](#page-39-2) function, see the reference there for examples.
...
Internally the function is implemented to use the function [PGTYPESdate\\_mdyjul](#page-38-1).
...
Internally the function is implemented to use the function [PGTYPESdate\\_dayofweek](#page-38-2).
...
Internally this function uses the [PGTYPEStimestamp\\_from\\_asc](#page-41-2) function. See the reference there for a table with example inputs.
...
This function is implemented by means of the [PGTYPEStimestamp\\_defmt\\_asc](#page-44-0) function. See the documentation there for a list of format specifiers that can be used.
...
Internally, this function uses the [PGTYPEStimestamp\\_fmt\\_asc](#page-41-1) function. See the reference there for information on what format mask specifiers can be used.
...
The function receives the type of the variable to test (t) as well a pointer to this variable (ptr). Note that the latter needs to be cast to a char\*. See the function [rsetnull](#page-113-0) for a list of possible variable types.
