...
## <span id="page-76-0"></span>**72.1. Row Estimation Examples**
...
How the planner determines the cardinality of tenk1 is covered in Section 14.2, but is repeated here for completeness. The number of pages and rows is looked up in pg\_class:
...
The planner examines the WHERE clause condition and looks up the selectivity function for the operator < in pg\_operator. This is held in the column oprrest, and the entry in this case is scalarltsel. The scalarltsel function retrieves the histogram for unique1 from pg\_statistic. For manual queries it is more convenient to look in the simpler pg\_stats view:
...
most_common_freqs | {0.00333333,0.003,0.003,0.003,0.003,0.003,
...
-------------------------------------------------------------------
-------------
 {AAAAAA,CQAAAA,FRAAAA,IBAAAA,KRAAAA,NFAAAA,PSAAAA,SGAAAA,VAAAAA,
XLAAAA,ZZAAAA}
```
...
```
selectivity = selectivity(unique1 < 1000) * selectivity(stringu1 =
 'xxx')
 = 0.100697 * 0.0014559
 = 0.0001466
