...
The view collation\_character\_set\_applicability identifies which character set the available collations are applicable to. In PostgreSQL, there is only one character set per database (see explanation in [Section 37.7](#page-98-0)), so this view does not provide much useful information.
