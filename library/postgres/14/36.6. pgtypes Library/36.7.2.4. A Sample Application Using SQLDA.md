...
The whole program is shown in [Example 36.1.](#page-33-0)

#### <span id="page-33-0"></span>**Example 36.1. Example SQLDA Program**
...
pg stat database s WHERE d.oid=s.datid AND ( d.datname=? OR
 d.oid=?)";
   ...
   EXEC SQL SELECT pg catalog.set config('search path', '',
 false); EXEC SQL COMMIT;
   EXEC SQL PREPARE stmt1 FROM :query;
