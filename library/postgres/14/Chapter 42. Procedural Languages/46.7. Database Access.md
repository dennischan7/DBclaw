...
Query parameters and result row fields are converted between PostgreSQL and Python data types as described in [Section 46.3.](#page-183-0)

When you prepare a plan using the PL/Python module it is automatically saved. Read the SPI documentation [\(Chapter 47\)](#page-198-0) for a description of what this means. In order to make effective use of this across function calls one needs to use one of the persistent storage dictionaries SD or GD (see [Section 46.4\)](#page-189-0). For example:
...
Do not confuse objects created by plpy.cursor with DB-API cursors as defined by the [Python Database API specification](https://www.python.org/dev/peps/pep-0249/)<sup>5</sup> . They don't have anything in common except for the name.

### <span id="page-193-0"></span>**46.7.2. Trapping Errors**
