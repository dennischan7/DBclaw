...
See [Section 19.9](#page-10-0) for more information about setting up SSL.
...
# <span id="page-28-2"></span>**20.4. Resource Consumption**

## <span id="page-28-0"></span>**20.4.1. Memory**
...
The use of huge pages results in smaller page tables and less CPU time spent on memory management, increasing performance. For more details about using huge pages on Linux, see [Sec](#page-4-0)[tion 19.4.5](#page-4-0).
...
Controls the size of huge pages, when they are enabled with [huge\\_pages.](#page-29-1) The default is zero (0). When set to 0, the default huge page size on the system will be used. This parameter can only be set at server start.

Some commonly available page sizes on modern 64 bit server architectures include: 2MB and 1GB (Intel and AMD), 16MB and 16GB (IBM POWER), and 64kB, 2MB, 32MB and 1GB (ARM). For more information about usage and support, see [Section 19.4.5](#page-4-0).
...
If you are not planning to use prepared transactions, this parameter should be set to zero to prevent accidental creation of prepared transactions. If you are using prepared transactions, you will probably want max\_prepared\_transactions to be at least as large as [max\\_connections,](#page-22-0) so that every session can have a prepared transaction pending.
...
Used to compute the maximum amount of memory that hash-based operations can use. The final limit is determined by multiplying work\_mem by hash\_mem\_multiplier. The default value is 1.0, which makes hash-based operations subject to the same simple work\_mem maximum as sort-based operations.
...
Note that when autovacuum runs, up to [autovacuum\\_max\\_workers](#page-73-0) times this memory may be allocated, so be careful not to set the default value too high. It may be useful to control for this by separately setting [autovacuum\\_work\\_mem](#page-31-0).
...
Specifies the maximum amount of memory to be used by each autovacuum worker process. If this value is specified without units, it is taken as kilobytes. It defaults to -1, indicating that the value of [maintenance\\_work\\_mem](#page-30-2) should be used instead. The setting has no effect on the behavior of VACUUM when run in other contexts. This parameter can only be set in the postgresql.conf file or on the server command line.
