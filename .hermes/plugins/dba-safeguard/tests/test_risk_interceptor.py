...
class TestRiskInterceptorAutoApprove:
    ...
    def test_select_passes(self):
        result = pre_tool_call_hook(
            ...
            args={"sql": "SELECT id FROM users WHERE id = 1", "dialect": "postgresql"},
        )
    ...
    def test_explain_passes(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "EXPLAIN SELECT * FROM users", "dialect": "postgresql"},
        )
...
class TestRiskInterceptorNotifyApprove:
    ...
    def test_insert_passes(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "INSERT INTO logs (msg) VALUES ('test')", "dialect": "postgresql"},
        )
...
class TestRiskInterceptorHumanConfirm:
    ...
    def test_update_with_where_blocks(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "UPDATE users SET status = 'inactive' WHERE id = 1", "dialect": "postgresql"},
        )
    ...
    def test_delete_with_where_blocks(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "DELETE FROM logs WHERE id = 1", "dialect": "postgresql"},
        )
        assert result is not None
        assert result["block"] is True
...
class TestRiskInterceptorForceBlock:
    ...
    def test_drop_table_blocked(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "DROP TABLE users", "dialect": "postgresql"},
        )
        assert result is not None
        assert result["block"] is True
    ...
    def test_delete_no_where_blocked(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "DELETE FROM users", "dialect": "postgresql"},
        )
        assert result is not None
        assert result["block"] is True
    ...
    def test_truncate_blocked(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "TRUNCATE TABLE users", "dialect": "postgresql"},
        )
        assert result is not None
        assert result["block"] is True
    ...
    def test_alter_drop_column_blocked(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "ALTER TABLE users DROP COLUMN temp", "dialect": "postgresql"},
        )
        assert result is not None
        assert result["block"] is True
...
class TestRiskInterceptorEdgeCases:
    ...
    def test_syntax_error_blocked(self):
        result = pre_tool_call_hook(
            tool_name="db_execute",
            args={"sql": "SELECTT * FORM users", "dialect": "postgresql"},
        )
